...
Method: 引入ICONIC-444数据集，包含超过310万张RGB图像，涵盖444个类别，使用原型工业分拣机采集。定义了四个参考任务来基准测试OOD检测方法，并为22种最先进的post-hoc OOD检测方法提供了基线结果。

Result: 创建了一个专门用于OOD检测研究的大规模工业图像数据集，填补了现有数据集的空白，提供了结构化、多样化的数据，支持跨任务复杂度的严格OOD评估。

Conclusion: ICONIC-444数据集通过提供大规模、高质量的工业图像数据，明确定义的OOD类别和参考任务，将促进OOD检测研究的基准测试和进展。
...
TL;DR: 评估三种主流视觉语言模型（GPT-4o、Florence 2、LLaVa-1.5）在建筑工地静态图像中识别工人行为和情绪的性能


<details>
  <summary>Details</summary>
Motivation: 随着机器人技术在建筑工作流程中的集成，理解人类行为对于安全有效协作至关重要。视觉语言模型（VLMs）具有无需大量领域特定训练即可识别人类行为的潜力，这在标注数据稀缺的建筑领域尤为重要，监测工人行为和情绪状态对安全和生产力至关重要。

Method: 使用包含1,000张图像的数据集，标注了10个行为类别和10个情绪类别。通过标准化推理流程和多种评估指标（F1分数、准确率、混淆矩阵分析）评估三种VLMs（GPT-4o、Florence 2、LLaVa-1.5）的性能。

Result: GPT-4o在两个任务中表现最佳：行为识别平均F1分数0.756、准确率0.799；情绪识别F1分数0.712、准确率0.773。Florence 2表现中等：行为F1分数0.497、情绪F1分数0.414。LLaVa-1.5表现最差：行为F1分数0.466、情绪F1分数0.461。所有模型在语义相近类别（如团队协作与主管沟通）上都存在混淆。

Conclusion: 通用VLMs可为建筑环境中的人类行为识别提供基础能力，但需要进一步改进（如领域适应、时序建模或多模态感知）才能达到实际应用的可靠性要求。
...
TL;DR: 研究发现ID准确率与OOD检测性能存在非单调关系：OOD性能随准确率提升而改善，但当训练方法将准确率推至基线以上时反而下降，且训练策略、检测器选择和OOD性能之间存在强相互依赖。


<details>
  <summary>Details</summary>
Motivation: OOD检测对于在开放世界环境中部署稳健可靠的机器学习系统至关重要，但现有OOD检测器与现代训练流程（旨在最大化ID准确率和泛化能力）之间的相互作用尚未得到充分探索。

Method: 采用ResNet-50架构，对56个通过不同训练策略获得的ImageNet训练模型，在8个OOD测试集上评估21种后处理、最先进的OOD检测方法，进行全面的实证研究。

Result: 发现ID准确率与OOD检测性能之间存在非单调关系：OOD性能随准确率提升而改善，但当高级训练方法将准确率推至基线以上时反而下降；训练策略、检测器选择和OOD性能之间存在强相互依赖，没有单一方法在所有情况下都是最优的。

Conclusion: OOD检测性能不仅取决于检测器本身，还高度依赖于训练策略，挑战了"更高ID准确率意味着更好OOD检测"的常见假设，强调需要综合考虑训练策略和检测器选择。
...
TL;DR: Medical SAM3：通过在全规模医学影像数据集上微调SAM3，构建了适用于医学图像分割的通用提示驱动基础模型，显著提升了在复杂医学场景下的分割性能。


<details>
  <summary>Details</summary>
Motivation: SAM3等可提示分割基础模型在医学图像分割中面临严重领域偏移、缺乏空间提示以及需要理解复杂解剖和体积结构等挑战，直接应用效果有限，需要进行全面的模型适应。

Method: 通过系统分析原始SAM3在医学数据上的表现，发现其性能严重依赖几何先验（如边界框），因此采用全模型参数微调策略，在33个数据集（涵盖10种医学成像模态）上进行训练，使用配对的2D/3D医学图像、分割掩码和文本提示。

Result: Medical SAM3在器官、成像模态和维度上均表现出显著且一致的性能提升，特别是在语义模糊、形态复杂和长距离3D上下文等挑战性场景中表现优异，成为通用的文本引导医学图像分割基础模型。

Conclusion: Medical SAM3证明了在严重领域偏移下实现鲁棒提示驱动分割需要全面的模型适应，而不仅仅是提示工程，为医学图像分割提供了强大的通用基础模型。
...
TL;DR: 提出FrankenMotion框架，通过构建具有原子级、时间感知的部分级文本标注的运动数据集，实现基于扩散模型的细粒度身体部位控制运动生成
...
Method: 1) 利用大语言模型构建具有原子级、时间感知的部分级文本标注的高质量运动数据集；2) 提出基于扩散模型的部分感知运动生成框架FrankenMotion，每个身体部位由其自身的时间结构化文本提示引导

Result: FrankenMotion在实验设置中优于所有先前基线模型，能够组合训练中未见过的运动，首次实现空间（身体部位）和时间（原子动作）双重控制

Conclusion: 该工作首次提供了原子级、时间感知的部分级运动标注，并开发了能够实现空间和时间双重控制的运动生成模型，显著提升了文本到运动生成的可控性和组合能力
...
<details>
  <summary>Details</summary>
Motivation: 胸部X光图像是诊断胸部疾病最常用的放射学检查方法之一，但多分类任务面临挑战。研究旨在系统比较不同方法在胸部X光图像多分类任务上的性能，为临床诊断提供技术支持。

Method: 1. 对多分类胸部X光图像诊断方法进行综述，包括DenseNet121等深度学习模型；2. 开发开源Web应用程序用于实际应用；3. 通过实验比较不同方法的性能表现；4. 深入分析所提方法的局限性。

Result: 1. 提供了多种方法在胸部X光图像多分类任务上的性能比较结果；2. 成功开发了可用的开源Web应用程序；3. 识别了现有方法的局限性；4. 代码已在GitHub上开源供社区使用。

Conclusion: 该研究系统评估了胸部X光图像多分类方法，开发了实用工具，并指出了未来改进方向，为胸部疾病诊断的自动化辅助系统发展提供了参考。
...
<details>
  <summary>Details</summary>
Motivation: 现实环境中的自动驾驶系统面临恶劣天气条件导致视觉退化的问题，这会严重降低模型性能和可靠性。现有多任务学习方法在应对这些视觉扰动时缺乏适应性，需要一种能够动态适应输入特征变化的鲁棒架构。

Method: 提出RobuMTL架构，采用混合专家模式，根据输入扰动动态选择任务特定的分层低秩适应（LoRA）模块和LoRA专家小队。该框架通过自适应专业化处理不同输入特征，提升在各种真实环境条件下的鲁棒性。

Result: 在PASCAL数据集上，RobuMTL相比MTL基线在单一扰动下获得+2.8%的平均相对提升，在混合天气条件下提升高达+44.4%。在NYUD-v2数据集上，跨任务平均相对提升+9.7%。性能优于单任务模型、标准MTL基线和最先进方法。

Conclusion: RobuMTL通过动态选择分层LoRA模块和专家小队，有效提升了多任务学习在恶劣天气条件下的鲁棒性，为自动驾驶系统在真实环境中的可靠运行提供了解决方案。代码已开源。
...
TL;DR: 该研究评估了五种深度学习架构在仅有150张标注图像的极端数据稀缺条件下进行树冠检测的性能，发现预训练的卷积模型（YOLOv11和Mask R-CNN）比预训练的Transformer模型（DeepLabv3、Swin-UNet和DINOv2）泛化能力显著更好。


<details>
  <summary>Details</summary>
Motivation: 树冠检测对于环境监测、城市规划和生态系统分析至关重要。Solafune树冠检测竞赛提供了仅有150张标注图像的小型不平衡数据集，模拟了现实中的数据标注稀缺情况，这对训练深度模型而不产生严重过拟合提出了重大挑战。

Method: 研究评估了五种代表性架构：YOLOv11、Mask R-CNN、DeepLabv3、Swin-UNet和DINOv2，以评估它们在极端数据稀缺条件下进行树冠分割的适用性。分析了训练策略、数据增强策略以及模型在小数据约束下的行为。

Result: 实验表明，预训练的卷积模型（特别是YOLOv11和Mask R-CNN）比预训练的Transformer模型泛化能力显著更好。DeepLabv3、Swin-UNet和DINOv2表现不佳，可能原因是语义分割与实例分割任务的差异、Vision Transformer的高数据需求以及缺乏强归纳偏置。

Conclusion: Transformer架构在没有大量预训练或数据增强的低数据情况下表现不佳，语义分割与实例分割的差异进一步影响模型性能。轻量级CNN方法在有限图像数据上进行树冠检测仍然是最可靠的。
...
<details>
  <summary>Details</summary>
Motivation: 传统AI医学诊断主要集中于图像分析，但缺乏患者报告症状限制了诊断准确性。需要模拟真实诊断过程中医生与患者的迭代对话来改善诊断效果。

Method: 提出预咨询对话框架(PCDF)，使用两个视觉语言模型：DocVLM基于图像和对话历史生成后续问题，PatientVLM使用基于真实诊断的症状配置文件进行回答。通过模拟多轮医患对话生成合成症状数据，并进行了小规模临床验证。

Result: 临床验证显示合成症状具有临床相关性、症状覆盖性和整体真实性。使用对话数据微调DocVLM相比仅使用图像训练有显著提升，证明了现实症状采集对诊断的价值。

Conclusion: PCDF框架通过模拟真实诊断对话生成高质量合成症状数据，有效提升AI医学诊断系统的准确性，为结合图像和症状信息的诊断方法提供了新途径。
...
Method: 结合隐式3D表示与2D检测器，设计LPIPS损失和多视角补偿损失（MVCL）联合优化渲染结果，能够聚合多视角受损图像信息。所有组件可端到端训练，合成高质量3D场景。建立了包含合成和真实世界干扰物的新基准数据集。

Result: IDDR-NGP在去除多种类型干扰物方面表现出有效性和鲁棒性，与现有SOTA去雪方法结果相当，能够准确去除真实和合成干扰物。建立了包含标注标签的干扰物基准数据集，支持相关研究。

Conclusion: IDDR-NGP是首个统一的3D场景干扰物去除方法，通过结合隐式3D表示与2D检测器，能够有效去除多种类型干扰物，为3D场景恢复提供了新解决方案，并建立了相关研究基准。
...
TL;DR: AIGVDBench是一个全面的AI生成视频检测基准，覆盖31个最先进的生成模型和44万+视频，对33个现有检测器进行1500+次评估，提供了8个深入分析和4个新发现。


<details>
  <summary>Details</summary>
Motivation: 当前AI生成视频检测领域存在两个关键限制：1）数据集角度：现有数据集规模有限，使用过时或范围狭窄的生成模型，难以捕捉现代生成技术的多样性和快速演变，且常重数量轻质量；2）基准角度：当前基准大多停留在数据集创建阶段，缺乏系统性的深入分析和基础问题探索。

Method: 提出AIGVDBench基准，覆盖31个最先进的生成模型和超过44万个视频，对33个现有检测器（属于四个不同类别）执行超过1500次评估，从多个角度进行8个深入分析。

Result: 该工作提出了8个深入分析，识别了4个新发现，为未来研究提供了有价值的见解。基准已开源在GitHub上。

Conclusion: AIGVDBench为推进AI生成视频检测领域提供了坚实基础，通过全面且具有代表性的基准解决了现有数据集和基准的局限性。
...
TL;DR: M3DDM+改进M3DDM视频外绘框架，通过统一训练掩码策略解决训练-推理不匹配问题，显著提升有限信息场景下的视觉质量和时间一致性


<details>
  <summary>Details</summary>
Motivation: M3DDM在相机运动有限或外绘区域较大的挑战性场景中，会出现空间模糊和时间不一致的质量退化问题。作者发现这是由于训练时使用随机掩码方向/宽度，而推理时需要一致方向外绘导致的训练-推理不匹配

Method: 提出M3DDM+，在训练时对所有帧应用统一的掩码方向和宽度，然后对预训练的M3DDM模型进行微调。这种方法保持了计算效率，同时解决了训练-推理不匹配问题

Result: 实验表明，M3DDM+在信息有限场景下显著提高了视觉保真度和时间一致性，同时保持了计算效率

Conclusion: 通过统一训练掩码策略解决训练-推理不匹配，M3DDM+有效改进了M3DDM在挑战性场景下的性能，为视频外绘提供了更鲁棒的解决方案
...
TL;DR: 提出首个物理感知强化学习范式用于视频生成模型，通过Mimicry-Discovery Cycle框架在生成过程中强制执行物理碰撞规则，提升视频物理真实性


<details>
  <summary>Details</summary>
Motivation: 当前基于transformer的视频生成模型忽视了物理原理，特别是在刚体运动渲染方面存在显著缺陷。虽然计算机图形学和物理模拟器能轻松建模碰撞，但现代预训练-微调范式在像素级全局去噪过程中丢弃了物体刚性的概念，将正确的数学约束仅视为优化条件，限制了生成视频的物理真实性。

Method: 1. 引入物理感知强化学习范式，在视频生成模型中直接在高维空间强制执行物理碰撞规则；2. 扩展为统一框架Mimicry-Discovery Cycle (MDcycle)，支持大规模微调同时完全保留模型利用物理基础反馈的能力；3. 构建新基准PhysRVGBench进行验证。

Result: 通过广泛的定性和定量实验验证了方法的有效性，在物理真实性方面表现出色。

Conclusion: 该研究首次将物理感知强化学习范式引入视频生成领域，通过MDcycle框架实现了物理规则在生成过程中的严格应用，显著提升了视频生成的物理真实性，为物理感知视频生成提供了新方向。
...
TL;DR: CoDance提出Unbind-Rebind框架，通过解绑姿态与参考图像间的刚性空间绑定，实现任意数量、类型和空间配置的多主体角色动画


<details>
  <summary>Details</summary>
Motivation: 现有方法在单角色动画上表现良好，但难以处理任意主体数量、多样角色类型以及参考图像与驱动姿态间的空间错位问题，主要受限于刚性空间绑定和无法将运动准确重绑定到目标主体

Method: 提出Unbind-Rebind框架：Unbind模块使用姿态偏移编码器，通过对姿态及其潜在特征引入随机扰动，打破姿态与参考间的刚性空间绑定，学习位置无关的运动表示；Rebind模块利用文本提示的语义引导和主体掩码的空间引导，将学习到的运动重定向到目标角色

Result: 在新建的多主体CoDanceBench和现有数据集上的实验表明，CoDance达到SOTA性能，在多样主体和空间布局上展现出卓越的泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 传统基于图的方法在3D点云分析中虽然能有效捕捉点之间的关系，但常常存在次优的图结构问题，特别是在边界点处连接稀疏，在连接区域存在噪声连接，这些限制了方法的性能。

Method: 提出两个核心模块：1) 图平滑模块，用于优化图结构，减少不可靠的稀疏和噪声连接的负面影响；2) 增强的局部几何学习模块，基于优化后的图结构，利用自适应几何描述符（基于特征向量）提取形状特征，并通过圆柱坐标变换获取分布特征。

Result: 在真实世界数据集上的实验结果表明，该方法在多种点云学习任务（分类、部件分割、语义分割）中均表现出有效性。

Conclusion: 通过集成图平滑和增强的局部几何学习，该方法能够有效解决传统图结构在边界点和连接区域的问题，提升3D点云分析的整体性能。
...
<details>
  <summary>Details</summary>
Motivation: 卫星地球观测系统产生了海量数据，但巨大的计算和存储需求阻碍了全球尺度分析的广泛应用。需要一种能够显著减少数据体积同时保持信息完整性的方法，以促进行星尺度研究。

Method: 使用ESDNet架构和有限标量量化(FSQ)技术，将Landsat系列(5,7,8,9)和MODIS Terra的高维多传感器观测数据转换为信息密集的量化潜在向量，将年度物候周期压缩为12个时间步长。

Result: 实现了约340倍的数据体积缩减，全球陆地表面单年数据仅需约2.4TB。重建保真度高(MAE:0.0130; RMSE:0.0179; CC:0.8543)。在土地覆盖分类中达到79.74%准确率，优于原始反射率融合的76.92%。

Conclusion: ESD通过显著的数据压缩和语义组织，为民主化行星尺度研究和推进下一代地理空间人工智能提供了多功能基础，具有强大的少样本学习能力和纵向一致性。
...
Method: 提出基于Rectified Flow的多模态联合推理算法，通过在结构化潜在空间中进行联合传输，实现配对样本的结构对齐生成，可建立在任意Rectified Flow模型之上

Result: 在图像、视频和3D形状生成领域验证方法有效性，相比编辑基和联合推理基方法，获得高度结构对齐的样本对和高质量视觉结果，在图像和视频生成上达到SOTA，3D生成质量相当但速度快数个数量级

Conclusion: 提出的基于Rectified Flow的联合推理方法能够高效实现结构对齐的配对样本生成，在多个模态上表现出优越性能，为多模态联合生成提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在图像分析中依赖大量标注数据，迁移学习虽能缓解此问题，但源域与目标域之间的差异会阻碍有效迁移，需要更优的微调策略。

Method: 提出BioTune自适应微调技术，利用进化优化算法自动选择需要冻结的层，并为解冻层调整学习率，实现跨域迁移的优化配置。

Result: 在9个图像分类数据集（包括自然图像和医学影像）上评估，BioTune在准确率和效率上均优于AutoRGN、LoRA等先进微调方法，且在4种不同CNN架构上表现一致优异。

Conclusion: BioTune通过进化优化实现了自适应微调，能有效应对不同数据特性和分布变化，为迁移学习提供了灵活高效的解决方案，消融研究验证了其关键组件的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 传统行人重识别方法通常针对单一场景设计，缺乏跨场景的统一框架。现实应用中需要处理多种场景（如跨分辨率、换装等），因此需要开发能够同时处理多个场景的无监督学习框架。

Method: 提出图像-文本知识建模三阶段框架：1）在图像编码器中引入场景嵌入并微调，自适应利用多场景知识；2）优化学习文本嵌入与伪标签关联，引入多场景分离损失增加场景间文本表示差异；3）引入聚类级和实例级异构匹配模块获取可靠异构正对，提出动态文本表示更新策略保持文本与图像监督信号一致性。

Result: 在多个场景上的实验结果表明，该方法不仅超越了现有的场景特定方法，而且通过整合多场景知识提升了整体性能，展示了优越性和泛化能力。

Conclusion: 提出的无监督多场景行人重识别任务和图像-文本知识建模框架有效解决了跨场景学习问题，利用视觉语言模型的表示能力，在多个场景上实现了性能提升和知识整合。
...
Result: 方法在28个基线方法中表现最优，在语言内检索基准上达到最先进准确率。在查询语言与目标语言不同的显式跨语言检索中验证了学习到的跨语言表示的有效性。仅需现有模型参数的一小部分即可实现强性能。

Conclusion: 该框架实现了准确且资源高效的跨脚本手写检索，为数字档案中的手写词检索提供了实用解决方案。
...
TL;DR: X-Distill通过跨架构知识蒸馏将大型DINOv2 ViT的视觉表示迁移到紧凑的ResNet-18，结合扩散策略头，在数据稀缺的机器人操作任务中实现高效学习


<details>
  <summary>Details</summary>
Motivation: 视觉运动策略常使用大型预训练ViT以获得泛化能力，但在数据稀缺的机器人学习环境中，其大量数据需求成为主要挑战，而紧凑CNN具有更强的归纳偏置且更容易优化。需要解决这种权衡问题。

Method: 提出X-Distill方法：1）在通用ImageNet数据集上进行离线跨架构知识蒸馏，将大型冻结DINOv2教师的丰富视觉表示迁移到紧凑ResNet-18学生；2）将蒸馏后的编码器与扩散策略头在目标操作任务上联合微调。

Result: 在34个模拟基准和5个具有挑战性的真实世界任务上的广泛实验表明，该方法始终优于使用从头训练ResNet或微调DINOv2编码器的策略。值得注意的是，X-Distill还超越了使用特权点云观测或更大视觉语言模型的3D编码器。

Conclusion: 这项工作展示了简单、理论基础扎实的蒸馏策略在实现数据高效机器人操作任务中达到最先进性能的有效性，为在数据稀缺环境中结合大型预训练模型和紧凑架构提供了实用解决方案。
...
TL;DR: SAMannot：一个开源本地框架，集成SAM2模型实现人机协同的视频实例分割标注，解决手动标注瓶颈和隐私问题
...
Method: 开发开源本地框架SAMannot，集成Segment Anything Model 2，通过修改SAM2依赖和实现处理层降低计算开销，包含持久实例身份管理、"锁定-精修"工作流、基于掩码骨架化的自动提示机制

Result: 工具能生成YOLO和PNG格式的研究就绪数据集及结构化交互日志，在动物行为追踪和LVOS、DAVIS基准数据集子集上验证有效，提供可扩展、隐私安全、成本效益高的视频标注方案

Conclusion: SAMannot为复杂视频标注任务提供了商业平台的可行替代方案，平衡了自动化效率与人工精度的需求，解决了研究中的隐私和成本问题
...
TL;DR: CASWiT：一种用于超高分辨率遥感图像语义分割的双分支Swin Transformer架构，通过上下文编码器和高分辨率编码器结合跨尺度融合模块，解决了Transformer在UHR图像中内存爆炸的问题，并采用SimMIM式预训练提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer模型在处理超高分辨率遥感图像时面临内存随token数量二次增长的问题，这限制了模型的上下文感知范围或空间分辨率。现有方法需要在上下文范围和空间细节之间做出权衡，难以同时保持全局语义理解和局部精细特征。

Method: 提出CASWiT双分支架构：1）上下文编码器处理下采样邻域捕获长距离依赖；2）高分辨率编码器提取UHR补丁的细粒度特征；3）跨尺度融合模块结合交叉注意力和门控特征注入，将全局上下文信息注入高分辨率token。此外，提出SimMIM式预训练策略：对高分辨率图像token和对应的低分辨率中心区域进行75%掩码，训练共享双编码器和小解码器重建原始UHR图像。

Result: 在IGN FLAIR-HUB大规模航空数据集上达到65.83% mIoU，比RGB基线提升1.78个百分点。在URUR数据集上达到49.1% mIoU，比当前最优方法提升+0.9%（按官方评估协议）。

Conclusion: CASWiT通过创新的双分支架构和预训练策略，有效解决了Transformer在超高分辨率遥感图像分割中的内存限制问题，实现了全局上下文与局部细节的平衡，在多个基准数据集上取得了新的最优性能。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型能够从图像和视频中生成复杂活动的高级可解释描述，在情境感知应用中具有价值。但在实际应用中需要高可靠性和准确性识别罕见但重要的事件，同时提取细粒度细节并评估识别质量。

Method: 提出将视觉语言模型与传统计算机视觉方法通过显式逻辑推理相结合的方法，包括：(a) 提取细粒度事件细节；(b) 采用智能微调策略，相比无指导选择实现显著更高的准确性；(c) 在推理过程中为VLM输出生成合理性验证。

Result: 智能微调机制提高了准确性，并在推理过程中提供了有价值的手段，可以确认VLM输出的有效性或指示其可能存在的问题。

Conclusion: 通过集成视觉语言模型与传统计算机视觉方法，结合显式逻辑推理，能够显著增强情境感知能力，特别是在细粒度事件识别、准确性提升和输出可靠性验证方面。
...
<details>
  <summary>Details</summary>
Motivation: 传统Beer-Lambert颜色反卷积方法在2-3种染色剂情况下表现良好，但对于K>3的多路免疫组化(mIHC)变得欠定和不稳定。需要一种能够处理多染色剂分离的方法，以支持染色标准化、标记物表达定量分析和细胞水平读取。

Method: 采用简单的数据驱动编码器-解码器架构：编码器是紧凑的U-Net，预测K个非负浓度通道；解码器是可微分的Beer-Lambert前向模型，具有可学习的染色矩阵（初始化为典型发色团色调）。训练是无监督的，使用感知重建目标，并通过损失项增强以抑制不必要的染色混合。

Result: 在包含5种染色剂（H、CDX2、MUC2、MUC5、CD8）的结直肠mIHC面板上，显示出优异的RGB重建效果，与基于矩阵的反卷积方法相比，显著减少了通道间串扰。

Conclusion: 该方法能够有效分离多路免疫组化RGB全切片图像中的染色贡献，为染色标准化、定量分析和细胞水平读取提供了可靠工具。代码和模型已开源。
...
TL;DR: 提出融合无人机与街景图像的机器学习框架，通过双模态跨视角学习评估建筑热相关属性，识别热暴露不平等性


<details>
  <summary>Details</summary>
Motivation: 气候变化加剧了全球南方城市中心的热暴露风险，但缺乏可扩展的方法评估建筑热相关属性，特别是低质量建筑材料和高热质量表面加剧了这一问题

Method: 提出耦合全局上下文视觉变换器(CGCViT)框架，融合公开可用的无人机(UAV)和街景(SV)图像进行双模态跨视角学习，利用HotSat-1热红外数据量化建筑属性与热相关健康风险的关系

Result: 双模态方法比最佳单模态模型性能提升9.3%；植被环绕、较亮屋顶、混凝土/粘土/木材屋顶材料与较低热红外值显著相关；在坦桑尼亚达累斯萨拉姆市成功识别了家庭层面的热暴露不平等

Conclusion: 无人机和街景图像提供了互补的城市结构视角，机器学习框架能够识别与建筑材料和社经劣势相关的热暴露不平等，强调本地化数据驱动风险评估对制定公平气候适应策略的重要性
...
TL;DR: TCS是一个无需训练的长视频理解框架，通过多查询推理和片段级快慢采样提升多模态大语言模型在长视频上的性能，在减少50%推理时间的同时实现可比精度。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在长视频理解上受限于计算约束和次优的帧选择策略，需要更高效的框架来提升长视频理解能力。

Method: 提出TCS框架，包含两个核心组件：1）多查询推理：生成多个查询以捕捉问题和视频的互补方面；2）片段级快慢采样：自适应平衡密集局部细节和稀疏全局上下文。

Result: 在MLVU、LongVideoBench和VideoMME三个基准测试中，TCS一致提升不同MLLMs的性能，最高提升6.9%准确率，且能以50%更少的推理时间成本达到可比精度。

Conclusion: TCS框架在长视频理解任务上展现出高效性和有效性，通过训练免费的方式显著提升多模态大语言模型的性能，平衡了计算效率和理解精度。
...
TL;DR: 本文提出HUG（异构不确定性引导）范式，通过细粒度概率学习框架解决组合图像检索中的内在噪声问题，使用高斯嵌入表示查询和目标，定制异构不确定性估计，并设计不确定性引导的目标函数，在基准测试中超越现有方法。
...
Method: 提出HUG范式：1）使用高斯嵌入表示查询和目标，捕获细粒度概念和不确定性；2）为多模态查询和单模态目标定制异构不确定性估计；3）捕获查询的单模态内容质量和多模态协调不确定性，通过可证明的动态加权机制获得综合查询不确定性；4）设计不确定性引导的目标函数，包括查询-目标整体对比和细粒度对比，配合全面的负采样策略。

Result: 在基准测试中，HUG超越了现有最先进的基线方法，通过忠实分析验证了技术贡献的有效性。

Conclusion: HUG范式通过细粒度概率学习框架和异构不确定性估计，有效解决了CIR中的内在噪声问题，提高了模型的鲁棒性和检索性能，为组合图像检索提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 3D语义占据预测作为自动驾驶全场景理解的关键任务，提供了体素级语义信息，但现有方法面临计算和内存开销过大的问题，阻碍了实时实际部署
...
Conclusion: SUG-Occ框架通过利用3D场景的固有稀疏性，在保持几何和语义完整性的同时显著降低了计算开销，为实时3D语义占据预测提供了有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 湿地制图需要密集像素级标注但成本高昂，实际应用通常依赖稀疏点标签，现有深度学习模型在此条件下表现不佳；湿地强烈的季节性和年际动态变化使单日期影像不足，导致显著制图误差；虽然SAM等基础模型从点提示中显示出良好泛化能力，但本质为静态图像设计，无法建模时间信息，在异质湿地中产生碎片化掩码

Method: 提出WetSAM框架：1) 时间提示分支通过分层适配器和动态时间聚合扩展SAM，从物候变异性中解耦湿地特征；2) 空间分支采用时间约束的区域增长策略生成可靠密集伪标签；3) 双向一致性正则化联合优化两个分支

Result: 在八个全球区域（每个约5000平方公里）的广泛实验中，WetSAM显著优于最先进方法，平均F1分数达到85.58%，以最小标注工作量提供准确且结构一致的湿地分割

Conclusion: WetSAM展示了强大的泛化能力和可扩展、低成本、高分辨率湿地制图的潜力，通过整合时间序列信息和稀疏点监督有效解决了湿地动态变化带来的挑战
...
TL;DR: Map2Thought是一个用于3D视觉语言模型的可解释空间推理框架，通过Metric-CogMap统一空间表示和Cog-CoT进行显式几何推理，在有限监督下实现接近全数据集的性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉语言模型缺乏显式和可解释的空间推理能力，难以进行精确的几何理解和关系推理，需要一种能够提供明确推理过程的空间推理框架。

Method: 提出Map2Thought框架，包含两个核心组件：Metric-CogMap（统一空间表示，结合离散网格进行关系推理和连续度量尺度表示进行几何理解）和Cog-CoT（通过向量运算、边界框距离和遮挡感知外观顺序等确定性操作进行显式几何推理，产生基于3D结构的可解释推理轨迹）。

Result: 在VSI-Bench上，仅使用一半监督数据达到59.9%准确率，接近全数据集训练的60.9%基线；在10%、25%、50%训练子集上分别比最先进方法高出5.3%、4.8%和4.0%；实现了可解释的3D理解。

Conclusion: Map2Thought通过显式和可解释的空间推理框架，显著提升了3D视觉语言模型在有限监督下的性能，为3D理解提供了可解释的推理过程。
...
TL;DR: 提出MHA2MLA-VLM框架，将现有多头注意力VLM高效转换为多头潜在注意力架构，显著减少KV缓存占用，同时保持性能
...
Method: 提出两种核心技术：1)模态自适应部分RoPE策略，通过选择性掩码支持传统和多模态设置；2)模态解耦低秩近似方法，独立压缩视觉和文本KV空间。结合参数高效微调，最小化输出激活误差而非参数距离

Result: 在三个代表性VLM上的实验表明，MHA2MLA-VLM能以最少监督数据恢复原始模型性能，显著减少KV缓存占用，并与KV量化无缝集成

Conclusion: MHA2MLA-VLM为现有VLM提供了一种参数高效、多模态感知的MLA转换框架，有效解决KV缓存瓶颈问题，具有实际部署价值
...
TL;DR: GeRo是一个用于VLA模型的即插即用框架，通过自回归展开策略联合执行规划和语言基础的未来交通场景生成，显著提升自动驾驶性能。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型主要依赖稀疏轨迹标注的模仿学习，未能充分利用其作为生成模型的潜力，需要更强大的语言基础推理和长时程规划能力。

Method: 1) 训练VLA模型将自车和智能体动态编码为潜在token；2) 语言条件自回归生成未来潜在token和文本响应；3) 使用展开一致性损失稳定预测，保持文本-动作对齐。

Result: 在Bench2Drive上，驾驶分数和成功率分别提升+15.7和+26.2；通过强化学习与生成展开结合，实现最先进的闭环和开环性能，展示强大的零样本鲁棒性。
...
TL;DR: ReScene4D：一种用于稀疏时间4D室内语义实例分割的新方法，通过跨观测共享信息实现一致的实例跟踪，并在3RScan数据集上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 室内环境会随时间变化（物体移动、出现或消失），需要在不连续采集的3D扫描中保持时间一致的实例身份。现有3DSIS方法缺乏时间推理需要离散匹配，而4D LiDAR方法依赖高频时间测量不适用于室内环境的长期演化

Method: 提出ReScene4D方法，将3DSIS架构适配到4DSIS任务，无需密集观测。通过跨观测共享信息的策略，不仅实现一致的实例跟踪，还提升标准3DSIS质量。定义了新指标t-mAP来奖励时间身份一致性

Result: 在3RScan数据集上达到最先进的性能，为理解演化室内场景建立了新基准

Conclusion: ReScene4D成功解决了稀疏时间4D室内语义实例分割任务，通过跨观测共享上下文信息实现了时间一致的实例跟踪，同时提升了分割质量，为动态室内场景理解提供了有效解决方案
...
TL;DR: ShapeR：从随意拍摄的图像序列中生成高质量3D物体形状的新方法，通过结合SLAM、3D检测和视觉语言模型提取多模态信息，使用整流流变换器生成度量3D形状。


<details>
  <summary>Details</summary>
Motivation: 现有3D形状生成方法大多依赖干净、无遮挡、分割良好的输入，而现实场景中很少满足这些条件。需要一种能够从随意拍摄的序列中生成3D物体形状的鲁棒方法。

Method: 1. 使用现成的视觉惯性SLAM、3D检测算法和视觉语言模型从图像序列中提取稀疏SLAM点、多视角图像和机器生成描述；2. 训练整流流变换器以这些模态为条件生成度量3D形状；3. 采用动态组合增强、跨对象和场景级数据集的课程训练方案，以及处理背景杂波的技术来确保鲁棒性。

Result: 1. 引入包含7个真实场景中178个野外物体的新评估基准；2. 在挑战性设置中显著优于现有方法，Chamfer距离比现有最佳方法提升2.7倍。

Conclusion: ShapeR能够从随意拍摄的序列中生成高质量的度量3D形状，解决了现实场景中数据不完美的挑战，为实际应用提供了有效的解决方案。
...
TL;DR: UniX是一个统一医学基础模型，通过解耦理解和生成任务，在胸部X光图像上实现了协同的理解和生成能力，性能优于现有方法且参数更少。


<details>
  <summary>Details</summary>
Motivation: 现有医学基础模型难以统一视觉理解和生成任务，因为这两个任务存在固有冲突：语义抽象vs像素级重建。基于参数共享自回归架构的方法通常导致一个或两个任务的性能妥协。

Method: 1) 将任务解耦为理解的自回归分支和生成的扩散分支；2) 引入跨模态自注意力机制，用理解特征动态指导生成过程；3) 采用严格数据清洗流程和多阶段训练策略。

Result: 在两个代表性基准测试中，UniX在理解性能（Micro-F1）上提升46.1%，在生成质量（FD-RadDino）上提升24.2%，仅使用LLM-CXR四分之一参数，性能达到任务特定模型水平。

Conclusion: UniX通过解耦架构和跨模态指导机制，建立了可扩展的协同医学图像理解和生成范式，实现了理解与生成任务的协同合作，同时利用扩散模型优势获得卓越生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM在离散博弈任务中推理效率与决策质量平衡问题，通过不确定性引导实现自适应推理

Method: 集成上下文学习与熵引导的链式思维推理，基于token级不确定性动态调整检索示例数量和推理路径

Result: 熵感知自适应推理将平均游戏结果从-11.6%提升至+9.5%，在保持较低LLM查询次数的同时显著改善决策质量

Conclusion: 不确定性引导的自适应推理能有效增强LLM在序列决策环境中的性能，为LLM在博弈场景的应用提供新思路
...
TL;DR: 研究发现，大语言模型的推理能力提升源于内部多智能体式交互（"思维社会"），而非单纯延长思维链；这种社会性思维结构通过多样化视角和辩论实现更优问题解决。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在复杂推理任务上表现出色，但其背后的机制尚不明确。现有研究通常将推理能力的提升归因于更长的思维链计算，但本文旨在探索是否存在更深层的机制。

Method: 采用定量分析和机制可解释性方法分析推理轨迹，研究DeepSeek-R1和QwQ-32B等推理模型；通过受控强化学习实验，观察模型在仅因推理准确率获得奖励时的行为变化；使用对话脚手架对模型进行微调。

Result: 推理模型比指令微调模型展现出更大的视角多样性，在推理过程中激活了更多异质性人格和专业知识特征之间的冲突；这种多智能体结构体现在对话行为（问答、视角转换、冲突观点调和）和社会情感角色中；强化学习实验显示，仅奖励推理准确率会增强对话行为，而对话脚手架微调能加速推理能力提升。

Conclusion: 思维的社会组织使模型能有效探索解空间，推理模型建立了与人类群体集体智能的计算平行关系，其中多样性的系统结构化能实现更优问题解决，这为智能体组织利用"群体智慧"提供了新机会。
...
TL;DR: EncodeRec是一种通过冻结语言模型参数、从物品描述直接学习紧凑信息嵌入的方法，旨在解决预训练语言模型在推荐系统中嵌入空间不够结构化、缺乏领域语义的问题。


<details>
  <summary>Details</summary>
Motivation: 预训练语言模型（PLMs）在推荐系统中的应用存在两个关键限制：1）PLMs没有明确优化以产生结构化和区分性的嵌入空间；2）其表示过于通用，往往无法捕捉推荐任务所需的领域特定语义。

Method: EncodeRec保持语言模型参数在推荐系统训练期间冻结，直接从物品描述中学习紧凑、信息丰富的嵌入，将文本表示与推荐目标对齐，同时保持计算效率而不牺牲语义保真度。

Result: 在核心推荐基准测试中，EncodeRec作为序列推荐模型的骨干和语义ID标记化方法都表现出有效性，相比基于PLM和嵌入模型的基线方法取得了显著提升。

Conclusion: 结果表明嵌入适应在弥合通用语言模型与实用推荐系统之间的差距中起着关键作用，EncodeRec提供了一种计算高效且语义保真的解决方案。
...
TL;DR: LLMs作为对话评估者时，对相同内容会因表述框架不同（陈述验证vs说话者归因）而产生不同的判断，这种现象被称为对话式顺从，DialDefer框架可检测和缓解这种框架诱导的判断偏移。


<details>
  <summary>Details</summary>
Motivation: LLMs越来越多地被用作第三方评估者，但其在评估对话中说话者时的可靠性尚不清楚。研究发现LLMs对相同内容会根据表述框架产生不同判断，这种框架效应可能影响评估的公正性和可靠性。

Method: 提出DialDefer框架和对话式顺从分数（DDS）来量化框架诱导的判断偏移。在9个领域、3000多个实例和4个模型上进行实验，比较陈述验证（"这个陈述正确吗？"）和说话者归因（"这个说话者正确吗？"）两种框架下的判断差异。

Result: 对话框架导致显著的判断偏移（|DDS|高达87个百分点，p<.0001），而准确率保持稳定（<2个百分点）。在自然Reddit对话中效应放大2-4倍。模型可能表现出顺从（同意）或怀疑（不同意），具体取决于领域。人类vsLLM归因驱动最大偏移（17.7个百分点摆动）。缓解尝试减少顺从但可能过度纠正为怀疑。

Conclusion: LLMs作为对话评估者存在框架依赖的判断偏移，这不仅仅是准确率优化问题，而是校准问题。人类归因特别影响模型判断，表明模型将不同意人类的成本视为高于不同意AI。需要超越准确率优化的校准方法来确保评估可靠性。
...
TL;DR: 提出PolyGloss模型，首次联合预测形态切分与词间注释，解决现有模型只生成语素级注释但不预测实际语素边界的问题，提升真实场景可用性。


<details>
  <summary>Details</summary>
Motivation: 现有自动词间注释预测模型（如GlossLM）虽然在基准测试中得分高，但在实际语言文档场景中存在关键障碍：它们生成语素级注释但将其分配给整个单词，而不预测实际的语素边界，导致预测结果难以解释且不可信。

Method: 首次研究神经模型联合预测原始文本的词间注释和相应的形态切分；通过实验确定平衡切分和注释准确性以及两者对齐的最佳训练方式；扩展GlossLM的训练语料，预训练PolyGloss系列seq2seq多语言模型，支持联合切分和注释；展示PolyGloss可通过低秩适配快速适应新数据集。

Result: PolyGloss在注释任务上优于GlossLM，在切分、注释和对齐任务上优于各种开源LLM；模型能够快速适应新数据集。

Conclusion: 联合预测形态切分和词间注释的神经模型能够解决现有模型的局限性，提高自动注释系统在真实语言文档场景中的可用性和可信度，PolyGloss为此类任务提供了有效的解决方案。
...
TL;DR: 论文探讨了社会科学家如何从数千个预训练大语言模型中选择合适模型，强调应以有效性、可靠性、可复现性和可复制性为指导原则，重点关注模型开放性、规模、训练数据、架构与微调等关键因素。


<details>
  <summary>Details</summary>
Motivation: 当前社会科学家面临数千个可用的大语言模型，但缺乏明确的选择标准。作者旨在为社会科学研究者提供基于科学原则（有效性、可靠性、可复现性、可复制性）的模型选择框架，以解决模型选择困境。

Method: 提出以有效性、可靠性、可复现性和可复制性为评估框架，分析四个关键维度：1) 模型开放性，2) 模型规模（footprint），3) 训练数据，4) 模型架构与微调。特别强调可复制性优先于事前有效性测试，建议从较小的开放模型开始，构建限定范围的基准测试来验证整个计算流程的有效性。

Result: 研究发现可复制性是选择语言模型更紧迫的指导原则，因为可靠复制特定发现需要可靠地复现任务。事前有效性测试（基准测试）虽然重要，但社会科学家无法完全避免事后验证计算测量。建议优先选择较小、开放的模型。

Conclusion: 社会科学家在选择语言模型时应优先考虑可复制性，从较小、开放的模型开始，通过构建限定范围的基准测试来验证整个计算流程。模型开放性、规模、训练数据和架构都是关键选择因素，但可复制性是最紧迫的指导原则。
...
TL;DR: 提出了首个中文患者模拟数据集Ch-PatientSim和无需训练的多阶段患者角色扮演框架MSPRP，显著提升了LLM在模拟患者行为方面的表现。


<details>
  <summary>Details</summary>
Motivation: 现有临床LLM模拟方法依赖通用或LLM生成的对话数据，限制了医患互动的真实性和多样性，需要更真实、多样化的患者模拟数据集和方法。

Method: 1) 基于真实临床交互场景构建中文患者模拟数据集Ch-PatientSim，采用五维人格结构模拟患者；2) 针对人格类别不平衡问题，使用少样本生成增强部分数据并进行人工验证；3) 提出无需训练的多阶段患者角色扮演框架MSPRP，将交互分解为三个阶段以确保个性化和真实性。

Result: 评估多种最先进LLM发现多数产生过于正式、缺乏个性的回答；提出的MSPRP框架在患者模拟的多个维度上显著提升了模型性能。

Conclusion: Ch-PatientSim数据集填补了中文临床患者模拟数据的空白，MSPRP框架有效解决了LLM在模拟患者时缺乏个性化和真实性的问题，为临床LLM发展和医学诊断教育提供了重要支持。
...
TL;DR: 提出一种无需训练、基于推理时logit干预的LLM控制方法，通过统计token分数表调整解码分布，实现细粒度、一致的多任务控制


<details>
  <summary>Details</summary>
Motivation: 现有LLM控制方法存在局限：基于激活的方法需要深度访问内部层，基于提示的方法难以提供一致或细粒度的控制。需要一种更有效的可控生成方法

Method: 提出无需训练的推理时logit干预方法，利用标注语料库的z归一化对数几率构建统计token分数表，在解码时调整分布

Result: 在写作复杂度、正式度和毒性三个数据集上的实验表明，该方法能有效引导输出特征，实现高达+47%准确率和50倍F1提升的控制增益

Conclusion: 基于统计的logit引导方法具有广泛适用性和任务无关性，能实现大规模、一致的多任务控制，为LLM可控生成提供了有效解决方案
...
TL;DR: 提出ZPD Detector框架，基于最近发展区理论动态选择训练数据，通过校准样本难度、估计模型能力并匹配两者，提高数据利用效率
...
Method: 提出ZPD Detector框架，基于最近发展区理论，采用模型与数据的双向视角，通过显式建模样本难度与模型当前能力的对齐。框架包含难度校准、基于项目反应理论的模型能力估计，以及能力-难度匹配评分，动态识别每个学习阶段最具信息量的样本

Result: 该方法提高了数据利用效率，动态匹配策略为训练策略设计提供了新见解。所有代码和数据将在论文被接受后发布以支持可重复研究

Conclusion: ZPD Detector通过建模模型与数据的动态关系，解决了现有数据选择方法的局限性，为在有限数据预算下高效训练大语言模型提供了创新解决方案
...
<details>
  <summary>Details</summary>
Motivation: 传统同步机器翻译仅使用读/写动作无法充分应对严格的实时约束，需要更丰富的动作空间来实现高质量翻译。

Method: 在LLM框架中引入四种自适应动作：Sentence_Cut（句子切分）、Drop（删除）、Partial_Summarization（部分摘要）和Pronominalization（代词化），通过动作感知提示构建训练参考，并开发延迟感知的TTS流水线评估质量和词级单调性。

Result: 在ACL60/60英-中、英-德和英-日基准测试中，该框架持续改善语义指标并实现比参考翻译和基于salami的基线更低的延迟。特别是Drop和Sentence_Cut的组合在流畅性和延迟平衡方面带来一致改进。

Conclusion: 丰富基于LLM的同步机器翻译的动作空间为缩小人机口译差距提供了有前景的方向。
...
<details>
  <summary>Details</summary>
Motivation: 在关键事实领域部署大语言模型时，准确评估模型置信度至关重要。虽然检索增强生成(RAG)被广泛采用以改善基础，但RAG设置中的置信度校准仍然理解不足。研究发现LLM在噪声检索上下文下表现出较差的校准性能，矛盾或不相关的证据倾向于夸大模型的虚假确定性，导致严重的过度自信。

Method: 提出NAACL规则(噪声感知置信度校准规则)为噪声下的过度自信提供原则性解决方案。设计NAACL框架，通过约2K个HotpotQA示例的监督合成，在NAACL规则指导下进行监督微调(SFT)，使模型具备内在的噪声感知能力，无需依赖更强的教师模型。

Result: 实证结果显示NAACL带来显著增益，域内ECE分数改善10.9%，域外改善8.0%。通过弥合检索噪声与口头校准之间的差距，NAACL为准确且认知可靠的大语言模型铺平道路。

Conclusion: NAACL框架通过噪声感知置信度校准有效解决了RAG中检索噪声导致的LLM过度自信问题，显著提高了模型的校准性能，为实现准确且认知可靠的大语言模型提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型展现出强大的翻译能力，但其内部机制仍不透明。研究者希望揭示这种内在翻译能力的核心组件，并利用这些机制洞察来改进模型训练

Method: 使用稀疏自编码器识别任务特定特征：首先召回在翻译输入上频繁共激活的特征，然后使用基于PCA的一致性指标过滤功能一致的特征，从而分离出一小部分翻译启动特征

Result: 成功识别出翻译启动特征，因果干预显示放大这些特征会引导模型正确翻译，而消融则导致幻觉和偏离任务输出。基于机制洞察的数据选择策略（优先训练机制上困难的样本）显著提高了数据效率并抑制了幻觉

Conclusion: 该工作不仅解码了LLM翻译机制的核心组件，还提供了利用内部模型机制创建更鲁棒和高效模型的蓝图，这些机制在同一家族的大模型中具有可迁移性
...
<details>
  <summary>Details</summary>
Motivation: 尽管机制可解释性研究已识别出检索头，但其在提升模型性能方面的作用尚未探索。本研究旨在探究是否可以利用检索头来增强LLM的长上下文能力。

Method: 提出RetMask方法：通过对比正常模型输出与掩码检索头的消融变体输出，生成训练信号。这种基于机制的方法在保持通用任务性能的同时，专门针对长上下文能力进行优化。

Result: 在Llama-3.1上，128K上下文长度的HELMET评估中提升2.28分，引用生成任务提升70%，段落重排序提升32%。实验发现效果取决于检索头组织模式：集中式模式响应强烈，分布式模式增益有限。
...
TL;DR: 论文研究了在有限计算预算下大语言模型的推理行为，提出了随时推理框架和Anytime Index指标，并利用LLM合成的偏好数据实现推理时自改进，在多个数据集上验证了方法在预算约束下提升推理质量和效率的效果。


<details>
  <summary>Details</summary>
Motivation: 现实世界任务（如行程规划）通常需要在固定推理预算内获得最佳输出，而大语言模型在有限计算资源下，快速产生有用的部分解决方案比详尽推理更实用，但现有方法缺乏量化评估推理质量随计算资源变化的指标。

Method: 1. 提出随时推理框架和Anytime Index指标，量化解决方案质量随推理token增加而改进的效果；2. 提出推理时自改进方法，利用LLM合成的偏好数据，让模型从自身推理比较中学习以产生更好的中间解决方案。

Result: 在NaturalPlan (Trip)、AIME和GPQA数据集上的实验表明，该方法在Grok-3、GPT-oss、GPT-4.1/4o和LLaMA模型上均取得一致提升，在预算约束下同时改善了推理质量和效率。

Conclusion: 该研究为有限计算预算下的LLM推理提供了系统框架和评估指标，提出的推理时自改进方法能有效提升模型在资源受限场景下的性能，对实际应用具有重要价值。
...
TL;DR: 论文提出REVIVE框架，通过谱分析揭示顺序知识编辑导致模型能力崩溃的机制，并设计保护主导奇异子空间的方法来稳定编辑过程。


<details>
  <summary>Details</summary>
Motivation: 顺序知识编辑在大型语言模型中常导致灾难性崩溃，现有方法通过启发式约束缓解问题，但崩溃机制尚未充分理解。需要深入分析编辑对模型权重谱结构的影响，并开发更有效的稳定方法。

Method: 提出REVIVE框架：1) 对顺序知识编辑进行谱分析，发现模型通用能力与预训练权重矩阵的主导奇异方向密切相关；2) 在原始权重的谱基中表示参数更新；3) 过滤会干扰受保护区域的组件，明确保护主导奇异子空间。

Result: REVIVE在多个模型和基准测试中一致提高编辑效果，同时在长序列编辑（包括多达20,000次编辑的极端设置）下显著保留模型通用能力。实验验证了主导奇异方向对扰动的高度敏感性及其与编辑效果崩溃的紧密关联。

Conclusion: 模型通用能力与预训练权重的主导奇异方向密切相关，这些方向对扰动敏感且易被重复编辑破坏。REVIVE通过谱分析和显式保护主导奇异子空间，为顺序知识编辑提供了稳定有效的解决方案。
...
Method: CoG框架包含两个核心模块：1) 关系蓝图引导模块（快速直觉过程），利用关系蓝图作为可解释的软结构约束，快速稳定搜索方向以抵抗噪声；2) 失败感知精炼模块（审慎分析过程），在遇到推理障碍时触发证据条件反射并执行受控回溯，克服推理停滞。

Result: 在三个基准测试上的实验结果表明，CoG在准确性和效率方面显著优于现有最先进方法。

Conclusion: CoG通过模拟直觉与审慎思考的双过程交互，有效解决了知识图谱增强大语言模型的认知刚性问题，为提升LLM在结构化知识推理中的可靠性和稳定性提供了新思路。
...
TL;DR: Integrity Shield是一种文档层水印系统，可在PDF评估文件中嵌入模式感知、项目级水印，防止多模态大语言模型直接解答受保护的考试PDF，同时保持人类可见外观不变。


<details>
  <summary>Details</summary>
Motivation: 大语言模型现在可以直接从上传的PDF评估文件中解答整个考试，这引发了关于学术诚信和成绩证书可靠性的紧迫担忧。现有的水印技术要么在token级别操作，要么假设能控制模型的解码过程，当学生使用教师提供的文档查询专有黑盒系统时，这些技术无效。

Method: 提出Integrity Shield文档层水印系统，在评估PDF中嵌入模式感知、项目级水印，同时保持人类可见外观不变。这些水印能持续阻止MLLMs回答受保护的考试PDF，并编码稳定的项目级签名，可从模型或学生回答中可靠恢复。

Result: 在涵盖STEM、人文和医学推理的30个考试中，Integrity Shield实现了极高的预防效果（91-94%考试级别阻止）和强大的检测可靠性（89-93%签名检索），在四个商业MLLMs上表现一致。演示展示了交互界面，教师可上传考试、预览水印行为、检查AI前后性能及作者证据。

Conclusion: Integrity Shield提供了一种有效的文档层水印解决方案，能够防止大语言模型直接解答受保护的考试PDF，同时保持人类可读性，为解决学术诚信问题提供了实用工具。
...
TL;DR: DOREMI是一个用于文档级关系抽取的迭代框架，通过有针对性的手动标注来优化长尾分布问题，可应用于现有DocRE模型以提高对罕见关系的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 文档级关系抽取面临两大挑战：需要跨句子上下文理解，以及关系类型的长尾分布问题（许多关系只有少量训练样本）。现有方法要么依赖大规模噪声数据，要么使用启发式去噪方法，效果有限。

Method: DOREMI是一个迭代框架，通过主动选择最具信息量的样本来进行最小化但有针对性的手动标注，从而增强对代表性不足关系的训练。该方法不依赖大规模噪声数据或启发式去噪，而是优化训练效率和鲁棒性。

Result: DOREMI能够有效缓解长尾偏差，提高对罕见关系的泛化能力。该框架可应用于任何现有的文档级关系抽取模型，提供了一种可扩展的解决方案。

Conclusion: DOREMI通过迭代式主动标注策略，为文档级关系抽取中的长尾分布问题提供了一种高效且可扩展的解决方案，能够显著改善对罕见关系的识别能力。
...
Result: 在数学推理基准测试上实现更高并行度解码且性能损失最小，并能收敛到替代解码调度方案Ŝ达到可比性能

Conclusion: T*提供了一种有效的渐进式块大小扩展方法，为掩码扩散语言模型的高效解码提供了新途径
...
<details>
  <summary>Details</summary>
Motivation: 在线虚假信息日益威胁社会，现有自动事实核查方法受限于缺乏反映真实世界复杂性的数据集，特别是多模态和多语言环境下的矛盾检测

Method: 首先创建MultiCaption数据集，通过多种策略标注指向相同图像或视频的声明对是否相互矛盾；然后使用基于transformer的架构、自然语言推理模型和大语言模型进行综合实验

Result: MultiCaption比标准NLI任务更具挑战性，需要任务特定的微调才能获得良好性能；多语言训练和测试带来的增益表明该数据集有潜力构建不依赖机器翻译的有效多语言事实核查流程

Conclusion: MultiCaption为多模态多语言虚假信息检测提供了独特资源，建立了未来研究的强基准，展示了在真实多语言环境中构建事实核查系统的可行性
...
TL;DR: 通过控制模型思考时使用的语言（思维语言），可以显著提升大语言模型的输出多样性，且思维语言与英语的差异越大，多样性增益越明显。


<details>
  <summary>Details</summary>
Motivation: 输出多样性对大语言模型的多元性和创造力至关重要。现有方法主要关注输出层面的控制，而本文探索思维语言这一新颖的结构性多样性来源，旨在通过控制模型内部思考过程使用的语言来增强输出多样性。

Method: 提出多语言思维方法，研究两种重复采样策略：单语言采样（使用单一非英语思维语言）和混合语言采样（聚合多种思维语言的样本）。通过控制输出为英语，研究不同思维语言对输出多样性的影响，并分析思维空间中的语言距离与多样性增益的关系。

Result: 实验表明：1）将思维语言从英语切换到非英语语言能持续增加输出多样性；2）思维语言与英语在思维空间中的距离与多样性增益呈正相关，距离越远增益越大；3）聚合多种思维语言的样本通过组合效应带来额外改进；4）通过语言异质性扩展采样能提升模型的多样性上限；5）这些发现在多元对齐场景中具有实际价值，能扩大文化知识和价值取向的覆盖范围。

Conclusion: 思维语言是控制大语言模型输出多样性的有效结构性来源，多语言思维策略能显著提升模型输出的多元性和创造力，为促进AI系统的文化包容性和价值多元性提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在知识密集型应用中经常生成事实错误的回答，需要有效的方法来纠正这些缺陷，而现有方法在跨领域适应和结构化反馈利用方面存在不足

Method: 提出FactCorrector后处理校正方法，无需重新训练即可跨领域适应，利用原始回答的事实性结构化反馈生成校正；同时开发VELI5基准数据集，包含系统注入的事实错误和真实校正

Result: 在VELI5和多个流行的长文本事实性数据集上的实验表明，FactCorrector方法显著提高了事实精确度，同时保持了相关性，优于强基线方法

Conclusion: FactCorrector是一种有效的后处理校正方法，能够显著改善LLM的事实准确性，且具有跨领域适应能力；VELI5基准为事实性校正方法的严格评估提供了支持
...
Method: 提出DDAIR方法：1) 使用Sentence Transformers检测LLM生成的意图导向增强示例中的歧义；2) 识别语义上更接近其他意图而非目标意图的合成示例；3) 提供迭代重新生成方法来减轻此类歧义

Result: 句子嵌入能有效帮助(重新)生成歧义较少的示例，在意图定义松散或宽泛的场景中显示出改善分类性能的潜力

Conclusion: DDAIR方法通过检测和减少LLM生成的数据增强示例中的意图歧义，为低资源意图识别任务提供了有效的解决方案，特别是在意图边界不清晰的场景中
...
Result: 综合实验表明，RT-RAG在F1分数上比最先进方法提升7.0%，在EM指标上提升6.0%，在复杂多跳问答任务中表现出显著优势。
...
TL;DR: 该论文提出使用多任务学习（MTL）方法，通过微调单个小型开源LLM来同时执行自动事实核查的三个子任务（声明检测、证据排序和立场检测），以替代需要多个专用模型的高成本方案。


<details>
  <summary>Details</summary>
Motivation: 大型专有LLM在自动事实核查中表现良好，但其封闭权重、复杂性和高成本限制了可持续性。微调小型开源模型针对单个任务需要多个专用模型，成本仍然很高。需要更高效的解决方案。

Method: 使用小型仅解码器LLM（如Qwen3-4b），探索三种多任务学习策略：分类头、因果语言建模头和指令调优。评估不同模型大小、任务顺序，并与标准非LLM基线进行比较。

Result: 多任务模型虽未在所有任务上超越单任务基线，但在声明检测、证据重排序和立场检测上分别实现了44%、54%和31%的相对提升（相对于零/少样本设置）。提供了基于实证的实用指南。

Conclusion: 多任务学习是自动事实核查中更高效的替代方案，能够通过单个模型同时处理多个子任务，显著提升性能并降低成本，为实践者提供了实用指导。
...
TL;DR: SimMIA：一种针对仅文本黑盒场景的鲁棒成员推理攻击框架，通过高级采样策略和评分机制实现，并在新基准WikiMIA-25上达到SOTA性能。
...
Method: 提出SimMIA框架，采用高级采样策略和评分机制，专门针对仅文本的黑盒设置。同时创建WikiMIA-25基准，用于评估现代专有LLM上的MIA性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前语音对话系统缺乏动态适应上下文的能力，限制了自然性和可用性。需要能够控制说话人声音、话题、对话行为（如回应、打断）和对话启动的可定制系统。

Method: 采用冻结音频编码器、仅微调语言模型的单阶段训练方法，只需2000小时数据，无需大规模预训练或多阶段优化。提出系统化的设计选择分析。

Result: 开发出首个开源、指令驱动的全双工会话语音模型，能够遵循显式指令控制多个对话维度，模型和训练代码将开源以促进可重复研究。

Conclusion: 该工作为可控全双工语音系统研究提供了高效、可复现的解决方案，通过指令驱动方法实现了对话行为的动态定制，有望提升语音对话系统的自然性和交互体验。
...
TL;DR: 该论文提出将自然语言题解作为编程竞赛评估的核心，区分算法推理与代码实现，并引入包含83个ICPC风格问题的数据集进行评测。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法将大语言模型在编程竞赛中的表现混为一谈，未能区分算法推理能力与代码实现能力。作者认为编程竞赛本质上是问题解决任务，应专注于自然语言题解来评估模型的核心推理能力。

Method: 提出以自然语言题解为中心的方法：1) 在生成代码前先生成题解；2) 引入包含83个ICPC风格问题的数据集，包含黄金题解和完整测试套件；3) 使用专家标注比较模型生成题解与黄金题解；4) 验证LLM-as-a-judge协议进行可扩展评估；5) 评估19个不同LLM。

Result: 1) 先生成题解再写代码能提高部分LLM的解题率，使用专家编写的黄金题解时提升更大；2) 即使有黄金题解，模型在实现阶段仍有困难；3) 生成题解与黄金题解之间的差距揭示了算法规范正确性和完整性的持续瓶颈；4) LLM-as-a-judge协议被验证可用于可扩展评估。

Conclusion: 编程竞赛评估应明确区分问题解决与代码实现两个阶段，自然语言题解应成为评估的核心。未来基准测试需要分离这两个维度，以更准确地评估LLM的算法推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM的顺序推理方法缺乏前瞻性，容易陷入次优推理路径并产生冗余步骤，需要更高效的推理策略

Method: 提出神经思维链搜索框架，将推理视为动态搜索最优思维策略的过程，使用双因子启发式（正确性和计算成本）评估候选推理算子，主动导航到稀疏的优质推理路径

Result: 在多个推理基准测试中实现帕累托改进：准确率提升超过3.5%，同时生成长度减少超过22%

Conclusion: NCoTS通过将推理重构为搜索问题并采用双因子优化，显著提升了推理效率和准确性，证明了稀疏优质推理路径的存在和可发现性
...
TL;DR: 该研究评估了大型语言模型在起草患者门户消息回复时与临床医生偏好的对齐程度，开发了新的评估框架和标注数据集，发现LLM在某些主题元素上表现良好，但在提问等主题上存在困难，需要针对个体医生偏好进行适配。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在起草患者门户消息回复方面显示出潜力，但其整合到临床工作流程中存在诸多担忧，特别是是否真正能节省临床医生的时间和精力。研究旨在调查LLM与个体临床医生偏好的对齐程度，以评估其在患者-临床医生沟通工作流程中的可靠性和负责任使用。

Method: 开发了临床医生回复中主题元素的新分类法；提出了在内容和主题层面评估临床医生对LLM起草回复编辑负荷的新评估框架；发布了专家标注的数据集；使用多种适配技术（包括主题提示、检索增强生成、监督微调和直接偏好优化）对本地和商业LLM进行了大规模评估。

Result: 研究揭示了LLM草稿与临床医生回复对齐中存在显著的认知不确定性。LLM在起草某些主题元素方面表现出能力，但在其他主题（特别是向患者提问以获取更多信息）上难以生成与临床医生对齐的内容。主题驱动的适配策略在大多数主题上带来了改进。

Conclusion: 研究结果强调了将LLM适配到个体临床医生偏好的必要性，以实现其在患者-临床医生沟通工作流程中的可靠和负责任使用。主题驱动的适配策略是提高对齐效果的有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 科学写作评估面临挑战：现有LLM评估模型主要针对通用基准设计，缺乏对科学领域稀疏知识的推理能力，无法适应任务依赖的多方面标准，且针对每个任务微调成本高昂，在低资源环境下不切实际。

Method: 提出两阶段训练框架：第一阶段优化科学评估偏好，第二阶段精炼推理能力。采用多方面评估设计和跨任务联合训练，实现细粒度评估和对动态标准及评分规则的鲁棒性。

Result: 实验分析表明训练方案显著改进LLM科学写作评估能力。模型能有效跨任务泛化，适应未见过的科学写作评估场景，允许单一训练评估器无需任务特定重训练即可复用。

Conclusion: 该研究开发了针对科学写作评估的成本效益高、开源奖励模型，通过创新的训练框架解决了现有评估方法的局限性，实现了跨任务泛化和动态标准适应，为科学写作评估提供了实用解决方案。
...
TL;DR: 研究发现大语言模型幻觉与关系线性度强相关：线性关系（如"X演奏Y乐器"）更容易导致模型对未知实体产生幻觉，而非线性关系（如"X的配偶是Y"）则较少出现幻觉。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的幻觉是一个核心失效模式。研究者关注模型对未知合成实体问题的幻觉现象，发现中等规模模型（如Gemma-7B-IT）经常产生幻觉，难以识别幻觉事实是否属于其知识范围。研究者假设关系线性度是导致幻觉的重要因素。

Method: 创建SyntHal数据集，包含6000个合成实体和六种关系。使用四个模型进行实验，测量每个关系的幻觉率，并使用Δcos度量关系的线性度。分析线性度与幻觉率之间的相关性。

Result: 发现关系线性度与幻觉率之间存在强相关性（r∈[0.78,0.82]）。线性关系（如"演奏乐器"）的幻觉率更高，而非线性关系（如"配偶关系"）的幻觉率较低。这支持了假设：关系的底层存储方式是影响模型自我评估知识能力的重要因素。

Conclusion: 关系线性度是导致大语言模型幻觉的关键因素。线性关系倾向于以更抽象的方式存储，使得模型难以评估其知识；而非线性关系则以更直接的方式存储，便于知识评估。这一发现对管理幻觉行为和改进LLM事实知识表示具有重要启示。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在安全担忧，现有编辑方法通常优化混合新旧知识的信息矩阵，但计算成本高且可能引发冲突

Method: 提出HORSE方法，关注信息矩阵的分层正交残差展开，从不同角度减少噪声梯度，实现更稳定的编辑

Result: 在两个数据集和多个LLM上的实验表明，HORSE能在不同场景下保持精确的大规模编辑

Conclusion: HORSE通过分层正交残差展开信息矩阵，提供了一种更稳定、计算效率更高的模型编辑方法
...
Method: 提出TTARAG方法，在推理时动态更新语言模型参数，通过让模型学习预测检索内容来自动调整参数以适应目标领域
...
TL;DR: 提出了CTest-Metric框架，首个用于评估CT放射学报告生成指标临床可行性的统一框架，包含三个测试模块，评估了8个常用指标在7个LLM上的表现。


<details>
  <summary>Details</summary>
Motivation: 在生成式AI时代，放射学报告生成的质量评估仍依赖次优指标，缺乏统一的框架来评估指标在临床环境中的稳健性和适用性。

Method: 提出CTest-Metric框架，包含三个模块：1) 写作风格泛化性测试（WSG），通过LLM重述报告；2) 合成错误注入（SEI），按严重程度注入错误；3) 指标与专家相关性（MvE），使用临床医生对175个"分歧"病例的评分。评估了8个指标在基于CT-CLIP编码器的7个LLM上的表现。

Result: 发现：词汇NLG指标对风格变化高度敏感；GREEN Score与专家判断最一致（Spearman~0.70）；CRG显示负相关；BERTScore-F1对事实错误注入最不敏感。

Conclusion: CTest-Metric为评估放射学报告生成指标的临床可行性提供了首个统一框架，有助于可重复的基准测试和未来指标开发，将发布框架、代码和匿名评估数据。
...
TL;DR: 该研究探讨大型推理模型（LRMs）生成的思维链（CoT）解释是否具有泛化性，即这些解释是否捕捉了问题的通用模式而非模型特有的模式。研究发现CoT解释确实能在不同LRMs间提高一致性，且这种一致性增强与人类偏好排名和强化学习后训练相关。


<details>
  <summary>Details</summary>
Motivation: 尽管大型推理模型生成的思维链解释提供了人类可读的问题解决过程，但尚不清楚这些解释是否真正捕捉了问题的通用模式，还是仅反映了特定模型的内部特性。这对于AI科学发现等需要可靠解释的应用至关重要。

Method: 通过评估一个具体的泛化概念：一个LRM生成的解释是否能诱导其他LRMs产生相同行为。研究分析了CoT解释在不同模型间的一致性，并提出了句子级别的集成策略来改进一致性。

Result: 研究发现CoT解释通常表现出跨模型泛化能力（即提高LRMs间的一致性），且这种增强的泛化性与人类偏好排名和强化学习后训练相关。提出的句子级集成策略能有效改善模型间的一致性。

Conclusion: 研究建议在使用LRM解释获得新见解时应保持谨慎，并提出了一个用于表征LRM解释泛化性的框架。结果表明CoT解释确实具有一定程度的泛化能力，但需要系统评估。
...
TL;DR: 论文对LLM中的tokenization进行了实证分析，发现不同模型和文本领域的token化存在显著差异，挑战了关于token长度的常见启发式假设


<details>
  <summary>Details</summary>
Motivation: 当前前沿LLM在学术界、社会和工业界广泛应用，token常被用作比较模型、输入输出和估算推理成本的稳定单位。然而，不同模型和文本领域的tokenization存在显著差异，这使得对token计数的简单解释存在问题。需要量化这种变化，挑战关于token长度的常见启发式假设。

Method: 通过提供全面的实证分析来量化tokenization的变化，探索不同文本数据分布下序列到token的压缩情况。分析不同模型和文本领域的tokenization差异。

Result: 研究发现tokenization在不同模型和文本领域存在显著差异，挑战了关于token长度的常见启发式假设，发现这些假设过于简化。提供了对当代LLM中tokenization的清晰见解和直觉。

Conclusion: tokenization在不同模型和文本领域存在显著差异，不能简单地将token视为稳定货币进行比较。研究为当代LLM中的tokenization提供了更清晰的理解，有助于更准确地比较模型和估算成本。
