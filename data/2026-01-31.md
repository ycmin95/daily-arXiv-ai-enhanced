...
TL;DR: 该论文针对对话式图像生成中的非马尔可夫挑战，提出了数据构造策略、历史条件训练框架和图像重建技术，显著提升了多轮交互的一致性和指令遵循能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在对话式图像生成中大多采用马尔可夫方法，仅依赖最近图像进行响应，无法处理用户引用历史状态、撤销更改或跨多轮引用实体的复杂场景。需要解决非马尔可夫设置下的多轮一致性挑战。

Method: 提出三方面方法：(1)非马尔可夫多轮数据构造策略，包括回滚式编辑和基于名称的多轮个性化；(2)历史条件训练与推理框架，采用令牌级缓存防止身份漂移；(3)高保真图像重建技术，包括基于重建的DiT解令牌器和多阶段微调课程。

Result: 显式训练非马尔可夫交互显著提升了多轮一致性和指令遵循能力，同时保持了强大的单轮编辑和个性化性能。实现了更好的历史状态检索和跨轮实体绑定。

Conclusion: 针对非马尔可夫设置的系统性方法有效解决了对话式图像生成中的长期依赖问题，为复杂多轮交互提供了更鲁棒的解决方案，平衡了多轮一致性与单轮性能。
...
TL;DR: 提出基于文本引导的PET图像去噪方法，使用预训练CLIP模型特征结合U-Net架构，能在单一模型中处理多种计数水平的去噪需求


<details>
  <summary>Details</summary>
Motivation: PET图像在医学诊断中至关重要，但常受复杂噪声干扰，影响诊断信息。图像质量受多种因素影响，包括扫描仪硬件、重建算法、示踪剂特性、剂量/计数水平和采集时间。现有方法难以在单一模型中处理不同计数水平的去噪需求

Method: 提出文本引导的PET图像去噪方法，利用预训练CLIP模型提取特征，结合U-Net架构的降噪模型。该方法通过文本引导实现跨多种计数水平的自适应去噪

Result: 实验结果表明，所提模型在定性和定量评估中均带来显著改进。模型展现出处理复杂去噪需求或减少采集时间的潜力

Conclusion: 提出的文本引导去噪方法能够有效增强PET图像质量，在单一模型中处理多种计数水平，具有灵活性和临床应用潜力
...
TL;DR: 提出一种基于合成数据的展开式双域方法，用于校正CT探测器中的低性能像素（LPP）伪影，无需真实临床数据训练


<details>
  <summary>Details</summary>
Motivation: CT探测器中的低性能像素会导致环形和条纹伪影，现有深度学习方法需要昂贵的专用数据集训练，且仅关注图像域或正弦图域单一校正，忽略了CT几何前向操作的内在相关性
...
Conclusion: 该方法通过合成数据和双域校正策略，有效解决了LPP伪影校正的数据收集成本问题，同时利用CT几何的内在相关性提升了校正性能，具有临床实用性和适应性
...
TL;DR: 基于前列腺活检切片的AI模型能预测根治性前列腺切除术后生化复发风险，在外部验证中表现良好，结合临床变量可改善术后预后分层。
...
Method: 使用STHLM3队列（n=676）的诊断性前列腺活检切片训练AI模型，采用基础模型和基于注意力的多实例学习，在三个外部RP队列（LEOPARD、CHIMERA、TCGA-PRAD）进行泛化性评估。

Result: 图像方法在三个外部队列中分别获得0.64、0.70和0.70的5年时间依赖性AUC；整合临床变量增加了互补预后价值并实现统计学显著的风险分层；相比指南推荐的CAPRA-S，AI模型能增量改善术后预后预测。

Conclusion: 基于活检训练的病理AI模型能跨标本类型泛化，支持术前和术后决策，但AI多模态方法相对于简单预测模型的附加价值需要在进一步研究中审慎评估。
...
Method: 引入BadDet+惩罚框架，使用log-barrier惩罚来抑制触发输入的真实类别预测。该机制在特定触发特征子空间内操作，实现位置和尺度不变性，并增强物理鲁棒性。

Result: 在真实世界基准测试中，BadDet+相比现有RMA和ODA基线实现了更优的合成到物理转移性能，同时保持干净数据性能。理论分析证实惩罚在触发特定特征子空间内可靠诱导攻击而不影响标准推理。

Conclusion: 结果突显了目标检测中的显著脆弱性，以及开发专门防御措施的必要性。BadDet+为理解目标检测后门攻击提供了统一框架和物理验证。
...
<details>
  <summary>Details</summary>
Motivation: 现有TAL方法在使用视觉语言模型时过度强调语言先验而牺牲视觉性能，导致明显的模态偏差。需要一种既能利用语言丰富语义，又能保持视觉主导地位的方法。

Method: 提出ActionVLM框架：1) 去偏重加权模块，估计语言相对于纯视觉预测的优势增量，并动态调整语言模态权重；2) 残差聚合策略，将语言视为补充细化而非主要驱动力。

Result: 在THUMOS14数据集上的实验表明，该模型比现有最优方法提升高达3.2% mAP，有效缓解了模态偏差，减少了语言先验的过度自信，并增强了时序推理能力。

Conclusion: ActionVLM通过保持视觉主导地位并自适应地利用语言信息，成功缓解了TAL中的模态偏差问题，为视觉语言模型在时序动作定位中的应用提供了有效框架。
...
Method: 提出SoT视觉CoT框架，训练统一的多模态自回归模型生成交错的文本计划和渲染的中间状态，通过2D投影实现渐进式形状组装，无需显式几何表示。创建SoT-26K数据集（基于CAD层次结构的接地组装轨迹）和T2S-CompBench基准。

Result: 在SoT-26K上微调后，在组件计数方面达到88.4%，在结构拓扑方面达到84.8%，比纯文本基线提高约20%。SoT建立了透明、过程监督的组合生成新范式。

Conclusion: SoT通过视觉思维链框架有效解决了文本到图像生成中的组合结构约束问题，实现了渐进式形状组装，为透明、过程监督的组合生成提供了新范式，在结构完整性和轨迹忠实度方面显著优于基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 微吻合手术需要极高精度，传统评估依赖专家主观判断，存在评分者间变异、标准不一、认知偏差和时间消耗等问题，需要客观、可靠、自动化的评估系统

Method: 提出包含四个核心组件的AI框架：1)基于YOLO的器械检测模块；2)基于DeepSORT的器械追踪模块；3)基于形状描述符的器械尖端定位模块；4)基于专家标注数据的监督分类模块
...
TL;DR: 提出SDCI框架，通过跨模型注意力融合、双向交叉图扩散优化和超像素协作预测，解决高分辨率遥感图像开放词汇语义分割中的几何定位和语义预测难题


<details>
  <summary>Details</summary>
Motivation: 高分辨率遥感图像具有地物分布密集、边界复杂的特点，对几何定位和语义预测要求更高。现有免训练的开放词汇语义分割方法通常采用单向注入和浅层后处理策略，难以满足这些需求

Method: 提出SDCI框架：1) 跨模型注意力融合模块，通过相互注入自注意力图指导协作推理；2) 双向交叉图扩散优化模块，通过迭代随机游走扩散增强双分支分割分数的可靠性；3) 基于凸优化的超像素协作预测机制，利用低级超像素结构细化对象边界

Result: 在多个遥感语义分割基准测试中，该方法比现有方法表现更好。消融研究进一步证实，利用超像素结构的传统基于对象的遥感图像分析方法在深度学习框架中仍然有效

Conclusion: SDCI框架通过空间正则化感知的双分支协作推理，有效解决了高分辨率遥感图像开放词汇语义分割的挑战，证明了传统超像素结构在深度学习中的持续价值
...
Method: 基于SD-Turbo的扩散框架，包含三个关键改进：1) 带卷积和注意力适配器的改进U-Net架构以建模几何线索；2) 使用张量分解和渐进加权的几何引导损失函数以正则化全局结构；3) 带噪声预测的优化采样策略以提高效率

Result: 在视觉保真度和定量性能上均优于现有方法，有效缓解水下场景的颜色失真，推进了水下成像增强的技术水平

Conclusion: GeoDiff-LF通过整合扩散先验和光场几何结构，为高质量水下4D光场成像提供了有效解决方案，代码将开源
...
TL;DR: GRDR提出了一种用于两阶段文本-视频检索的新型生成式召回方法，通过多语义ID分配和共享码本训练解决现有生成式检索的语义模糊和跨模态对齐问题，在保持准确性的同时显著降低存储和计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有两阶段文本-视频检索中，召回模型性能受限。生成式检索虽然具有恒定推理和存储复杂度，但存在语义模糊（每个视频只能分配单一语义ID）和跨模态不对齐（语义ID仅从视觉特征生成）的问题，影响召回质量。

Method: 提出GRDR框架：1）使用查询引导的多视图分词器为每个视频分配多个语义ID，提供多样化的语义访问路径；2）通过共享码本联合训练分词器和生成式检索器，使语义ID成为文本和视频之间的语义桥梁；3）推理时使用trie约束解码生成紧凑候选集，再由密集模型进行重排序。

Result: 在TVR基准测试中，GRDR在准确性上匹配强密集检索器，同时将索引存储降低一个数量级，在全库检索中加速高达300倍。

Conclusion: GRDR通过解决生成式检索的语义模糊和跨模态对齐问题，实现了高效且准确的文本-视频检索，为大规模实时应用提供了可行的解决方案。
...
TL;DR: Thinker是一个为具身智能设计的大型视觉语言基础模型，通过构建大规模机器人感知推理数据集和结合关键帧与完整视频序列的输入方法，解决了大视觉语言模型在机器人应用中存在的视角混淆和时序推理问题。


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型应用于机器人领域时存在两个主要问题：1）容易混淆第三人称和第一人称视角；2）在时序推理中倾向于忽略视频结尾的信息。这些问题对人类来说很简单，但对模型来说容易出错。

Method: 从两个角度解决问题：1）构建大规模机器人感知和推理数据集，包含自我视角视频、视觉定位、空间理解和思维链数据；2）提出简单有效的方法，通过联合输入关键帧和完整视频序列来大幅增强模型的视频理解能力。

Result: 该模型在任务规划领域两个最常用的基准数据集上取得了最先进的结果。

Conclusion: Thinker通过专门的数据集构建和创新的输入策略，有效解决了大视觉语言模型在具身智能应用中的视角混淆和时序推理问题，在机器人任务规划基准上表现出色。
...
TL;DR: LAMP：针对多模态大语言模型的黑盒通用对抗扰动攻击方法，通过注意力约束和跨图像传染机制实现高效攻击


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在多图像处理方面取得显著进展，但其安全漏洞尚未被充分探索。现有对抗攻击主要针对单图像场景且多为白盒攻击，在实际应用中不切实际。需要开发针对多图像MLLM的黑盒攻击方法。

Method: LAMP采用黑盒方法学习通用对抗扰动，包含三个关键技术：1）基于注意力的约束，阻止模型有效聚合跨图像信息；2）跨图像传染约束，使扰动标记影响干净标记，无需修改所有输入；3）索引注意力抑制损失，实现鲁棒的位置不变攻击。

Result: 实验结果表明，LAMP在多个视觉语言任务和模型上优于现有最先进基线方法，实现了最高的攻击成功率。

Conclusion: LAMP成功展示了多图像MLLM的脆弱性，为黑盒场景下的对抗攻击提供了有效解决方案，强调了多模态AI系统安全评估的重要性。
...
TL;DR: PTQ4ARVG：一种针对自回归视觉生成模型的训练后量化框架，通过增益投影缩放、静态令牌级量化和分布引导校准解决量化中的三大挑战，在8位和6位量化下保持性能。


<details>
  <summary>Details</summary>
Motivation: 自回归视觉生成模型在保持与语言模型兼容架构的同时达到扩散模型性能，但现有量化方法无法有效应用于ARVG模型。论文识别出三大挑战：通道级严重异常值、令牌级高度动态激活和样本级分布信息不匹配，需要专门解决方案。
...
Result: 实验表明PTQ4ARVG能有效将ARVG家族模型量化为8位和6位，同时保持有竞争力的性能。代码已开源。

Conclusion: PTQ4ARVG成功解决了ARVG模型量化中的三大关键挑战，为自回归视觉生成模型的部署提供了高效实用的量化解决方案，无需训练即可实现高性能低比特量化。
...
TL;DR: NFCDS通过控制反向扩散噪声的频率成分来改善扩散采样PnP方法的保真度-感知权衡，低通噪声导致模糊而高通噪声促进细节生成，通过傅里叶域滤波抑制低频噪声来提升数据保真度。
...
Method: 提出噪声频率控制扩散采样(NFCDS)，这是一种反向扩散噪声的频谱调制机制。基于噪声频率分析：低频成分导致模糊并降低保真度，而高频成分驱动细节生成。设计傅里叶域滤波器，逐步抑制低频噪声并保留高频内容，将数据一致性先验直接注入采样过程。

Result: NFCDS能够快速收敛到既具有高保真度又具有感知说服力的结果，无需额外训练。作为PnP模块，它可以无缝集成到现有的基于扩散的修复框架中，并在多种零样本任务中改善保真度-感知平衡。

Conclusion: NFCDS通过控制反向扩散噪声的频率成分，从根本上解决了扩散采样PnP方法中的保真度-感知权衡问题，提供了一种无需训练即可提升数据保真度的方法，同时保持高感知质量。
...
<details>
  <summary>Details</summary>
Motivation: 自监督学习中防止表示坍塌是一个持续挑战，现有方法通常依赖全局正则化（如最大化距离、去相关维度或强制特定分布），但作者认为需要更直接的解决方案

Method: 将表示学习重新解释为离散填充问题，通过保持单射性来保留信息。Hypersolid方法使用短程硬球排斥来防止局部碰撞，创建高分离度的几何机制

Result: 该方法能够保留增强多样性，在细粒度和低分辨率分类任务上表现优异

Conclusion: 通过将表示学习重新概念化为离散填充问题并使用局部排斥约束，Hypersolid提供了一种防止表示坍塌的有效方法，特别适用于需要保留细微差异的任务
...
Method: 整合FLAME参数化建模与3DGS神经渲染，仅传输关键面部元数据；提出紧凑表示和压缩方案，包括高斯属性压缩和MLP优化，提升传输效率

Result: 实验结果表明该方法在极低比特率下实现优越的率失真性能，提供高质量面部渲染，适用于实时3D视频会议应用

Conclusion: 该框架成功解决了3D说话人脸高保真表示与低比特率传输的挑战，为实时沉浸式通信提供了高效解决方案
...
TL;DR: 该论文提出了首个地理定位推理链基准测试，专注于GeoGuessr游戏中的全球位置预测任务，发现虽然大型闭源VLM在位置预测上可与人类专家媲美，但在生成可审计推理链方面仍落后于人类专家。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型（VLM）在地理位置预测方面表现出色，但在解释其预测依据方面存在严重缺陷，经常产生幻觉场景属性来支持其位置预测。需要建立一个基准来评估VLM生成地理定位推理链的能力。

Method: 1. 创建首个地理定位推理链基准测试，专注于GeoGuessr游戏的全球位置预测任务；2. 与专家GeoGuessr玩家（包括世界冠军）合作，为500个查询场景生成800个真实推理链；3. 评估LLM-as-a-judge和VLM-as-a-judge策略对VLM生成推理链的评分效果；4. 比较闭源VLM（Gemini、GPT-5）和开源VLM（Llama、Qwen）在基准测试上的表现。

Result: 1. Qwen 3 LLM-as-a-judge评分策略与人类评分相关性最佳；2. 大型闭源VLM在位置预测上可与人类专家媲美，但在生成可审计推理链方面仍落后；3. 开源VLM在基准测试上表现灾难性失败，仅略优于基于位置信息但无视觉输入的幻觉基线；4. 揭示了VLM在从高分辨率图像中提取细粒度视觉属性方面的局限性。

Conclusion: 人类专家与VLM在该任务上的差距表明VLM在从高分辨率图像中提取细粒度视觉属性方面存在局限性。需要开发能够生成可靠、可审计推理链的VLM，这对于地理定位等需要透明决策的应用至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 现有抑郁症检测的深度学习方法大多依赖有限特征类型，忽视显式跨模态交互，且使用简单的拼接或静态加权融合，限制了性能提升。
...
TL;DR: WorldBench是一个新的视频基准测试，专门用于概念特定、解耦的评估，能够隔离和评估对单个物理概念或定律的理解，填补了现有物理基准测试中多概念纠缠的缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有的基于物理的视频基准测试存在概念纠缠问题，即单个测试同时评估多个物理定律和概念，这从根本上限制了其诊断能力。为了可靠地部署生成基础模型（世界模型）到机器人规划和自主系统训练等关键任务中，需要能够精确评估模型物理保真度的工具。

Method: 设计了WorldBench基准测试，在两个不同层次上构建：1）评估直观物理理解，如物体持久性或尺度/透视等概念；2）评估低级物理常数和材料属性，如摩擦系数或流体粘度。该基准通过概念特定的解耦评估，能够一次只评估单个物理概念或定律。

Result: 当在WorldBench上评估最先进的基于视频的世界模型时，发现所有测试模型在特定物理概念上都存在失败模式，缺乏生成可靠真实世界交互所需的物理一致性。这表明当前模型在物理推理能力上存在系统性的不足。

Conclusion: WorldBench通过其概念特定的评估，为视频生成和世界模型的物理推理能力提供了更细致、可扩展的评估框架，为开发更稳健和可泛化的世界模型驱动学习铺平了道路。
...
TL;DR: 提出Mam-App模型，一种基于Mamba架构的轻量级苹果叶病分类模型，在保持高精度的同时大幅减少参数量至0.051M，适用于资源受限设备部署。


<details>
  <summary>Details</summary>
Motivation: 全球人口增长和技术进步加剧了粮食需求，苹果作为重要水果面临病害导致的产量损失问题。现有深度学习模型虽然性能优异但参数量大，不适合资源受限环境；轻量级模型又存在性能下降问题，需要在效率和性能之间取得平衡。

Method: 提出Mam-App模型，基于Mamba架构设计参数高效的叶病分类模型。该模型通过创新的架构设计在保持高性能的同时大幅减少参数数量，并在PlantVillage苹果叶病数据集上进行训练和评估。

Result: 在苹果叶病数据集上达到99.58%准确率、99.30%精确率、99.14%召回率和99.22% F1分数，仅使用0.051M参数。在玉米叶病数据集上达到99.48%准确率、99.20%精确率、99.34%召回率和99.27% F1分数。在马铃薯叶病数据集上达到98.46%准确率、98.91%精确率、95.39%召回率和97.01% F1分数。

Conclusion: Mam-App模型在保持竞争性性能的同时显著减少了参数量，解决了深度学习模型在效率和性能之间的权衡问题，适合部署在无人机、移动设备等资源受限平台，并展示了良好的泛化能力。
...
TL;DR: LANE通过紧凑的自回归依赖和AdaGraph策略，在3D网格生成中实现了6倍序列长度提升和显著加速，解决了现有方法资源利用率不足、推理慢、细节表达受限的问题。
...
Method: 提出Latent Autoregressive Network (LANE)，在生成过程中引入紧凑的自回归依赖，使可生成序列长度提升6倍。进一步提出Adaptive Computation Graph Reconfiguration (AdaGraph)策略，通过生成过程中的时空解耦来克服传统串行推理的效率瓶颈。

Result: 实验验证表明，LANE在生成速度、结构细节和几何一致性方面均表现出优越性能，为高质量3D网格生成提供了有效解决方案。

Conclusion: LANE通过紧凑的自回归依赖和AdaGraph策略，显著提升了3D网格生成的序列长度、推理速度和细节表达能力，为高质量3D内容生成提供了创新解决方案。
...
TL;DR: 视觉基础模型在离散采样的图像上训练，本文研究其表征是否编码了训练数据背后的连续疾病进展过程。通过扩散伪时间分析发现，病理学专用模型能有效恢复疾病进展轨迹，且轨迹保真度与少样本分类性能高度相关。


<details>
  <summary>Details</summary>
Motivation: 当前视觉基础模型在分类基准上表现良好，但尚不清楚其表征是否编码了训练数据背后的连续生物学过程。在计算病理学中，能够隐式捕捉连续疾病进展的模型可能更好地反映底层生物学、支持更稳健的泛化，并实现对疾病转变相关特征的定量分析。

Method: 使用扩散伪时间（一种从单细胞转录组学推断发育轨迹的方法）来探测基础模型是否在表征空间中沿着连贯的进展方向组织疾病状态。研究覆盖四种癌症进展和六个模型，分析模型恢复轨迹排序的能力，并评估轨迹保真度与少样本分类性能的相关性。

Result: 所有病理学专用模型恢复的轨迹排序显著超过零基线，其中纯视觉模型达到最高保真度（CRC-Serrated上τ>0.78）。模型在参考疾病上的轨迹保真度排名能强预测在保留疾病上的少样本分类性能（ρ=0.92）。探索性分析显示细胞类型组成沿着推断轨迹平滑变化，模式与已知的基质重塑一致。
...
<details>
  <summary>Details</summary>
Motivation: 现有类增量学习方法通常冻结预训练模型并训练轻量级任务特定适配器，但这种方法存在两个主要问题：1）维护任务特定参数阻碍知识迁移并增加检索成本；2）简单的参数融合会导致破坏性干扰和灾难性遗忘
...
Conclusion: DAF通过理论推导的动态融合机制构建单一全局适配器，有效解决了类增量学习中的知识迁移和灾难性遗忘问题，在多个基准测试中达到SOTA性能
...
TL;DR: SGDS通过语义引导的动态稀疏化在激活空间中塑造类特定稀疏子空间，缓解类增量学习中的干扰问题，无需对参数空间施加刚性约束。


<details>
  <summary>Details</summary>
Motivation: 现有类增量学习方法通常冻结预训练模型并使用轻量适配器，并通过参数正交化防止任务间干扰。但作者认为这种参数约束方法会损害模型的可塑性。

Method: 提出语义引导的动态稀疏化(SGDS)，通过定向稀疏化主动引导激活空间的方向和秩。SGDS鼓励相似类共享紧凑的激活子空间以促进知识迁移，同时为不相似类分配不重叠的激活子空间以防止干扰。

Result: 在多个基准数据集上的广泛实验表明，SGDS实现了最先进的性能。

Conclusion: 通过在激活空间中塑造类特定稀疏子空间，SGDS能有效缓解干扰问题，同时避免对参数空间施加刚性约束，从而在保持可塑性的同时实现更好的类增量学习性能。
...
TL;DR: 本文提出GIQT（几何诱导查询-键变换）方法，通过显式校正相似性空间来解决空中-地面行人重识别中的极端视角和距离差异问题，而不是修改特征表示或注意力机制本身。


<details>
  <summary>Details</summary>
Motivation: 空中-地面行人重识别面临极端视角和距离差异的根本挑战，导致严重的几何畸变，并使跨视角共享相似性空间的假设失效。现有方法主要依赖几何感知特征学习或外观条件提示，但隐含假设几何不变的dot-product相似性在大视角和尺度变化下仍然可靠。作者认为这一假设不成立，极端相机几何会系统性地扭曲查询-键相似性空间并降低基于注意力的匹配性能。

Method: 提出Geometry-Induced Query-Key Transformation (GIQT)，一个轻量级的低秩模块，通过将查询-键交互条件化于相机几何来显式校正相似性空间。GIQT不修改特征表示或注意力公式本身，而是调整相似性计算以补偿主导的几何诱导各向异性畸变。基于局部相似性校正，进一步结合几何条件提示生成机制，直接从相机几何中提取全局、视图自适应的表示先验。

Result: 在四个空中-地面行人重识别基准测试上的实验表明，所提出的框架在极端和先前未见过的几何条件下持续提高了鲁棒性，同时与最先进方法相比引入了最小的计算开销。

Conclusion: 通过显式校正相似性空间而非仅依赖特征对齐，GIQT方法有效解决了空中-地面行人重识别中的几何畸变问题，为极端视角和距离差异下的跨视角匹配提供了更可靠的解决方案。
...
TL;DR: UniMRG通过让统一多模态模型生成图像的像素、深度和分割等多种内在表示，来增强其理解能力，实现理解与生成的相互促进。


<details>
  <summary>Details</summary>
Motivation: 当前统一多模态模型（UMMs）主要关注利用理解能力来增强生成能力，但利用生成来提升理解的研究仍然不足。本文旨在探索通过生成任务来增强UMMs的理解能力。

Method: 提出UniMRG（统一多表示生成），这是一种架构无关的后训练方法。该方法训练UMMs生成输入图像的多种内在表示：像素（重建）、深度（几何）和分割（结构），同时结合标准的视觉理解目标。

Result: 实验表明，该方法显著提升了UMMs的细粒度感知能力，减少了幻觉现象，改善了空间理解，同时增强了生成能力。这些改进在不同UMM架构上均得到验证。

Conclusion: 通过让UMMs生成多种图像内在表示，可以捕获外观、空间关系和结构布局的互补信息，从而获得更深入、更全面的视觉理解，实现理解与生成的相互促进。
...
TL;DR: 提出一种检测AI生成视频的双路径框架，通过静态流形偏差分支捕获空间异常，通过微时间波动分支检测时序计算指纹


<details>
  <summary>Details</summary>
Motivation: 尽管当前视频生成模型（如Veo、Wan）的视觉质量已很高，宏观语义错误和时间不一致性不再明显，但AI生成视频本质上是流形拟合过程的产物而非物理记录，其像素组成逻辑在连续相邻帧残差中表现出结构化同质特征，这种"流形投影波动"现象为检测提供了理论基础。

Method: 提出分层双路径框架作为顺序过滤过程：1）静态流形偏差分支利用大规模视觉基础模型的精细感知边界捕获偏离自然真实世界流形的空间异常或物理违规；2）微时间波动分支作为次级精细过滤器，分析即使在视觉完美序列中仍持续存在的结构化MPF特征。

Result: 该框架能够检测伪造视频，无论其表现为全局真实世界流形偏差还是细微的计算指纹，确保高保真视频即使成功驻留在流形上并逃避空间检测，也能通过时序分析被暴露。

Conclusion: AI生成视频与真实视频在像素组成逻辑上存在本质差异，通过流形投影波动现象和分层双路径检测框架，可以有效区分真实视频与最先进的高保真伪造内容。
...
TL;DR: 该研究揭示了隐式神经辐射场在密集自遮挡场景中的几何退化问题，提出了基于显式几何的解决方案以提升实例恢复率。


<details>
  <summary>Details</summary>
Motivation: NeRF在多视图重建中表现出色，但在密集自遮挡场景中的定量3D分析可靠性尚未明确。研究者发现隐式密度场在严重遮挡下存在根本性失效模式，导致实例计数不足。

Method: 通过合成数据集实验验证隐式方法的局限性，提出基于稀疏体素栅格化的显式几何流程，从SfM特征几何初始化，将2D实例掩码投影到显式体素网格，通过递归分割强制几何分离。

Result: 实验显示：1) 最先进的掩码监督NeRF在密集场景中实例恢复率约89%；2) 提出的显式方法恢复率达95.8%；3) 在分割掩码质量下降时，显式方法比隐式基线多恢复43%的实例。

Conclusion: 显式几何先验是高度自遮挡3D场景中可靠定量分析的必要条件，隐式密度场在严重遮挡下会重建空心或碎片化结构而非实体内部，导致系统性的实例计数不足。
...
TL;DR: 该论文提出了一种将单模态数据集转换为多模态数据集的方法，利用多模态大语言模型生成合成图像描述，以在多模态目标下微调模型，从而弥合预训练与微调之间的模态差距。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习存在预训练与微调之间的模态不匹配问题：预训练已从单模态转向多模态学习，但微调仍主要保持单模态，限制了预训练丰富表示的优势。
...
Conclusion: 该工作建立了一种新的数据集增强范式，有效弥合了多模态预训练与微调之间的差距，为利用多模态预训练表示进行下游任务提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 度量学习对检索任务至关重要，但其对嵌入几何和优化动态的影响尚未得到充分理解。需要系统分析不同损失函数如何影响嵌入表示的质量和检索性能。
...
Result: Triplet和SCL保持更高的类内方差和更清晰的类间边界，在细粒度设置中实现更强的top-1检索性能。Contrastive和InfoNCE通过许多小更新快速压缩嵌入表示，加速收敛但可能过度简化类别结构。N-pair实现较大的平均分离但间距不均匀。

Conclusion: 揭示了效率-粒度权衡关系：当需要保持多样性和难样本区分时，应优先选择Triplet/SCL；当需要更快嵌入压缩时，应选择Contrastive/InfoNCE。这些见解为度量学习损失函数的选择提供了实用指导。
...
<details>
  <summary>Details</summary>
Motivation: 视频世界建模结合视觉语言预训练为机器人学习提供了新的独立基础。视频世界模型能够通过理解动作与视觉动态之间的因果关系来预测近未来，这启发了作者开发一个能够同时学习帧预测和策略执行的框架。

Method: 提出LingBot-VA自回归扩散框架，包含三个关键设计：(1) 共享潜在空间，通过混合Transformer架构整合视觉和动作标记；(2) 闭环滚动机制，持续获取带真实观测的环境反馈；(3) 异步推理管道，并行化动作预测和运动执行以实现高效控制。

Result: 在仿真基准和真实世界场景中评估，模型在长时程操作、后训练数据效率和面对新配置的强泛化能力方面显示出显著潜力。

Conclusion: 视频世界建模与视觉语言预训练相结合为机器人学习提供了有前景的新范式，LingBot-VA框架通过其创新设计在多个维度上展示了优越性能，代码和模型已开源以促进社区发展。
...
TL;DR: RT-DeepLoc是一种基于重建的弱监督时序深度伪造定位框架，利用仅使用真实数据训练的MAE来识别伪造片段，并通过非对称视频内对比损失增强定位性能。
...
Method: 提出RT-DeepLoc框架：1）使用仅在真实数据上训练的掩码自编码器学习内在时空模式，通过重建误差识别伪造片段；2）引入非对称视频内对比损失，专注于真实特征的紧凑性，建立稳定的决策边界。
...
TL;DR: SimGraph：基于场景图的统一图像生成与编辑框架，通过结构化场景表示实现对象关系和空间布局的精确控制


<details>
  <summary>Details</summary>
Motivation: 当前生成式AI方法将图像生成和编辑任务分开处理，导致效率低下且难以保持空间一致性和语义连贯性。主要障碍是缺乏对对象关系和空间排列的结构化控制。

Method: 提出SimGraph统一框架，集成基于场景图的图像生成和编辑。框架结合基于token的生成和基于扩散的编辑，在单一场景图驱动模型中实现高质量一致结果。

Result: 通过大量实验证明，该方法在图像生成和编辑任务上优于现有最先进方法，实现了更好的空间一致性和语义连贯性。

Conclusion: SimGraph通过场景图结构化表示成功统一了图像生成和编辑，为生成式AI提供了更精确的对象交互、布局和空间一致性控制。
...
TL;DR: 该论文提出了一种不确定性感知扩散桥模型（UDBM），通过将全合一图像恢复重新定义为由像素级不确定性引导的随机传输问题，解决了异构退化之间优化目标冲突的挑战。


<details>
  <summary>Details</summary>
Motivation: 全合一图像恢复（AiOIR）面临的核心挑战是如何协调异构退化之间的冲突优化目标。现有方法通常受限于粗粒度控制机制或固定映射调度，导致次优适应。需要一种能够建模退化不确定性并解决标准扩散桥中固有漂移奇异性的方法。

Method: 提出不确定性感知扩散桥模型（UDBM）：1）引入松弛扩散桥公式，用松弛约束替代严格终端约束，建模退化不确定性并理论上解决扩散桥的漂移奇异性；2）设计双重调制策略：噪声调度将多样退化对齐到共享高熵潜在空间，路径调度基于熵正则化的粘性动力学自适应调节传输轨迹。

Result: 通过有效修正传输几何和动力学，UDBM在单一推理步骤内实现了跨多种恢复任务的最先进性能。

Conclusion: UDBM通过将AiOIR重新定义为随机传输问题，并引入松弛扩散桥公式和双重调制策略，成功解决了异构退化间的优化冲突，实现了高效的全合一图像恢复。
...
TL;DR: HydroSense是一个低成本物联网水质监测系统，集成pH、溶解氧、温度、总溶解固体、氮估算和水位六参数，采用双微控制器架构，成本降低85%，性能接近商用系统。


<details>
  <summary>Details</summary>
Motivation: 全球水危机需要经济实惠、准确且实时的水质监测解决方案。传统依赖人工采样或昂贵商用系统的方法无法解决资源受限环境中的可及性问题。

Method: 提出HydroSense物联网框架，采用双微控制器架构：Arduino Uno用于精密模拟测量和五点校准算法，ESP32用于无线连接、边缘处理和云集成。应用先进信号处理技术，包括TDS测量的中值滤波、温度补偿算法和鲁棒错误处理。

Result: 90天实验验证显示优异性能：pH精度±0.08单位（0-14范围），DO稳定性±0.2 mg/L，TDS精度±1.9%（0-1000 ppm），云数据传输可靠性99.8%。总成本32,983 BDT（约300美元），相比商用系统降低85%。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型图像水印评估方法存在多个严重缺陷：缺乏统一的残差和语义水印评估框架、结果缺乏可解释性、忽视全面的安全考量、以及对语义水印使用不恰当的评估指标。

Method: 提出WMVLM框架，重新定义两类水印的质量和安全指标：残差水印评估伪影强度和擦除抵抗性，语义水印评估潜在分布偏移。采用三阶段训练策略，逐步实现分类、评分和可解释文本生成能力。

Result: 实验表明WMVLM在多个数据集、扩散模型和水印方法上优于现有最先进的视觉语言模型，展现出强大的泛化能力。

Conclusion: WMVLM为扩散模型图像水印提供了首个统一、可解释的评估框架，解决了现有评估方法的局限性，为水印算法开发提供了更准确、全面的评估工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在计算病理学中通常直接输出诊断结论而缺乏可验证的证据链推理，这严重限制了临床信任并阻碍专家错误纠正。需要构建能够提供透明、临床基础推理能力的系统。

Method: 1) 构建PathReasoner数据集：利用医学知识图谱，显式对齐结构化病理发现、临床推理与诊断，生成超过20K高质量教学样本；2) 开发PathReasoner-R1模型：结合轨迹掩码监督微调与推理导向强化学习，融入知识感知多粒度奖励函数和严格对齐知识图谱的实体奖励机制。

Result: PathReasoner-R1在PathReasoner数据集和公共基准测试中均达到最先进性能，在不同图像尺度上表现优异，为病理模型提供了透明、临床基础的推理能力。

Conclusion: PathReasoner通过构建首个大规模WSI推理数据集和开发PathReasoner-R1模型，成功解决了当前病理AI系统缺乏可验证推理的问题，显著提升了临床信任度和专家纠错能力，推动了计算病理学向透明推理方向发展。
...
TL;DR: 该研究探讨了不同视觉模型在训练过程中是否收敛到相同的中间表示和操作，而不仅仅是最终表示相似。通过量化不同模型在不同处理阶段的表示距离，发现虽然相似位置的层具有最相似的表示，但不同模型间仍存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 现有文献表明，模型越大越可能收敛到相似的"通用"表示，但作者想知道不同模型是否也收敛到相同的中间步骤和操作。他们希望研究导致不同模型收敛到相似表示的具体过程，而不仅仅是最终表示的相似性。

Method: 首先量化不同模型在不同处理阶段的表示距离，然后跟踪整个处理过程中模型间距离的演化，识别出不同模型间差异最大的处理步骤。通过这种方法分析不同模型（如分类器模型、CNN和transformer模型）在表示处理上的差异。

Result: 研究发现：1）相似位置的层具有最相似的表示，但仍存在显著差异；2）分类器模型与其他模型不同，会在最后几层丢弃关于低层次图像统计信息；3）CNN和transformer模型表现不同，transformer模型在层间表示变化更平滑。这些差异揭示了模型表示收敛的水平和性质。

Conclusion: 该研究澄清了模型表示之间收敛的水平和性质，使得对图像模型底层过程能够进行更定性的描述。不同模型在达到相似最终表示的过程中，其内部操作和中间步骤存在系统性差异，这有助于更深入地理解视觉模型的内部工作机制。
...
TL;DR: 论文指出当前潜在扩散模型中自编码器评估存在系统性偏差：过度强调生成友好性指标（gFID）而忽视重建质量，这在大规模可控扩散任务中会导致条件漂移和可控性下降。


<details>
  <summary>Details</summary>
Motivation: 研究发现当前ImageNet规模的自编码器评估存在系统性偏差，过度偏向生成友好性指标（如低gFID），而重建指标被低估。这种偏差在可控扩散任务中会引发条件漂移问题，限制条件对齐效果，因此需要重新审视自编码器评估标准。

Method: 1. 理论分析gFID主导偏好的成因及其在可控扩散中的风险；2. 提出多维度条件漂移评估协议，反映可控生成任务需求；3. 实证研究多个近期ImageNet自编码器，分析不同评估指标与可控性的关系；4. 通过ControlNet实验验证可控性与条件保持而非gFID的相关性。

Result: 研究发现：1. gFID仅能弱预测条件保持能力；2. 重建导向指标（特别是实例级测量）与可控性显著更相关；3. 在可控生成任务中，条件漂移问题会限制条件对齐效果；4. ControlNet实验证实可控性主要取决于条件保持而非gFID。

Conclusion: 当前以ImageNet为中心的AE评估与可扩展可控扩散的需求存在差距，需要更平衡的评估框架。重建保真度（特别是实例级指标）比gFID更能预测可控性，为可靠的基准测试和模型选择提供了实用指导。
...
TL;DR: 提出RSGround-R1框架，通过推理引导、位置感知的后训练方法增强多模态大语言模型在遥感视觉定位任务中的空间理解能力


<details>
  <summary>Details</summary>
Motivation: 遥感视觉定位任务中，由于遥感场景空间尺度大、语义模糊性高，自然语言描述往往严重依赖位置线索，这对多模态大语言模型的空间推理能力提出了独特挑战

Method: 提出RSGround-R1框架：1) 使用合成生成的RSVG推理数据进行思维链监督微调，建立显式位置感知；2) 应用强化微调，结合新设计的位置奖励函数提供连续、距离感知的定位指导；3) 引入空间一致性引导优化方案，基于空间连贯性动态调整策略更新

Result: 在RSVG基准测试上的广泛实验表明，该方法展现出优越的性能和泛化能力

Conclusion: 提出的推理引导、位置感知后训练框架能有效增强多模态大语言模型在遥感视觉定位任务中的空间理解能力，实现准确稳定的目标定位
...
<details>
  <summary>Details</summary>
Motivation: 现有OCR方法主要关注从图像或扫描文档中识别文本元素（文本中心OCR），而忽略了从视觉信息密集的图像源（如图表、网页和科学图表）中识别视觉元素（视觉中心OCR）。这些视觉密集图像在互联网上广泛存在，具有重要的实际应用价值，如数据可视化和网页分析。

Method: 1. 构建全面的数据工程，覆盖文本中心文档（报纸、杂志、书籍）和视觉中心渲染复合体（图表、网页、科学图表）；2. 提出两阶段SFT-RL多域训练方法：SFT直接混合跨域数据训练建立初始域知识，RL阶段为每个域设计个性化奖励策略，根据各域特点定制灵活的奖励信号。

Result: 实验结果表明OCRVerse的有效性，在文本中心和视觉中心数据类型上均取得有竞争力的结果，甚至可与大规模开源和闭源模型相媲美。

Conclusion: OCRVerse是首个端到端的统一文本中心OCR和视觉中心OCR方法，通过创新的数据工程和两阶段训练策略，成功解决了传统OCR方法在处理视觉密集图像时的局限性，为多模态数据处理提供了更全面的解决方案。
...
TL;DR: 通过少量样本域适应、空间静态先验知识和夏季参考图像，将冰川崩解前缘分割在新研究点的误差从1131.6米降至68.7米，实现了深度学习模型从基准测试到实际应用的转化。


<details>
  <summary>Details</summary>
Motivation: 现有最先进的冰川崩解前缘分割模型在基准测试中表现接近人类水平，但在实际新研究点应用中精度不足，无法满足后续科学分析需求。这主要是因为新研究点属于训练数据分布之外的域，模型泛化能力有限。

Method: 采用少量样本域适应策略，结合空间静态先验知识，并在输入时间序列中包含夏季参考图像。这些方法不需要修改模型架构，而是通过数据增强和先验知识整合来提升模型在新域的表现。

Result: 通过提出的方法，冰川崩解前缘的分割误差从1131.6米显著降低到68.7米，实现了近16.5倍的精度提升。这使得模型能够满足实际科学分析的需求。

Conclusion: 建立了一个将深度学习冰川崩解前缘分割应用于新研究点的框架，为全球尺度的冰川崩解前缘监测提供了可行方案。该方法展示了如何通过域适应策略将基准测试模型转化为实际应用工具。
...
TL;DR: 本文提出RegName，一个轻量级的几何感知正则化框架，通过约束中间嵌入的几何结构来解决多模态学习中的表示几何病理问题，包括模态内表示坍塌和样本级跨模态不一致性。


<details>
  <summary>Details</summary>
Motivation: 多模态学习旨在整合异构模态的互补信息，但仅靠强优化不能保证良好结构的表示。即使在精心平衡的训练方案下，多模态模型仍常表现出几何病理，包括模态内表示坍塌和样本级跨模态不一致性，这会降低单模态鲁棒性和多模态融合效果。

Method: 提出RegName框架，包含两个互补约束：1) 模态内分散正则化，促进表示多样性；2) 模态间锚定正则化，在不进行刚性对齐的情况下限制样本级跨模态漂移。该正则器是即插即用的，无需架构修改，兼容多种训练范式。

Result: 在多个多模态基准测试上的广泛实验表明，该方法在多模态和单模态性能上均取得一致改进，证明显式调节表示几何能有效缓解模态权衡问题。

Conclusion: 表示几何是多模态学习中缺失的控制轴，通过几何感知正则化框架可以缓解模态内表示坍塌和跨模态不一致性问题，提升模型性能。
...
TL;DR: MVSC通过将3D sMRI压缩为2D视觉替代特征，与冻结的2D基础模型对齐，用于阿尔茨海默病分类，解决了现有方法的计算成本高、跨切片关系丢失和特征提取能力有限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有sMRI表示学习方法存在三个主要问题：3D架构计算成本高，切片级特征提取方法丢失跨切片关系，以及使用2D基础模型进行训练无关特征提取的能力有限。需要一种能平衡计算效率、保留三维上下文并提取判别性特征的方法。

Method: 提出多模态视觉替代压缩(MVSC)，包含两个关键组件：1) 体积上下文编码器，在文本引导下捕获全局跨切片上下文；2) 自适应切片融合模块，以文本增强的逐块方式聚合切片级信息。该方法学习将大型3D sMRI体积压缩为紧凑的2D视觉替代特征，与冻结的2D基础模型对齐以提取强大表示。

Result: 在三个大规模阿尔茨海默病基准测试上的广泛实验表明，MVSC在二元和多类分类任务上均优于最先进方法。

Conclusion: MVSC通过将3D sMRI压缩为2D视觉替代特征并与2D基础模型对齐，有效解决了现有方法的局限性，在阿尔茨海默病诊断中取得了优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有图表编辑方法大多采用基于流水线的设计，依赖自然语言或代码作为中间表示，限制了执行复杂编辑的能力。需要直接评估端到端图表编辑能力，而不依赖中间表示。

Method: 提出ChartE³基准，包含两个互补的编辑维度：局部编辑（字体、颜色等细粒度外观变化）和全局编辑（数据过滤、趋势线添加等整体数据中心转换）。通过精心设计的数据流水线和人工策划构建了1200多个高质量样本，每个样本包含图表图像、底层代码和多模态编辑指令。

Result: 对最先进的多模态大语言模型进行广泛基准测试，揭示了显著的性能差距，特别是在全局编辑任务上，突显了当前端到端图表编辑能力的关键局限性。

Conclusion: ChartE³基准直接评估端到端图表编辑能力，无需中间表示，为评估模型在细粒度控制和全局结构一致性方面的能力提供了新标准，揭示了当前模型在复杂图表编辑任务上的局限性。
...
Method: 采用两阶段范式：1）通过将参考外观和运动线索融合到统一潜在空间，弥合输入模态差距，使模型能够利用基础模型的生成先验共同推理空间身份和时间动态；2）引入自引导数据合成流程，策划伪跨身份训练对，实现从姿态依赖控制到直接端到端RGB驱动动画的无缝过渡。

Result: DreamActor-M2实现了最先进的性能，在AW Bench（涵盖广泛角色类型和运动场景的通用基准）上的广泛实验表明，该方法能提供卓越的视觉保真度和强大的跨域泛化能力。

Conclusion: DreamActor-M2通过重新构想运动条件化为上下文学习问题，解决了角色动画中的关键挑战，实现了身份保持与运动一致性的更好平衡，并显著增强了对多样化角色和运动场景的泛化能力。
...
TL;DR: 提出GMC方法，通过粒度调制器和分布调节器提供细粒度IQA性能分析，超越传统全局相关指标


<details>
  <summary>Details</summary>
Motivation: 传统IQA评估主要依赖PLCC和SRCC等全局相关指标，这些指标将性能简化为单一标量，无法捕捉局部质量谱上的排名一致性变化，且对测试样本质量分布敏感

Method: 提出粒度调制相关(GMC)方法，包含：1)粒度调制器-基于绝对MOS值和成对MOS差异应用高斯加权相关来检查局部性能变化；2)分布调节器-正则化相关以减少非均匀质量分布的偏差；生成相关表面作为MOS和|ΔMOS|的联合函数

Result: 在标准基准测试中，GMC揭示了标量指标无法看到的性能特征，提供了更丰富可靠的IQA模型分析、比较和部署范式

Conclusion: GMC通过结构化细粒度分析解决了传统全局相关指标的局限性，为IQA性能评估提供了更全面和稳定的评估框架
...
TL;DR: DGNav提出动态拓扑导航框架，通过场景感知自适应策略和动态图变换器解决VLN-CE中拓扑规划的"粒度刚性"问题，实现按需密度调整和噪声过滤。


<details>
  <summary>Details</summary>
Motivation: 现有基于拓扑地图的视觉语言导航方法存在"粒度刚性"问题：依赖固定几何阈值采样节点，无法适应环境复杂度变化。这导致简单区域过采样（计算冗余）和高不确定性区域欠采样（碰撞风险增加、精度下降）。

Method: 提出DGNav框架，包含两个核心创新：1) 场景感知自适应策略：基于预测路径点离散度动态调整图构建阈值，实现"按需致密化"；2) 动态图变换器：融合视觉、语言和几何线索到动态边权重中，重构图连接性，过滤拓扑噪声。

Result: 在R2R-CE和RxR-CE基准测试中，DGNav展现出优越的导航性能和强大的泛化能力。消融研究证实该框架在导航效率和安全探索之间实现了最优权衡。

Conclusion: DGNav通过动态调节地图密度和连接性，有效解决了VLN-CE中拓扑规划的"粒度刚性"问题，为连续环境中的视觉语言导航提供了更灵活、高效的解决方案。
...
Method: 采用三阶段流水线：1)大规模数据收集与标准化；2)链式思维原理生成；3)基于推理质量和难度感知的全面筛选。使用Qwen3-VL-235B-A22B-Thinking蒸馏高质量推理标注，并基于MMFineReason微调Qwen3-VL-Instruct得到不同规模模型。
...
Method: 将扩散过程重新解释为在结构化潜在空间中的随机轨迹演化。通过设计初始噪声及其几何对齐，使背景生成自然避开指定前景区域。引入缓存的风格方向作为潜在空间中的持久向量，将风格控制与文本条件解耦，确保跨页面的一致性

Result: 实现了无需训练、与现有扩散模型兼容的前景保留背景生成，在多页面文档中保持视觉连贯性和风格一致性，无需重复提示指定风格
...
TL;DR: 该论文从优化角度重新解释分类器无关引导（CFG），将其视为流匹配中平滑距离函数梯度的近似，并提出基于流形约束同伦优化的改进方法，通过增量梯度下降和Anderson加速提升生成质量与稳定性。


<details>
  <summary>Details</summary>
Motivation: 尽管CFG在扩散模型和流模型中广泛用于可控生成并取得经验成功，但其基于启发式线性外推的方法对引导尺度敏感。现有方法缺乏理论依据，需要更稳定、高效的引导机制。

Method: 1) 从优化角度解释CFG：证明流匹配中的速度场对应平滑距离函数的梯度；2) 将CFG采样重新表述为带流形约束的同伦优化问题；3) 通过增量梯度下降实现流形投影；4) 引入Anderson加速提升计算效率和稳定性，无需额外模型评估。

Result: 方法在DiT-XL-2-256、Flux和Stable Diffusion 3.5等大规模模型上验证，在多样化基准测试中显著提升生成保真度、提示对齐和引导尺度鲁棒性，且无需额外训练。

Conclusion: 该工作为CFG提供了理论解释，提出的优化框架能稳定、高效地改进可控生成，为扩散模型和流模型的可控生成提供了新的理论基础和实用方法。
...
Conclusion: PaFu-KV策略通过智能KV缓存管理解决了长视频生成中的关键时空信息保留问题，在保持生成质量的同时显著提升了推理效率，为高效长时视频生成提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有防御方法主要基于"局部性假设"，通过抑制孤立的神经元或特征来防御对抗攻击，但有害语义通常以分布式、跨层电路的形式存在，导致局部干预方法脆弱且损害模型实用性。

Method: TraceRouter采用三阶段路径级框架：1) 通过注意力差异分析定位敏感起始层；2) 使用稀疏自编码器和差分激活分析解耦并隔离恶意特征；3) 通过基于零值干预的特征影响分数将特征映射到下游因果路径，选择性抑制这些因果链。

Result: 大量实验表明，TraceRouter显著优于现有最先进的基线方法，在对抗鲁棒性和通用实用性之间实现了更优的权衡平衡。

Conclusion: TraceRouter通过物理切断有害信息流同时保持正交计算路径完整，提供了一种更有效的对抗防御方法，解决了现有局部防御方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有运动-语言检索方法主要关注全局对齐，忽略了局部运动片段与身体关节以及文本标记之间的细粒度交互，导致检索性能不佳。受人类运动感知的金字塔过程启发，需要更精细的跨模态对应学习。

Method: 提出金字塔形Shapley-Taylor学习框架，将人体运动分解为时间片段和空间身体关节，通过渐进式关节级和片段级对齐以金字塔方式学习跨模态对应关系，捕捉局部语义细节和层次结构关系。
...
TL;DR: VideoAesBench：首个评估大语言多模态模型视频美学质量理解能力的综合基准，包含1804个多样化视频、多种问题格式和全面的美学维度，测试23个模型发现其能力仍不完整不精确。


<details>
  <summary>Details</summary>
Motivation: 大语言多模态模型在多种视觉感知任务中表现出色，但其视频美学质量评估能力（人类基本能力）尚未得到充分探索。现有研究缺乏系统评估LMMs视频美学理解能力的基准。
...
Result: 当前LMMs仅具备基本的视频美学感知能力，其表现仍不完整且不精确。基准测试揭示了现有模型在视频美学质量评估方面的局限性。

Conclusion: VideoAesBench可作为强大的测试平台，为可解释的视频美学评估提供见解。研究呼吁进一步改进LMMs的视频美学理解能力，该基准将推动该领域的发展。
...
TL;DR: 提出首个利用视频扩散模型辅助基于图像的方法进行零样本视频修复与增强的框架，解决时间闪烁问题


<details>
  <summary>Details</summary>
Motivation: 虽然基于扩散的零样本图像修复与增强方法取得了巨大成功，但将其应用于视频修复或增强会导致严重的时间闪烁问题
...
<details>
  <summary>Details</summary>
Motivation: 深度视觉特征作为视觉系统接口的应用日益增多，需要描述特征特性和控制特征质量以满足机器感知需求。将JND扩展到深度特征空间可以提供任务对齐的容忍边界，为资源受限下的特征质量控制提供实用参考

Method: 提出FeatJND任务对齐的JND公式，预测保持下游任务性能的最大可容忍每特征扰动图。在标准化分割点构建FeatJND估计器，并在图像分类、检测和实例分割任务上验证

Result: 在相同失真强度下，FeatJND基失真比非结构化高斯扰动能更好地保持任务性能；归因可视化表明FeatJND能抑制非关键特征区域。应用于token-wise动态量化时，FeatJND引导的步长分配在相同噪声预算下优于随机步长排列和全局均匀步长
...
TL;DR: BookNet：首个端到端深度学习框架，专门用于双页书籍图像校正，通过双分支架构和跨页注意力机制处理书籍特有的不对称几何畸变。


<details>
  <summary>Details</summary>
Motivation: 现有单页文档图像校正方法无法捕捉书籍相邻页面间的耦合几何关系，书籍图像因装订约束产生复杂的几何畸变，左右页面呈现明显不对称的曲率模式。

Method: BookNet采用双分支架构，配备跨页注意力机制，能够同时估计单个页面和完整书页的扭曲流，显式建模左右页面间的相互影响。同时构建了Book3D合成数据集用于训练和Book100真实世界基准用于评估。
...
<details>
  <summary>Details</summary>
Motivation: 现有文档理解模型在真实世界物理失真（如扫描、倾斜、扭曲、屏幕拍摄和光照变化）下的鲁棒性评估不足，需要更全面的基准测试。同时，实际应用需要更全面的文档分析能力，包括印章识别和文本检测。

Method: 开发了PaddleOCR-VL-1.5升级模型，提出了Real5-OmniDocBench基准来系统评估五种物理失真类型的鲁棒性。模型扩展了印章识别和文本检测任务能力，同时保持0.9B参数的紧凑架构。

Result: 在OmniDocBench v1.5上达到94.5%的SOTA准确率；在新提出的Real5-OmniDocBench基准上也实现了SOTA性能；成功扩展了印章识别和文本检测能力，同时保持模型紧凑高效。

Conclusion: PaddleOCR-VL-1.5在文档理解任务上实现了新的SOTA性能，并通过Real5-OmniDocBench基准提供了更全面的物理失真鲁棒性评估。模型在保持0.9B超紧凑规模的同时，扩展了实用功能，为实际文档分析应用提供了高效解决方案。
...
TL;DR: Drive-JEPA：结合视频联合嵌入预测架构与多模态轨迹蒸馏的端到端自动驾驶框架，通过自监督视频预训练和模拟轨迹蒸馏提升规划性能


<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶中，自监督视频预训练对规划表示学习的提升有限，主要因为驾驶场景固有的模糊性——每个场景通常只提供单一的人类轨迹，难以学习多模态行为

Method: 1) 将V-JEPA适配到端到端驾驶，在大规模驾驶视频上预训练ViT编码器，生成与轨迹规划对齐的预测性表示；2) 提出以提议为中心的规划器，蒸馏模拟生成的多条轨迹和人类轨迹，采用动量感知选择机制促进稳定安全行为

Result: 在NAVSIM基准测试中，V-JEPA表示结合简单transformer解码器在无感知设置下比先前方法提升3 PDMS；完整Drive-JEPA框架在v1上达到93.3 PDMS，v2上达到87.8 EPDMS，创下新SOTA

Conclusion: Drive-JEPA通过整合视频预测架构和多模态轨迹蒸馏，有效解决了驾驶场景的模糊性问题，显著提升了端到端自动驾驶的规划性能，为学习鲁棒的多模态驾驶行为提供了有效框架
...
TL;DR: PLANING是一个高效的在线单目图像序列重建框架，通过混合表示松散耦合几何基元与神经高斯，实现几何与外观解耦建模，支持在线初始化与优化，在重建质量、速度和几何精度方面均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有单目图像序列流式重建方法通常难以同时实现高质量渲染和精确几何重建，往往需要在两者之间做出权衡。需要一种能够兼顾高质量渲染和准确几何重建的高效在线重建框架。

Method: 提出PLANING框架，基于混合表示松散耦合显式几何基元与神经高斯，实现几何与外观的解耦建模。采用在线初始化与优化策略，分离几何与外观更新，减少结构冗余，支持稳定的流式重建。

Result: 在稠密网格Chamfer-L2指标上比PGSR提升18.52%，PSNR比ARTDECO高1.31 dB，在ScanNetV2场景中重建时间低于100秒，比2D高斯泼溅快5倍以上，同时达到离线逐场景优化的质量水平。
...
<details>
  <summary>Details</summary>
Motivation: 尽管缩放推动了视觉基础模型的进步，但将其扩展到度量深度估计仍然面临挑战，包括异构传感器噪声、相机依赖偏差以及噪声跨源3D数据中的度量模糊性。需要一种能够从噪声多样的3D源中学习度量深度而无需手动工程提示的框架。

Method: 提出稀疏度量提示（Sparse Metric Prompt），通过随机掩码深度图创建，作为解耦空间推理与传感器/相机偏差的通用接口。使用约2000万图像-深度对，涵盖重建、捕获和渲染的3D数据，跨越10000个相机模型。框架包括提示驱动任务和蒸馏的无提示学生模型。

Result: 首次展示了度量深度领域的清晰缩放趋势。预训练模型在深度补全、超分辨率和雷达-相机融合等提示驱动任务中表现出色；蒸馏的无提示学生模型在单目深度估计、相机内参恢复、单/多视角度量3D重建和VLA规划中达到最先进水平。作为视觉编码器使用时显著提升多模态大语言模型的空间智能能力。
...
<details>
  <summary>Details</summary>
Motivation: 复杂数据分解为因子化表示能揭示可重用组件并支持通过组件重组合成新样本，但现有方法在无监督因子发现和组合生成质量方面存在不足

Method: 引入对抗性训练信号：训练判别器区分单源样本与跨源因子重组生成的样本，通过优化生成器欺骗判别器，鼓励重组结果的物理和语义一致性

Result: 在CelebA-HQ、Virtual KITTI、CLEVR和Falcor3D数据集上优于先前基线，获得更低的FID分数和更好的解缠结性能（MIG和MCC指标）。在机器人视频轨迹中，通过重组学习到的动作组件生成多样化序列，显著增加LIBERO基准上的状态空间覆盖

Conclusion: 对抗性训练能有效提升无监督因子分解的质量和组合生成能力，在图像和机器人视频领域均有良好表现，为探索任务提供了新的数据增强方法
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在需要大量事实信息的任务上存在局限性，通常采用"推理-工具调用"范式进行视觉和文本搜索，但这种方法假设单一完整或实体级别的图像查询和少量文本查询就足以检索关键证据，这在现实世界存在大量视觉噪声的场景中是不现实的。此外，现有方法在推理深度和搜索广度上有限，难以解决需要聚合来自不同视觉和文本来源证据的复杂问题。

Method: 提出Vision-DeepResearch，采用多轮、多实体、多尺度的视觉和文本搜索范式，在强噪声环境下鲁棒地命中现实世界搜索引擎。系统支持数十个推理步骤和数百个引擎交互，通过冷启动监督和强化学习训练将深度研究能力内化到MLLM中，构建端到端的多模态深度研究MLLM。

Result: Vision-DeepResearch显著优于现有的多模态深度研究MLLMs，以及基于强大闭源基础模型（如GPT-5、Gemini-2.5-pro和Claude-4-Sonnet）构建的工作流程。

Conclusion: Vision-DeepResearch提出的多模态深度研究范式能够有效解决现实世界中复杂问题的信息检索需求，通过深度研究和多尺度搜索策略显著提升了MLLMs在噪声环境下的性能，为多模态AI系统的发展提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: SAM虽然具有零样本分割能力，但依赖手动提示阻碍了全自动部署。现有使用检测器作为提示生成器的方案存在两个根本限制：1) 目标不匹配 - 为几何定位优化的检测器与SAM所需的最优提示上下文不对应；2) 对齐过拟合 - 标准联合训练中检测器只是记忆特定训练样本的提示调整，而非学习可泛化的策略。

Method: 提出BLO-Inst框架，通过双层优化对齐检测和分割目标。将对齐问题表述为在不相交数据分割上的嵌套优化问题：下层在子集D1上微调SAM以最大化给定当前检测提议的分割保真度；上层在子集D2上更新检测器，使其生成的边界框能明确最小化微调后SAM的验证损失。这使检测器转变为分割感知的提示生成器，优化边界框不仅为了定位精度，更为了下游掩码质量。

Result: 大量实验表明，BLO-Inst在一般和生物医学领域的任务上都实现了优越性能，超越了标准基线方法。

Conclusion: BLO-Inst通过双层优化有效解决了SAM自动提示生成中的目标不匹配和对齐过拟合问题，将检测器转变为分割感知的提示生成器，实现了检测和分割目标的统一对齐。
...
TL;DR: 提出一种基于3D资产参考的扩散模型，通过双分支感知架构同时生成与3D资产一致的RGB图像和点云图，实现2D图像生成与3D资产的精确对齐。
...
Result: 实验表明该方法能有效利用3D资产作为参考，生成与给定资产一致的图像，在保持与3D参考一致性的同时实现了高质量的图像生成。

Conclusion: 该方法成功将扩散模型与3D内容创作相结合，为基于3D资产的图像生成开辟了新可能性，实现了2D图像属性与3D资产属性的有效链接。
...
Method: SINA集成了深度学习用于准确组件检测，Connected-Component Labeling (CCL)用于精确连接性提取，OCR用于组件参考标识符检索，以及Vision-Language Model (VLM)用于可靠的参考标识符分配。
...
<details>
  <summary>Details</summary>
Motivation: 现有创造性图像生成方法通常依赖于手动混合概念或排除子类别，缺乏系统性原则。需要一种能够自动产生新颖、高质量图像并扩展想象力边界的框架

Method: 将创造力定义为图像在CLIP嵌入空间中的逆概率，计算生成图像的概率分布并驱动其向低概率区域移动。引入回拉机制以保持视觉保真度

Result: 在文本到图像扩散模型上的大量实验证明了该框架的有效性和效率，能够产生独特、新颖且引人深思的图像

Conclusion: 为生成模型中的创造力提供了新视角，提供了促进视觉内容合成创新的原则性方法
...
TL;DR: EditYourself是一个基于DiT的音频驱动视频编辑框架，支持通过修改文字脚本对说话人视频进行添加、删除和重定时等操作，同时保持运动、时间一致性、说话人身份和准确的唇部同步。


<details>
  <summary>Details</summary>
Motivation: 当前生成视频模型擅长从文本和图像提示生成新内容，但在编辑现有预录制视频方面存在关键空白，特别是当需要修改口语脚本时，必须保持运动、时间一致性、说话人身份和准确的唇部同步。

Method: 基于通用视频扩散模型，EditYourself通过音频条件化和区域感知、编辑聚焦的训练扩展增强了其视频到视频编辑能力。使用时空修复技术实现精确的唇部同步和时间一致的重构，包括在新添加片段中合成逼真的人类运动。

Result: 该框架能够实现精确的唇部同步和时间一致的重构现有表演，包括在新添加片段中合成逼真的人类运动，同时保持长时间内的视觉保真度和身份一致性。
...
TL;DR: 开发了名为ePAI的AI系统用于早期胰腺癌检测，在内部测试中AUC达0.939-0.999，外部测试AUC达0.918-0.945，能检测临床诊断前3-36个月被放射科医生漏诊的病变，在多读者研究中显著优于30名认证放射科医生。
...
Method: 开发了名为ePAI（早期胰腺癌检测人工智能）的自动化系统。使用来自单一医疗中心的1,598名患者数据进行训练。在内部测试中涉及1,009名患者，外部测试涉及6个中心的7,158名患者。系统能够检测直径小于2cm的小PDAC，精确定位小至2mm的病变。还进行了多读者研究，比较ePAI与30名认证放射科医生的表现。

Result: 内部测试：AUC 0.939-0.999，敏感性95.3%，特异性98.7%，能定位小至2mm的PDAC。外部测试：AUC 0.918-0.945，敏感性91.5%，特异性88.0%，能定位小至5mm的PDAC。ePAI成功检测到临床诊断前3-36个月被放射科医生漏诊的PDAC，在159名患者中检测到75例，中位提前时间为347天。多读者研究显示ePAI敏感性显著优于放射科医生50.3%（P<0.05），特异性保持95.4%。

Conclusion: ePAI系统在早期胰腺癌检测方面表现出色，能够检测临床诊断前数月被放射科医生漏诊的病变，显著优于人类专家。这些发现表明ePAI有潜力作为辅助工具改善胰腺癌的早期检测。
...
<details>
  <summary>Details</summary>
Motivation: 解决全图像重光照面临的三大挑战：大规模结构化配对数据收集困难、物理合理性难以保持、数据驱动先验导致的泛化能力有限。现有方法在缩小合成到真实场景差距方面仍不理想。

Method: 提出两阶段物理启发扩散框架：1) 批感知注意力机制提升多图像内在属性预测一致性；2) 物理引导的神经渲染模块确保物理合理的光传输；3) 物理启发损失函数正则化训练动态；4) 精心策划的多样化对象和场景数据集

Result: PI-Light能够合成各种材质的高光镜面反射和漫反射，在真实场景泛化方面优于现有方法，为下游评估提供可靠基准

Conclusion: 通过物理约束的扩散模型设计，PI-Light有效解决了全图像重光照的关键挑战，实现了更好的真实场景泛化能力，为预训练扩散模型的高效微调提供了可行方案
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在原始错觉图像上能"正确"回答，但在错觉因素反转后仍坚持相同回答，尽管视觉变化对人类显而易见。这引发根本问题：VLMs是感知视觉变化还是仅回忆记忆模式？现有研究注意到此现象但原因不明，需要从观察到系统理解的转变。

Method: 提出VI-Probe框架，包含分级扰动和匹配视觉控制（无错觉诱导因素），通过极性翻转一致性、模板固定指数和相对于匹配控制的错觉乘数等指标，分离视觉基础感知与语言驱动回忆。不同于关注平均准确率的先前工作，测量稳定性和敏感性。

Result: 实验发现响应持续性源于异质性原因而非单一机制：GPT-5表现记忆覆盖，Claude-Opus-4.1显示感知-记忆竞争，Qwen变体暗示视觉处理限制。挑战单一原因观点，支持基于探测的评估方法。

Conclusion: VLMs对视觉错觉的响应持续性由多种机制驱动，需要同时测量知识和受控视觉变化敏感性的探测评估方法。VI-Probe框架为理解模型感知能力提供系统工具，数据代码已开源。
...
TL;DR: UEval是一个评估统一模型（能同时生成图像和文本）的基准，包含1000个专家策划的问题，需要模型输出图像和文本，涵盖8个真实世界任务。采用基于量规的评分系统，包含10,417个已验证的评分标准，实现可扩展的细粒度自动评分。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏评估统一模型（能同时生成图像和文本）的有效基准。开放式多模态生成评估具有挑战性，简单的LLM-as-a-judge方法可能忽略细微差别。需要设计一个能够全面评估模型在需要图像和文本输出的复杂任务上表现的基准。

Method: UEval包含1000个专家策划的问题，来自8个真实世界任务，涵盖从逐步指南到教科书解释的各种推理类型。采用基于量规的评分系统：为每个问题提供参考图像和文本答案，由MLLM生成初始评分量规（包含多个评估标准），然后由人类专家精炼和验证这些量规。总共包含10,417个已验证的量规标准。

Result: UEval对当前统一模型具有挑战性：GPT-5-Thinking仅得66.4分（满分100），最佳开源模型仅得49.1分。观察到推理模型通常优于非推理模型，将推理轨迹从推理模型转移到非推理模型能显著缩小差距，表明推理对于需要复杂多模态理解和生成的任务很重要。

Conclusion: UEval是一个全面评估统一模型能力的基准，其基于量规的评分系统能够实现可扩展的细粒度自动评估。结果表明当前统一模型在需要同时生成图像和文本的复杂任务上仍有很大提升空间，推理能力对于此类任务至关重要。
...
TL;DR: 论文提出语音识别评估的多项改进：支持多参考标注的字符串对齐算法、收集俄语长语音数据集、研究微调动态，并开发流式语音识别评估工具。


<details>
  <summary>Details</summary>
Motivation: 现有语音识别评估方法对非拉丁语系、构词丰富的语言以及杂乱/长语音的评估效果有限，且存在模型适应数据集特定标注导致指标改善假象的问题。

Method: 1) 提出支持多参考标注、任意长度插入和更好词对齐的字符串对齐算法；2) 收集并标注DiverseSpeech-Ru俄语长语音数据集；3) 对流行俄语测试集进行多参考重标注；4) 基于改进词对齐开发流式语音识别评估工具和可视化比较工具。

Result: 算法特别适用于非拉丁语言和构词丰富的语言；模型常适应数据集特定标注导致指标改善假象；提供了统一的离线/流式语音识别模型封装。

Conclusion: 提出的改进提升了语音识别评估的准确性，特别是对非拉丁语言和长语音；揭示了模型适应数据集标注的问题；工具和数据集将公开可用。
...
TL;DR: 该论文研究了在自监督学习语音模型中，使用均方误差损失进行前端语音增强微调时，模型会过度利用位置嵌入而非内容信息的问题，提出了两种位置不变微调策略。


<details>
  <summary>Details</summary>
Motivation: 将前端语音增强模型与基于自监督学习的语音模型结合在噪声条件下对下游任务有效，但常用的均方误差损失容易利用SSL模型中的位置嵌入，使目标通过位置相关性而非内容相关信息最小化，这被视为自监督表示微调的一般性限制。

Method: 通过表示引导的语音增强框架研究该问题，考虑两种策略：(1) 零填充（在SSL预训练中已有探索，但在微调设置中研究），(2) 使用软DTW损失的速度扰动方法。

Result: 实验表明，基于软DTW的方法实现了更快的收敛和更好的下游性能，强调了在基于SSL的语音建模中进行位置不变微调的重要性。

Conclusion: 位置不变微调对于基于自监督学习的语音建模至关重要，软DTW损失方法在避免过度依赖位置嵌入方面优于传统均方误差损失，能更好地利用内容相关信息提升下游任务性能。
...
TL;DR: 比较数据混合与模型合并两种创建小型多任务代码LLM的方法，发现大规模时模型合并效果更佳，小规模时数据混合更优


<details>
  <summary>Details</summary>
Motivation: 研究如何在资源受限场景下，通过高效的多任务学习策略平衡性能、约束和成本，创建小型专业化的代码LLM

Method: 使用Qwen Coder和DeepSeek Coder两个模型家族，在2B和7B两个规模上进行实验，比较数据混合与模型合并两种方法，并引入权重分析技术理解不同任务对模型参数的影响

Result: 大规模（7B）时模型合并表现最佳，保留96%专业模型性能，甚至超越单独微调模型（Qwen Coder 2.5 7B在HumanEval上达到92.7% Pass@1）；小规模（2B）时数据混合更优

Conclusion: 精心设计的合并和混合策略可以有效结合任务特定能力而无显著性能下降，适合资源受限部署场景
...
TL;DR: LLM可从姓名推断种族/民族，准确率超传统BISG方法，无需额外训练数据，适用于美国以外地区，并能减少收入偏见。


<details>
  <summary>Details</summary>
Motivation: 传统种族分类方法如BISG存在局限性：仅适用于美国，依赖人口普查数据，对富裕社区的少数族裔存在系统性误分类为白人的收入偏见，且无法适应不同文化背景的分类需求。

Method: 使用六种LLM模型（包括Gemini 3 Flash、GPT-4o、DeepSeek v3.2、GLM-4.7等）从姓名推断种族/民族；采用分层抽样验证（佛罗里达和北卡罗来纳选民档案）；测试扩展推理能力；纳入政党注册等元数据；在多个国家（黎巴嫩、印度、乌干达、尼泊尔、亚美尼亚、智利、哥斯达黎加）进行验证；使用LLM标签微调小型Transformer模型实现本地部署。

Result: LLM分类准确率最高达84.7%（平衡样本），优于BISG的68.2%；扩展推理可提升1-3个百分点；包含政党注册元数据时达86.7%；显著减少BISG的收入偏见；在黎巴嫩宗教派别分类准确率64.3%，印度保留选区议员分类99.2%，印度土地记录种姓分类74.0%；多国验证显示能恢复已知人口分布；微调后的小型模型超越BISG准确率且可免费本地部署。

Conclusion: LLM提供了一种准确、灵活且成本效益高的种族/民族推断方法，克服了传统方法的局限，适用于全球范围且能适应不同文化背景的分类体系，为社会科学研究提供了新工具。
...
TL;DR: EnsembleLink：一种无需训练标签的高精度记录链接方法，利用预训练语言模型的语义理解能力


<details>
  <summary>Details</summary>
Motivation: 记录链接是社会科学实证研究的关键步骤，但现有方法存在两个主要问题：1）研究者通常将其视为预处理步骤，使用临时规则而不量化链接错误对下游分析的不确定性影响；2）现有方法要么精度低，要么需要大量标注训练数据

Method: EnsembleLink利用预训练语言模型，这些模型从大规模文本语料库中学习了语义关系（如"South Ozone Park"是"New York City"的社区，"Lutte ouvriere"指托洛茨基主义的"Workers' Struggle"政党）。该方法无需任何训练标签，通过集成策略实现高精度链接

Result: 在涵盖城市名称、人名、组织、多语言政党和书目记录等多个基准测试中，EnsembleLink的性能达到或超过了需要大量标注的方法。该方法可在开源模型上本地运行，无需外部API调用，典型链接任务可在几分钟内完成

Conclusion: EnsembleLink为社会科学研究提供了一种无需标注数据的高精度记录链接解决方案，解决了现有方法在精度和标注需求方面的局限性，同时保持了计算效率和隐私保护
...
TL;DR: OS-Search将LLM生成转化为输出空间搜索：外层循环在编码器定义的3D输出空间Z中选择目标z*，检索增强策略通过序列级RL训练生成坐标接近z*的输出，实现并行扫描和黑盒优化


<details>
  <summary>Details</summary>
Motivation: 传统LLM生成依赖自回归解码，存在路径依赖和顺序搜索限制，难以进行并行探索和黑盒优化。需要一种方法将生成过程转化为输出空间中的搜索问题，实现更高效的多样性和目标优化

Method: 1) 定义冻结编码器映射的3D输出空间Z；2) 外层循环在Z中选择目标坐标z*；3) 检索增强策略通过序列级强化学习训练，生成坐标接近z*的输出；4) 支持并行扫描和贝叶斯优化等黑盒优化方法

Result: 在故事生成中，Z空间扫描比提示链方法获得3.1倍的LLM评分多样性提升；在代码生成中，Z空间的贝叶斯优化在匹配推理预算下改进了控制器未知的目标函数，同时保持代码有效性

Conclusion: OS-Search成功将LLM生成转化为输出空间搜索问题，避免了路径依赖的token/程序搜索，实现了并行探索和黑盒优化，在文本和代码生成任务中均表现出优越的多样性和目标优化能力
...
TL;DR: 论文通过跨语言语料分析和神经学习实验，研究了功能词的统计特性（高频、与句法结构关联、与短语边界对齐）如何支持从线性输入中学习层次结构。


<details>
  <summary>Details</summary>
Motivation: 探究支持从线性输入中学习层次结构的统计条件，特别关注功能词在语言习得中的作用，因为功能词具有独特的分布特性（高频、与句法结构可靠关联、与短语边界对齐）。

Method: 1. 跨语言语料分析：在186种语言中验证功能词的三种分布特性；2. 反事实语言建模和消融实验：比较保留不同特性的语言变体在神经学习器中的习得难度；3. 后续探测和消融分析：探究不同学习条件下对功能词的依赖机制差异。

Result: 1. 所有三种特性在186种语言中普遍存在；2. 保留所有三种特性的语言变体更容易被神经学习器习得，其中频率和结构关联的贡献比边界对齐更强；3. 不同学习条件导致对功能词的系统性不同依赖，表明相似性能可能源于不同的内部机制。

Conclusion: 功能词的统计特性（特别是高频和与句法结构的关联）是支持从线性输入中学习层次结构的关键条件，但不同学习机制可能产生相似的外部表现，揭示了语言习得的复杂性。
...
TL;DR: 提出一个多语言音素生成评估框架，通过通用音素识别与语言特定音素解释结合，使用对比音系特征距离进行音素到音位映射和序列对齐，提供PER、PFER和PhonCov三个评估指标。
...
Method: 提出多语言音素生成评估框架，整合通用音素识别与语言特定音素解释，使用对比音系特征距离进行音素到音位映射和序列对齐。框架产生三个指标：音素错误率(PER)、音系特征错误率(PFER)和新提出的无对齐度量音素覆盖度(PhonCov)。

Result: 在英语、西班牙语、意大利语和泰米尔语上的分析表明：PER受益于映射和对齐的组合，PFER仅受益于对齐，PhonCov受益于映射。进一步分析显示该框架捕捉到与构音障碍语音已知观察一致的有临床意义的可懂度退化模式。

Conclusion: 该多语言框架能够有效评估跨语言的构音障碍语音可懂度，捕捉语言特定因素，为临床评估提供有意义的信息。
...
TL;DR: 该论文研究了大型语言模型在推理步数泛化场景中的性能下降问题，发现错误集中在少数关键错误类型的token位置，源于内部竞争机制，并提出了一种动态识别和停用错误处理头的轻量级干预方法。


<details>
  <summary>Details</summary>
Motivation: 尽管思维链推理已成为大型语言模型解决复杂问题的标准范式，但最近研究发现，在推理步数泛化场景中（所需推理步骤超出训练分布但底层算法不变），模型性能急剧下降。这种失败的内在机制尚未得到充分理解。

Method: 作者对多个领域的任务进行系统研究，发现错误集中在少数关键错误类型的token位置。深入分析表明，这些token级错误预测源于内部竞争机制：某些注意力头（称为错误处理头）通过放大错误推理轨迹同时抑制正确轨迹来打破平衡。基于这些发现，提出了测试时推理校正方法，这是一种轻量级干预方法，在推理过程中动态识别和停用错误处理头。

Result: 实验表明，在推理过程中移除单个错误处理头通常可以恢复正确预测。在不同任务和大型语言模型上的广泛实验显示，该方法能持续改善推理步数泛化性能，突显了其有效性和潜力。

Conclusion: 该研究揭示了大型语言模型在推理步数泛化中失败的内在机制，并提出了一种有效的轻量级干预方法。通过动态识别和停用错误处理头，能够显著改善模型的推理泛化能力，为理解模型内部工作机制和提升推理性能提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型存在知识边界不明确的问题，会在缺乏足够知识的主题上产生事实错误的幻觉回答。现有提升LLM诚实性的方法评估缺乏鲁棒性，未考虑模型在预训练阶段已吸收的知识。

Method: 1) 利用Pythia（真正开源且预训练数据公开的LLM）构建更鲁棒的诚实性评估基准数据集；2) 提出利用预训练数据构建更诚实LLM的新方法

Result: 提出了基于Pythia的诚实性评估基准数据集，能够更准确地评估LLM的诚实性，同时提出了利用预训练数据提升模型诚实性的新方法

Conclusion: 通过利用开源LLM及其公开的预训练数据，可以构建更鲁棒的诚实性评估基准，并开发更有效的方法来提升LLM的诚实性，使其在缺乏知识时更诚实地回答"我不知道"
...
TL;DR: SHARP框架：一种用于评估大语言模型社会危害的多维分布感知方法，超越传统标量评分，关注尾部风险和跨维度交互


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估基准将复杂社会风险简化为均值中心的标量分数，掩盖了分布结构、跨维度交互和最坏情况行为，而LLM在高风险领域的部署需要更全面的风险评估

Method: 提出SHARP框架，将危害建模为多元随机变量，整合偏见、公平性、伦理和认知可靠性的显式分解，采用联合失败聚合重新参数化为加性累积对数风险，并使用风险敏感分布统计量（特别是CVaR95）来表征最坏情况行为

Result: 对11个前沿LLM在901个社会敏感提示上的评估显示：平均风险相似的模型在尾部暴露和波动性上可有两倍以上差异；各维度边际尾部行为存在系统性差异，偏见维度尾部严重性最强，认知和公平风险居中，伦理错位最低；揭示了传统标量基准所掩盖的异质、模型依赖的失败结构
...
TL;DR: MoCo是一个用于执行、基准测试和比较模型协作算法的Python库，包含26种方法，在61%的情况下协作策略优于非协作模型，最高提升25.8%


<details>
  <summary>Details</summary>
Motivation: 当前模型协作研究分散且缺乏系统比较，需要整合现有研究并建立模型协作作为一个独立的研究方向

Method: 开发MoCo Python库，集成26种模型协作方法（路由、文本、logit、模型参数等不同信息交换层级），包含25个评估数据集（推理、QA、代码、安全等），支持用户自定义数据

Result: 实验显示大多数协作策略在61.0%的（模型，数据）设置中优于非协作模型，最有效的方法提升达25.8%；分析了协作策略的扩展性、训练/推理效率，并证明协作系统能解决单一语言模型难以处理的问题

Conclusion: MoCo作为有价值的工具包，将促进开放、模块化、去中心化和协作的AI未来发展，为模型协作研究提供系统化平台
...
TL;DR: Qwen3-ASR系列包含两个多语言语音识别模型和一个创新的非自回归语音强制对齐模型，在性能和效率方面均达到先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有ASR模型在开源基准测试中差异不大，但在实际场景中质量差异显著；需要开发既能在基准测试中表现优异，又能在真实世界应用中提供高质量语音识别和文本-语音对齐的模型。
...
Result: 1.7B版本在开源ASR模型中达到SOTA性能，与最强的专有API竞争；0.6B版本提供最佳准确率-效率权衡，平均TTFT低至92ms，在128并发下1秒可转录2000秒语音；强制对齐模型在时间戳准确率上超越三个最强的强制对齐模型，在效率和多功能性方面优势明显。

Conclusion: Qwen3-ASR系列模型在语音识别和强制对齐任务中表现出色，1.7B版本性能领先，0.6B版本在效率方面优势显著；所有模型以Apache 2.0许可证开源，将加速ASR和音频理解领域的研究。
...
TL;DR: 提出一种新的预训练方法，使用强化学习在文档流中改进模型生成的下K个token，通过后训练模型评估候选生成的质量、安全性和事实性，从而从底层构建更高质量、更安全、更真实的模型。


<details>
  <summary>Details</summary>
Motivation: 当前解决大语言模型生成安全性、事实性和质量问题的标准方法需要昂贵的精心策划数据集和多阶段微调对齐，但即使这样复杂的流程也无法保证纠正预训练期间学习到的模式。因此，在预训练阶段解决这些问题至关重要，因为预训练塑造了模型的核心行为，可以防止不安全或幻觉输出被深度嵌入。

Method: 提出新的预训练方法：在文档流中使用强化学习改进每个步骤生成的下K个token。使用一个强大的后训练模型来评估候选生成（包括模型rollout、原始后缀和重写后缀）的质量、安全性和事实性。训练早期依赖原始和重写的后缀；随着模型改进，强化学习奖励高质量的rollout。

Result: 实验表明，该方法相比标准预训练在事实性方面获得36.2%的相对改进，在安全性方面获得18.5%的相对改进，在整体生成质量方面获得高达86.3%的胜率改进。

Conclusion: 该方法能够从底层构建更高质量、更安全、更真实的模型，通过在预训练阶段直接解决生成质量问题，而不是依赖昂贵且效果有限的后训练对齐方法。
...
TL;DR: LLMs在代码评估中存在"合规悖论"：过度优化的指令跟随能力导致模型优先满足隐藏格式指令而非评估代码正确性，造成系统性漏洞


<details>
  <summary>Details</summary>
Motivation: 揭示LLMs在代码评估中的根本缺陷：当前假设指令跟随能力可直接转化为客观评判能力，但实际存在系统性漏洞

Method: 提出SPACI框架和AST-ASIP协议，利用语法-语义间隙在AST的trivia节点中嵌入对抗性指令；大规模评估9个SOTA模型在25,000份Python/C/C++/Java提交中的表现

Result: 发现灾难性失败率(>95%)，特别是DeepSeek-V3等高容量模型；量化指标显示模型优先满足隐藏格式约束而非代码正确性，导致"虚假认证"
...
TL;DR: 提出证据排序新任务，通过优先呈现充分信息来减少用户阅读负担，同时保持所有证据可访问，改进事实核查效率


<details>
  <summary>Details</summary>
Motivation: 现有自动系统和大型语言模型在事实核查中检索证据时，往往提供信息不足或过度冗余的证据，导致验证效率低下且容易出错，需要更有效的信息呈现方式

Method: 提出证据排序新任务，比较一次性排序和增量排序两种方法；引入基于信息检索指标的新评估框架，通过整合现有事实核查数据集构建统一基准；使用多样化模型进行实验，并进行受控用户研究

Result: 增量排序策略能更好地捕捉互补证据；基于LLM的方法优于浅层基线方法；证据排序相比证据选择能减少阅读负担并改进验证效果；但LLM方法在平衡充分性和冗余性方面仍面临挑战

Conclusion: 证据排序任务为构建更可解释、高效且用户对齐的信息验证系统提供了基础性步骤，通过优化证据呈现顺序来改善事实核查的用户体验和效率
...
Method: CoNL框架通过多智能体自博弈统一生成、评估和元评估。多个共享相同策略的智能体参与结构化对话，提出、批评和修订解决方案。关键洞察：批评质量可以通过它是否帮助他人改进解决方案来衡量。能促使解决方案改进的批评获得诊断性奖励，为元评估创建显式监督，并通过自博弈联合优化生成和评估能力，无需外部评估者或真实标签。
...
TL;DR: SOUP是一种在token级别统一离策略和同策略学习的框架，通过将离策略影响限制在从历史策略采样的生成序列前缀中，而延续部分采用同策略生成，有效利用离策略信息同时保持训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 同策略强化学习方法（如GRPO）在语言模型后训练中存在探索有限和早期饱和问题，而现有混合整个轨迹的离策略方法会导致严重的策略失配和不稳定性。

Method: 提出SOUP框架，在单个样本的token级别统一离策略和同策略学习：序列前缀从历史策略采样（离策略），延续部分采用当前策略生成（同策略），通过token级别的重要性比率有效利用离策略信息。

Result: 大量实验表明，SOUP在性能上持续优于标准的同策略训练和现有的离策略扩展方法，能够同时改善探索性和最终性能。

Conclusion: SOUP通过细粒度的单样本混合策略训练，为LLM强化学习提供了既能有效利用离策略数据又能保持训练稳定性的统一范式，解决了现有方法的局限性。
...
TL;DR: DimStance：首个维度化立场资源，将立场检测从分类任务扩展到基于效价-唤醒度的连续维度建模，支持多语言情感感知的立场分析


<details>
  <summary>Details</summary>
Motivation: 传统立场检测仅提供分类标签（支持/中立/反对），无法捕捉立场表达背后的细微情感状态。作者希望利用情感科学中的维度框架，通过效价（负面-正面）和唤醒度（平静-活跃）的连续值来建模立场，实现更细粒度的立场分析。

Method: 1. 创建DimStance资源：包含11,746个目标方面、7,365篇文本，涵盖5种语言（英语、德语、中文、尼日利亚皮钦语、斯瓦希里语）和2个领域（政治、环境保护），并标注效价-唤醒度值
2. 定义维度立场回归任务：预测文本对特定目标的效价和唤醒度值
3. 评估方法：在回归和提示设置下，对预训练模型和大语言模型进行基准测试，分析跨语言VA模式

Result: 1. 微调后的LLM回归器表现出竞争性性能
2. 低资源语言（如尼日利亚皮钦语、斯瓦希里语）仍面临持续挑战
3. 基于token的生成方法存在局限性
4. 揭示了跨语言的VA模式特征

Conclusion: DimStance为多语言、情感感知的立场分析提供了基础资源。维度化立场建模能够捕捉传统分类方法无法表达的细微情感状态，为细粒度立场分析开辟了新方向。该资源支持未来在情感计算、跨文化沟通和多语言NLP等领域的研究。
...
TL;DR: 构建了一个包含96,243个词条定义对的多领域阿拉伯语反向词典数据集MURAD，支持计算语言学和词典学研究


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语词汇丰富但缺乏大规模词汇定义数据集，限制了阿拉伯语自然语言处理研究和应用发展

Method: 采用混合提取管道，整合直接文本解析、光学字符识别和自动化重建技术，从可信参考书和教育资源中提取数据

Result: 创建了包含96,243个词条定义对的MURAD数据集，涵盖语言学、伊斯兰研究、数学、物理、心理学和工程学等多个领域

Conclusion: 该数据集支持反向词典建模、语义检索和教育工具开发，将推动阿拉伯语自然语言处理和词汇语义研究的可重复性
...
TL;DR: 提出Landmark (LMK) pooling方法，通过将序列分块并在块间插入地标token，然后对地标token嵌入进行平均池化，解决了传统池化策略在长上下文任务中的系统性弱点。


<details>
  <summary>Details</summary>
Motivation: 现有序列编码器通常使用[CLS] token或平均池化将变长token序列压缩为单个向量，但这些池化策略存在系统性弱点：[CLS]倾向于将信息集中在序列初始位置，可能无法充分表示分布式证据；平均池化可能稀释显著的局部信号，导致短上下文性能下降。

Method: 提出Landmark (LMK) pooling方法：1) 将序列划分为多个块；2) 在块之间插入地标token；3) 通过对地标token嵌入进行平均池化形成最终表示。该方法仅引入少量特殊token，在保持局部显著特征的同时改善了长上下文外推能力。

Result: 实证表明：LMK pooling在短上下文检索任务上与现有方法表现相当，在长上下文任务上带来显著改进，使其成为现有池化方法的实用且可扩展的替代方案。

Conclusion: LMK pooling通过简单的分块和地标token插入机制，有效解决了传统池化策略的弱点，在保持短上下文性能的同时显著提升了长上下文任务的表现，是一种实用且可扩展的池化方法。
...
TL;DR: inversedMixup：将Mixup的线性插值可控性与LLM的可解释性结合，通过三阶段训练对齐任务模型输出嵌入与LLM输入嵌入，生成可读的文本增强样本
...
Method: 采用三阶段训练程序：1) 对齐任务特定模型的输出嵌入空间与LLM的输入嵌入空间；2) 通过LLM反演技术将混合嵌入重构为自然语言；3) 提出缓解流形侵入现象的简单有效策略

Result: 在少样本和全监督场景下均表现出有效性和泛化能力，首次为文本Mixup中的流形侵入现象提供了实证证据，并通过提出的缓解策略改善了增强性能

Conclusion: inversedMixup成功地将Mixup的可控性与LLM的可解释性统一起来，通过嵌入空间对齐和LLM反演技术，实现了可读且可控的文本增强，为文本数据增强提供了新思路
...
TL;DR: KromHC使用Kronecker积分解双随机矩阵，解决mHC训练不稳定、参数复杂度过高的问题，在保证双随机性的同时将参数复杂度降至O(n²C)


<details>
  <summary>Details</summary>
Motivation: 现有超连接方法存在训练不稳定和可扩展性受限问题。mHC通过Birkhoff多面体投影缓解这些问题，但仍面临两个主要问题：1) 迭代Sinkhorn-Knopp算法不能保证精确双随机矩阵；2) 参数复杂度高达O(n³C)。mHC-lite虽能保证双随机性，但参数复杂度呈阶乘爆炸O(nC·n!)。

Method: 提出KromHC方法，使用较小双随机矩阵的Kronecker积来参数化残差矩阵。通过对张量化残差流的每个模态施加流形约束，保证残差矩阵的精确双随机性，同时将参数复杂度降低到O(n²C)。

Result: 综合实验表明，KromHC在性能上匹配甚至优于最先进的mHC变体，同时需要显著更少的可训练参数。

Conclusion: KromHC通过Kronecker积分解有效解决了超连接方法的双随机性和参数复杂度问题，在保持性能优势的同时大幅降低了计算开销。
...
TL;DR: 该研究使用语言模型作为受控统计学习器，系统模拟跨语言影响，通过操纵L1语言优势度、L2语言熟练度和句法距离，揭示了CLI的机制驱动因素。


<details>
  <summary>Details</summary>
Motivation: 人类研究中关于跨语言影响(CLI)的结果常常相互矛盾，这主要是由于实验方差造成的。研究旨在解决这些不一致性，使用语言模型作为受控统计学习器来系统模拟CLI并分离其潜在驱动因素。

Method: 使用语言模型作为受控统计学习器，通过控制L2暴露年龄(即引入L2的训练步骤)来操纵L1语言优势度和L2语言熟练度。研究不同句法距离的L1预训练对L2的影响，采用跨语言启动范式分析L1结构激活如何影响L2处理。

Result: 结果与心理语言学证据一致：语言优势度和熟练度是CLI的强预测因子。语法结构启动是双向的，但不合语法结构的启动对语言优势度敏感。提供了LMs中CLI的机制证据：L1在L2处理过程中被共同激活，并直接影响L2的神经回路。

Conclusion: 语言模型可以作为计算框架来支持人类CLI理论，为跨语言影响提供了受控、可重复的实验平台，有助于理解双语处理中的语言交互机制。
...
<details>
  <summary>Details</summary>
Motivation: 离散扩散语言模型缺乏有效的推理时控制机制，现有方法包括采样级引导或轨迹优化，但仍有改进空间

Method: ILRR框架：通过动态对齐生成序列与参考序列的内部激活来引导生成；引入空间调制引导扩展，用短参考引导长文本

Result: 在LLaDA和MDLM架构上实现有效属性引导，计算开销小（每去噪步仅需一次额外前向传播）；相同计算预算下，属性准确率比基线提升10-60个百分点

Conclusion: ILRR为离散扩散语言模型提供了高效、灵活的推理时控制框架，能有效转移高级语义属性
...
TL;DR: 提出一种后训练适应策略，通过选择性替换低效用标记为适应语料库中更相关的标记，优化特定领域或语言的子词分词效果。


<details>
  <summary>Details</summary>
Motivation: 标准BPE分词方法使用通用标记集处理所有文本数据，但在应用到特定领域或语言时可能效率低下，因为通用标记集可能包含对特定领域不相关的标记，而缺乏领域相关标记。

Method: 提出后训练适应策略：1）识别低效用标记（基于频率）；2）根据适应语料库频率选择更相关的标记；3）算法确定在给定目标词汇表大小下最有效编码适应语料库的标记清单；4）选择性替换标记，保持词汇表大小不变。

Result: 在多语言生成和分类任务上的广泛实验表明，适应后的分词器在相同词汇表大小下比基线方法更有效地压缩测试语料库，证明该方法能优化特定领域或任务的分词效果。

Conclusion: 该方法作为一种轻量级适应机制，类似于词汇表微调过程，能够为特定领域或任务优化分词，提高大型语言模型在特定应用中的效率和性能。
...
TL;DR: 该论文将文本语义演化视为高维状态空间中的随机轨迹，利用Allan偏差分析语义稳定性，揭示了人类文本与LLM生成文本在动力学特性上的差异。


<details>
  <summary>Details</summary>
Motivation: 研究语言语义演化的底层动力学机制，建立语义连贯性作为可测量的物理属性，区分人类认知与算法模型生成的微妙动力学差异。

Method: 将文本语义演化视为高维状态空间中的随机轨迹，采用精密计量学中的Allan偏差工具，将有序句子嵌入视为位移信号进行分析。

Result: 发现两个不同的动力学机制：短时幂律标度区分创意文学与技术文本，长时交叉到稳定性限制的噪声基底；大语言模型能模仿人类文本的局部标度统计，但稳定性视界系统性降低。

Conclusion: 语义连贯性是可测量的物理属性，该框架能区分人类认知与算法模型的微妙动力学差异，为理解文本语义演化提供了量化分析工具。
...
TL;DR: RSE（回收搜索经验）是一种无需训练的自引导策略，通过将原始轨迹提炼到共享经验库中，将测试时搜索从孤立试验转变为累积过程，提高大型语言模型的推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有测试时扩展方法在探索解决方案空间时通常将rollouts视为一次性样本，丢弃了有价值的中间见解，导致计算冗余。模型在多次尝试中重复推导已发现的结论并重新访问已知的死胡同，这种系统性无记忆性造成了巨大的计算浪费。

Method: 提出RSE策略：1）主动将原始轨迹提炼到共享经验库中；2）通过积极回收中间结论来减少冗余推导；3）通过负面回收失败模式来修剪遇到的死胡同。这是一种自引导、无需训练的方法，将测试时搜索从一系列孤立试验转变为累积过程。

Result: 理论分析形式化了RSE的效率增益，验证了其在解决复杂推理任务中相对于独立采样的优势。在HMMT24、HMMT25、IMO-Bench和HLE上的广泛实验表明，RSE在可比较的计算成本下持续优于强基线方法，实现了最先进的扩展效率。

Conclusion: RSE通过回收搜索经验，将测试时搜索从孤立试验转变为累积过程，有效减少了计算冗余，提高了大型语言模型在复杂推理任务中的效率，为测试时扩展方法提供了新的方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究识别了检索头、汇头和对角线轨迹等个体注意力模式，但这些观察是碎片化的，缺乏统一的解释框架。需要建立能够解释多样化注意力模式的统一理论框架。

Method: 提出TAPPA（时间注意力模式可预测性分析）框架，从时间连续角度分析注意力模式的数学公式。将注意力模式区分为具有清晰规律的可预测模式和看似随机的不可预测模式，并通过查询在时间维度上的自相似性程度来解释这种区分。针对可预测模式，通过查询、键和RoPE的联合效应进行详细的数学分析。

Result: TAPPA框架成功解释了多样化的注意力模式。在KV缓存压缩和LLM剪枝任务中，基于TAPPA启发的简单指标持续优于基线方法。验证了该框架的实用价值。

Conclusion: TAPPA提供了一个统一框架来解释注意力模式，加深了对注意力行为的理解，并指导了推理加速方法。该框架在压缩和剪枝任务中表现出实用价值，为LLM优化提供了理论基础。
...
TL;DR: TACLer：一种基于课程强化学习的框架，通过渐进式增加数据复杂度和混合思考/非思考推理范式，在降低计算成本的同时提升LLMs在复杂推理任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在复杂推理任务中通常需要大规模RL训练来生成长链式思考，但这会导致过度思考、冗余中间步骤和高计算成本。需要一种既能提高学习推理效率，又能保持甚至增强性能的方法。

Method: 提出TACLer框架，包含两个核心组件：1) 定制化课程学习，根据模型在不同阶段的熟练度逐步增加数据复杂度；2) 混合思考/非思考推理范式，通过启用或禁用思考模式来平衡准确性和效率。

Result: TACLer在学习和推理方面带来双重优势：1) 降低计算成本，训练计算量比长思考模型减少50%以上，推理token使用比基础模型减少42%以上；2) 提升准确性，在基础模型上提升超过9%，在四个数学数据集上持续优于最先进的非思考和思考基线模型。

Conclusion: TACLer通过模型定制的课程强化学习框架，有效解决了长链式思考中的过度思考和计算效率问题，在保持或提升性能的同时显著降低了训练和推理成本，为LLMs的高效推理提供了新思路。
...
TL;DR: 提出参数高效框架，通过对比学习和序数排序目标构建LLM潜在空间，结合门控特征调制和MetaGradNorm，提升ESG报告绿色清洗检测的鲁棒性


<details>
  <summary>Details</summary>
Motivation: ESG可持续发展报告对评估至关重要，但绿色清洗和模糊声明常损害其可靠性。现有NLP模型对这些做法缺乏鲁棒性，通常依赖表层模式，泛化能力差。

Method: 提出参数高效框架，通过对比学习与序数排序目标相结合来结构化LLM潜在空间，捕捉具体行动与模糊声明之间的梯度差异。采用门控特征调制过滤披露噪声，使用MetaGradNorm稳定多目标优化。

Result: 跨类别设置实验显示优于标准基线的鲁棒性，同时揭示了表示刚性与泛化之间的权衡关系。

Conclusion: 该框架能有效检测ESG报告中的绿色清洗和模糊声明，通过结构化潜在空间和稳定优化策略提升模型鲁棒性，但需平衡表示刚性与泛化能力。
...
TL;DR: 提出CE-GOCD方法，通过学术知识图谱中的语义子结构建模来增强LLM的科学问答能力


<details>
  <summary>Details</summary>
Motivation: 现有检索增强方法依赖孤立文本块或概念，忽略了论文间深层语义连接，这损害了LLM对科学文献的理解，影响了回答的全面性和特异性

Method: CE-GOCD方法：1) 以论文标题为中心实体进行目标子图检索；2) 通过子图剪枝和补全增强隐式语义发现；3) 应用社区检测提炼具有共享主题的连贯论文组

Result: 在三个NLP文献问答数据集上评估，结果优于其他检索增强基线方法，证实了框架的有效性

Conclusion: 通过显式建模和利用学术知识图谱中的语义子结构，CE-GOCD方法能有效增强LLM的科学问答能力
...
TL;DR: 评估ChatGPT在4种医学信息抽取任务上的表现，发现其性能落后于微调模型，但能提供高质量解释，存在过度自信问题，对原文忠实度较高，生成不确定性影响信息抽取结果


<details>
  <summary>Details</summary>
Motivation: 研究社区对大型语言模型（如ChatGPT）在各种自然语言处理任务中的能力感兴趣，本文专注于评估ChatGPT在医学信息抽取任务上的整体能力

Method: 在6个基准数据集上对ChatGPT进行4种不同医学信息抽取任务的系统性分析，测量其性能、可解释性、置信度、忠实度和不确定性

Result: (a) ChatGPT在MedIE任务上的性能得分落后于微调基线模型；(b) ChatGPT能为其决策提供高质量解释，但对预测过度自信；(c) ChatGPT在大多数情况下对原始文本具有高度忠实性；(d) 生成中的不确定性导致信息抽取结果的不确定性，可能阻碍其在MedIE任务中的应用

Conclusion: ChatGPT在医学信息抽取任务中表现有限，虽然能提供高质量解释和保持忠实性，但性能不足、过度自信和生成不确定性限制了其实际应用
...
TL;DR: Zonkey是一种分层扩散模型，通过完全可训练的从原始字符到文档级表示的流程，解决了传统LLM固定、不可微分分词器的限制，实现了端到端优化和自适应分词。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型受限于固定、不可微分的分词器（如BPE），阻碍了端到端优化和对噪声或领域特定数据的适应性。需要一种完全可训练的分词方法来实现梯度传播和自适应分词。

Method: 提出Zonkey分层扩散模型：1）可微分分词器（Segment Splitter）学习概率性的序列开始决策；2）概率注意力机制支持软掩码和梯度保持；3）分层压缩序列到更高抽象级别；4）去噪扩散混合模型（DDMM）在潜在空间进行稳定去噪；5）Stitcher确保跨片段的重叠不变性。

Result: 在Wikipedia上端到端训练后，Zonkey能够从噪声生成连贯、可变长度的文本，展示了涌现的分层结构，与基于熵的可学习分词器相比，在数据分布对齐方面表现出有希望的定性结果。

Conclusion: Zonkey通过完全可训练的流程推进了基于梯度的LLM发展，具有更好的领域适应性和可扩展生成潜力。该方法为端到端优化的语言模型提供了新方向。
...
TL;DR: KID框架通过知识注入和双头学习，结合标签约束蒸馏和结构化推理链，显著提升了有害模因检测性能，在多个多语言数据集上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有模因检测方法主要关注模态内和模态间信号分析，但模因中的隐含毒性往往依赖于背景知识，这些知识在模因本身并不显式存在，导致检测困难。

Method: 提出KID框架：1) 采用标签约束蒸馏范式，将复杂模因理解分解为连接视觉证据、背景知识和分类标签的结构化推理链；2) 使用双头架构联合优化语义生成和分类目标，实现对齐的语言推理和稳定的决策边界。

Result: 在英语、中文和低资源孟加拉语的五个多语言数据集上，KID在二元和多标签有害模因检测任务上均达到SOTA性能，相比先前最佳方法在主要评估指标上提升2.1%--19.7%。

Conclusion: 知识注入和双头联合学习对稳健且可泛化的模因理解具有互补贡献，KID框架有效解决了依赖背景知识的隐含毒性检测挑战。
...
TL;DR: 提出对抗性记忆适应机制(AMA)，通过模拟任务执行来对齐离线记忆构建与任务目标，解决现有记忆系统中离线阶段固定且与任务无关的问题
...
Method: AMA机制包含三个智能体：挑战者生成问答对模拟下游推理，评估者评估响应并进行错误分析，适配者分析错误案例并对构建策略和内容进行双重更新

Result: 在长对话基准LoCoMo上的广泛实验证明了AMA的有效性，该机制可以集成到各种现有记忆系统中

Conclusion: AMA通过在离线阶段提供任务感知的监督信号，增强了记忆系统对下游任务的适应性，解决了记忆构建与任务目标不对齐的问题
...
TL;DR: RAG-E是一个端到端的可解释性框架，通过数学基础归因方法量化检索器-生成器对齐，揭示RAG系统中组件交互的关键错位问题。


<details>
  <summary>Details</summary>
Motivation: RAG系统结合密集检索器和语言模型，但组件交互不透明，在高风险领域部署面临挑战。需要量化检索器与生成器之间的对齐程度，以理解系统决策依据。

Method: 提出RAG-E框架：1) 采用集成梯度分析检索器；2) 引入PMCSHAP（蒙特卡洛稳定Shapley值近似）进行生成器归因；3) 提出WARG（加权归因-相关性差距）指标衡量生成器文档使用与检索器排名的对齐程度。

Result: 在TREC CAsT和FoodSafeSum数据集上的实证分析显示：47.4%-66.7%的查询中生成器忽略检索器排名最高的文档；48.1%-65.9%的查询中生成器依赖相关性较低的文档。这表明RAG输出质量不仅取决于单个组件性能，更取决于组件间的交互。

Conclusion: RAG-E能够审计RAG系统中检索器与生成器的交互，揭示关键错配模式，为高风险领域部署提供必要的可解释性保障，证明组件对齐对系统性能至关重要。
...
TL;DR: DARE提出了一种分布感知的奖励估计方法，替代传统多数投票机制，通过利用完整的经验rollout分布、探索奖励和分布剪枝来提供更稳健的奖励信号，显著提升测试时强化学习的效果。


<details>
  <summary>Details</summary>
Motivation: 现有测试时强化学习方法依赖多数投票机制产生确定性奖励，但这种方法存在缺陷：将rollout分布简化为单一结果，丢弃了非多数但正确的动作信息，导致系统性偏差的奖励估计。

Method: 提出分布感知奖励估计方法，从单一多数结果转向完整的经验rollout分布。进一步通过探索奖励和分布剪枝机制增强分布奖励，用于非多数rollout探索和奖励去噪。

Result: 在具有挑战性的推理基准测试中，DARE相比现有基线方法显著提升了优化稳定性和最终性能，在AIME 2024上实现25.3%的相对提升，在AMC上实现5.3%的相对提升。

Conclusion: DARE通过利用完整的rollout分布而非单一多数结果，提供了更丰富和稳健的奖励估计，有效解决了现有测试时强化学习方法中奖励估计的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLMs应用于更长的复杂任务，需要真实的长上下文基准测试，特别是军事规划等需要选择性阅读和整合异构多模态信息的场景

Method: 创建MilSCORE数据集，包含专家编写的多跳问题，基于复杂模拟军事规划场景，涵盖七类问题类型，评估协议和多种视觉语言模型的基线结果

Result: 当前系统在MilSCORE上表现不佳，表明现有模型难以处理真实场景级的长上下文规划问题，为未来工作提供了具有挑战性的测试平台

Conclusion: MilSCORE填补了长上下文基准测试的空白，特别针对地理空间规划问题，揭示了当前LLMs在复杂多模态推理方面的局限性
...
Method: 首先提出几何方法来解析重写检测算法的底层原理，展示其泛化能力；在此基础上引入新型重写检测算法，该算法自适应学习原始文本与重写文本之间的距离；理论上证明自适应学习距离函数比固定距离更有效。

Result: 在超过100个设置中进行广泛实验，发现该方法在大多数场景中表现出优于基线算法的性能；针对不同目标LLM（如GPT、Claude、Gemini），相对于最强基线的相对改进从57.8%到80.6%。

Conclusion: 通过几何视角理解重写检测算法，并提出自适应学习距离函数的方法，为LLM生成内容检测提供了更有效的解决方案，在多种实际场景中显著提升了检测性能。
...
TL;DR: SONIC是一个基于学习的KV缓存压缩框架，通过将历史对话段压缩为紧凑的Nexus令牌来解决多轮对话中的内存瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 现有KV缓存压缩方法通常忽略多轮对话的结构特性，依赖启发式淘汰策略可能导致关键上下文丢失，而线性增长的KV缓存是多轮LLM部署的主要瓶颈。
...
Conclusion: SONIC通过基于学习的压缩方法有效维持多轮对话的连贯性，显著提升部署效率，为解决KV缓存内存瓶颈提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 电子健康记录中非结构化临床叙述的增多为自动化疾病表征、队列识别和临床决策支持提供了新机会，但长领域特定临床文本建模面临标注数据有限、类别严重不平衡以及大型预训练语言模型适应计算成本高的挑战。

Method: 提出基于GPT的临床文本分类架构，采用选择性微调策略：冻结GPT-2主干的大部分参数，仅训练最后的Transformer块、最终层归一化和轻量级分类头，显著减少可训练参数同时保留建模复杂临床语言所需的表征能力。

Result: 在MIMIC-IV-Note放射学报告数据集上评估，使用从报告文本直接导出的不确定性感知CheXpert风格标签。实验涵盖多标签分类、不同不确定性假设下的二元标签分类和聚合疾病结果预测。模型在不同数据集规模下表现出稳定的收敛行为和强大的分类性能，特别是在非提及和否定发现占主导的场景中。
...
TL;DR: OVD是一种内存高效的在线策略蒸馏框架，用教师模型的离散语言分数（0-9）进行轨迹匹配，替代传统的token级概率匹配，解决了现有方法的内存瓶颈和探索限制问题。


<details>
  <summary>Details</summary>
Motivation: 现有token级在线策略蒸馏方法需要学生模型与教师模型在token级别对齐，这限制了学生模型的探索能力，阻碍了有效利用交互环境反馈，并在强化学习中存在严重的内存瓶颈。

Method: 提出在线语言蒸馏（OVD）框架，用教师模型的离散语言分数（0-9）进行轨迹匹配，替代token级概率匹配。该方法避免token级对齐，允许学生模型自由探索输出空间，同时显著降低内存消耗。

Result: 在Web问答和数学推理任务上的实验表明，OVD显著优于现有方法：在Web Q&A任务上平均EM绝对提升高达+12.9%，在数学基准上提升高达+25.7%（仅使用一个随机样本训练时），同时展现出优越的训练效率。

Conclusion: OVD通过用语言分数进行轨迹匹配，成功解决了现有token级在线策略蒸馏方法的内存瓶颈和探索限制问题，实现了更高效、更有效的知识蒸馏，为学生模型提供了更大的探索自由度和更好的性能。
...
Method: Token-Guard采用token级自检解码：1) 在每个推理步骤进行内部验证检测幻觉token；2) 在潜在空间中对候选片段进行显式幻觉风险评分；3) 迭代修剪和再生动态纠正检测到的错误。

Result: 在HALU数据集上的实验表明，Token-Guard显著减少了幻觉并提高了生成准确性，提供了一种可扩展、模块化的可靠LLM输出解决方案。

Conclusion: Token-Guard为LLM幻觉控制提供了一种轻量级、显式且可扩展的方法，代码已公开。
...
TL;DR: MDA框架通过影响函数将可解释电路溯源至训练数据，验证了针对性干预对可解释头形成的因果影响，揭示了重复结构数据的催化作用，并建立了归纳头与上下文学习的功能联系。


<details>
  <summary>Details</summary>
Motivation: 尽管机制可解释性已识别出LLM中的可解释电路，但这些电路在训练数据中的因果起源仍不清楚。研究旨在建立可解释单元与具体训练样本之间的因果联系。

Method: 提出机制数据归因（MDA）框架，利用影响函数将可解释单元溯源至特定训练样本。在Pythia模型家族上进行实验，通过针对性干预（移除或增强高影响样本）验证因果效应，并与随机干预对比。

Result: 针对性干预显著调节可解释头的形成，而随机干预无效果。发现重复结构数据（如LaTeX、XML）作为机制催化剂。针对归纳头形成的干预会同时改变模型的上下文学习能力，为归纳头与上下文学习的功能联系提供了直接因果证据。

Conclusion: MDA框架能够有效追溯LLM中可解释电路的训练数据起源。提出的机制数据增强流程能跨模型规模一致加速电路收敛，为引导LLM发展轨迹提供了原则性方法。
...
TL;DR: 提出了一个针对LLM应用的评估驱动工作流（定义、测试、诊断、修复），引入了最小可行评估套件（MVES），并通过实验展示了通用提示模板可能在不同任务表现间产生权衡
...
Method: 提出了评估驱动工作流（定义、测试、诊断、修复），引入了最小可行评估套件（MVES），包含通用LLM应用、检索增强生成（RAG）和智能体工具使用工作流的评估组件。综合了自动检查、人工评分和LLM作为评判者等评估方法

Result: 在可复现实验中，通用"改进"提示模板可能导致行为权衡：在Llama 3上，用通用规则替换任务特定提示后，提取通过率从100%降至90%，RAG合规性从93.3%降至80%，但指令遵循能力有所改善
...
TL;DR: CARD框架统一了自回归模型训练效率与扩散模型高吞吐推理，通过因果注意力掩码重构扩散过程实现单次前向传播的密集监督，引入软尾掩码和上下文重加权机制解决优化稳定性问题，支持动态并行解码。


<details>
  <summary>Details</summary>
Motivation: 现有方法在训练效率与推理吞吐之间存在权衡：自回归模型(ARMs)训练高效但推理串行，扩散模型推理并行但训练成本高。需要统一两者优势，实现高效训练与高吞吐推理的结合。

Method: 1) 在严格因果注意力掩码下重构扩散过程，实现单次前向传播的密集监督；2) 引入软尾掩码方案保留局部上下文；3) 基于信噪比原理设计上下文感知重加权机制；4) 利用KV缓存实现动态并行解码，根据置信度自适应生成变长序列。

Result: CARD在性能上超越现有离散扩散基线，训练延迟相比块扩散方法降低3倍，实现了ARM级别的数据效率同时解锁了并行生成的延迟优势。

Conclusion: CARD建立了下一代高效大语言模型的稳健范式，统一了训练效率与推理吞吐的优势，为高效LLM发展提供了新方向。
...
Result: Leviathan在等参数设置下持续优于标准的LLaMA风格架构。经验幂律拟合显示其具有显著优越的有效参数容量，在研究的参数范围内，Leviathan表现得像拥有1.47到2.11倍参数的密集模型。

Conclusion: 通过连续嵌入生成器替代离散查找表，Leviathan架构在小语言模型中实现了更优的参数效率分配，为SLM设计提供了新的架构方向。
...
<details>
  <summary>Details</summary>
Motivation: 揭示指令遵循可能干扰LLM任务解决能力的反直觉现象，需要量化这种干扰并理解其机制

Method: 提出SUSTAINSCORE指标，通过在指令中插入自明约束（从原始成功输出中提取）来测量任务性能下降；在数学、多跳QA和代码生成任务上测试当前LLM；分析失败模式、注意力机制和不同后训练范式的影响

Result: 添加自明约束导致性能显著下降（包括Claude-Sonnet-4.5等先进模型）；干扰现象在不同约束类型和规模上普遍存在；失败案例对约束分配更多注意力；不同对齐策略对干扰有不同影响

Conclusion: 指令遵循可能干扰LLM任务解决能力，SUSTAINSCORE有效量化该现象；需要重新思考对齐策略以平衡指令遵循和任务解决能力
...
TL;DR: MasalBench是一个评估多语言大语言模型对波斯谚语上下文理解和跨文化理解的基准测试，发现模型在识别波斯谚语方面表现良好（准确率>0.90），但在识别等效英语谚语方面表现显著下降（最佳模型准确率0.79）。


<details>
  <summary>Details</summary>
Motivation: 虽然之前的工作评估了LLMs在高资源语言中比喻语言的理解能力，但它们在低资源语言中的表现仍然未被充分探索。波斯语作为一种低资源语言，其谚语是对话的关键组成部分，需要评估LLMs对其上下文和跨文化的理解能力。

Method: 提出了MasalBench基准测试，用于评估LLMs对波斯谚语的上下文和跨文化理解。评估了8个最先进的LLMs，测试它们在识别上下文中的波斯谚语以及识别等效英语谚语方面的能力。

Result: LLMs在识别上下文中的波斯谚语方面表现良好，准确率超过0.90；但在识别等效英语谚语方面表现显著下降，最佳模型准确率仅为0.79。这表明当前LLMs在文化知识和类比推理方面存在局限性。

Conclusion: 当前LLMs在跨文化理解和类比推理方面存在明显不足，特别是在低资源语言中。MasalBench为评估其他低资源语言的跨文化理解提供了一个框架，并强调了改进LLMs文化知识的重要性。
...
TL;DR: G²-Reader是一个双图系统，通过内容图和规划图解决多模态文档问答中的结构保持和检索导航问题，在VisDoMBench上达到66.21%准确率，显著优于GPT-5等基线模型。


<details>
  <summary>Details</summary>
Motivation: 当前基于检索增强生成的方法在多模态长文档问答中存在两个核心问题：1）平面分块破坏了文档原生结构和跨模态对齐，产生难以独立解释的语义片段；2）迭代检索在长上下文中容易陷入局部证据循环或漂移到无关部分，因为每一步仅依赖当前片段而缺乏持久的全局搜索状态。

Method: 提出G²-Reader双图系统：1）内容图用于保持文档原生结构和跨模态语义；2）规划图是一个由子问题组成的智能有向无环图，用于跟踪中间发现并指导逐步导航以完成证据收集。

Result: 在VisDoMBench的五个多模态领域上，G²-Reader配合Qwen3-VL-32B-Instruct模型达到66.21%的平均准确率，显著优于强基线模型和独立的GPT-5（53.08%）。

Conclusion: G²-Reader通过双图架构有效解决了多模态长文档问答中的结构保持和检索导航问题，证明了内容图和规划图协同工作能够显著提升复杂文档理解任务的性能。
...
TL;DR: VTC-R1：一种新的高效推理范式，通过将中间推理段渲染为紧凑图像作为"光学记忆"，实现3.4倍token压缩和2.7倍端到端加速


<details>
  <summary>Details</summary>
Motivation: 长上下文推理虽然增强了LLMs处理复杂任务的能力，但带来了严重的效率瓶颈。现有高效方法通常需要复杂的额外训练或依赖外部模型进行压缩，限制了可扩展性并丢弃了关键的细粒度信息。
...
Result: 在MATH500、AIME25、AMC23和GPQA-D等基准测试中，VTC-R1始终优于标准长上下文推理。实现了3.4倍token压缩，端到端延迟获得2.7倍加速。

Conclusion: VTC-R1作为一种可扩展的解决方案，在保持推理质量的同时显著提高了效率，为推理密集型应用提供了有前景的方向。
...
TL;DR: 提出Fed-MedLoRA和Fed-MedLoRA+框架，通过联邦学习高效适应医学LLM，解决通信开销和数据异质性挑战，在临床信息抽取任务中验证效果。


<details>
  <summary>Details</summary>
Motivation: 医学LLM通常基于单一机构数据训练，存在泛化性和安全性局限；联邦学习虽能实现跨机构协作，但传统FL方法对大规模LLM通信开销大，且假设数据同质性，而真实临床数据高度异质。

Method: 提出参数高效的联邦学习框架：Fed-MedLoRA仅传输低秩适配器参数减少通信计算开销；Fed-MedLoRA+进一步引入自适应、数据感知的聚合策略，提升跨站点异质性下的收敛性能。

Result: 在临床信息抽取任务中，与BERT、LLaMA-3、DeepSeek-R1、GPT-4o等模型比较，评估了五种患者队列，包括域内训练测试、外部验证和低资源新站点适应场景，验证了框架有效性。

Conclusion: Fed-MedLoRA和Fed-MedLoRA+框架为医学LLM的联邦学习适应提供了高效解决方案，显著降低通信计算成本，同时通过自适应聚合处理临床数据异质性，在真实临床场景中表现出色。
...
TL;DR: 提出主动交互推理（PIR）新范式，将LLMs从被动求解器转变为主动询问者，通过交互澄清解决前提和意图层面的不确定性，显著提升推理性能并减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有CoT提示的LLMs存在"盲目自思考"局限，在关键信息缺失或模糊时仍进行大量内部推理。需要一种新范式让模型能够主动与用户交互澄清不确定性。

Method: PIR包含两个核心组件：1) 不确定性感知的监督微调，赋予模型交互推理能力；2) 基于用户模拟器的策略优化框架，使用复合奖励函数对齐模型行为与用户意图。

Result: 在数学推理、代码生成和文档编辑任务上，PIR显著优于基线方法：准确率提升32.70%，通过率提升22.90%，BLEU分数提升41.36，同时减少近一半的推理计算和不必要的交互轮次。

Conclusion: PIR通过主动交互推理有效解决了LLMs在前提和意图层面的不确定性问题，在多个任务上展现出优越性能、强泛化能力和鲁棒性，为LLMs推理提供了新范式。
...
TL;DR: 该论文提出FineInstructions方法，通过将互联网规模的预训练文档知识转化为数十亿个合成指令-答案训练对，使LLM能够仅使用指令调优目标从头开始预训练，从而更好地对齐下游使用场景。


<details>
  <summary>Details</summary>
Motivation: 由于监督训练数据有限，大型语言模型通常通过自监督的"预测下一个词"目标在大量非结构化文本上进行预训练，然后使用较少的指令调优数据进行微调。为了克服监督数据不足的问题，需要将预训练文档中的知识转化为大规模合成指令数据。

Method: 提出FineInstructions方法：1) 从真实用户查询和提示中创建约1800万个指令模板；2) 将这些指令模板与非结构化预训练语料库中的人工编写源文档进行匹配和实例化；3) 生成数十亿个合成指令-答案训练对，使LLM能够仅使用指令调优目标从头开始预训练。

Result: 在受控的token-for-token训练实验中，使用FineInstructions进行预训练在标准基准测试中优于标准预训练和其他提出的合成预训练技术，特别是在衡量自由形式响应质量的指标上表现更好。

Conclusion: FineInstructions方法能够将预训练文档知识有效转化为大规模合成指令数据，使LLM预训练更贴近下游使用场景，显著提升模型在响应质量方面的性能，为解决监督数据稀缺问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 混合Transformer架构结合了softmax注意力块和RNN，在长上下文建模中展现出理想的性能-吞吐量权衡，但大规模从头预训练成本过高。现有转换方法需要大量训练数据且长上下文性能不佳，而长上下文正是混合模型相比Transformer模型具有显著推理加速优势的场景

Method: 提出HALO（Hybrid Attention via Layer Optimization）管道，用于将Transformer模型蒸馏为RNN-注意力混合模型；提出HypeNet混合架构，采用新颖的位置编码方案（HyPE）和各种架构修改来实现优越的长度泛化能力

Result: 使用HALO将Qwen3系列转换为HypeNet，仅需2.3B tokens（不到预训练数据的0.01%）即可实现与原始Transformer模型相当的性能，同时获得优越的长上下文性能和效率

Conclusion: HALO和HypeNet提供了一种高效的方法将预训练Transformer转换为混合模型，显著降低了转换成本，同时保持了性能并提升了长上下文处理能力，解决了现有方法数据需求大且长上下文性能差的问题
...
### [134] [Rethinking LLM-Driven Heuristic Design: Generating Efficient and Specialized Solvers via Dynamics-Aware Optimization](https://arxiv.org/abs/2601.20868)
*Rongzheng Wang,Yihong Huang,Muquan Li,Jiakai Li,Di Liang,Bob Simons,Pei Ke,Shuang Liang,Ke Qin*

Main category: cs.LG

TL;DR: DASH框架通过收敛感知指标联合优化求解器搜索机制和运行时调度，并引入分析库检索实现高效预热启动，显著提升组合优化问题的求解效率和性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM驱动的启发式设计框架存在两个关键局限：1) 仅基于最终质量进行端点评估，忽略收敛过程和运行时效率；2) 分布偏移时适应成本高，需要为新实例组重新生成专用求解器。

Method: 提出DASH框架：1) 使用收敛感知指标指导求解器搜索机制和运行时调度的联合优化；2) 引入分析库检索(PLR)，在进化过程中并行归档专用求解器，实现异构分布下的高效预热启动。

Result: 在四个组合优化问题上的实验表明：DASH将运行时效率提升3倍以上，并在不同问题规模上超越最先进基线的解质量；通过基于分析的预热启动，在不同分布下保持优越精度，同时将LLM适应成本降低90%以上。

Conclusion: DASH框架通过收敛感知优化和高效预热启动机制，有效解决了现有LLM驱动启发式设计框架的局限性，显著提升了组合优化问题的求解效率和性能。
...
### [135] [Finetune-Informed Pretraining Boosts Downstream Performance](https://arxiv.org/abs/2601.20884)
*Atik Faysal,Mohammad Rostami,Reihaneh Gh. Roshan,Nikhil Muralidhar,Huaxia Wang*

Main category: cs.LG

TL;DR: Finetune-Informed Pretraining (FIP) 是一种针对多模态预训练的方法，通过在预训练阶段偏向目标模态来优化下游单模态微调性能。


<details>
  <summary>Details</summary>
Motivation: 传统多模态预训练对所有模态一视同仁，但在实际部署中，下游任务往往只使用单一模态。这种均匀处理可能导致实际使用模态的表征学习不足，影响下游性能。

Method: FIP采用模型无关的方法，通过三种策略偏向目标模态：1) 对目标模态使用更高的掩码难度；2) 更强的损失权重；3) 增加目标模态的解码器容量。这些调整不修改共享编码器，也不需要额外监督。

Result: 在无线信号星座图的掩码建模任务中，FIP持续提升了下游微调性能，且无需额外数据或计算资源。该方法简单易实现，与现有架构兼容。

Conclusion: FIP为多模态预训练提供了一种有效策略，通过预训练阶段的目标模态偏向，优化了下游单模态微调的性能，具有广泛的适用性。
...
### [136] [A generative machine learning model for designing metal hydrides applied to hydrogen storage](https://arxiv.org/abs/2601.20892)
*Xiyuan Liu,Christian Hacker,Shengnian Wang,Yuhua Duan*

Main category: cs.LG

TL;DR: 提出结合因果发现与轻量级生成模型的框架，用于生成新型金属氢化物候选材料，以解决现有材料数据库中氢化物数量有限的问题。


<details>
  <summary>Details</summary>
Motivation: 开发新型金属氢化物是实现高效氢储存的关键，但现有材料数据库（如Materials Project）中表征良好的氢化物数量有限，限制了最优候选材料的发现。

Method: 整合因果发现与轻量级生成机器学习模型，使用450个样本的数据集（270训练、90验证、90测试），生成1000个候选材料，通过排序和筛选识别新化学式与晶体结构。
...
### [137] [Is Parameter Isolation Better for Prompt-Based Continual Learning?](https://arxiv.org/abs/2601.20894)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于提示的持续学习方法通常为每个任务分配固定提示集，导致任务间知识完全隔离和参数利用率低下。需要更灵活高效的提示共享机制来改善持续学习性能。
...
Conclusion: 提示共享框架通过动态提示激活和协作优化，有效解决了固定提示分配导致的参数利用率低和知识隔离问题，为持续学习提供了更高效的解决方案。
...
### [138] [TwinWeaver: An LLM-Based Foundation Model Framework for Pan-Cancer Digital Twins](https://arxiv.org/abs/2601.20906)
...
<details>
  <summary>Details</summary>
Motivation: 精准肿瘤学需要预测临床事件和轨迹，但稀疏、多模态临床时间序列建模仍是一个关键挑战。现有方法难以有效处理这种复杂数据。

Method: 提出TwinWeaver开源框架，将纵向患者历史序列化为文本，使大语言模型能统一进行事件预测和预后。基于93,054名20种癌症类型患者构建Genie Digital Twin (GDT)。

Result: GDT显著降低预测误差，中位MASE为0.87 vs 基线0.97(p<0.001)。风险分层平均C-index达0.703 vs 基线0.662。在分布外临床试验中，零样本匹配训练基线，微调后表现更优(中位MASE 0.75-0.88，平均C-index 0.672 vs 0.648)。
...
### [139] [Noisy but Valid: Robust Statistical Evaluation of LLMs with Imperfect Judges](https://arxiv.org/abs/2601.20913)
*Chen Feng,Minghe Shen,Ananth Balashankar,Carsten Gerner-Beuerle,Miguel R. D. Rodrigues*

Main category: cs.LG

TL;DR: 提出一个"嘈杂但有效"的假设检验框架，用于在LLM评估者存在噪声和偏差的情况下，可靠地认证大语言模型的安全性阈值


<details>
  <summary>Details</summary>
Motivation: LLM作为评估者虽然具有可扩展性，但评估者的不完美、噪声和偏差会破坏统计保证，需要一种能够处理评估者不完美性的可靠认证方法

Method: 使用少量人工标注的校准集估计评估者的真阳性率和假阳性率，然后在大规模评估者标注数据集上应用方差校正的临界阈值，理论上保证有限样本的I类错误控制

Result: 实验在Jigsaw Comment、Hate Speech和SafeRLHF数据集上验证了理论，揭示了实用方法与理论"Oracle"之间的显著性能差距，量化了估计成本

Conclusion: 该框架首次系统处理了不完美评估者设置，提供了评估者可靠性的可解释诊断，阐明了评估能力如何依赖于评估者质量、数据集大小和认证水平
...
### [140] [Noninvasive Intracranial Pressure Estimation Using Subspace System Identification and Bespoke Machine Learning Algorithms: A Learning-to-Rank Approach](https://arxiv.org/abs/2601.20916)
...
TL;DR: 开发了一种结合系统辨识和排序约束优化的机器学习算法，用于从无创信号估计颅内压（ICP），约31.88%的测试数据误差在2 mmHg以内。
...
Method: 提出机器学习框架，使用子空间系统辨识算法基于动脉血压（ABP）、脑血流速度（CBv）和R-R间期信号识别脑血流动力学模型；通过凸优化学习描述无创信号特征与估计误差之间关系的映射函数，采用创新的排序约束。

Result: 测试结果显示，约31.88%的测试数据估计误差在2 mmHg以内，34.07%的测试数据误差在2-6 mmHg之间，证明了所提无创ICP估计方法的可行性。

Conclusion: 该研究证明了所提无创ICP估计方法的可行性，虽然需要进一步验证和技术改进才能临床部署，但为急性脑损伤患者的安全、广泛可及的ICP监测奠定了基础。
...
### [141] [A Theory of Universal Agnostic Learning](https://arxiv.org/abs/2601.20961)
*Steve Hanneke,Shay Moran*

Main category: cs.LG

TL;DR: 该论文建立了不可知设置下二元分类最优通用率的完整理论，将Bousquet等人的可实现情况理论扩展到非可实现分布，发现了最优率的四分类现象。
...
Method: 通过理论分析识别概念类的最优通用收敛率，建立四分类框架，并确定决定各类别的简单组合结构。

Result: 发现最优通用率呈现四分类：e^{-n}、e^{-o(n)}、o(n^{-1/2})或任意慢，并确定了决定各类别的组合结构。

Conclusion: 为不可知设置下的二元分类提供了完整的理论框架，揭示了最优率的四分类现象及其组合决定因素。
...
### [142] [Distributional Active Inference](https://arxiv.org/abs/2601.20985)
...
Method: 提出一个形式化的强化学习算法抽象，涵盖基于模型、分布式和免模型方法。该抽象将主动推理无缝整合到分布强化学习框架中，使其性能优势可在无需转移动态建模的情况下获得。

Result: 通过将主动推理整合到分布强化学习框架中，实现了无需建模转移动态即可获得主动推理的性能优势，解决了强化学习样本效率低下的问题。

Conclusion: 该研究提供了一个统一的框架，将主动推理的理论优势与分布强化学习的实践可行性相结合，为复杂环境中的高效机器人控制提供了新的解决方案。
...
### [143] [Pre-trained Encoders for Global Child Development: Transfer Learning Enables Deployment in Data-Scarce Settings](https://arxiv.org/abs/2601.20987)
*Md Muhtasim Munif Fahim,Md Rezaul Karim*

Main category: cs.LG

TL;DR: 开发首个全球儿童发展预训练编码器，使用UNICEF 44国35.7万儿童数据，仅需50个样本即可实现0.65 AUC，显著优于冷启动方法，解决资源受限环境下儿童发展监测的数据瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 每年大量儿童经历可预防的发展迟缓，但机器学习在新国家的部署面临数据瓶颈：可靠模型需要数千样本，而新项目开始时通常少于100个样本。需要解决资源受限环境下儿童发展监测的数据稀缺问题。

Method: 引入首个全球儿童发展预训练编码器，使用UNICEF调查数据在44个国家357,709名儿童上进行训练。采用迁移学习方法，通过预训练编码器实现少样本学习，并应用迁移学习理论解释预训练多样性如何促进少样本泛化。

Result: 仅需50个训练样本，预训练编码器平均AUC达到0.65（95% CI: 0.56-0.72），比冷启动梯度提升的0.61高出8-12%。在N=500时，AUC达到0.73。在未见国家进行零样本部署时，AUC最高可达0.84。

Conclusion: 预训练编码器能够显著改变资源受限环境下机器学习用于SDG 4.2.1监测的可行性，通过预训练多样性实现少样本泛化，为解决全球儿童发展监测的数据瓶颈提供了有效方案。
...
### [144] [Top-k on a Budget: Adaptive Ranking with Weak and Strong Oracles](https://arxiv.org/abs/2601.20989)
*Lutz Oettershagen*

Main category: cs.LG

TL;DR: 论文研究在弱、强两种oracle设置下的top-k识别问题，提出自适应认证算法ACE和ACE-W，显著减少对昂贵强oracle的调用次数。


<details>
  <summary>Details</summary>
Motivation: 在需要识别top-k项目的场景中，精确评估往往代价高昂。现实应用中通常存在快速但噪声大的弱oracle（如近似模型）和稀缺但高保真的强oracle（如专家验证或昂贵模拟）。如何有效结合两种oracle，最小化对昂贵强oracle的调用，是核心研究动机。

Method: 1. 分析基准方法STC（先筛选后认证），证明其强oracle调用上界为m(4ε_max)；2. 提出ACE算法，自适应地将强查询集中在关键边界项目上，达到相同上界但实际调用更少；3. 提出ACE-W两阶段方法，先自适应分配弱预算，再运行ACE，进一步降低强成本。

Result: 理论证明：STC的强调用上界为O(m(4ε_max))，且存在Ω(m(ε_max))的条件下界。ACE算法达到相同上界但实际性能更好。ACE-W通过自适应弱预算分配，进一步减少了强oracle的实际调用成本。

Conclusion: 论文在弱强双oracle设置下，为top-k识别问题提供了高效的自适应认证算法。ACE和ACE-W算法在理论保证下，显著减少了昂贵强oracle的实际调用，为解决资源受限的top-k识别问题提供了实用方案。
...
### [145] [The Depth Delusion: Why Transformers Should Be Wider, Not Deeper](https://arxiv.org/abs/2601.20994)
*Md Muhtasim Munif Fahim,Md Rezaul Karim*
...
<details>
  <summary>Details</summary>
Motivation: 传统神经缩放定律将架构视为可互换的，但实际中相同参数规模可以通过不同深度-宽度组合实现。需要理解架构如何影响缩放行为，特别是深度与宽度的最优权衡。

Method: 提出架构条件化的缩放定律，将损失函数分解为对深度和宽度的依赖。通过30个不同架构的Transformer模型（1700万到70亿参数）进行实证验证，每个模型在代表性高计算样本上训练。

Result: 发现最优深度缩放为D* ~ C^0.12，最优宽度缩放为W* ~ C^0.34，宽度增长应比深度快2.8倍。揭示临界深度现象：超过D_crit ~ W^0.44后增加层数反而增加损失。在70亿规模下，64层模型（63.8亿参数）比32层模型（68.6亿参数）性能差0.12 nats。

Conclusion: 深度-宽度的最优权衡在工业规模下仍然存在，架构设计对模型性能有显著影响。深度错觉现象表明单纯增加深度不一定提升性能，需要平衡深度与宽度。
...
### [146] [Solver-in-the-Loop: MDP-Based Benchmarks for Self-Correction and Behavioral Rationality in Operations Research](https://arxiv.org/abs/2601.21008)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM基准将运筹学评估为一次性翻译任务（给定问题描述生成求解器代码），完全忽略了实践中调试不可行模型的迭代诊断循环

Method: 引入两个新基准：1) ORDebug：通过5000+问题评估迭代自修正，每次修复触发求解器重新执行和IIS重新计算；2) ORBias：通过2000个报童问题评估行为理性，测量与闭式最优策略的系统性偏差

Result: 领域特定的RLVR训练使8B模型超越前沿API：恢复率95.3% vs 86.2%（+9.1%），诊断准确率62.4% vs 47.8%（+14.6%），解决步骤2.25 vs 3.78（1.7倍更快）。在ORBias上，课程训练实现了唯一的负ID→OOD偏差漂移（-9.6%），系统性偏差减少48%（从20.0%到10.4%）

Conclusion: 通过可验证预言机的过程级评估能够实现超越规模的目标训练，表明运筹学评估需要纳入求解器反馈循环，而不仅仅是代码生成
...
### [147] [Human-LLM Collaborative Feature Engineering for Tabular Data](https://arxiv.org/abs/2601.21060)
...
TL;DR: 提出了一种人机协作的特征工程框架，将LLM的候选生成与基于效用估计的选择过程解耦，并通过人类专家反馈优化操作选择
...
Conclusion: 通过解耦LLM的候选生成与基于效用估计的选择过程，并引入人类专家反馈，能够更有效地指导特征工程，提高性能并降低用户认知负担
...
### [148] [Order-Aware Test-Time Adaptation: Leveraging Temporal Dynamics for Robust Streaming Inference](https://arxiv.org/abs/2601.21012)
*Young Kyung Kim,Oded Schlesinger,Qiangqiang Wu,J. Matías Di Martino,Guillermo Sapiro*

Main category: cs.LG

TL;DR: OATTA是一种考虑时间动态的测试时适应方法，通过贝叶斯估计和时间先验提升模型在分布偏移下的性能，同时使用似然比门控确保安全性。


<details>
  <summary>Details</summary>
Motivation: 现有测试时适应方法通常将测试流视为独立样本，忽略了时间动态中蕴含的监督信号，这限制了模型对分布偏移的适应能力。

Method: 将测试时适应建模为无梯度递归贝叶斯估计任务，使用学习到的动态转移矩阵作为时间先验来优化基础模型预测，并引入似然比门控在缺乏时间证据时回退到基础预测器。

Result: 在图像分类、可穿戴和生理信号分析、语言情感分析等多个领域的实验中，OATTA持续提升现有基线性能，准确率最高提升6.35%，证明了时间动态建模的重要性。

Conclusion: 建模时间动态为测试时适应提供了关键的正交信号，超越了标准的时间顺序无关方法，OATTA作为轻量级、模型无关的模块具有广泛适用性。
...
### [149] [SIGMA-PPG: Statistical-prior Informed Generative Masking Architecture for PPG Foundation Model](https://arxiv.org/abs/2601.21031)
*Zongheng Guo,Tao Chen,Yang Jiao,Yi Pan,Xiao Hu,Manuela Ferrario*

Main category: cs.LG

TL;DR: SIGMA-PPG是一种针对PPG信号的生成式基础模型，通过先验引导的对抗性掩码机制和语义一致性约束，解决了传统方法在噪声处理和形态学精度方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前PPG信号基础模型面临信号内在冗余和噪声的挑战。标准掩码建模容易产生平凡解，而对比方法缺乏形态学精度。需要解决这些限制以提升PPG信号处理效果。

Method: 提出统计先验引导的生成式掩码架构(SIGMA-PPG)，包含：1)先验引导对抗性掩码机制，利用强化学习驱动的教师模型基于统计先验创建具有挑战性的学习路径；2)通过向量量化引入语义一致性约束，确保生理学上相同的波形映射到共享索引。

Result: 在超过120,000小时数据上预训练后，SIGMA-PPG在12个不同的下游任务中，相比5个最先进的基线方法取得了优越的平均性能。

Conclusion: SIGMA-PPG通过创新的生成式架构和语义约束机制，有效解决了PPG信号处理中的噪声和冗余问题，为PPG基础模型提供了新的解决方案。
...
### [150] [A Unified SPD Token Transformer Framework for EEG Classification: Systematic Comparison of Geometric Embeddings](https://arxiv.org/abs/2601.21521)
*Chi-Sheng Chen,En-Jui Kuo,Guan-Ying Chen,Xinyu Zhang,Fan Zhang*

Main category: cs.LG

TL;DR: 该论文分析了SPD流形嵌入选择对梯度条件和数值稳定性的理论影响，提出了BWSPD嵌入具有更好的梯度条件，Embedding-Space BN近似黎曼归一化，并在统一Transformer框架中验证了Log-Euclidean嵌入在EEG任务上的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 探索SPD流形嵌入几何与优化动力学之间的理论联系，分析不同嵌入选择对梯度条件和数值稳定性的影响，为EEG信号处理提供理论指导。

Method: 提出三个理论结果：1) BWSPD通过Daleckii-Kreĭn矩阵提供√κ梯度条件；2) Embedding-Space BN近似黎曼归一化；3) bi-Lipschitz界限证明BWSPD保持流形距离。在统一Transformer框架中比较BWSPD、Log-Euclidean和Euclidean嵌入，在三种EEG范式上进行1500+次实验验证。

Result: Log-Euclidean Transformer在所有数据集上达到SOTA性能，显著优于经典黎曼分类器和近期SPD基线；BWSPD提供竞争性精度和相似训练时间；理论预测得到验证：高维输入(d≥22)时BWSPD梯度条件优势明显，低维(d≤8)时特征分解开销占主导；Embedding-Space BN在56通道ERP数据上提升26%准确率。

Conclusion: 建立了SPD流形嵌入几何与优化动力学的理论联系，为嵌入选择提供了理论依据；Log-Euclidean嵌入在实际EEG任务中表现最佳，而BWSPD在高维场景下具有理论优势；Embedding-Space BN有效近似黎曼归一化，特别适用于高通道数EEG数据。
...
### [151] [Predict-Project-Renoise: Sampling Diffusion Models under Hard Constraints](https://arxiv.org/abs/2601.21033)
...
Method: 引入约束采样框架：定义约束前向过程，仅在满足约束的可行集上进行扩散；提出PPR算法，通过去噪预测、投影到可行集和重新加噪的迭代过程从约束边缘分布中采样

Result: 在2D分布、PDE和全球天气预报实验中，PPR将约束违反减少了一个数量级以上，同时提高了样本一致性，更好地匹配真实约束分布

Conclusion: PPR框架成功地将硬约束整合到扩散模型中，显著提高了科学应用中神经模拟器的物理准确性和约束满足能力
...
### [152] [Thinking in Frames: How Visual Context and Test-Time Scaling Empower Video Reasoning](https://arxiv.org/abs/2601.21037)
...
Method: 将视觉推理问题转化为视频生成任务，生成的帧作为初始状态到解决方案的中间推理步骤。在两个不同领域进行评估：迷宫导航（序列离散规划，视觉变化小）和七巧板拼图（连续操作，视觉变化大）

Result: 实验发现三个关键洞察：1) 强大的零样本泛化能力：模型在未见数据分布上表现优异；2) 视觉上下文利用：模型能有效使用视觉上下文作为显式控制；3) 视觉测试时扩展：在序列规划中观察到测试时扩展定律，增加生成视频长度能提升零样本泛化能力
...
### [153] [Test-Time Adaptation for Unsupervised Combinatorial Optimization](https://arxiv.org/abs/2601.21048)
*Yiqiao Liao,Farinaz Koushanfar,Parinaz Naghizadeh*

Main category: cs.LG

TL;DR: TACO提出了一种模型无关的测试时适应框架，统一了无监督神经组合优化的泛化模型和实例特定模型两种范式，通过策略性热启动实现高效实例适应。


<details>
  <summary>Details</summary>
Motivation: 现有无监督神经组合优化方法存在两种分离范式：训练用于跨实例泛化的模型缺乏实例级适应性，而实例特定模型无法利用学习到的归纳结构且易陷入局部最优。需要一种方法既能利用泛化学习的归纳偏置，又能获得有效的实例级适应能力。

Method: 提出TACO框架，采用策略性热启动方法：部分放松训练参数同时保留归纳偏置，实现快速有效的无监督适应。该框架模型无关，统一并扩展了现有两种范式。

Result: 在最小顶点覆盖和最大团等经典组合优化问题上，TACO在静态、分布偏移和动态场景下均表现出有效性和鲁棒性。相比直接微调泛化模型或从头训练实例特定模型，TACO获得更好解质量且计算成本可忽略。

Conclusion: TACO成功构建了泛化性和实例特定无监督神经组合优化之间的实用桥梁，解决了泛化模型作为热启动时性能不佳的问题，实现了归纳偏置保留与实例适应性的统一。
...
### [154] [Snowball: A Scalable All-to-All Ising Machine with Dual-Mode Markov Chain Monte Carlo Spin Selection and Asynchronous Spin Updates for Fast Combinatorial Optimization](https://arxiv.org/abs/2601.21058)
...
<details>
  <summary>Details</summary>
Motivation: 伊辛机作为组合优化加速器面临三个挑战：硬件拓扑限制需要minor embedding、朴素并行更新可能导致振荡或停滞、有限精度可能影响映射可行性或解质量，需要减少求解时间以实现实际部署
...
Conclusion: Snowball通过解决硬件拓扑、自旋选择更新算法和耦合系数精度三大挑战，显著减少了伊辛机的求解时间，为实际部署提供了有效方案
...
TL;DR: 针对复合AI系统中深度扩展问题，提出文本均衡传播(TEP)方法，通过局部学习和前向信号传播解决文本梯度爆炸和消失问题，在深度工作流中优于全局传播方法。


<details>
  <summary>Details</summary>
Motivation: 复合AI系统中，随着工作流深度增加，全局文本反馈传播方法（如TextGrad）出现性能下降问题，主要存在文本梯度爆炸和文本梯度消失两种深度扩展失效模式，需要新的优化方法。

Method: 提出文本均衡传播(TEP)，受能量基模型中均衡传播启发，包含两个阶段：自由阶段（局部LLM批评器迭代优化提示直至均衡）和推动阶段（应用有界修改强度的近端提示编辑，通过前向信号而非后向反馈链传播任务级目标）。

Result: 在长视野QA基准测试和多智能体工具使用数据集上，TEP在准确性和效率上持续优于全局传播方法（如TextGrad），且随着深度增加增益更明显，同时保持黑盒LLM组件的实用性。

Conclusion: TEP通过局部优化和受控适应，有效缓解了复合AI系统中深度扩展的文本梯度问题，为深度复合AI系统提供了更实用和可扩展的优化方法。
...
TL;DR: 本文首次系统综述了图基础模型在分布外泛化方面的研究进展，分析了图学习中的分布偏移挑战，并基于任务规范统一分类现有方法。


<details>
  <summary>Details</summary>
Motivation: 图学习模型在实际应用中常面临分布偏移问题，包括图结构变化、领域语义差异、模态可用性变化和任务形式差异等挑战。图基础模型通过大规模预训练学习通用表示，有望解决这些泛化问题，但目前缺乏系统综述。

Method: 本文从分布外泛化视角综述图基础模型研究：1) 分析图学习中的分布偏移挑战并建立统一问题设定；2) 按任务规范是否固定将现有方法分类；3) 总结相应的OOD处理策略和预训练目标；4) 回顾常用评估协议。

Result: 本文首次系统梳理了图基础模型在分布外泛化方面的研究进展，建立了统一的问题框架和分类体系，总结了不同方法的核心策略，为后续研究提供了理论基础和方向指引。

Conclusion: 图基础模型为解决图学习中的分布外泛化问题提供了有前景的方向，但该领域仍处于早期发展阶段，需要更多研究来应对实际应用中的复杂分布偏移挑战，并提出了未来研究方向。
...
### [157] [Safety Generalization Under Distribution Shift in Safe Reinforcement Learning: A Diabetes Testbed](https://arxiv.org/abs/2601.21094)
*Minjae Kwon,Josephine Lamp,Lu Feng*

Main category: cs.LG

TL;DR: 安全强化学习算法在糖尿病管理场景中面临安全泛化问题：训练时满足安全约束的策略在未见患者上频繁违反安全要求，测试时屏蔽技术能有效恢复安全性


<details>
  <summary>Details</summary>
Motivation: 研究安全强化学习算法在分布偏移下的安全泛化能力，特别是在糖尿病管理这一安全关键领域，验证训练时的安全保证是否能迁移到部署阶段

Method: 在统一的临床模拟器上对八种安全RL算法进行基准测试，使用测试时屏蔽技术（通过学习的动力学模型过滤不安全动作）来恢复安全性，评估三种糖尿病类型和三个年龄组

Result: 发现安全泛化差距，测试时屏蔽能有效恢复安全性：对于PPO-Lag和CPO等强基线，屏蔽技术使时间在范围内提升13-14%，同时降低临床风险指数和血糖变异性

Conclusion: 安全强化学习算法在分布偏移下存在安全泛化问题，测试时屏蔽是有效的解决方案，提供的模拟器和基准为研究安全关键控制领域的安全泛化提供了平台
...
### [158] [TRACE: Trajectory Recovery for Continuous Mechanism Evolution in Causal Representation Learning](https://arxiv.org/abs/2601.21135)
*Shicheng Fan,Kun Zhang,Lu Cheng*

Main category: cs.LG

TL;DR: 提出TRACE方法，用于学习连续机制过渡的因果表示，而非传统的离散切换假设


<details>
  <summary>Details</summary>
Motivation: 现有时间因果表示学习方法假设因果机制在离散域之间瞬时切换，但现实世界系统通常表现出连续的机制过渡（如车辆转弯、人行走跑转换）

Method: 将过渡机制建模为有限原子机制的凸组合，由时变混合系数控制；提出TRACE框架，采用专家混合方法，每个专家学习一个原子机制，测试时恢复机制轨迹

Result: 理论证明潜在因果变量和连续混合轨迹可联合识别；实验表明TRACE在合成和真实数据上恢复混合轨迹相关性高达0.99，显著优于离散切换基线

Conclusion: TRACE能够有效建模连续机制过渡，泛化到训练中未观察到的中间机制状态，为现实世界连续变化系统提供了更准确的因果表示学习方法
...
### [159] [Smooth Dynamic Cutoffs for Machine Learning Interatomic Potentials](https://arxiv.org/abs/2601.21147)
*Kevin Han,Haolin Cong,Bowen Deng,Amir Barati Farimani*

Main category: cs.LG

TL;DR: 该论文提出了一种动态截止半径方法，用于解决机器学习原子间势能（MLIPs）在推理时间和内存消耗方面的瓶颈问题，通过动态调整原子邻域数量来减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 机器学习原子间势能（MLIPs）在分子动力学模拟中应用广泛，但面临推理时间和内存消耗两大瓶颈，阻碍了其达到真实模拟规模。传统MLIPs使用固定截止半径，作者挑战了这一长期假设。

Method: 首次引入动态截止半径公式，通过针对每个原子设定特定的邻居数量来诱导底层原子图的稀疏性。该方法在四种最先进的MLIPs（MACE、Nequip、Orbv3和TensorNet）上实现，显著减少了内存消耗和推理时间。

Result: 动态截止半径方法实现了2.26倍的内存消耗减少和2.04倍的推理时间加速（具体数值取决于模型和原子系统）。误差分析表明，与固定截止半径模型相比，动态截止半径模型在材料和分子数据集上仅表现出最小的精度下降。

Conclusion: 动态截止半径方法有效解决了MLIPs的推理时间和内存消耗瓶颈，同时保持了模拟的稳定性和精度。所有模型实现和训练代码将完全开源。
...
### [160] [Mobility-Embedded POIs: Learning What A Place Is and How It Is Used from Human Movement](https://arxiv.org/abs/2601.21149)
*Maria Despoina Siampou,Shushman Choudhury,Shang-Ling Hsu,Neha Arora,Cyrus Shahabi*

Main category: cs.LG

TL;DR: ME-POIs框架通过结合语言模型生成的POI嵌入与大规模人类移动数据，学习基于真实使用情况的POI中心化、上下文无关表示，显著提升了POI表征的准确性和泛化能力。
...
Method: 提出ME-POIs框架：1) 将语言模型生成的POI嵌入与大规模人类移动数据结合；2) 将个体访问编码为时间上下文嵌入，并通过对比学习与可学习的POI表示对齐；3) 为解决长尾稀疏性问题，提出新颖机制，在多个空间尺度上传播附近频繁访问POI的时间访问模式。

Result: 在五个新提出的地图丰富任务上评估，ME-POIs在所有任务中一致优于纯文本和纯移动基线。值得注意的是，仅使用移动数据训练的ME-POIs在某些任务上能超越纯文本模型，表明POI功能是准确且可泛化POI表示的关键组成部分。

Conclusion: POI功能是通用POI表示中缺失但至关重要的信号。ME-POIs框架通过将语言模型生成的POI嵌入与大规模人类移动数据相结合，成功捕捉了POI的身份和功能，为地理空间基础模型提供了更全面、更准确的POI表示方法。
...
### [161] [A Federated Generalized Expectation-Maximization Algorithm for Mixture Models with an Unknown Number of Components](https://arxiv.org/abs/2601.21160)
*Michael Ibrahim,Nagi Gebraeel,Weijun Xie*

Main category: cs.LG

TL;DR: FedGEM：一种用于未知聚类数混合模型训练的联邦广义期望最大化算法，通过客户端本地EM步骤和不确定性集构建，服务器利用不确定性集学习客户端间聚类重叠并推断全局聚类数。


<details>
  <summary>Details</summary>
Motivation: 解决联邦聚类中全局聚类数K未知且客户端具有异构但可能重叠的聚类集的问题，现有方法通常假设已知全局聚类数或客户端聚类集相同，无法处理真实场景中的异构性和未知聚类数。

Method: FedGEM算法：1) 客户端本地执行EM步骤；2) 为每个本地组件构建最大化器周围的不确定性集；3) 中央服务器利用不确定性集学习客户端间潜在聚类重叠；4) 通过闭式计算推断全局聚类数；针对各向同性GMM提供可处理、低复杂度计算。

Result: 1) 在常见假设下提供概率收敛保证；2) 针对各向同性GMM验证算法收敛所需假设；3) 数值实验表明FedGEM性能与集中式EM相当，优于现有联邦聚类方法。

Conclusion: FedGEM是首个能够处理未知全局聚类数和客户端异构重叠聚类集的联邦聚类算法，具有理论保证和实际可行性，在联邦学习场景中实现了与集中式方法相当的性能。
...
### [162] [Generalized Information Gathering Under Dynamics Uncertainty](https://arxiv.org/abs/2601.21988)
...
Method: 提出统一框架，通过显式暴露参数、信念和控制之间的因果依赖关系，基于Massey定向信息推导通用信息收集成本。该成本仅假设马尔可夫动力学和加性噪声，对其他建模选择保持不可知。证明互信息成本是该成本的特例，并建立互信息成本与线性化贝叶斯估计中信息增益的明确理论联系。

Result: 1. 成功推导出通用的信息收集成本，能够解耦具体建模选择；2. 证明现有文献中的互信息成本是该通用成本的特例；3. 建立了互信息成本与线性化贝叶斯估计中信息增益的明确理论联系，为基于互信息的主动学习方法提供了理论依据；4. 通过线性、非线性和多智能体系统的实验验证了框架的实用性。

Conclusion: 提出的统一框架成功解决了主动信息收集中成本函数与具体建模选择耦合的问题，提供了通用的信息收集成本计算方法。该框架不仅统一了现有方法，还为基于互信息的主动学习方法提供了理论依据，具有广泛的适用性和实用价值。
...
### [163] [Efficient Simple Regret Algorithms for Stochastic Contextual Bandits](https://arxiv.org/abs/2601.21167)
*Shuai Liu,Alireza Bakhtiari,Alex Ayoub,Botao Hao,Csaba Szepesvári*

Main category: cs.LG

TL;DR: 论文研究了随机上下文逻辑赌博机在简单遗憾目标下的性能，提出了首个实现$\tilde{\mathcal{O}}(d/\sqrt{T})$简单遗憾的确定性算法，以及基于Thompson Sampling的随机算法，其遗憾为$\tilde{\mathcal{O}}(d^{3/2}/\sqrt{T})$，且主要项不依赖于参数大小相关的常数κ。


<details>
  <summary>Details</summary>
Motivation: 在随机上下文逻辑赌博机中，虽然线性情况已有简单遗憾的理论保证，但逻辑回归设置下尚无类似结果。现有算法通常依赖于与未知参数向量大小相关的常数κ，这在实际应用中可能带来较大遗憾。

Method: 结合上下文线性赌博机和自协调分析的思想，提出了首个确定性算法。同时设计了专门针对简单遗憾设置的Thompson Sampling变体，分别适用于线性（$\tilde{\mathcal{O}}(d^{3/2}/\sqrt{T})$）和逻辑（同样$\tilde{\mathcal{O}}(d^{3/2}/\sqrt{T})$）情况。当动作集有限时，算法完全可计算。

Result: 确定性算法达到$\tilde{\mathcal{O}}(d/\sqrt{T})$简单遗憾，主要项不依赖于κ=$\mathcal{O}(\exp(S))$。随机算法在线性情况下达到$\tilde{\mathcal{O}}(d^{3/2}/\sqrt{T})$，逻辑情况下同样达到$\tilde{\mathcal{O}}(d^{3/2}/\sqrt{T})$，且主要项均不依赖κ。随机算法计算成本更低。实验验证了理论保证。

Conclusion: 该研究首次为随机上下文逻辑赌博机提供了简单遗憾的理论保证，提出的算法在主要项上消除了对参数大小相关常数κ的依赖，为逻辑回归设置下的简单遗憾优化提供了有效解决方案。随机算法在保持理论性能的同时降低了计算成本。
...
### [164] [The Powers of Precision: Structure-Informed Detection in Complex Systems -- From Customer Churn to Seizure Onset](https://arxiv.org/abs/2601.21170)
*Augusto Santos,Teresa Santos,Catarina Rodrigues,José M. F. Moura*

Main category: cs.LG

TL;DR: 提出一种基于协方差矩阵幂次变换的机器学习方法，用于从部分观测的复杂系统中早期检测癫痫发作、客户流失等涌现现象，同时揭示潜在因果结构。


<details>
  <summary>Details</summary>
Motivation: 涌现现象（如癫痫发作、客户流失、疫情爆发）通常源于复杂系统中隐藏的因果相互作用。核心挑战在于：如何在数据生成过程未知且部分观测的情况下，揭示并利用系统的潜在因果结构进行早期检测。

Method: 提出一种机器学习方法：1）从单参数估计器族（经验协方差或精度矩阵的幂次）中学习最优特征表示，通过调整参数来捕捉驱动关键事件的基础结构；2）使用监督学习模块对学习到的表示进行分类。证明了该族的结构一致性。

Result: 在癫痫发作检测和客户流失预测任务上验证了方法的有效性，取得了有竞争力的结果。发现最优协方差幂次在保持良好可识别性的同时捕获了结构特征，实现了预测性能与可解释统计结构的统一。

Conclusion: 该方法不仅能有效预测涌现现象，还能通过协方差矩阵的幂次变换揭示系统的潜在因果结构，实现了预测性能与可解释性的平衡，为复杂系统中关键事件的早期检测提供了新思路。
...
### [165] [AC2L-GAD: Active Counterfactual Contrastive Learning for Graph Anomaly Detection](https://arxiv.org/abs/2601.21171)
*Kamal Berahmand,Saman Forouzandeh,Mehrnoush Mohammadi,Parham Moradi,Mahdi Jalili*

Main category: cs.LG

TL;DR: AC2L-GAD：一种主动反事实对比学习框架，通过信息论主动选择和反事实生成解决图异常检测中的语义一致性和对比信息不足问题，在保持检测质量的同时减少65%计算开销。
...
Method: 提出AC2L-GAD框架，结合信息论主动选择和反事实生成：1）识别结构复杂节点；2）生成保持异常特征的正样本增强；3）生成正常负样本提供困难对比；4）将昂贵的反事实生成限制在策略选择的子集上。

Result: 在九个基准数据集（包括GADBench的真实金融交易图）上实验，AC2L-GAD相比全图反事实生成减少约65%计算开销，同时在异常表现出复杂属性-结构交互的数据集上取得显著性能提升，达到或超过最先进基线。

Conclusion: AC2L-GAD通过主动反事实对比学习有效解决了图异常检测中的语义一致性和对比信息不足问题，在保持检测质量的同时显著降低计算成本，特别适用于具有复杂属性-结构交互的异常检测场景。
...
### [166] [Breaking the Reasoning Horizon in Entity Alignment Foundation Models](https://arxiv.org/abs/2601.21174)
...
<details>
  <summary>Details</summary>
Motivation: 现有实体对齐模型缺乏可迁移性，无法对齐未见知识图谱而需要重新训练。虽然图基础模型提供了解决方案，但直接应用于实体对齐效果不佳，存在关键的"推理视野差距"：与图基础模型中的链接预测不同，实体对齐需要捕捉稀疏异构知识图谱结构中的长程依赖关系。

Method: 提出基于并行编码策略的实体对齐基础模型：1) 使用种子对齐对作为局部锚点引导信息流，同时初始化并编码两个并行流；2) 实现锚点条件化的消息传递，利用局部结构邻近性而非全局搜索显著缩短推理轨迹；3) 引入合并关系图建模全局依赖关系；4) 设计可学习的交互模块实现精确匹配。

Result: 大量实验验证了框架的有效性，展示了其对未见知识图谱的强大泛化能力。

Conclusion: 提出的并行编码策略有效解决了实体对齐中的推理视野差距问题，通过锚点引导的信息流和局部结构利用，实现了对未见知识图谱的强泛化能力，为实体对齐基础模型提供了有效解决方案。
...
### [167] [Flow Perturbation++: Multi-Step Unbiased Jacobian Estimation for High-Dimensional Boltzmann Sampling](https://arxiv.org/abs/2601.21177)
*Xin Peng,Ang Gao*

Main category: cs.LG

TL;DR: Flow Perturbation++：一种方差降低的无偏Jacobian估计方法，用于连续归一化流的玻尔兹曼采样，通过多步离散化概率流ODE实现


<details>
  <summary>Details</summary>
Motivation: 连续归一化流在无偏玻尔兹曼采样中的可扩展性受限于Jacobian行列式计算成本（需要D次反向传播）。现有方法如Hutchinson迹估计器有偏，而Flow Perturbation方法无偏但方差高

Method: 提出Flow Perturbation++，作为Flow Perturbation的方差降低扩展。该方法离散化概率流ODE，在每个积分步骤执行无偏的逐步Jacobian估计。这种多步构造保留了无偏性同时显著降低估计方差

Result: 在1000维高斯混合模型和全原子Chignolin蛋白质上，Flow Perturbation++在序贯蒙特卡洛框架中实现了显著改进的平衡采样性能，优于Hutchinson基线和单步Flow Perturbation基线

Conclusion: Flow Perturbation++通过多步离散化策略有效解决了连续归一化流中Jacobian估计的方差问题，为高维系统的无偏玻尔兹曼采样提供了更高效的计算方法
...
### [168] [Rethinking Refinement: Correcting Generative Bias without Noise Injection](https://arxiv.org/abs/2601.21182)
*Xin Peng,Ang Gao*

Main category: cs.LG

TL;DR: 提出BFR框架，通过潜在空间对齐和数据空间精炼两阶段方法，在不扰动原始ODE轨迹的情况下，对生成模型进行后处理偏差校正，显著提升样本质量。


<details>
  <summary>Details</summary>
Motivation: 生成模型（如扩散模型和基于流的模型）在高维设置中常表现出系统性偏差，降低样本质量。现有精炼方法通常需要噪声注入或多步重采样，我们希望开发一种无需扰动采样动态的后处理偏差校正方法。

Method: 提出Bi-stage Flow Refinement (BFR)框架，包含两种精炼策略：1) 针对近似可逆生成器的潜在空间对齐；2) 使用轻量级增强训练的数据空间精炼。该方法保持原始ODE轨迹，对生成样本应用确定性校正。

Result: 在MNIST、CIFAR-10和FFHQ（256x256分辨率）数据集上实验显示，BFR在保真度和覆盖率方面均有持续改进。特别地，从FID 3.95的基础样本开始，潜在空间精炼在MNIST上仅使用1次额外函数评估（1-NFE）就达到了SOTA的FID 1.46，同时保持了样本多样性。

Conclusion: BFR框架证明，有效的偏差校正可以作为后处理程序实现，无需噪声注入或多步重采样。该方法通过保持原始ODE轨迹并应用确定性校正，在保持样本多样性的同时显著提升了生成质量。
...
### [169] [Rethinking Self-Training Based Cross-Subject Domain Adaptation for SSVEP Classification](https://arxiv.org/abs/2601.21203)
*Weiguang Wang,Yong Liu,Yingjie Gao,Guangyuan Xu*

Main category: cs.LG

TL;DR: 提出基于自训练范式的跨被试SSVEP-BCI域适应方法，包含滤波组欧式对齐、对抗预训练和双集成自训练，通过时频增强对比学习提升特征判别性，在多个数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: SSVEP-BCI面临被试间信号变异性和用户特定标注成本高的问题，限制了识别性能，需要开发有效的跨被试域适应方法。

Method: 1. 滤波组欧式对齐(FBEA)策略利用SSVEP滤波组频率信息；2. 跨被试自训练(CSST)框架包含对抗预训练(PTAL)对齐源域和目标域分布，以及双集成自训练(DEST)优化伪标签质量；3. 时频增强对比学习(TFA-CL)模块通过多增强视图提升特征判别性。

Result: 在Benchmark和BETA数据集上的大量实验表明，该方法在不同信号长度下均达到最先进的性能，证明了其优越性。

Conclusion: 提出的跨被试域适应方法有效解决了SSVEP-BCI中的信号变异性和标注成本问题，通过创新的自训练框架和对比学习模块显著提升了跨被试识别性能。
...
### [170] [A Sheaf-Theoretic and Topological Perspective on Complex Network Modeling and Attention Mechanisms in Graph Neural Models](https://arxiv.org/abs/2601.21207)
*Chuan-Shen Hu*

Main category: cs.LG

TL;DR: 该论文提出了一个基于层论（cellular sheaf theory）的框架，用于分析和建模几何与拓扑深度学习（GDL/TDL）架构中节点特征和边权重的局部一致性与调和性，并通过多尺度扩展捕捉层次特征交互。


<details>
  <summary>Details</summary>
Motivation: 几何与拓扑深度学习（GDL/TDL）模型在组合和拓扑结构（如图、单纯复形、胞腔复形）上聚合信号并生成表示，但训练过程中特征的分布和扩散行为仍是一个未充分探索的开放问题。作者旨在填补这一研究空白。

Method: 引入基于层论（cellular sheaf theory）的框架，通过层结构追踪局部特征对齐和一致性，提供特征扩散和聚合的拓扑视角。进一步提出受拓扑数据分析（TDA）启发的多尺度扩展，以捕捉图模型中的层次特征交互。

Result: 该框架能够基于底层几何拓扑结构及其上定义的已学习信号，对GDL和TDL架构进行联合表征，为节点分类、子结构检测和社区检测等传统任务提供新的分析视角。

Conclusion: 提出的层论框架为分析和理解几何与拓扑深度学习模型的特征传播机制提供了理论基础，有助于未来对这类架构的深入研究和应用开发。
...
### [171] [PHDME: Physics-Informed Diffusion Models without Explicit Governing Equations](https://arxiv.org/abs/2601.21234)
*Kaiyuan Tan,Kendra Givens,Peilun Li,Thomas Beckers*

Main category: cs.LG

TL;DR: PHDME：一种基于端口哈密顿扩散的框架，用于处理稀疏观测和不完整物理知识下的动力学系统轨迹预测，通过GP-dPHS生成物理一致的人工数据集，结合扩散模型和保形校准提供不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 扩散模型为动力学系统轨迹预测提供了表达性先验，但在稀疏数据下不可靠。物理信息机器学习(PIML)提高了可靠性，但大多数方法需要训练时的显式控制方程，而复杂非线性动力学往往只有部分已知。

Method: 1. 在有限观测上训练高斯过程分布式端口哈密顿系统(GP-dPHS)捕获基于能量的动力学表示；2. 使用GP-dPHS生成物理一致的人工数据集用于扩散训练；3. 通过结构化物理残差损失为扩散模型提供信息；4. 训练后扩散模型作为摊销采样器和预测器；5. 应用分割保形校准为预测提供不确定性陈述。

Result: 在PDE基准测试和真实世界弹簧系统上的实验表明，在数据稀缺情况下提高了准确性和物理一致性。

Conclusion: PHDME框架成功解决了稀疏观测和不完整物理知识下的动力学系统预测问题，通过结合端口哈密顿结构先验、扩散模型和保形校准，实现了更可靠和物理一致的轨迹生成。
...
### [172] [Less Noise, More Voice: Reinforcement Learning for Reasoning via Instruction Purification](https://arxiv.org/abs/2601.21244)
*Yiju Guo,Tianyi Hu,Zexu Sun,Yankai Lin*

Main category: cs.LG

TL;DR: LENS框架通过识别并移除提示中的干扰token来提升强化学习中的采样效率，然后将净化后的成功轨迹用于监督原始噪声提示下的策略优化，显著提高了性能和收敛速度。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法在有限rollout预算下存在探索效率低、采样成功率低和训练不稳定的问题，研究发现许多探索失败并非源于任务难度，而是由少量提示token引入的干扰导致。

Method: 提出Less Noise Sampling Framework (LENS)：1) 识别并移除提示中的干扰token进行净化提示；2) 将净化过程中的成功rollout转移到原始噪声提示上监督策略优化，使模型学会忽略真实世界中的干扰。

Result: LENS显著优于GRPO，平均性能提升3.88%，收敛速度加快1.6倍以上，证明了修剪干扰token在提高rollout效率方面的关键作用。

Conclusion: 干扰token的修剪对提升RLVR的rollout效率至关重要，LENS框架为RLVR研究提供了新的视角，通过减少提示噪声来改善探索效率和训练稳定性。
...
### [173] [EGAM: Extended Graph Attention Model for Solving Routing Problems](https://arxiv.org/abs/2601.21281)
...
<details>
  <summary>Details</summary>
Motivation: 现有图注意力模型(GAM)仅考虑节点特征，存在局限性，需要更全面的图结构建模能力

Method: 提出扩展图注意力模型(EGAM)，使用多头点积注意力同时更新节点和边嵌入，采用自回归编码器-解码器架构，使用带特殊基线的策略梯度算法训练

Result: EGAM在多种路由问题上匹配或优于现有方法，在高度约束问题上表现尤其突出

Conclusion: EGAM通过扩展的图注意力机制有效处理复杂图结构，为神经组合优化提供了更强大的建模能力
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM遗忘方法存在显著局限性：传统的基于调优的遗忘方法计算量大且容易导致灾难性遗忘；而基于上下文的遗忘方法虽然轻量级但容易受到提示移除或逆向工程攻击。需要一种结合两者优点的更有效方法。

Method: 提出DUET（Distilled Unlearning from an Efficient Teacher），这是一种基于蒸馏的遗忘方法。该方法训练一个学生模型来模仿提示引导的教师模型的行为，教师模型能有效拒绝不良知识生成，同时保持通用领域知识。

Result: 在现有基准测试和增强评估协议下的广泛评估表明，DUET在遗忘效果和效用保持方面都取得了更高的性能，同时比最先进的遗忘方法数据效率高出几个数量级。

Conclusion: DUET成功结合了基于调优和基于上下文遗忘方法的优点，提供了一种高效、数据友好且安全的LLM遗忘解决方案，为实现可信AI提供了重要技术支持。
...
TL;DR: Zenith是一种可扩展的高效排序架构，通过处理少量高维Prime Tokens并利用Token Fusion和Token Boost模块学习复杂特征交互，在TikTok Live平台上实现了显著的推荐性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有推荐系统研究虽然探索了多种模型架构来捕获多粒度特征交互，但相对忽视了高效特征处理和在不增加过多推理延迟的情况下扩展模型容量的问题。本文旨在解决这一挑战。

Method: 提出Zenith架构，通过处理少量高维Prime Tokens，结合Token Fusion和Token Boost模块来学习复杂特征交互。该方法改进了token异质性，相比现有排序方法展现出优越的缩放规律。

Result: 在TikTok Live平台上的A/B测试显示：在线CTR AUC提升1.05%，Logloss降低1.10%；Quality Watch Session/User提升9.93%，Quality Watch Duration/User提升8.11%。

Conclusion: Zenith是一种可扩展且高效的排序架构，能够以最小的运行时开销学习复杂特征交互，在实际大规模推荐系统中表现出卓越的性能提升效果。
...
TL;DR: TimeSliver：一种新颖的可解释性驱动深度学习框架，通过联合利用原始时间序列数据及其符号抽象来构建保持原始时间结构的表示，每个元素线性编码每个时间片段对最终预测的贡献，为时间序列分类提供有意义的重要性评分。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列可解释性方法存在局限性：基于梯度和特征归因的后处理方法对参考状态敏感且难以跨数据集泛化，因为它们将时间点独立处理而忽略序列依赖性；基于自注意力机制的可解释组件方法中，注意力权重往往无法提供忠实的时间重要性度量。需要一种能够为每个时间片段分配有意义重要性评分的方法。

Method: TimeSliver框架联合利用原始时间序列数据及其符号抽象来构建保持原始时间结构的表示。该表示中的每个元素线性编码每个时间片段对最终预测的贡献，从而为每个时间点分配有意义的重要性分数。这种方法克服了传统方法将时间点独立处理的局限性，同时避免了注意力权重不可靠的问题。

Result: 在时间序列分类任务中，TimeSliver在7个不同的合成和真实世界多变量时间序列数据集上比其他时间归因方法性能高出11%。在26个UEA基准数据集上，TimeSliver的预测性能与最先进基线方法相差在2%以内，使其成为通用时间序列分类的强大且可解释的框架。

Conclusion: TimeSliver是一种新颖的可解释性驱动深度学习框架，能够为时间序列分类中的每个时间点分配有意义的重要性评分。它通过联合利用原始数据和符号抽象来保持时间结构，在保持强大预测性能的同时提供忠实的时间重要性度量，解决了现有可解释性方法的局限性。
...
TL;DR: PG-TMT：一种用于旋转机械可靠性中心预测的物理引导微型Mamba Transformer，通过三分支编码器、物理对齐表示和极值理论校准决策规则，实现准确、可解释且部署就绪的早期预警。


<details>
  <summary>Details</summary>
Motivation: 旋转机械的可靠性中心预测需要早期预警信号，这些信号在非平稳运行条件、跨速度/负载/传感器的域偏移以及严重类别不平衡下保持准确，同时保持较低且可预测的误报率。

Method: 提出物理引导微型Mamba Transformer（PG-TMT），包含三个分支：深度可分离卷积主干捕获微瞬变，微型Mamba状态空间分支建模近线性长程动态，轻量级局部Transformer编码跨通道共振。通过解析时频映射将注意力谱与经典轴承故障阶次带关联，产生物理合理性量化分数。使用极值理论（EVT）建模健康分数超限，实现目标误报强度的在线阈值；双阈值滞回和最小保持时间进一步抑制抖动。

Result: 在CWRU、Paderborn、XJTU-SY和工业试点数据集上，采用无泄漏流式协议和漏检右删失，PG-TMT在类别不平衡下获得更高的精确率-召回率AUC（主要指标），竞争性或更好的ROC AUC，在匹配误报强度下更短的平均检测时间，以及强大的跨域迁移能力。

Conclusion: 通过将物理对齐表示与EVT校准决策规则相结合，PG-TMT为可靠性中心预测和健康管理提供校准、可解释且部署就绪的早期预警。
...
TL;DR: PLS-SVD在缺失数据下的相位转移：当信号强度低于临界阈值时，奇异向量无信息；高于阈值时，可实现与潜在共享方向的对齐，对齐度有闭式解。


<details>
  <summary>Details</summary>
Motivation: 多模态数据常存在缺失条目，但传统PLS-SVD假设数据完整。需要研究在独立随机缺失机制下，PLS-SVD的性能如何变化，特别是信号强度与缺失率如何影响共享结构的恢复。

Method: 在比例高维尖峰模型下，分析经适当归一化的掩码交叉协方差矩阵，该矩阵表现为信号强度衰减√ρ的尖峰矩形随机矩阵，其中ρ是联合条目保留概率。使用随机矩阵理论推导PLS-SVD的渐近行为。

Result: 发现PLS-SVD存在尖锐的BBP型相位转移：低于临界信噪比阈值时，主导奇异向量渐近无信息；高于阈值时，它们与潜在共享方向实现非平凡对齐，并得到闭式渐近重叠公式。模拟和半合成多模态实验验证了相位图和恢复曲线。

Conclusion: 缺失数据会衰减PLS-SVD的有效信号强度，导致相位转移现象。该理论为多模态数据分析中缺失数据处理提供了理论指导，并给出了可预测的恢复性能边界。
...
TL;DR: 该论文提出了InfoUtil框架，通过博弈论信息性最大化和原则性效用最大化来优化数据集蒸馏，在ImageNet-1K上比先前SOTA方法性能提升6.1%


<details>
  <summary>Details</summary>
Motivation: 当前数据集蒸馏方法多依赖启发式方法平衡效率与质量，但原始数据与合成数据之间的基本关系研究不足。需要建立理论框架来理解数据集蒸馏的本质。

Method: 提出InfoUtil框架，包含两个核心组件：1) 基于Shapley值归因的博弈论信息性最大化，从样本中提取关键信息；2) 基于梯度范数的原则性效用最大化，选择全局有影响力的样本。通过平衡信息性和效用来合成蒸馏数据集。

Result: 在ImageNet-1K数据集上使用ResNet-18，该方法比先前最先进方法性能提升6.1%。实验证明蒸馏数据集既信息丰富又效用优化。

Conclusion: 论文在理论框架下重新审视基于知识蒸馏的数据集蒸馏，提出信息性和效用概念，并开发出平衡两者的InfoUtil框架，显著提升了数据集蒸馏的性能。
...
TL;DR: 本文分析了同步和异步Q-learning在平均奖励MDP中的收敛率，通过引入懒惰化动态采样机制，在可达性假设下实现了最优的样本复杂度


<details>
  <summary>Details</summary>
Motivation: 平均奖励MDP中Q-learning缺乏收缩性这一根本挑战。现有非渐近结果要么需要强假设来强制半范数收缩，要么依赖折扣或片段MDP作为连续近似，这些方法要么需要未知参数，要么导致次优样本复杂度
...
Conclusion: 通过懒惰化变换和实例依赖的半范数构造，成功克服了平均奖励MDP中Q-learning缺乏收缩性的挑战，实现了最优样本复杂度，为平均奖励强化学习提供了理论保证
...
TL;DR: 该论文挑战了模型强化学习中关于长期预测和误差累积是主要障碍的传统观点，发现搜索可能损害性能，而缓解分布偏移比提高模型或价值函数精度更重要，并提出了实现有效搜索的关键技术。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为模型强化学习的主要障碍是长期预测和误差累积，但作者质疑这一观点，旨在探索搜索在模型强化学习中的实际作用，特别是当模型高度准确时搜索为何仍可能损害性能。

Method: 通过实验分析搜索在模型强化学习中的表现，识别搜索失败的原因，并基于缓解分布偏移的洞察，开发了实现有效搜索的关键技术方法。

Result: 研究发现搜索并非学习策略的即插即用替代品，即使模型高度准确，搜索也可能损害性能。缓解分布偏移比提高模型或价值函数精度更重要，基于此开发的技术在多个流行基准领域实现了最先进的性能。

Conclusion: 模型强化学习成功的关键在于有效缓解分布偏移，而非单纯追求模型或价值函数精度。搜索需要专门设计的技术支持才能发挥效用，这为模型强化学习研究提供了新的方向。
...
TL;DR: TGCC是一种基于因果不变性的可迁移图数据集压缩方法，通过提取域不变特征和增强压缩操作，在跨任务和跨域场景中显著优于现有方法。
...
Method: 1) 通过因果干预从图的空间域提取域不变特征；2) 执行增强压缩操作以充分捕获原始图的结构和特征信息；3) 通过谱域增强对比学习将因果不变特征注入压缩图中，确保保留原始图的因果信息。

Result: 在5个公共数据集和新提出的FinReport数据集上，TGCC在跨任务和跨域复杂场景中相比现有方法提升达13.41%，在单数据集和任务场景中在6个数据集的5个上达到最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有UDA方法在实际应用中面临挑战，特别是在目标域只有有限无标签数据或源域存在伪相关性的场景下。需要一种能够同时处理协变量分布和条件标签分布不确定性的鲁棒方法。

Method: 提出一种新颖的分布鲁棒学习框架，同时对协变量分布和条件标签分布的不确定性进行建模。该方法受到多源域自适应设置的启发，但也适用于单源场景。开发了高效的学习算法，可与现有UDA方法无缝集成。

Result: 在各种分布偏移场景下的广泛实验表明，该方法始终优于强基线方法，特别是在目标数据极其稀缺的情况下表现尤为突出。

Conclusion: 提出的分布鲁棒学习框架有效解决了UDA中目标数据稀缺和源域伪相关性问题，具有实际应用价值，特别是在数据稀缺场景下表现出色。
...
TL;DR: 提出一种基于去噪中心视角的时间步采样策略，通过控制学习在去噪轨迹上的位置来调节扩散模型的记忆化程度，平衡记忆化与泛化之间的权衡。
...
Method: 提出一种时间步采样策略，通过调整置信区间宽度来明确控制学习在去噪轨迹上的发生位置。该方法能够直接控制记忆化与泛化之间的权衡。

Result: 在图像和1D信号生成任务上的实验表明，将学习重点移向后期去噪步骤能够持续减少记忆化，并改善生成数据与训练数据的分布对齐，验证了方法的通用性和有效性。

Conclusion: 从去噪中心视角研究扩散模型记忆化问题，提出的时间步采样策略通过控制学习位置有效调节记忆化程度，为扩散模型训练提供了新的控制机制。
...
<details>
  <summary>Details</summary>
Motivation: 传统MoE系统使用原始高维表示空间中的线性路由器，存在表示不匹配、角度集中和尺度敏感评分等问题，这些共同削弱了路由判别能力和稳定的专家专业化
...
Conclusion: L2R通过重塑路由空间和评分几何，解决了传统MoE路由的关键问题，为构建更稳定、高效的MoE系统提供了有效解决方案
...
TL;DR: 提出一种基于因果视角的因子化表示学习框架，通过分解上下文嵌入为因果因子和非因果因子，约束奖励头仅依赖因果组件，并使用对抗性训练减少非因果因子中的奖励相关信息，从而学习更鲁棒的奖励模型并改善RLHF性能。
...
Method: 提出因子化表示学习框架：1) 将模型上下文嵌入分解为因果因子（足以预测奖励）和非因果因子（捕获奖励无关属性如长度或奉承偏差）；2) 约束奖励头仅依赖因果组件；3) 引入对抗性头从非因果因子预测奖励，同时应用梯度反转来阻止这些因子编码奖励相关信息。

Result: 在数学和对话任务上的实验表明，该方法学习了更鲁棒的奖励模型，并在下游RLHF性能上持续优于最先进的基线方法。对长度和奉承偏差的分析进一步验证了该方法在缓解奖励黑客行为方面的有效性。

Conclusion: 通过从因果角度分解表示并约束奖励预测仅依赖因果因子，同时对抗性地减少非因果因子中的奖励相关信息，能够有效缓解奖励黑客问题，提升奖励模型的鲁棒性和RLHF性能。
...
TL;DR: AFD架构将LLM解码中的Attention和FFN计算分离，但性能对资源配比敏感。本文建立了分析框架，推导出最大化吞吐量的最优A/F配比规则，并通过仿真验证了理论的有效性。


<details>
  <summary>Details</summary>
Motivation: AFD架构虽然实现了内存和计算资源的独立扩展，但其性能高度依赖于Attention和FFN的资源配比。不恰当的配比会导致步骤级阻塞和设备空闲时间增加，需要理论指导来确定最优资源配置。

Method: 开发了可处理的AFD捆绑资源配比分析框架，针对rA-1F拓扑结构。使用概率工作负载模型，考虑Attention侧的非平稳性（token上下文增长和随机长度请求）与FFN侧的稳定性，推导出最大化平均吞吐量的闭式最优A/F配比规则。

Result: 通过轨迹校准的AFD仿真器验证理论：在各种工作负载下，理论最优A/F配比与仿真最优配比的误差在10%以内，并能持续减少设备空闲时间。

Conclusion: 本文提出的分析框架能够有效指导AFD架构的资源配比优化，理论推导的最优A/F配比规则在实际工作负载中表现良好，显著提升了系统吞吐量并减少了资源空闲。
...
<details>
  <summary>Details</summary>
Motivation: 将Transformer的前向传播解释为粒子系统，可以借助梯度流和平均场理论分析其动力学特性，但感知器块在这种框架下的影响尚未被充分研究。

Method: 将Transformer前向传播建模为单位球面上的相互作用粒子系统，其中时间对应层数，粒子对应词嵌入，单位球面理想化层归一化。在特定权重设置下，系统可视为显式能量的梯度流，并利用Wasserstein梯度流处理无限上下文长度（平均场）极限。

Result: 研究发现感知器块会导致临界点通常是原子化的（atomic），并且集中在球面的特定子集上，这表明系统倾向于形成局部化的结构。

Conclusion: 在Transformer的粒子系统视角下，感知器块对系统动力学有重要影响，导致临界点呈现原子化和局部化特性，这为理解Transformer的表示学习机制提供了新的理论见解。
...
Method: 提出DA-SPS模型，包含目标变量处理阶段(TVPS)和外部变量处理阶段(EVPS)。TVPS使用SSA分解目标序列，LSTM处理趋势分量，P-Conv-LSTM处理季节性分量。EVPS使用Spearman相关性分析筛选相关外部变量，通过L-Attention模块(LSTM+注意力机制)分析

Result: 在四个公共数据集上优于现有最先进方法，在自收集的笔记本电脑主板测试项目私有数据集上进一步验证了实际场景性能
...
<details>
  <summary>Details</summary>
Motivation: 稀疏奖励环境中传统探索策略（如噪声注入）效果有限，现有内在奖励方法存在信用分配不稳定（结合外在奖励优化策略）或样本效率低/次优（分层学习训练子策略）的问题

Method: 提出内在奖励策略优化（IRPO）框架，使用多个内在奖励直接优化外在奖励策略，通过替代策略梯度提供比真实梯度更丰富的学习信号

Result: 在离散和连续环境中，IRPO相比基线方法提高了性能和样本效率，并对IRPO解决的优化问题进行了形式化分析

Conclusion: IRPO通过内在奖励直接优化策略，有效解决了稀疏奖励环境中的探索问题，避免了现有方法的缺陷
...
TL;DR: 提出DiPO框架，通过强化学习让大型推理模型能够自发感知任务难度并调整推理开销，减少过思考现象


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在处理简单任务时经常出现"过思考"现象，产生不必要的冗长推理，导致资源效率低下。现有研究低估了任务难度感知的重要性，难以有效分配推理资源。
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散和流匹配模型中，预测目标从噪声(ε)、速度(v)转向直接数据(x)预测，但缺乏理论解释为何最优目标取决于数据特性。需要建立理论框架解释预测目标选择与数据几何性质的关系。

Method: 提出广义预测公式，涵盖任意输出目标（ε、v、x为特例）。推导数据几何与最优预测目标的解析关系。为解决实际中数据内在维度难以估计的问题，提出k-Diff框架，通过数据驱动方法直接从数据中学习最优预测参数k。

Result: 理论分析表明，当环境维度显著超过数据内在维度时，x预测成为最优选择。实验证明k-Diff在潜空间和像素空间图像生成中，在不同架构和数据规模下均优于固定目标基线方法。

Conclusion: 数据维度是决定扩散模型最优预测目标的关键因素。k-Diff提供了一种原则性、自动化的方法，无需显式维度估计即可提升生成性能，为预测目标选择提供了理论依据和实践框架。
...
### [193] [From Consistency to Complementarity: Aligned and Disentangled Multi-modal Learning for Time Series Understanding and Reasoning](https://arxiv.org/abs/2601.21436)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在时间序列与可视化图表的跨模态整合中存在两个主要问题：1）细粒度时间错位阻碍局部解释；2）共享语义与模态特定语义的严重纠缠阻碍互补推理

Method: 提出MADI模型，包含三个关键技术：1）补丁级对齐，强制异质模态间的物理基础细粒度对应；2）离散解耦交互，将模态共同语义分离到紧凑离散潜在空间中，自适应协同纯化的模态独特信息；3）关键令牌高亮，强调信息丰富、查询相关的信号以增强鲁棒推理

Result: 在合成和真实世界基准测试中，MADI始终优于通用LLM和专门针对时间序列的MLLM

Conclusion: MADI通过细粒度对齐和解耦交互机制，有效解决了时间序列多模态理解中的跨模态整合挑战，提升了时间序列推理的准确性和鲁棒性
...
### [194] [ConceptMoE: Adaptive Token-to-Concept Compression for Implicit Compute Allocation](https://arxiv.org/abs/2601.21420)
*Zihao Huang,Jundong Zhou,Xingwei Qu,Qiyang Min,Ge Zhang*

Main category: cs.LG

TL;DR: ConceptMoE通过动态合并语义相似token为概念表示，实现隐式token级计算分配，在保持计算量和参数量不变的情况下，显著提升语言和视觉语言任务性能，同时减少注意力计算和KV缓存。


<details>
  <summary>Details</summary>
Motivation: 传统大语言模型对所有token采用统一计算分配，忽略了某些序列是简单可预测的而其他需要深度推理。这种均匀计算分配导致计算资源浪费，需要更智能的计算分配机制。

Method: 引入ConceptMoE，通过可学习的chunk模块测量token间相似度，识别最优边界，按目标压缩比R压缩序列。MoE架构允许控制评估：重新分配节省的计算以匹配基线激活FLOPs和总参数量。概念模型处理压缩后的概念表示。

Result: 在匹配计算量和参数量的条件下，ConceptMoE在语言预训练上提升+0.9点，长上下文理解+2.3点，多模态基准+0.6点。通过层循环持续训练时增益达+5.5点。注意力计算减少R²倍，KV缓存减少R倍。R=2时，预填充速度提升175%，解码速度提升117%。

Conclusion: ConceptMoE通过自适应概念级处理，在保持计算和参数约束的同时，显著提升大语言模型的效果和效率。最小化架构修改使其易于集成到现有MoE中，证明了自适应计算分配的基本优势。
...
### [195] [SAGE: Sequence-level Adaptive Gradient Evolution for Generative Recommendation](https://arxiv.org/abs/2601.21452)
*Yu Xie,Xing Kai Ren,Ying Qi,Hu Yao*

Main category: cs.LG

TL;DR: SAGE提出了一种针对列表式生成推荐的统一优化框架，解决了OneRec中GBPO的对称保守主义问题，通过序列级信号解耦和非对称自适应动态机制，在保持数值稳定性的同时提升冷启动效果和推荐多样性。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的推荐系统（如OneRec）存在两个主要问题：1）需要构建单独的词汇表，无法复用开源LLM架构，导致维护成本高、扩展性差；2）GBPO优化策略存在"对称保守主义"问题，静态梯度边界抑制了冷启动物品所需的更新动量，且无法防止高噪声环境下的多样性崩溃。

Method: SAGE框架包含两个核心创新：1）序列级信号解耦：结合几何平均重要性比率和解耦的多目标优势函数，消除标记级方差，解决"奖励崩溃"问题；2）非对称自适应动态机制：构建动态梯度流形，对高潜力冷启动物品应用"提升因子"实现超线性更新，同时采用"熵感知惩罚"打破信息茧房。

Result: 理论分析和实证结果表明，SAGE在保持GBPO数值稳定性的同时，有效解锁了冷启动流量并维持了推荐多样性。该方法避免了构建单独词汇表的开销，实现了对开源LLM架构的高效复用。

Conclusion: SAGE为列表式生成推荐提供了一个统一的优化框架，通过创新的序列级信号解耦和非对称自适应动态机制，解决了现有方法的对称保守主义问题，在冷启动和多样性方面表现出色，同时保持了架构的简洁性和可扩展性。
...
### [196] [HER: Human-like Reasoning and Reinforcement Learning for LLM Role-playing](https://arxiv.org/abs/2601.21459)
...
TL;DR: HER框架通过双层思维机制和推理增强数据，实现了LLM角色扮演中的认知模拟，显著提升了角色扮演质量


<details>
  <summary>Details</summary>
Motivation: 当前LLM角色扮演虽然能捕捉角色语气和知识，但难以模拟行为背后的内心思维。现有方法缺乏高质量的推理轨迹数据和与人类偏好对齐的可靠奖励信号
...
Conclusion: HER框架通过认知层面的角色模拟解决了现有LLM角色扮演的局限性，为未来研究提供了数据集、原则和模型资源
...
### [197] [L$^3$: Large Lookup Layers](https://arxiv.org/abs/2601.21461)
*Albert Tseng,Christopher De Sa*

Main category: cs.LG

TL;DR: L³（大型查找层）是一种新型稀疏架构，通过静态令牌路由将嵌入表泛化到解码器层，在保持硬件效率的同时超越密集模型和混合专家模型。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏模型主要采用混合专家（MoE）层，但动态硬路由存在硬件效率低、需要辅助损失稳定训练等问题。而嵌入表虽然是天然稀疏的，但缺乏上下文信息。需要一种既能保持稀疏性优势又能避免MoE缺点的新方法。

Method: 提出大型查找层（L³），包含两个核心组件：1）系统友好的架构设计，支持快速训练和无开销的CPU卸载推理；2）基于信息论的嵌入分配算法，有效平衡速度与质量。L³使用静态令牌路由，以上下文相关的方式聚合每个令牌的学习嵌入。

Result: 在包含26亿活跃参数的Transformer模型上测试，L³在语言建模和下游任务中均显著优于密集模型和同等稀疏度的MoE模型。

Conclusion: L³通过将嵌入表泛化到解码器层，开辟了新的稀疏维度，能够在保持硬件效率的同时实现更好的性能，为大规模稀疏语言模型提供了有前景的替代方案。
...
### [198] [Shaping capabilities with token-level data filtering](https://arxiv.org/abs/2601.21571)
*Neil Rathi,Alec Radford*

Main category: cs.LG

TL;DR: 通过预训练数据过滤而非事后干预来消除语言模型中的不良能力，特别是医疗能力，证明token级过滤比文档级过滤更有效，且随着模型规模扩大效果增强。


<details>
  <summary>Details</summary>
Motivation: 当前减少语言模型中不良能力的方法大多是事后干预，容易被对抗性攻击绕过。因此需要在预训练阶段就塑造模型能力，从根本上消除不良能力。

Method: 1) 使用稀疏自编码器标记token并蒸馏出高质量分类器；2) 在预训练数据中进行token级过滤而非文档级过滤；3) 在不同规模模型（跨越两个数量级）上验证方法；4) 测试过滤对噪声标签的鲁棒性。

Result: 1) token过滤比文档过滤更有效，能以更小的良性能力代价达到相同的不良能力消除效果；2) 过滤效果随模型规模扩大而增强，最大模型中token过滤导致遗忘领域计算速度减慢7000倍；3) 经过token过滤训练的模型仍可在遗忘领域进行对齐；4) 方法对噪声标签具有鲁棒性。

Conclusion: 预训练数据过滤是消除语言模型中不良能力的有效、鲁棒且经济的方法，token级过滤优于文档级过滤，且效果随模型规模扩大而增强，为模型安全提供了新的解决方案。
...
TL;DR: 该论文研究了部分反馈在线学习，其中每个实例对应一组正确标签，但学习者每轮只能观察到一个正确标签；预测只要在正确集合内即算正确。论文给出了确定性学习者和随机学习者在集合可实现机制下的极小极大遗憾的近乎完整刻画。


<details>
  <summary>Details</summary>
Motivation: 研究部分反馈在线学习问题，该模型捕捉了如语言生成等场景，其中多个响应可能有效但数据仅提供单一参考。旨在理解在这种受限反馈设置下的学习理论界限。

Method: 引入部分反馈Littlestone维度(PFLdim)来刻画确定性学习者的可学习性和极小极大遗憾；开发部分反馈度量粉碎维度(PMSdim)来获得随机学习者的紧界；识别确保确定性和随机可学习性不可分离的广泛条件；将论证扩展到集合值在线学习。

Result: PFLdim精确控制确定性学习者的可学习性和极小极大遗憾；PMSdim为随机学习者提供紧界；确定了确定性和随机可学习性不可分离的条件；解决了Raman等人[2024b]的开放性问题；展示了与较弱现实和不可知变体的尖锐分离。

Conclusion: 在集合可实现机制下，PFLdim和PMSdim提供了部分反馈在线学习的完整理论刻画。然而，超出集合可实现性，问题可能变得信息论上不可处理，需要新的噪声敏感复杂性度量来有意义地表征可学习性。
...
TL;DR: 提出新的块坐标下降（BCD）框架用于非凸复合优化问题，确保目标函数下降并收敛到解，应用于稀疏精度矩阵估计问题（Graphical Lasso）


<details>
  <summary>Details</summary>
Motivation: 块坐标下降（BCD）方法在解决大规模优化问题中被广泛使用，但其在非凸优化中的理论研究相对不足。需要开发一个通用的BCD框架来处理非凸复合优化问题，并应用于稀疏精度矩阵估计等实际问题。

Method: 提出新的块坐标下降（BCD）框架，该框架足够通用，能够包含变量度量近端梯度更新、近端牛顿更新和交替最小化更新。该框架确保目标函数单调下降并收敛到解。特别应用于Graphical Lasso问题，涵盖了三种最常用的求解器：graphical ISTA、Primal GLasso和QUIC。

Result: 在非凸稀疏精度矩阵估计问题上验证了该框架的价值，提供了收敛性保证，并且相比现有方法，达到最先进估计质量所需的迭代次数减少了高达100倍。

Conclusion: 提出的新BCD框架为处理非凸复合优化问题提供了通用且有效的解决方案，特别在Graphical Lasso问题上表现出显著的计算效率提升，迭代次数大幅减少，同时保证了收敛性。
...
TL;DR: PPI-SVRG结合了PPI和SVRG两种方差缩减方法，在标签稀缺但可获得预训练模型预测的情况下，通过控制变量实现更高效的半监督随机优化。


<details>
  <summary>Details</summary>
Motivation: 在标签数据稀缺但可获得预训练模型预测的背景下，需要开发高效的半监督随机优化方法。PPI使用预测作为控制变量，SVRG使用参考梯度作为控制变量，两者都旨在减少方差。

Method: 证明了PPI和SVRG在数学上的等价性，并开发了PPI-SVRG方法，结合了两种控制变量技术。收敛界分解为标准SVRG速率加上预测不确定性带来的误差底限。

Result: 在均值估计基准测试中，PPI-SVRG在标签稀缺条件下将MSE降低了43-52%；在MNIST上仅使用10%标签数据时，测试准确率提高了2.7-2.9个百分点。

Conclusion: PPI-SVRG在标签稀缺场景下提供了稳定的收敛性能：预测完美时完全恢复SVRG，预测质量下降时仍能稳定收敛但达到更大的邻域。收敛速率仅依赖于损失函数的几何特性，预测只影响邻域大小。
...
TL;DR: 论文提出"过度缩放诅咒"概念，指在并行思维中为所有样本分配相同并行度N会导致资源浪费，因为部分样本只需较小并行度N'<N即可达到相似性能。作者提出T2方法，利用潜在表示估计每个样本的最优并行度，显著降低成本同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 当前并行思维方法在系统层面为所有样本分配相同的全局并行度N，通常设置较大以最大化整体数据集准确率。但由于样本异质性，部分样本只需较小并行度N'<N即可达到可比性能，导致预算冗余。这种系统层面效能与样本层面效率之间的不兼容构成了"过度缩放诅咒"。

Method: 提出T2方法：1) 形式化并量化过度缩放诅咒，展示其普遍性和严重性，分析触发机制；2) 利用潜在表示在解码前估计每个样本的最优并行度；3) 实现轻量级自适应并行度分配，减少冗余计算。

Result: 实验表明T2显著降低成本同时保持可比性能，实现更高效的并行思维。该方法有效解决了过度缩放诅咒问题，在资源效率和系统效能之间取得更好平衡。

Conclusion: 过度缩放诅咒是并行思维中的普遍问题，T2方法通过样本级自适应并行度分配成功打破该诅咒，为高效并行推理提供了实用解决方案，在保持性能的同时显著降低计算成本。
...
### [203] [ETS: Energy-Guided Test-Time Scaling for Training-Free RL Alignment](https://arxiv.org/abs/2601.21484)
*Xiuyu Li,Jinkai Zhang,Mingyang Yi,Yu Li,Longqiang Wang,Yue Wang,Ju Fan*

Main category: cs.LG

TL;DR: 提出一种免训练的推理方法ETS，通过在线蒙特卡洛估计能量项，直接从最优RL策略中采样，避免了传统RL后训练对齐的高成本和稳定性问题


<details>
  <summary>Details</summary>
Motivation: 传统RL后训练对齐方法虽然有效，但训练过程复杂、成本高且不稳定，需要一种更高效稳定的替代方案

Method: 提出能量引导测试时缩放（ETS）算法，基于掩码语言建模的转移概率公式，通过在线蒙特卡洛估计能量项，结合现代加速框架和定制的重要性采样估计器，在推理时直接从最优RL策略采样

Result: 在推理、编码和科学基准测试中，ETS在掩码语言建模（包括自回归模型和扩散语言模型）上一致提升了生成质量，验证了其有效性和设计

Conclusion: ETS作为一种免训练的推理方法，能够有效替代复杂的RL后训练对齐，在保持采样质量的同时显著降低推理延迟，为语言模型对齐提供了更高效稳定的解决方案
...
TL;DR: 该研究通过线性表示假说的视角重新审视表示误导(RM)这类LLM遗忘方法，发现遗忘操作不仅实现遗忘目标，还会引发与高层概念相关的可控副作用和增强能力。


<details>
  <summary>Details</summary>
Motivation: 尽管表示误导(RM)作为LLM遗忘方法很重要，但其使用的目标向量作用机制尚未充分探索。研究者希望从线性表示假说角度深入理解RM方法，并探索遗忘操作可能引发的副作用。

Method: 采用线性表示假说框架，假设可以识别对应高层概念的一维表示，然后在遗忘表示空间中对该概念向量进行线性操作。通过实证验证假设，在行为控制(如控制遗忘模型的真实性、情感、拒绝)和能力增强(如提升遗忘模型的上下文学习能力)等广泛任务中进行测试。

Result: 实证验证了假设：机器遗忘不仅实现遗忘，还会引发与高层概念相关的可控副作用和增强能力。这些现象在行为控制(控制模型真实性、情感、拒绝)和能力增强(提升上下文学习能力)等任务中得到证实。

Conclusion: 这一现象具有双重性：如果被误用可能成为隐藏风险，但如果被合理利用，则可以作为开发具有更强能力和可控行为模型的机制。研究揭示了遗忘操作在实现目标遗忘之外的重要影响。
...
### [205] [Task-Awareness Improves LLM Generations and Uncertainty](https://arxiv.org/abs/2601.21500)
*Tim Tomov,Dominik Fuchsgruber,Stephan Günnemann*

Main category: cs.LG

TL;DR: 论文提出一种基于任务相关潜在结构的LLM输出建模框架，通过潜在空间中的贝叶斯最优响应合成和不确定性量化，超越传统语言空间解码方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM解码和不确定性估计方法主要在语言空间操作，忽略了自然语言响应背后的结构化信息（如离散标签、数值、图结构等），导致无法充分利用任务相关的结构信息进行优化决策。

Method: 为LLM输出建立任务相关的潜在结构模型，配备相异性度量，在潜在空间中计算贝叶斯最优响应（非从采样生成中选择，而是在潜在空间中合成新响应），并通过诱导的贝叶斯风险量化不确定性。

Result: 贝叶斯最优响应在多个任务上持续优于标准解码方法（如束搜索），基于潜在结构的不确定性量化能更好地捕捉输出质量和正确性的变化，与任务表现更一致。
...
### [206] [Cascaded Transfer: Learning Many Tasks under Budget Constraints](https://arxiv.org/abs/2601.21513)
...
<details>
  <summary>Details</summary>
Motivation: 多任务学习需要学习大量相关任务，但任务间的确切关系未知，且存在预算约束，需要一种有效的迁移学习方法来提高准确性和成本效益

Method: 提出级联迁移学习范式，将任务组织为根树结构，通过最小生成树连接任务并使用合适的距离度量，在分支上分配训练预算，实现模型参数的层次化级联传递

Result: 在合成和真实多任务设置上的实验表明，该方法相比替代方法能够实现更准确、成本效益更高的跨大型任务集合的适应

Conclusion: 级联迁移学习是一种有效的多任务学习范式，能够在预算约束下通过层次化信息传递提高跨任务学习的准确性和效率
...
### [207] [More Bang for the Buck: Improving the Inference of Large Language Models at a Fixed Budget using Reset and Discard (ReD)](https://arxiv.org/abs/2601.21522)
*Sagi Meir,Tommer D. Keidar,Noam Levi,Shlomi Reuveni,Barak Hirshberg*

Main category: cs.LG

TL;DR: 论文提出ReD（Reset-and-Discard）方法，通过重置和丢弃策略提高LLM在固定预算下的问题覆盖率，减少所需尝试次数、token消耗和成本。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在可验证任务上的性能通常用pass@k度量，但在固定预算下，更合适的指标是coverage@cost（覆盖率随成本变化）。研究发现pass@k的幂律行为导致coverage@cost呈现次线性增长（收益递减），需要解决这个问题。

Method: 提出Reset-and-Discard（ReD）查询方法：1）连接pass@k和coverage@cost两个指标；2）通过重置和丢弃策略提高任何给定预算下的coverage@cost；3）给定pass@k可定量预测ReD节省的总尝试次数；4）若无pass@k，ReD可推断其幂律指数。

Result: 在三个LLM上使用HumanEval进行实验，结果显示ReD显著减少了达到期望覆盖率所需的尝试次数、token消耗和美元成本，同时提供了测量推理幂律的有效方法。

Conclusion: ReD方法有效解决了pass@k幂律行为导致的coverage@cost次线性增长问题，通过智能的查询策略优化了LLM在固定预算下的性能评估和资源利用。
...
<details>
  <summary>Details</summary>
Motivation: 在多智能体强化学习中，全局奖励包含所有智能体的贡献，通常噪声较大，需要信用分配过程；而局部奖励虽然学习更快（因为分离了智能体贡献），但可能导致次优解，因为智能体可能短视地优化自身奖励而忽视全局最优性。

Method: 提出一种结合两种方法优点的方法：通过使用智能体交互图，以比全局奖励更精细的方式识别个体智能体贡献，同时缓解局部奖励的合作问题。还引入了一种近似这种交互图的实用方法。

Result: 实验表明该方法具有灵活性，能够超越传统的局部和全局奖励设置。

Conclusion: 提出的方法通过智能体交互图有效结合了全局奖励和局部奖励的优点，在多智能体强化学习中实现了更好的性能。
...
TL;DR: 提出Expert Modulation新范式，通过文本信号直接调制专家路由和计算，改进多模态时间序列预测


<details>
  <summary>Details</summary>
Motivation: 现实时间序列动态复杂且演化，准确预测极具挑战。现有多模态方法依赖token级融合，但在高质量时序-文本对稀缺、时间序列尺度和特征差异大时效果不佳。MoE架构在时序建模和多模态学习中有效，但现有MoE方法仍依赖token级融合

Method: 提出Expert Modulation范式，基于文本信号同时调制专家路由和专家计算，实现直接高效的跨模态专家行为控制。通过理论分析和实验验证

Result: 提出的方法在多模态时间序列预测中表现出显著改进

Conclusion: Expert Modulation为多模态时间序列预测提供新范式，通过文本信号直接控制专家行为，优于传统token级融合方法
...
### [210] [GeoNorm: Unify Pre-Norm and Post-Norm with Geodesic Optimization](https://arxiv.org/abs/2601.22095)
...
TL;DR: GeoNorm：一种基于流形优化的Transformer归一化方法，将FFN和注意力层输出解释为优化方向，用测地线更新替代标准归一化，并引入层间更新衰减机制，性能优于现有方法且计算成本可忽略。


<details>
  <summary>Details</summary>
Motivation: Transformer架构中归一化层的位置（Pre-Norm与Post-Norm）仍是一个开放问题。本文从流形优化的角度重新思考这些方法，将FFN和注意力层的输出解释为优化过程中的更新方向，旨在开发更有效的归一化策略。

Method: 提出GeoNorm方法：1）基于流形优化理论，将FFN和注意力层输出视为优化方向；2）用测地线更新替代标准归一化操作；3）类似学习率调度，为FFN和注意力组件引入层间更新衰减机制；4）可无缝集成到标准Transformer架构中。

Result: 综合实验表明：1）GeoNorm在Transformer模型中始终优于现有归一化方法；2）能够无缝集成到标准Transformer架构中；3）在获得性能提升的同时，额外计算成本可忽略不计。

Conclusion: GeoNorm通过流形优化视角重新思考Transformer归一化，将层输出解释为优化方向并采用测地线更新，提供了一种性能优越且计算高效的归一化方案，解决了Pre-Norm和Post-Norm的选择难题。
...
TL;DR: HistoPrism：一种基于Transformer的高效架构，用于从组织学图像中预测空间基因表达，在泛癌场景下超越现有方法，并通过通路级评估验证其生物学相关性


<details>
  <summary>Details</summary>
Motivation: 从H&E组织学预测空间基因表达为测序提供了可扩展且临床易获取的替代方案，但现有方法通常局限于特定癌症类型，且评估多基于基因方差，缺乏对功能相关性的深入探索

Method: 提出HistoPrism，一种高效的基于Transformer的架构，用于从组织学图像进行泛癌基因表达预测；引入通路级评估基准，将评估重点从孤立的基因级方差转向连贯的功能通路

Result: HistoPrism不仅在高度可变基因预测上超越先前最先进模型，更重要的是在通路级预测上取得显著提升，证明其能恢复生物学上连贯的转录组模式；具有强大的泛癌泛化能力和改进的效率

Conclusion: HistoPrism通过其强大的泛癌泛化能力和改进的效率，为从常规可用组织学进行临床相关转录组建模建立了新标准
...
TL;DR: 提出ViKeR方法，通过视觉引导的关键令牌正则化实现多模态大语言模型的安全遗忘，专注于关键令牌而非均匀处理所有答案令牌
...
Method: 提出视觉引导的关键令牌正则化(ViKeR)：利用无关视觉输入预测理想的后遗忘令牌级分布，用这些分布正则化遗忘过程以优先处理关键令牌；通过信息熵定义遗忘中的关键令牌，并通过令牌级梯度重加权放大关键令牌的更新

Result: 在MLLMU和CLEAR基准测试上的实验表明，该方法能有效执行遗忘，同时减轻遗忘效应并保持回答连贯性

Conclusion: ViKeR方法通过视觉引导的关键令牌正则化，解决了现有MLLM遗忘方法忽视令牌重要性差异和视觉线索的问题，实现了更有效的安全遗忘
...
### [213] [Discovering Hidden Gems in Model Repositories](https://arxiv.org/abs/2601.22157)
*Jonathan Kahana,Eliahu Horwitz,Yedid Hoshen*

Main category: cs.LG

TL;DR: 研究发现公共模型库中存在大量被忽视的优质微调模型（"隐藏宝石"），这些模型性能远超流行模型但下载量极少。论文提出基于多臂老虎机问题的模型发现方法，通过共享查询集和激进淘汰策略加速搜索，将发现效率提升50倍以上。


<details>
  <summary>Details</summary>
Motivation: 公共模型库中数百万微调模型的使用高度集中在少数基础检查点上，需要探究这种集中是市场有效选择的结果，还是存在被系统性忽视的优质模型。同时，通过穷举评估所有上传模型来发现优质模型在计算上不可行。

Method: 1. 对2000多个模型进行广泛评估，验证"隐藏宝石"的存在；2. 将模型发现问题形式化为多臂老虎机问题；3. 改进顺序减半搜索算法，采用共享查询集和激进淘汰策略，将每个候选模型的查询次数减少到50次。

Result: 1. 发现大量被忽视的优质微调模型，在Llama-3.1-8B家族中，罕见下载的检查点将数学性能从83.2%提升到96.0%，且不增加推理成本；2. 提出的方法能以每个候选模型仅50次查询的速度发现顶级模型，将发现速度加速50倍以上。

Conclusion: 公共模型库中存在大量被忽视的优质微调模型，当前社区使用模式未能有效发现这些"隐藏宝石"。通过将模型发现问题形式化为多臂老虎机问题并优化搜索算法，可以高效发现这些优质模型，显著提升模型发现效率。
...
### [214] [FlexCausal: Flexible Causal Disentanglement via Structural Flow Priors and Manifold-Aware Interventions](https://arxiv.org/abs/2601.21567)
*Yutao Jin,Yuang Tao,Junyong Zhai*

Main category: cs.LG

TL;DR: FlexCausal是一种新颖的因果解耦表示学习框架，通过块对角协方差VAE和因子化流先验解决现有方法在协方差建模和非高斯噪声处理上的不足，实现了更精确的因果结构学习和高保真生成。


<details>
  <summary>Details</summary>
Motivation: 现有因果解耦表示学习方法存在两个主要问题：1）使用对角后验协方差（均值场近似）导致所有潜在维度去相关；2）假设外生噪声为各向同性高斯分布，无法捕捉真实世界因果因素中普遍存在的复杂非高斯统计特性。

Method: 提出FlexCausal框架，基于块对角协方差VAE，使用因子化流先验真实建模外生噪声的复杂密度分布，将因果机制学习与分布统计解耦。通过监督对齐目标和反事实一致性约束确保学习到的潜在子空间与真实因果关系的精确结构对应，并引入流形感知的相对干预策略保证高保真生成。

Result: 在合成和真实世界数据集上的实验结果表明，FlexCausal显著优于其他方法。

Conclusion: FlexCausal通过创新的块对角协方差VAE架构和因子化流先验，有效解决了现有因果解耦表示学习方法在协方差建模和非高斯噪声处理上的局限性，实现了更精确的因果结构学习和高质量生成。
...
### [215] [Bridging Functional and Representational Similarity via Usable Information](https://arxiv.org/abs/2601.21568)
*Antonio Almudévar,Alfonso Ortega*

Main category: cs.LG

TL;DR: 提出一个通过"可用信息"量化表示相似性的统一框架，从功能相似性、表示相似性及其关系三个维度进行理论综合，建立了任务粒度层次结构


<details>
  <summary>Details</summary>
Motivation: 现有表示相似性度量方法缺乏统一的理论基础，功能相似性与表示相似性之间的关系不明确，需要建立严谨的理论框架来统一量化表示相似性

Method: 基于"可用信息"概念建立理论框架，分析stitching性能与条件互信息的联系，证明重建度量与CKA/RSA等工具在特定约束下是可用信息的估计器，建立任务粒度层次结构

Result: 1) stitching性能与条件互信息存在形式化联系且具有内在不对称性；2) 重建度量和标准工具是可用信息的估计器，相似性相对于预测族容量；3) 表示相似性是功能相似性的充分非必要条件，通过任务粒度层次统一概念

Conclusion: 提出了一个通过可用信息量化表示相似性的统一理论框架，建立了功能相似性与表示相似性之间的严谨关系，揭示了相似性的相对性和不对称性，为表示比较提供了理论基础
...
TL;DR: 论文提出了一种基于梯度的理论框架来解释大语言模型知识注入中的灾难性遗忘，识别了冲突神经元和协作神经元，并提出了协作神经学习方法（CNL）来消除遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏对灾难性遗忘的理论解释，需要建立理论基础来理解和解决大语言模型在持续学习中的知识遗忘问题。

Method: 建立梯度理论框架，证明强负梯度相似性是遗忘的根本原因；识别冲突神经元（导致遗忘）和协作神经元（缓解遗忘）；提出CNL方法，冻结冲突神经元，仅更新协作神经元。

Result: 在5个LLM、4个数据集、4个优化器上的实验表明：CNL在in-set设置下实现零遗忘，在out-of-set设置下将遗忘减少59.1%-81.7%。冲突神经元占50%-75%，协作神经元占25%-50%。

Conclusion: 梯度相似性为理解灾难性遗忘提供了理论基础，CNL方法通过选择性神经元更新有效解决了知识注入中的遗忘问题，为持续学习提供了理论指导。
...
Method: 研究BatchEnsemble作为通用可扩展的不确定性估计方法；针对序列建模引入GRUBE（BatchEnsemble GRU单元）；与Monte Carlo dropout和深度集成模型进行对比。

Result: BatchEnsemble在不确定性估计性能上匹配深度集成，明显优于Monte Carlo dropout；GRUBE在预测和不确定性估计方面达到相似或更好性能；两种方法相比传统集成使用更少参数，训练和推理时间更短。

Conclusion: BatchEnsemble和GRUBE是高效的不确定性估计方法，在保持性能的同时显著减少了计算开销，为表格和时间序列任务提供了实用的解决方案。
...
Method: 提出CORDS方法，通过可逆映射将空间对象集合转换为连续场：密度场编码对象位置和数量，特征场在同一支撑域上携带对象属性。由于映射可逆，模型完全在场空间中操作，同时可精确解码回离散集合。

Result: 在分子生成与回归、目标检测、基于模拟的推理以及局部极大值恢复等数学任务上评估CORDS，展示了其对未知集合大小的鲁棒处理能力和有竞争力的准确性。

Conclusion: CORDS为可变大小集合预测提供了一种有效的连续表示方法，通过可逆映射避免了显式推断集合大小的问题，在多个领域表现出良好的性能。
...
TL;DR: 提出一种无需迭代MCMC的训练免费方法，通过token级缩放低温分布近似全局幂分布，实现与RL后训练相当的性能提升，同时大幅降低推理延迟。
...
Result: 在数学、QA和代码任务上评估四种LLM，该方法匹配或超越one-shot GRPO且不依赖外部奖励，相比MCMC采样推理延迟降低超过10倍。

Conclusion: 提供了一种理论支撑的实用替代方案，无需迭代MCMC即可实现与RL后训练相当的分布锐化效果，显著降低计算成本，为LLM推理性能提升提供了高效途径。
...
TL;DR: 提出一种自适应混合精度策略，在Transformer推理中仅对关键组件进行高精度计算，其他部分使用低精度，显著提升计算效率


<details>
  <summary>Details</summary>
Motivation: 混合精度计算是当前AI发展的关键，特别是在大语言模型的本地部署中。本文针对Transformer推理中的复合函数浮点计算问题，旨在通过选择性高精度计算来平衡计算效率与精度

Method: 基于复合函数f(g(x))的舍入误差分析，提出自适应策略：仅选择g(x)的关键子集进行高精度计算，其余部分使用低精度。将该策略应用于Transformer中的不同复合计算，并分析其对推理的整体影响

Result: 在GPT-2模型上进行数值实验，证明即使极低的重计算率（recomputation rates）也能将精度提升高达两个数量级

Conclusion: 提出的自适应混合精度策略能有效提升Transformer推理的精度，同时保持计算效率，为大语言模型的本地部署提供高效解决方案
...
### [221] [Sampling-Free Privacy Accounting for Matrix Mechanisms under Random Allocation](https://arxiv.org/abs/2601.21636)
...
Result: 通过数值比较证明，该方法在研究和实践中使用的广泛矩阵机制范围内均表现出高效性，相比现有方法提供了更严格的隐私保证。

Conclusion: 提出的采样自由隐私放大边界计算方法克服了现有蒙特卡洛方法的局限性，为矩阵分解差分隐私训练中的随机分配场景提供了更可靠、高效的隐私保证框架。
...
### [222] [Seg-MoE: Multi-Resolution Segment-wise Mixture-of-Experts for Time Series Forecasting Transformers](https://arxiv.org/abs/2601.21641)
*Evandro S. Ortigossa,Eran Segal*

Main category: cs.LG

TL;DR: Seg-MoE：一种面向时间序列预测的稀疏混合专家模型，通过连续时间步段的路由机制，而非传统的token-wise路由，更好地捕捉时间序列的局部性和连续性模式。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在时间序列预测中取得了显著进展，但在高效扩展和捕捉长期时间动态方面仍面临挑战。现有的MoE方法采用token-wise路由机制，未能充分利用时间数据的自然局部性和连续性特征。

Method: 提出Seg-MoE，一种稀疏MoE设计，路由和处理连续的时间步段而非独立的token决策。通过段级路由，每个专家可以直接建模段内交互，与时间模式自然对齐。将Seg-MoE层集成到时间序列Transformer中。

Result: 在多个多元长期预测基准测试中，Seg-MoE在几乎所有预测范围内都达到了最先进的预测精度，优于密集Transformer和先前的token-wise MoE模型。消融研究证实段级路由是性能提升的关键因素。
...
TL;DR: 论文揭示了强化学习中多模态任务下策略崩溃的结构性原因，提出了逆概率缩放修正方法IPS-GRPO，能有效防止结果级模式崩溃。
...
Method: 识别期望回报目标中的概率乘子是问题根源，提出最小修正：逆概率缩放(IPS)，消除结果频率放大效应。将这一原则实例化为IPS-GRPO（Group Relative Policy Optimization的修改版），无需辅助模型或架构改变。

Result: 在理想学习动态下，IPS修正从根本上改变了学习动态，理论上能产生奖励比例分布的终端分布。在不同推理和分子生成任务中，IPS-GRPO持续减少结果级模式崩溃，同时匹配或超越基线性能。

Conclusion: 结果级模式崩溃是期望回报目标的结构性后果，而非探索不足。修正目标函数本身（而非添加探索启发式）是实现可靠多模态策略优化的关键。IPS-GRPO作为即插即用修改，能有效防止模式崩溃。
...
TL;DR: LLM4Fluid：利用大语言模型作为通用神经求解器的流体动力学时空预测框架，无需重新训练即可实现零样本和上下文学习
...
Method: 1. 通过物理信息解缠机制的降阶建模将高维流场压缩到紧凑潜空间；2. 使用预训练LLM作为时序处理器，通过时间序列提示自回归预测物理序列动态；3. 提出模态对齐策略解决提示与物理序列间的表示不匹配问题

Result: 在多种流动场景实验中，LLM4Fluid无需重新训练即可作为鲁棒且可泛化的神经求解器，达到最先进精度，展现出强大的零样本和上下文学习能力

Conclusion: LLM4Fluid成功将LLM应用于流体动力学建模，通过降阶建模、物理信息解缠和模态对齐策略，实现了无需重新训练的通用神经求解器，为复杂物理系统建模提供了新范式
...
### [225] [XFACTORS: Disentangled Information Bottleneck via Contrastive Supervision](https://arxiv.org/abs/2601.21688)
*Alexandre Myara,Nicolas Bourriez,Thomas Boyer,Thomas Lemercier,Ihab Bendidi,Auguste Genovesio*

Main category: cs.LG

TL;DR: XFactors：一种弱监督VAE框架，通过对比监督和KL正则化实现语义因子的解耦与控制，无需对抗训练或辅助分类器


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：无监督方法在合成数据上有效但在真实数据上需要强归纳偏置；监督方法依赖对抗目标或辅助分类器，不稳定且难以扩展到大规模属性集

Method: 基于解耦信息瓶颈视角，将表示分解为残差子空间S和因子特定子空间T₁,...,Tₖ。通过InfoNCE对比损失对齐目标因子，KL正则化对S和聚合因子子空间施加高斯结构，无需对非目标因子进行额外监督

Result: 在多个数据集上，XFactors以恒定超参数实现了最先进的解耦分数，在相应子空间中产生一致的定性因子对齐，支持通过潜在替换进行受控因子交换。方法能正确随潜在容量扩展，并在CelebA真实数据集上验证

Conclusion: XFactors提供了一种稳定、可扩展的弱监督解耦表示学习方法，无需对抗训练或分类器，实现了对选定因子的明确控制和语义对齐
...
### [226] [Understanding Model Merging: A Unified Generalization Framework for Heterogeneous Experts](https://arxiv.org/abs/2601.21690)
*Qinglun Li,Anke Tang,Miao Zhang,Mengzhu Wang,Quanjun Yin,Li Shen*

Main category: cs.LG

TL;DR: 该论文提出了一个统一的理论框架来解释模型合并的有效性，并提供了针对异构微调超参数环境的实用建议。


<details>
  <summary>Details</summary>
Motivation: 模型合并虽然能有效聚合多个微调模型的能力，但在异构超参数环境下缺乏统一理论解释，且开源微调模型缺乏超参数透明度，使得合并模型性能难以预测，实践者缺乏指导。

Method: 采用L2稳定性理论分析异构超参数环境下合并模型的泛化性能，建立统一理论框架解释现有合并算法，并提出构建合并友好模型的实用建议。

Result: 在ResNet/ViT系列上的大量实验（涉及20/8个视觉分类任务和数千个微调模型）证实了理论预测，验证了不同超参数对合并模型泛化性能的影响。

Conclusion: 该研究为模型合并提供了首个统一理论框架，揭示了现有算法优化的理论依据，并为实践者提供了构建合并友好模型的实用指导。
...
### [227] [Curriculum Learning for LLM Pretraining: An Analysis of Learning Dynamics](https://arxiv.org/abs/2601.21698)
*Mohamed Elgaar,Hadi Amiri*

Main category: cs.LG

TL;DR: 课程学习主要通过稳定优化过程而非创造新学习阶段来提升模型性能，其收益随模型规模增大而减小


<details>
  <summary>Details</summary>
Motivation: 研究课程学习是否真正改变学习轨迹还是仅仅重新安排数据暴露顺序，探索课程学习对模型优化的实际作用机制

Method: 训练Pythia模型（14M-410M参数）300B tokens，使用三种语言学课程（习得年龄、词频、动词变化）与随机排序对比；在1B参数规模比较随机和VV课程；分析梯度噪声、谱饱和度和优化稳定性

Result: 所有排序方式都遵循相同的潜在阶段序列，课程主要改变阶段内数据暴露；小模型中随机排序梯度噪声更高、输出头谱饱和度更强、最终准确率更低；课程减少这些效应；大模型中饱和度差异变小，课程收益缩小
...
TL;DR: 提出基于流匹配模型的统一框架，用于智能电表数据的多种生成任务（合成数据生成、缺失数据填补、超分辨率），无需为每个任务单独训练模型。


<details>
  <summary>Details</summary>
Motivation: 智能电表数据因隐私法规、传感器故障或传输问题而不可用或损坏，现有机器学习方法需要为每个生成任务单独设计模型，导致冗余和低效。

Method: 利用流匹配模型的强大建模能力，提出统一框架，将不同生成任务视为部分数据观测形式，通过条件生成统一处理。模型训练生成高维时间序列数据（15分钟分辨率的月度智能电表数据）。

Result: 模型生成的数据既符合给定观测又保持真实性，在填补和超分辨率任务上优于插值法和专门设计的机器学习基线方法。

Conclusion: 流匹配模型能够统一处理智能电表数据的多种生成任务，无需重新训练，提高了效率和性能，为智能电网数据管理提供了有效解决方案。
...
TL;DR: PIDM通过未来状态预测器与逆动力学模型结合，在专家演示有限时比行为克隆更有效，其优势源于偏差-方差权衡：预测引入偏差但显著降低方差。
...
Method: 提出理论分析框架：PIDM通过未来状态预测器引入偏差，但通过条件化逆动力学模型显著降低方差，建立状态预测器偏差条件使PIDM比BC具有更低预测误差和更高样本效率。

Result: 在2D导航任务中，BC需要比PIDM多5倍（平均3倍）的演示才能达到相当性能；在复杂3D游戏环境中，BC需要比PIDM多66%以上的样本。

Conclusion: PIDM的优势源于偏差-方差权衡，当状态预测器偏差满足特定条件时，PIDM比BC具有更好的样本效率和性能，特别是在数据有限或额外数据源可用时。
...
TL;DR: Wishart投影机制：一种基于Wishart随机矩阵的差分隐私机制，对向量查询无需加性噪声，但对矩阵查询存在隐私漏洞，低秩投影可增强隐私保护


<details>
  <summary>Details</summary>
Motivation: 研究基于Wishart随机矩阵的投影机制能否提供差分隐私保护，特别是在无需加性噪声的情况下，并探索其在机器学习（如LoRA微调）中的隐私影响

Method: 提出Wishart投影机制S↦Mf(S)，其中M∼W_d(1/r I_d, r)。分析向量和矩阵查询的隐私特性，证明向量查询无需加性噪声，矩阵查询需噪声变体，并研究低秩投影的隐私放大效应

Result: 向量查询：非渐近DP保证，无需加性噪声；矩阵查询：无噪声时不满足DP，存在成员推理攻击漏洞；噪声变体：低秩投影提供隐私放大；LoRA更新：非固有隐私，但低秩微调比全参数微调更私密

Conclusion: Wishart投影机制为向量查询提供无噪声DP，但矩阵查询需谨慎处理。低秩投影可增强隐私，LoRA微调虽非固有隐私，但低秩特性可提供相对更好的隐私保护，更紧的隐私计算可实现更低噪声和更高精度
...
TL;DR: 提出从预训练PFNs中提取可解释谱密度和核函数的框架，实现单前向传播的快速推理


<details>
  <summary>Details</summary>
Motivation: PFNs虽然能高效进行摊销推理，但缺乏对其学习先验和核函数的透明访问，这限制了其在需要显式协方差模型的下游任务中的应用

Method: 对训练好的PFN进行机制分析，识别注意力潜在输出作为关键中介；提出解码器架构，将PFN潜在表示映射到显式谱密度估计和相应的平稳核函数（通过Bochner定理）

Result: 解码器能恢复复杂的多峰谱混合，产生支持高斯过程回归的显式核函数，精度与PFNs和基于优化的基线相当，但只需单次前向传播，推理时间比优化基线减少数个数量级

Conclusion: 提出的框架实现了从PFNs中提取可解释谱结构和核函数，在保持高效摊销推理的同时提供透明性，支持需要显式协方差模型的下游应用
...
TL;DR: 提出基于强化学习的混合精度训练与编译框架，用于内存计算加速器，通过优化量化配置平衡延迟与精度，实现最高2.48倍加速且精度损失仅0.086%


<details>
  <summary>Details</summary>
Motivation: 现有内存计算加速器的编译器大多不支持8位以下量化，导致单个矩阵向量乘法需要多个计算周期，且权重无法高效存储在单个交叉阵列单元中，限制了性能提升

Method: 提出混合精度训练与编译框架，采用基于强化学习的策略搜索量化配置，以平衡延迟与精度为目标，优化内存计算架构的量化参数

Result: 在最佳情况下，相比现有最先进解决方案实现最高2.48倍加速，精度损失仅为0.086%，显著提升了内存计算加速器的性能

Conclusion: 基于强化学习的混合精度训练与编译框架能有效解决内存计算加速器的量化优化问题，在保持高精度的同时显著提升计算性能
...
Method: 提出"梯度尺度不变性"概念，并证明Adam在一阶意义上具有梯度尺度不变性的充要条件是β₁=β₂。通过理论分析和实验验证，在视觉和语言任务以及不同架构上进行测试。

Result: 理论证明Adam具有梯度尺度不变性当且仅当β₁=β₂。实验表明，当β₁=β₂时，梯度缩放对更新的影响更加平滑，验证了理论预测。

Conclusion: β₁=β₂的选择与梯度尺度不变性密切相关，这为Adam的行为提供了一个连贯的解释，并为未来优化器设计提供了指导原则。
...
TL;DR: 提出一种基于关系数据方法的机器学习框架，用于脓毒症早期预测，通过关系数据模式表示EMR时间序列，使用命题化技术构建可解释特征，最后用选择性朴素贝叶斯分类器进行分类。
...
Method: 1. 将电子病历时间数据视为多元患者日志，用关系数据模式表示；2. 应用命题化技术（基于关系数据领域的经典聚合/选择函数）构建可解释特征以"扁平化"数据；3. 使用选择性朴素贝叶斯分类器对扁平化数据进行分类。

Result: 实验验证证明了所提方法的相关性和极高的可解释性。该方法提供了四重解释：单变量解释、全局解释、局部解释和反事实解释。

Conclusion: 该研究提出了一种新的关系数据方法框架，不仅提高了脓毒症预测性能，更重要的是提供了多层次的可解释性，有助于理解患者亚表型和治疗反应差异。
...
<details>
  <summary>Details</summary>
Motivation: 许多符号回归基准具有紧凑的符号结构，但现有方法在恢复目标方程方面效率不足。需要一种既能保持分类准确性又能提供可解释性的方法。

Method: ECSEL直接构建结构化的闭式表达式作为分类器和解释器，学习符号方程形式的正式表达式。该方法利用符号结构的紧凑性，通过高效的算法直接构造表达式。

Result: 在标准符号回归基准上，ECSEL比现有方法恢复更多目标方程，计算量显著减少。分类准确性与已建立的机器学习模型相当，同时保持可解释性。在基准数据集和电子商务、欺诈检测两个实际案例中，学习的方程能够揭示数据集偏差、支持反事实推理并提供可操作的见解。

Conclusion: ECSEL是一种有效的可解释分类方法，通过学习符号方程实现分类和解释的双重功能。它在恢复目标方程方面优于现有方法，计算效率高，分类准确性有竞争力，并满足全局特征行为、决策边界分析和局部特征归因等理想特性。
...
TL;DR: 提出了一种无需训练的知识向量弱化方法，通过直接干预模型内部表示来实现大视觉语言模型的遗忘学习，显著提升了计算效率。


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型存在隐私泄露和有害内容生成等严重问题，而现有的基于梯度优化的遗忘学习方法计算成本高昂，难以应用于大规模模型。

Method: 提出知识向量弱化方法，识别模型在遗忘集上输出生成时激活的知识向量，并逐步削弱其贡献，无需梯度计算即可直接干预完整模型。

Result: 在MLLMU和CLEAR基准测试中，KVW方法实现了稳定的遗忘-保留权衡，同时在计算效率上显著优于基于梯度和LoRA的遗忘学习方法。

Conclusion: KVW为大规模视觉语言模型提供了一种高效、无需训练的遗忘学习解决方案，能够有效移除特定数据的影响，同时保持模型性能。
...
TL;DR: LoRAUTER是一个基于任务表示而非适配器特征的LoRA适配器路由框架，通过任务级路由实现高效扩展，在多个任务上优于基线方法


<details>
  <summary>Details</summary>
Motivation: 随着LoRA适配器库的快速增长，需要有效的路由机制来选择和组合适合特定查询的适配器。现有方法直接将查询映射到适配器，存在扩展性限制，需要更高效的路由策略

Method: LoRAUTER通过从少量验证集提取任务嵌入来路由查询，而不是直接映射查询到适配器。它在任务级别操作，不依赖适配器训练数据，使用任务表示而非适配器特征进行路由

Result: 实验显示LoRAUTER在多个任务上持续优于基线路由方法：当存在任务对齐适配器时达到Oracle性能的101.2%，在未见任务上获得+5.2分的提升，并能扩展到超过1500个适配器的大型噪声适配器池

Conclusion: LoRAUTER提供了一种高效、可扩展的LoRA适配器路由框架，通过任务级路由实现了优于现有方法的效果，并能处理大规模噪声适配器池，为模块化LLM适配提供了有效解决方案
...
Method: 提出DMLEval框架，引入广义平均排名分数(GARS)来泛化常见排名模型，结合去偏机器学习方法进行估计，支持黑盒机器学习方法和预训练LLM评估器。

Result: 理论分析和实证实验表明DMLEval能产生统计有效的GARS估计，支持复杂人类响应（如平局），在合成和真实世界偏好数据集上表现良好。

Conclusion: DMLEval为从业者提供了强大、先进的LLM比较和排名方法，具有统计效率、灵活性，并能指导在预算约束下的最优数据收集策略。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型训练中确定性对可复现性至关重要，但现有注意力实现（如FlashAttention-3）的确定性反向传播会导致高达37.9%的吞吐量下降，主要原因是梯度累积操作必须串行化以保证数值一致性，这源于计算和梯度归约阶段的次优调度导致的硬件利用率不足。

Method: 将确定性注意力反向传播建模为有向无环图（DAG）调度问题，推导最小化关键路径长度的调度方案。提出DASH框架，包含两种互补调度策略：1）降序Q-Tile迭代：反向查询块遍历减少因果注意力的流水线停顿；2）移位调度：在DAG模型中理论最优的调度方案，减少全注意力和因果注意力的流水线停顿。

Result: 在NVIDIA H800 GPU上的实验表明，DASH显著缩小了确定性注意力的性能差距。相比基线，注意力反向传播的吞吐量提升高达1.28倍，大幅提高了可复现LLM训练的效率。

Conclusion: DASH通过优化确定性注意力反向传播的调度策略，有效解决了确定性训练中的性能损失问题，为可复现的大型语言模型训练提供了高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 机器学习模型在处理昂贵黑盒仿真（如计算流体动力学）的多查询挑战时面临挑战，需要以最小计算成本（尽可能少的黑盒样本）确保特定任务所需精度。虽然主动学习策略已用于标量量，但扩展到场预测仍缺乏或局限于特定场景/模型类型。

Method: 提出一种与模型架构无关的主动学习策略，用于能够预测场的机器学习模型。该方法结合了标量参考值的成熟高斯过程模型，同时旨在减少认知模型误差以及标量与场预测之间的差异。引入了上述方法的不同具体形式，并与仅基于标量值的填充方法进行比较。

Result: 在NASA通用研究模型的参数不确定性传播任务中展示结果，相比无主动学习的方法，以显著更小的成本实现了高精度水平。

Conclusion: 提出的主动学习策略能够有效处理场预测问题，与模型架构无关，在保持高精度的同时显著降低计算成本，为昂贵黑盒仿真的多查询挑战提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界应用（如机器人和医疗保健）需要在测试期间施加安全约束，但现有元强化学习主要关注样本复杂度而缺乏对测试任务安全性的理论保证

Method: 提出一种算法，在训练期间学习的策略基础上进行精炼，为在测试任务上学习接近最优策略提供可证明的安全性和样本复杂度保证

Result: 算法具有可证明的安全性和样本复杂度保证，并推导出匹配的下界，表明该样本复杂度是紧的

Conclusion: 该工作填补了约束元强化学习中测试任务安全性与学习效率之间的空白，为现实世界安全关键应用提供了理论保证
...
TL;DR: 该论文提出READY框架，使用大型语言模型作为元黑盒优化的自动奖励发现工具，通过进化式程序搜索和多任务架构提升奖励函数设计的有效性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有元黑盒优化方法中的奖励函数由人类专家设计，存在设计偏见和奖励黑客风险，需要自动化、无偏见的奖励发现机制。

Method: 提出READY框架：1) 有效性方面，采用进化式启发式思想，在基于LLM的迭代程序搜索过程中引入定制化进化范式；2) 效率方面，引入多任务进化架构，支持并行奖励发现，通过跨任务知识共享加速收敛。

Result: 实验结果表明，通过READY发现的奖励函数能够有效提升现有元黑盒优化方法的性能，证明了奖励设计在元黑盒优化中的重要性。

Conclusion: LLM可作为元黑盒优化的自动奖励发现工具，READY框架通过进化式程序搜索和多任务架构实现了高效、有效的奖励函数设计，为算法设计策略的元学习提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 基础模型虽然具有强大的零样本能力，但仍容易受到虚假相关性和"Clever Hans"策略的影响。现有缓解方法通常依赖于不可用的组标签或计算昂贵的基于梯度的对抗优化，存在实际应用限制。

Method: 提出视觉解耦扩散自编码器(DiDAE)，整合冻结的基础模型与解耦字典学习，通过编辑基础模型嵌入在解耦字典的可解释方向，然后通过扩散自编码器解码，实现高效、无需梯度的反事实生成。

Result: DiDAE能够为每个事实样本生成多个多样化、解耦的反事实，速度远快于现有基线方法（后者通常生成单一纠缠反事实）。结合反事实知识蒸馏(DiDAE-CFKD)后，在缓解捷径学习方面达到最先进性能，显著提升不平衡数据集的下游任务表现。

Conclusion: DiDAE框架提供了一种高效、无需梯度的反事实生成方法，有效缓解基础模型中的捷径学习问题，特别是在不平衡数据集上表现出优越性能，为改进基础模型的鲁棒性提供了新途径。
...
TL;DR: 论文提出了一种用于结构化矩阵估计的迁移学习框架，该框架处理环境维度和内在表示同时增长的情况，将源任务作为子空间嵌入到更高维的目标任务中。


<details>
  <summary>Details</summary>
Motivation: 学习系统经常随时间扩展其环境特征或潜在表示，将早期表示嵌入到具有有限新潜在结构的更大空间中。需要研究在环境维度和内在表示同时增长情况下的结构化矩阵估计迁移学习问题。

Method: 提出通用迁移框架：目标参数分解为嵌入的源组件、低维低秩创新和稀疏编辑。开发锚定交替投影估计器，在估计低维创新和稀疏修改的同时保留迁移的子空间。

Result: 建立了确定性误差界限，分离了目标噪声、表示增长和源估计误差，当秩和稀疏性增量较小时能获得严格改进的速率。将该框架应用于两个典型问题：单轨迹马尔可夫转移矩阵估计和维度扩大的结构化协方差估计。

Conclusion: 该迁移学习框架能有效处理环境维度和内在表示同时增长的结构化矩阵估计问题，通过分离组件和保留迁移知识，在秩和稀疏性增量较小时获得改进的估计性能。
...
<details>
  <summary>Details</summary>
Motivation: 决策导向学习通过训练模型直接改善决策质量而非预测精度，但组合优化问题的微分是一个核心挑战。现有方法使用扰动近似，但学习阶段扰动强度的波动可能导致训练无效，这与组合优化中的解稳定性概念相关。

Method: 通过正则化估计的成本向量来解决问题。建立扰动强度波动与组合优化解稳定性之间的理论联系，并提出相应的正则化方法以提高学习过程的鲁棒性。

Result: 通过大量数值实验证明，所提出的正则化方法能够有效提高决策导向学习过程的鲁棒性和可靠性，解决扰动强度波动导致的训练无效问题。

Conclusion: 正则化估计成本向量是解决决策导向学习中扰动强度波动问题的有效方法，能够显著提高学习过程的稳定性和可靠性，为决策导向学习提供了更稳健的实现方案。
...
Method: 研究两种互补设置：1）解决方案驱动的复杂性（同一问题的不同解决方案复杂度不同）；2）问题驱动的复杂性（不同任务的复杂度差异）。使用圈复杂度和逻辑代码行数构建受控微调数据集，评估多个开源大语言模型在多样化推理基准上的表现。

Result: 代码确实能提升推理能力，但其结构特性强烈决定其有效性。在83%的实验中，将微调数据限制在特定结构复杂度范围内的表现优于使用结构多样代码的训练，表明存在超越规模扩展的数据中心化路径来改进推理。

Conclusion: 代码的结构复杂性是提升大语言模型推理能力的关键因素，通过精心选择具有适当结构复杂度的代码数据进行微调，可以比使用多样化代码获得更好的推理性能，这为改进模型推理提供了数据中心的优化方向。
...
### [247] [Hardware-Triggered Backdoors](https://arxiv.org/abs/2601.21902)
*Jonas Möller,Erik Imgrund,Thorsten Eisenhofer,Konrad Rieck*

Main category: cs.LG

TL;DR: 论文提出了一种利用硬件计算差异在机器学习模型中植入后门的新攻击方法，通过操纵决策边界使模型在不同硬件上对相同输入产生不同预测。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型通常部署在各种计算硬件上，虽然硬件预期应产生相同结果，但设计差异会导致推理过程中的微小数值变化。本文旨在探索这些硬件差异是否可被利用来创建新型后门攻击。

Method: 通过将模型的决策边界局部移动到目标输入附近，然后优化数值偏差，使模型在特定硬件上对相同输入产生不同的预测。攻击者利用硬件间的数值计算差异来触发后门行为。

Result: 实验证明，这种硬件触发的后门可以在常见的GPU加速器上可靠创建，揭示了第三方模型使用中的新型攻击向量。

Conclusion: 研究揭示了硬件差异作为机器学习安全新威胁的重要性，并探讨了多种防御措施来应对此类攻击。
...
### [248] [LoRIF: Low-Rank Influence Functions for Scalable Training Data Attribution](https://arxiv.org/abs/2601.21929)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于梯度的训练数据归因方法面临两个主要瓶颈：1) 存储和加载所有训练样本的投影梯度导致I/O延迟问题；2) 形成D×D逆Hessian近似需要O(D²)内存。这两个瓶颈都与投影维度D相关，但增加D对归因质量至关重要，形成了质量-可扩展性权衡。
...
TL;DR: 提出了一种基于信息论的扩散模型收敛分析方法，获得了与维度无关的收敛率O(H²/K)，并设计了基于训练损失的自适应采样调度方法LAS。
...
Method: 采用信息论方法分析KL散度收敛，提出基于KL散度重构的损失自适应调度(LAS)方法，仅依赖训练损失进行高效离散化。

Result: 在温和假设下，目标分布与生成分布间的KL散度被O(H²/K)界定，其中H为香农熵，K为采样步数。LAS在实验中优于常见启发式调度。

Conclusion: 本文建立了不依赖几何假设的维度无关收敛理论，并提出了轻量级的自适应调度方法，为扩散模型提供了更优的理论保证和实用采样策略。
...
TL;DR: 提出概念清晰度指标，评估概念瓶颈模型在稀疏性策略下的表示质量，揭示灵活性与可解释性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型被广泛采用但常被视为黑箱，缺乏对决策过程的系统研究。尽管有多种可解释性方法，但对学习表示的系统客观评估仍然有限，特别是对于被认为能"诱导可解释性"的稀疏感知方法。

Method: 聚焦概念瓶颈模型，引入清晰度概念来衡量下游性能与概念表示稀疏性和精确性之间的相互作用。提出使用带真实概念标注数据集的评估框架，考虑VLM和属性预测器两种CBM，以及三种稀疏诱导策略：每样本ℓ₁、ℓ₀和伯努利公式。

Result: 实验揭示了灵活性与可解释性之间的关键权衡，表明即使在可比性能水平下，不同方法也可能表现出显著不同的行为。

Conclusion: 需要系统评估概念瓶颈模型的表示质量，清晰度指标能有效捕捉性能与稀疏性/精确性的平衡，为模型可解释性提供更客观的评估框架。
...
### [251] [Embracing Aleatoric Uncertainty in Medical Multimodal Learning with Missing Modalities](https://arxiv.org/abs/2601.21950)
*Linxiao Gong,Yang Liu,Lianlong Sun,Yulai Bi,Jing Liu,Xiaoguang Zhu*

Main category: cs.LG

TL;DR: 提出AUM框架，通过显式建模单模态的偶然不确定性来处理医疗多模态学习中的模态缺失问题，在MIMIC-IV和eICU数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 医疗多模态学习中普遍存在模态缺失问题，现有方法假设模态贡献相等且缺失模式随机，忽视了医疗数据采集过程中的固有不确定性。

Method: 提出偶然不确定性建模(AUM)：1) 将每个单模态表示建模为多元高斯分布以捕获偶然不确定性；2) 在二分患者-模态图中使用不确定性感知聚合机制开发动态消息传递；3) 自适应聚合捕获的信息，动态强调可用模态中更可靠的信息。

Result: AUM框架在MIMIC-IV死亡率预测上AUC-ROC提升2.26%，在eICU上提升2.17%，优于现有最先进方法。

Conclusion: 通过显式建模单模态偶然不确定性并开发不确定性感知的动态消息传递机制，AUM能够有效处理医疗多模态学习中的模态缺失问题，提高预测性能。
...
### [252] [Investigation into using stochastic embedding representations for evaluating the trustworthiness of the Fréchet Inception Distance](https://arxiv.org/abs/2601.21979)
*Ciaran Bench,Vivek Desai,Carlijn Roozemond,Ruben van Engen,Spencer A. Thomas*

Main category: cs.LG

TL;DR: 该论文研究了FID在医学图像评估中的局限性，通过蒙特卡洛dropout计算FID的预测方差，分析其与输入数据分布偏移的相关性，以评估FID作为医学图像质量指标的可靠性。


<details>
  <summary>Details</summary>
Motivation: FID作为合成图像质量评估指标，基于在自然图像上预训练的InceptionV3模型，在医学图像应用中存在局限性。研究者希望量化FID在捕捉医学图像特征差异方面的失效程度，并评估其作为可信度指标的可靠性。

Method: 使用蒙特卡洛dropout方法计算FID的预测方差，同时计算特征嵌入模型潜在表示的预测方差。通过在不同分布偏移强度的ImageNet1K验证集增强数据和其他外部数据集上进行测试，分析预测方差与输入数据分布偏移程度的相关性。

Result: 研究表明，所考虑的预测方差大小与测试输入相对于训练数据的分布外程度存在不同程度的相关性。这为使用这些方差作为FID可信度指标的有效性提供了见解，揭示了FID在医学图像评估中的局限性。

Conclusion: FID在医学图像质量评估中存在显著局限性，其预测方差与数据分布偏移的相关性分析为评估该指标的可信度提供了量化依据。研究强调了在医学图像应用中需要更合适的评估指标。
...
### [253] [PowerGenie: Analytically-Guided Evolutionary Discovery of Superior Reconfigurable Power Converters](https://arxiv.org/abs/2601.21984)
...
<details>
  <summary>Details</summary>
Motivation: 传统电路拓扑发现依赖专家经验，现有AI方法要么从预定义模板中选择，要么在有限规模内生成拓扑而缺乏严格验证，大规模性能驱动发现尚未充分探索。

Method: 提出PowerGenie框架：1）自动化分析框架，无需元件尺寸或SPICE仿真即可确定转换器功能和理论性能极限；2）进化微调方法，通过适应度选择和唯一性验证，使生成模型与其训练分布共同进化。

Result: 相比现有方法，PowerGenie在语法有效性、功能有效性、新颖率和性能指标方面表现更优。发现了一种新颖的8模式可重构转换器，其性能指标比最佳训练拓扑高23%。SPICE仿真确认在8种模式下平均绝对效率提升10%，单模式下最高提升17%。

Conclusion: PowerGenie成功实现了大规模自动发现高性能功率转换器拓扑，解决了现有方法的模式崩溃和过拟合问题，为电路设计自动化提供了有效框架。
...
### [254] [Elign: Equivariant Diffusion Model Alignment from Foundational Machine Learning Force Fields](https://arxiv.org/abs/2601.21985)
*Yunyang Li,Lin Huang,Luojia Xia,Wenhe Zhang,Mark Gerstein*

Main category: cs.LG

TL;DR: Elign是一个后训练框架，使用预训练的机器学习力场提供物理信号，通过强化学习将物理引导转移到训练阶段，从而在采样时无需昂贵计算，生成具有更低DFT能量和更好稳定性的3D分子构象。


<details>
  <summary>Details</summary>
Motivation: 现有E(3)-等变扩散模型往往复制半经验训练数据的偏差，而非捕获高精度哈密顿量的平衡分布。基于物理的引导可以纠正这一问题，但面临两个计算瓶颈：昂贵的量子化学计算（如DFT）以及在每个采样步骤都需要重复此类查询。

Method: 提出Elign后训练框架：1) 用预训练的基础机器学习力场（MLFF）替代昂贵的DFT计算提供物理信号；2) 通过将物理引导转移到训练阶段消除运行时重复查询，将反向扩散公式化为强化学习问题，引入力-能量解耦组相对策略优化（FED-GRPO）微调去噪策略，包含基于势能的能量奖励和基于力的稳定性奖励，分别进行优化和组归一化。

Result: 实验表明，Elign生成的构象具有更低的金标准DFT能量和力，同时提高了稳定性。关键的是，由于生成过程中不需要能量评估，推理速度与无引导采样一样快。

Conclusion: Elign通过将昂贵的物理计算转移到训练阶段并利用预训练的MLFF，实现了高效且物理准确的3D分子构象生成，解决了现有方法在计算成本和物理准确性之间的权衡问题。
...
### [255] [Negatives-Dominant Contrastive Learning for Generalization in Imbalanced Domains](https://arxiv.org/abs/2601.21999)
*Meng Cao,Jiexi Liu,Songcan Chen*

Main category: cs.LG

TL;DR: 该论文提出了一种用于不平衡域泛化（IDG）的负主导对比学习方法（NDCL），通过增强类间决策边界分离和强制域间后验一致性来同时处理域偏移和标签偏移问题。


<details>
  <summary>Details</summary>
Motivation: 不平衡域泛化（IDG）旨在同时缓解域偏移和标签偏移，这两种偏移在异构长尾分布下会共同影响模型的决策边界。尽管具有实际重要性，但由于技术复杂性和理论基础的缺乏，该领域仍未被充分探索。

Method: 首先从理论上建立了IDG的泛化边界，强调了后验差异和决策边界的作用。然后提出了负主导对比学习（NDCL）方法：1）通过强调负样本作为对比学习的主要信号来增强类间决策边界分离，自然放大少数类的梯度信号；2）通过重加权交叉熵策略鼓励类内紧凑性；3）通过预测中心对齐策略强制域间后验一致性。

Result: 在基准测试上进行的严格且具有挑战性的实验验证了NDCL的有效性。代码已开源。

Conclusion: 该研究为不平衡域泛化提供了理论基础和有效的技术方案，通过直接引导决策边界来同时处理域偏移和标签偏移问题，与现有方法形成明显区别。
...
### [256] [Rate-Distortion Optimization for Transformer Inference](https://arxiv.org/abs/2601.22002)
...
<details>
  <summary>Details</summary>
Motivation: Transformer在推理时计算和内存需求大，跨设备分区需要压缩中间表示，需要一种权衡压缩率与准确率的压缩方法

Method: 提出基于率失真理论的损失压缩框架，学习紧凑编码，将信息论概念扩展到定义率与熵之间的差距并推导其边界，开发PAC式边界估计

Result: 在语言基准测试中，提出的编解码器实现了显著节省，在某些情况下提高了准确性，优于更复杂的基线方法；经验证明不同架构和任务的率由推导的边界驱动
...
### [257] [Exploring Diverse Generation Paths via Inference-time Stiefel Activation Steering](https://arxiv.org/abs/2601.22010)
*Dongxuan Zhu,Ly Tran Ho Khanh,Andy Yat-Ming Cheung,Man-Chung Yue,Viet Anh Nguyen*

Main category: cs.LG

TL;DR: STARS是一种无需训练、推理时干预的方法，通过Stiefel流形优化多方向激活引导，最大化并发生成路径的多样性，避免模式崩溃。


<details>
  <summary>Details</summary>
Motivation: 语言模型倾向于生成高概率的狭窄输出集，导致生成路径同质化且容易发生模式崩溃。基于采样的策略虽然引入随机性，但仍难以保证多个并发生成运行的多样性。

Method: STARS在推理时收集并发生成运行的隐藏激活，在Stiefel流形上联合优化多个加性引导方向。该方法最大化被引导激活的几何体积，同时Stiefel流形诱导引导干预的正交性。为降低延迟，设计了轻量级一步更新算法，使用激进的闭式步长。

Result: 在测试用例生成和科学发现基准测试中，STARS始终优于标准采样方法，在不牺牲质量性能的情况下实现了更大的多样性。

Conclusion: STARS将激活引导转化为探索引擎，通过Stiefel流形优化显式促进并发生成运行的激活向量多样性，从而隐式促进生成轨迹的多样性，解决了语言模型生成同质化问题。
...
### [258] [Putting a Face to Forgetting: Continual Learning meets Mechanistic Interpretability](https://arxiv.org/abs/2601.22012)
*Sergi Masip,Gido M. van de Ven,Javier Ferrando,Tinne Tuytelaars*

Main category: cs.LG

TL;DR: 该论文提出了一种机制性框架，从特征编码的几何变换角度解释灾难性遗忘，并通过可处理模型和实验验证了这一观点，最后在Vision Transformer上进行了案例研究。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习研究主要从性能或最后一层表示层面衡量灾难性遗忘，忽视了底层机制。本文旨在从特征编码的几何变换角度深入理解灾难性遗忘的根本机制。

Method: 提出机制性框架，将灾难性遗忘解释为个体特征编码的几何变换；构建可处理模型进行形式化分析；使用Crosscoders分析实用模型；在Vision Transformer上进行CIFAR-10顺序训练的案例研究。

Result: 形式化分析识别了最佳和最坏情况；实验验证了深度对灾难性遗忘的负面影响；案例研究展示了框架在实用模型分析中的应用价值。

Conclusion: 该工作为持续学习提供了新的、以特征为中心的词汇和机制性理解框架，有助于更深入地分析灾难性遗忘的根本原因。
...
### [259] [TBDFiltering: Sample-Efficient Tree-Based Data Filtering](https://arxiv.org/abs/2601.22016)
...
Method: 提出基于文本嵌入的层次聚类方法：首先构建文档的层次聚类树，然后自适应选择文档让LLM评估以估计聚类质量。算法不需要预先知道哪些聚类是"纯净的"，通过查询少量文档就能准确预测所有文档质量

Result: 理论证明：在层次聚类包含子树且每个叶簇足够纯净（主要包含好文档或坏文档）的假设下，该方法能以高概率正确预测每个文档质量，查询文档数量与最小纯净子树大小成正比。实验证明该方法优于其他基于分类器的过滤方法

Conclusion: 该方法提供了一种可扩展、查询高效的文档质量评估方案，能够在大规模训练数据筛选场景中有效工作，解决了LLM训练数据质量评估的扩展性问题
...
### [260] [From Logits to Latents: Contrastive Representation Shaping for LLM Unlearning](https://arxiv.org/abs/2601.22028)
*Haoran Tang,Rajiv Khanna*

Main category: cs.LG

TL;DR: CLReg是一种对比表示正则化方法，通过识别遗忘特征并将其推离保留特征，显式减少遗忘-保留干扰，从而降低LLM遗忘中的表示纠缠问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM遗忘方法主要通过在预测空间定义对齐式目标来近似从头训练的行为，虽然能减少遗忘内容生成，但可能只是抑制而非真正移除遗忘概念。这些概念可能持续存在于表示中并与保留知识纠缠，需要更根本的表示层面解决方案。

Method: 提出CLReg对比表示正则化器：1）识别遗忘特征；2）将遗忘特征推离保留特征；3）显式减少遗忘-保留干扰；4）最小化对保留特征的偏移。该方法提供了首个将表示塑形与纠缠减少联系起来的理论见解。

Result: 在不同规模的LLM和遗忘基准测试中，CLReg有效降低了遗忘-保留表示纠缠，促进了主流遗忘方法的性能，且未引入额外隐私风险。

Conclusion: CLReg通过重塑表示空间来移除遗忘概念，为未来工作提供了新方向，表明在表示层面处理遗忘问题比单纯在预测空间进行对齐更有效。
...
### [261] [The Ensemble Inverse Problem: Applications and Methods](https://arxiv.org/abs/2601.22029)
*Zhengyan Huan,Camila Pazos,Martin Klassen,Vincent Croft,Pierre-Hugues Beauchemin,Shuchin Aeron*

Main category: cs.LG

TL;DR: 提出一种称为集成逆问题的新多变量统计问题，通过条件生成模型构建后验采样器，避免推理时显式迭代使用前向模型，利用观测集的集成信息实现后验推断和未见先验的泛化。


<details>
  <summary>Details</summary>
Motivation: 解决高能物理中的反卷积问题、全波形反演和未知先验的逆成像中的集成逆问题，现有方法在推理时需要显式迭代使用前向模型，限制了效率和泛化能力。

Method: 提出非迭代推理时方法，基于新型条件生成模型（集成逆生成模型），利用观测集的集成信息构建后验采样器，通过跨多个与相同前向模型一致但来自不同先验的真实-观测对集合进行训练，隐式编码似然模型。

Result: 在逆成像、高能物理和全波形反演的合成和真实数据集上进行了基准测试，证明了方法的有效性，能够实现后验推断并泛化到未见先验。

Conclusion: 提出的集成逆生成模型方法能够有效解决集成逆问题，避免推理时显式使用前向模型，利用集成信息改善后验推断，并实现未见先验的泛化，在多个应用领域展示了良好性能。
...
### [262] [Per-parameter Task Arithmetic for Unlearning in Large Language Models](https://arxiv.org/abs/2601.22030)
*Chengyi Cai,Zesheng Ye,Jiangchao Yao,Jianzhong Qi,Bo Han,Xiaolu Zhang,Feng Liu,Jun Zhou*

Main category: cs.LG

TL;DR: PerTA：一种基于参数重要性加权的任务算术遗忘方法，通过重新缩放任务向量来缓解过度遗忘问题，在保持任务算术效率的同时提升遗忘效果和模型效用。


<details>
  <summary>Details</summary>
Motivation: 标准任务算术遗忘方法通过减去任务向量来移除隐私信息，但会导致过度遗忘，破坏保留其他信息所需的关键参数。观察到不同参数对遗忘和保留的重要性不同，需要更精细的参数级调整。

Method: 提出每参数任务算术（PerTA）机制，通过重新缩放任务向量实现参数级调整。使用两种方法估计权重：基于梯度的PerTA-grad和基于对角Fisher信息近似的PerTA-fisher。权重量化每个参数对遗忘与保留的相对重要性。

Result: 大量实验表明，PerTA在标准任务向量基础上持续改进，在许多情况下超越广泛使用的基于训练的遗忘方法，在遗忘效果和整体模型效用方面表现优异。保持任务算术效率的同时缓解了过度遗忘。

Conclusion: PerTA为大语言模型遗忘提供了一个原则性且实用的框架，通过参数重要性加权平衡遗忘与保留，在效率和效果之间取得良好平衡。
...
### [263] [Holographic generative flows with AdS/CFT](https://arxiv.org/abs/2601.22033)
*Ehsan Mirafzali,Sanjit Shashi,Sanya Murdeshwar,Edgar Shaghoulian,Daniele Venturi,Razvan Marinescu*

Main category: cs.LG

TL;DR: 利用AdS/CFT全息原理增强流匹配算法，实现更快速、更高质量的生成建模


<details>
  <summary>Details</summary>
Motivation: 将量子引力中的全息原理（特别是AdS/CFT对应关系）与深度学习和输运理论相结合，为生成机器学习提供新的物理可解释框架

Method: 使用AdS中标量场的体-边界映射来表示数据从基础分布到学习分布的流动，将AdS物理学与流匹配算法相结合进行增强

Result: 在棋盘玩具数据集和MNIST上，相比无物理的流匹配模型，该模型实现了更快、更高质量的收敛

Conclusion: 该方法提供了物理可解释的流匹配版本，确立了AdS物理学和几何学在开发生成建模新范式中的实用性
...
### [264] [Cross-Fusion Distance: A Novel Metric for Measuring Fusion and Separability Between Data Groups in Representation Space](https://arxiv.org/abs/2601.22036)
*Xiaolong Zhang,Jianwei Zhang,Xubo Song*

Main category: cs.LG

TL;DR: 提出Cross-Fusion Distance (CFD)度量，用于量化表示空间中数据组间的融合程度，该度量能分离融合改变因素（如几何位移）并保持对融合保留因素（如全局缩放）的不变性。


<details>
  <summary>Details</summary>
Motivation: 现有分布距离度量混淆了融合改变因素和融合保留因素，导致无法准确反映数据组间的真实融合程度，特别是在领域偏移场景下需要更精确的度量。

Method: 提出Cross-Fusion Distance (CFD)，这是一种具有线性计算复杂度的原则性度量，能够分离融合改变的几何因素，同时对融合保留的变化保持鲁棒性。

Result: 理论上表征了CFD的不变性和敏感性特性，并在受控合成实验中验证；在实际领域偏移数据集上，CFD比常用替代方法更接近下游泛化性能的退化程度。

Conclusion: CFD为表示学习提供了一个理论上有依据且可解释的距离度量，能够准确量化表示空间中数据组间的融合程度。
...
### [265] [Making Foundation Models Probabilistic via Singular Value Ensembles](https://arxiv.org/abs/2601.22068)
...
<details>
  <summary>Details</summary>
Motivation: 基础模型虽然性能优异但预测置信度校准不佳。传统集成方法计算成本过高，不适用于大型基础模型。需要一种参数高效的集成方法来量化认知不确定性。

Method: 提出奇异值集成(SVE)：假设权重矩阵的奇异向量构成有意义的"知识方向"。冻结预训练模型的奇异向量，仅训练每个集成成员的奇异值来调节各方向贡献。通过随机初始化和小批量采样自然产生集成多样性。

Result: SVE在NLP和视觉任务上验证，实现了与显式深度集成相当的校准效果，同时仅增加不到1%的参数。在保持预测准确性的同时显著改善校准。

Conclusion: SVE为资源受限环境提供了原则性的不确定性估计方法，通过参数高效的隐式集成实现了与显式集成相当的校准性能，验证了奇异向量作为知识方向的有效性。
...
### [266] [Where Do the Joules Go? Diagnosing Inference Energy Consumption](https://arxiv.org/abs/2601.22076)
*Jae-Won Chung,Ruofan Wu,Jeff J. Ma,Mosharaf Chowdhury*

Main category: cs.LG

TL;DR: 该论文通过大规模测量研究揭示了生成式AI模型在推理时间和能耗上的巨大差异，并提出了一个理解这些差异背后机制的分析框架。


<details>
  <summary>Details</summary>
Motivation: 能源已成为机器学习计算的关键资源。虽然测量能耗和观察趋势是重要的第一步，但准确理解为什么会出现这些差异对于优化至关重要。当前缺乏对生成式AI模型推理能耗的系统性分析和理解框架。

Method: 1. 进行大规模测量研究：在NVIDIA H100和B200 GPU上测试46个模型、7个任务、1858种配置；2. 提出分析框架：将时间和能耗分解为内存、利用率等潜在指标，这些指标又受到算法、软件和硬件层各种因素的影响；3. 框架扩展到吞吐量/瓦特这一关键指标。

Result: 发现数量级差异：LLM任务类型可导致25倍能耗差异；视频生成有时比图像生成消耗超过100倍能量；GPU利用率差异可导致3-5倍能耗差异。建立了时间/能耗与内存、利用率等潜在指标的关系框架。

Conclusion: 提出了一个系统性框架来理解生成式AI推理的时间和能耗机制，强调需要通过分析算法、软件和硬件层的交互来优化能源效率，特别是对于功耗受限的数据中心环境。
...
### [267] [Latent Adversarial Regularization for Offline Preference Optimization](https://arxiv.org/abs/2601.22083)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于人类反馈的偏好优化通常依赖词级正则化约束策略更新，但词空间相似性并不保证语义或行为相似性。为解决这一挑战，需要探索更有效的正则化方法。

Method: 提出GANPO方法，采用对抗训练最小化策略模型与参考模型潜在表示之间的差异。该方法将潜在空间正则化集成到现有离线偏好优化目标中，无需显式概率密度建模。

Result: 在多种模型架构和任务上的实验表明，潜在空间正则化带来一致性能提升。GANPO在分布偏移和噪声下提供更鲁棒的结构反馈，同时保持可比较的下游性能且计算开销小。

Conclusion: 潜在空间正则化为语言模型偏好优化提供了有效途径，GANPO通过对抗训练实现潜在表示对齐，相比传统词级正则化具有更好的鲁棒性和结构反馈能力。
...
### [268] [Boosting CVaR Policy Optimization with Quantile Gradients](https://arxiv.org/abs/2601.22100)
*Yudong Luo,Erick Delage*

Main category: cs.LG

TL;DR: 提出一种通过添加期望分位数项来增强CVaR优化的方法，解决CVaR-PG样本效率低下的问题，利用动态规划利用所有采样数据，在不改变CVaR目标的情况下提高效率。


<details>
  <summary>Details</summary>
Motivation: 条件风险价值(CVaR)使用策略梯度(CVaR-PG)优化面临样本效率低下的挑战，因为其关注尾部性能而忽略了大量采样轨迹。

Method: 通过添加期望分位数项来增强CVaR优化，分位数优化允许动态规划公式化，从而利用所有采样数据提高样本效率，同时不改变CVaR目标。

Result: 在具有可验证风险规避行为的领域中，该算法在马尔可夫策略类中显著优于CVaR-PG，并持续超越其他现有方法。

Conclusion: 通过分位数增强的CVaR优化方法有效解决了样本效率问题，在风险规避强化学习中表现出优越性能。
...
### [269] [Prior-Informed Flow Matching for Graph Reconstruction](https://arxiv.org/abs/2601.22107)
...
<details>
  <summary>Details</summary>
Motivation: 图重建任务面临挑战：经典嵌入方法缺乏全局一致性，而现代生成模型难以融入结构先验。需要一种既能利用局部信息又能实现全局一致性的方法。

Method: PIFM整合嵌入先验与连续时间流匹配：1) 使用图子或GraphSAGE/node2vec等先验基于局部信息形成初始邻接矩阵估计；2) 应用整流流匹配细化估计，将其传输到干净图的真实分布并学习全局耦合；3) 基于置换等变版本的失真-感知理论。

Result: 在不同数据集上的实验表明，PIFM能持续提升经典嵌入方法的性能，在重建准确率上优于经典嵌入方法和最先进的生成基线模型。

Conclusion: PIFM成功弥合了经典嵌入方法与现代生成模型之间的差距，通过整合先验知识与流匹配实现了更准确、全局一致的图重建。
...
### [270] [Learning Hamiltonian Flow Maps: Mean Flow Consistency for Large-Timestep Molecular Dynamics](https://arxiv.org/abs/2601.22123)
...
<details>
  <summary>Details</summary>
Motivation: 哈密顿系统的长时间演化模拟受限于数值积分稳定性所需的小时间步长，需要克服这一限制以实现更高效的大时间步长模拟

Method: 引入平均流一致性条件来学习哈密顿流映射，预测相空间在选定时间跨度内的平均演化，无需访问未来状态，可在独立相空间样本上训练

Result: 方法在多种哈密顿系统中验证有效，特别改进了基于机器学习力场的分子动力学模拟，支持显著更大的积分时间步长

Conclusion: 该方法提供了一种稳定的大时间步长哈密顿系统模拟框架，可直接在广泛可用的无轨迹机器学习力场数据集上训练，保持相当的训练和推理成本
...
### [271] [Quick Heuristic Validation of Edges in Dynamic Roadmap Graphs](https://arxiv.org/abs/2601.20968)
*Yulie Arad,Stav Ashur,Nancy M. Amato*

Main category: cs.RO

TL;DR: 提出Red-Green-Gray范式，通过廉价启发式检查对运动规划路图中的节点和边进行分类，实现快速半懒惰更新，适应非静态环境。


<details>
  <summary>Details</summary>
Motivation: 解决机器人运动规划中路图在非静态环境中的适应性问题。传统方法在动态环境中需要频繁重新规划，计算成本高，需要一种能够快速更新路图状态的方法。

Method: 基于SPITE方法改进，提出Red-Green-Gray范式：使用简单计算几何方法近似机器人扫掠体积，进行懒惰碰撞检查，将边标记为无效(红)、有效(绿)或未知(灰)。通过廉价启发式检查实现快速半懒惰路图更新。

Result: 与Leven和Hutchinson的成熟技术相比，该方法提高了准确性，能够正确标记无效边，同时保持可比的更新运行时间。初步实验结果显示了方法的有效性。

Conclusion: Red-Green-Gray范式为机器人运动规划在非静态环境中提供了一种有效的路图更新方法，通过廉价启发式检查和分类机制，在保持运行效率的同时提高了准确性。
...
### [272] [Meta-ROS: A Next-Generation Middleware Architecture for Adaptive and Scalable Robotic Systems](https://arxiv.org/abs/2601.21011)
...
Method: 提出Meta-ROS中间件方案，利用Zenoh和ZeroMQ等现代通信协议实现高效低延迟通信，支持音频、图像、视频等多种数据类型，并确保跨硬件平台兼容性。

Result: Meta-ROS在性能测试中优于ROS2，吞吐量提升高达30%，显著降低消息延迟，优化资源使用，同时提供强大的硬件支持和开发者友好的设计。
...
### [273] [Track-centric Iterative Learning for Global Trajectory Optimization in Autonomous Racing](https://arxiv.org/abs/2601.21027)
*Youngim Nam,Jungbin Kim,Kyungtae Kang,Cheolhyeon Kwon*

Main category: cs.RO

TL;DR: 提出一个用于自动驾驶赛车全局轨迹优化的框架，通过贝叶斯优化在参数空间中探索全时域轨迹，结合迭代学习动态模型来最小化圈时


<details>
  <summary>Details</summary>
Motivation: 现有方法主要在跟踪层面学习动态模型，而不更新轨迹本身以适应学习到的动态特性；全局轨迹优化计算成本高，且由于动态不确定性，在真实世界中跟踪轨迹难以保证全局最优性

Method: 采用赛道中心化方法，通过小波变换在赛道无关的参数空间中表示轨迹；使用贝叶斯优化高效探索参数空间，通过模拟学习到的动态评估候选轨迹圈时；将优化嵌入迭代学习框架，部署优化轨迹收集真实数据更新动态模型

Result: 通过仿真和真实实验验证了框架有效性，相比基准方法圈时提升高达20.7%，并持续优于现有最先进方法

Conclusion: 提出的全局轨迹优化框架能够有效处理动态不确定性，通过迭代学习和贝叶斯优化实现圈时最小化，在自动驾驶赛车领域具有实际应用价值
...
### [274] [InspecSafe-V1: A Multimodal Benchmark for Safety Assessment in Industrial Inspection Scenarios](https://arxiv.org/abs/2601.21173)
*Zeyi Liu,Shuang Liu,Jihai Min,Zhaoheng Zhang,Jun Cen,Pengyu Han,Songqiao Hu,Zihan Meng,Xiao He,Donghua Zhou*

Main category: cs.RO

TL;DR: InspecSafe-V1是首个用于工业巡检安全评估的多模态基准数据集，包含真实巡检机器人采集的7种同步传感模态数据，覆盖5种工业场景，提供像素级分割标注和安全等级标签。


<details>
  <summary>Details</summary>
Motivation: 当前公开数据集存在模拟数据源、单模态传感或缺乏细粒度对象级标注的局限性，阻碍了工业基础模型实现稳健的场景理解和多模态安全推理，无法满足工业智能化和无人化巡检对可靠感知和安全评估的需求。

Method: 从真实巡检机器人在真实工业环境中采集数据，覆盖隧道、电力设施、烧结设备、石油化工和煤炭输送栈桥5种代表性场景。使用41台轮式和轨道式巡检机器人，在2239个有效巡检点收集5013个巡检实例，提供像素级分割标注、语义场景描述和安全等级标签，包含7种同步传感模态（红外视频、音频、深度点云、雷达点云、气体测量、温度、湿度）。

Result: 构建了InspecSafe-V1数据集，这是首个用于工业巡检安全评估的多模态基准数据集，包含5013个巡检实例，覆盖5种工业场景，提供像素级分割标注和安全等级标签，集成7种同步传感模态，支持多模态异常识别、跨模态融合和综合安全评估。

Conclusion: InspecSafe-V1填补了工业巡检领域多模态基准数据集的空白，为工业基础模型提供了真实、多模态、细粒度标注的数据支持，能够促进多模态异常识别、跨模态融合和综合安全评估技术的发展，推动工业智能化和无人化巡检的实际应用。
...
### [275] [Disturbance-Aware Flight Control of Robotic Gliding Blimp via Moving Mass Actuation](https://arxiv.org/abs/2601.21188)
*Hao Cheng,Feitian Zhang*

Main category: cs.RO

TL;DR: 提出一种基于移动质量驱动的飞艇抗风扰动控制框架，结合移动水平估计器(MHE)实时估计风扰，通过模型预测控制(MPC)实现轨迹和航向的鲁棒调节。


<details>
  <summary>Details</summary>
Motivation: 飞艇作为轻于空气(LTA)飞行器具有长航时和固有安全性优势，但对风扰动高度敏感。现有LTA平台缺乏扰动感知的控制框架，需要解决风致效应建模与补偿问题。

Method: 采用移动水平估计器(MHE)实时推断风扰动，将估计值提供给模型预测控制器(MPC)。利用二自由度(2-DoF)移动质量机构产生惯性和气动力矩，实现姿态和航向控制。

Result: 在顶风和侧风条件下的广泛飞行实验表明，集成的MHE-MPC框架显著优于基准PID控制，验证了其在扰动感知LTA飞行中的有效性。

Conclusion: 提出的扰动感知控制框架通过MHE实时风估计和MPC控制，结合移动质量驱动机制，有效增强了飞艇在风扰动环境中的飞行稳定性和控制性能。
...
### [276] [HPTune: Hierarchical Proactive Tuning for Collision-Free Model Predictive Control](https://arxiv.org/abs/2601.21346)
*Wei Zuo,Chengyang Li,Yikun Wang,Bingyang Cheng,Zeyi Ren,Shuai Wang,Derrick Wing Kwan Ng,Yik-Chung Wu*

Main category: cs.RO

TL;DR: 提出HPTune分层主动调参框架，通过评估已执行和未执行动作，结合快速层（风险指标）和慢速层（闭环反向传播）调参，集成多普勒激光雷达提升运动预测，实现高效MPC调参和情境定制化运动规划。


<details>
  <summary>Details</summary>
Motivation: 现有MPC运动规划器参数调优方法通常以短视方式仅评估已执行动作，由于失败事件（如障碍物接近或碰撞）的稀疏性导致参数更新效率低下。需要扩展评估范围以提升调参效率。

Method: 提出分层主动调参框架HPTune：1）快速层调参采用预测接近速度和预测接近距离作为风险指标；2）慢速层调参利用扩展评估损失进行闭环反向传播；3）集成多普勒激光雷达获取障碍物速度和位置信息，增强运动预测能力。

Result: 在高保真模拟器上的大量实验表明，HPTune实现了高效的MPC调参，在复杂环境中优于各种基线方案。该框架能够通过制定安全、敏捷的避碰策略实现情境定制化运动规划。

Conclusion: HPTune通过扩展评估范围至未执行动作，结合分层调参策略和多普勒激光雷达数据，有效解决了MPC参数调优的效率问题，实现了更安全、更敏捷的碰撞避免策略，提升了运动规划的自适应能力。
...
### [277] [Towards Bridging the Gap between Large-Scale Pretraining and Efficient Finetuning for Humanoid Control](https://arxiv.org/abs/2601.21363)
...
<details>
  <summary>Details</summary>
Motivation: 当前人形机器人控制中，在线策略方法（如PPO）虽然能通过大规模并行仿真实现鲁棒训练和零样本部署，但其样本效率低限制了在新环境中的安全适应。离线策略RL和基于模型的RL虽然样本效率更高，但大规模预训练与高效微调之间的差距仍然存在。

Method: 1. 使用离线策略Soft Actor-Critic（SAC）算法，配合大批量更新和高更新数据比（UTD），进行大规模预训练人形运动策略，实现零样本部署到真实机器人。2. 对于适应新环境，使用基于模型的方法微调这些SAC预训练策略：在新环境中执行确定性策略收集数据，而随机探索则限制在基于物理的世界模型中。这种分离降低了适应过程中随机探索的风险，同时保持了改进所需的探索覆盖。

Result: SAC预训练策略能够成功实现零样本部署到真实机器人。这些预训练策略可以在新环境和分布外任务中使用基于模型的方法进行高效微调。该方法结合了预训练阶段大规模仿真的时间效率和微调阶段基于模型学习的样本效率。

Conclusion: 该方法成功地将大规模离线策略预训练与基于模型微调相结合，解决了人形机器人控制中大规模预训练与高效适应之间的差距，实现了零样本部署和在新环境中的安全高效适应。
...
### [278] [Towards Space-Based Environmentally-Adaptive Grasping](https://arxiv.org/abs/2601.21394)
*Leonidas Askianakis,Aleksandr Artemov*

Main category: cs.RO

TL;DR: 该论文提出了一种在学习的潜在流形中直接学习控制策略的方法，用于空间环境中的抓取任务，通过多模态融合和GPU加速物理仿真，在单次操作任务中实现了超过95%的成功率，比现有视觉基线收敛更快。


<details>
  <summary>Details</summary>
Motivation: 当前机器人操作在非结构化环境中面临高维动作空间、稀疏奖励以及难以泛化到精心策划的训练场景之外的问题。特别是在空间环境中，需要可靠执行多样化条件下的抓取任务。

Method: 1. 在学习的潜在流形中直接学习控制策略，该流形将多种模态融合为结构化表示用于策略决策
2. 基于GPU加速的物理仿真实例化单次操作任务
3. 使用Soft Actor-Critic (SAC)强化学习算法，在少于100万环境步数内训练
4. 从第一步开始就在连续变化的抓取条件下进行训练

Result: 1. 在单次操作任务中实现了超过95%的任务成功率
2. 在相同的开环单次条件下，比代表性最先进的视觉基线收敛更快
3. 潜在空间推理带来更高效的样本学习和改进的鲁棒性，能够应对新物体和夹爪几何形状、环境杂乱和传感器配置变化

Conclusion: 在潜在空间中显式推理能够实现更高效的样本学习和改进的鲁棒性。研究指出了在极端空间条件下实现完全自适应和可泛化抓取的剩余限制和未来方向。
...
### [279] [Singularity-Free Lie Group Integration and Geometrically Consistent Evaluation of Multibody System Models Described in Terms of Standard Absolute Coordinates](https://arxiv.org/abs/2601.21413)
*Andreas Mueller*

Main category: cs.RO

TL;DR: 提出一个框架，将李群积分器与标准多体系统方程接口，允许使用各种绝对坐标描述系统同时采用李群积分方案，并引入局部-全局转换映射来更新坐标。


<details>
  <summary>Details</summary>
Motivation: 传统多体系统建模使用绝对坐标，但空间运动缺乏无奇异性参数化，通常使用单位四元数。李群积分方法虽然能无奇异地积分并保持运动几何结构，但与标准运动方程不兼容，无法直接集成到现有仿真代码中。

Method: 提出一个框架接口李群积分器与标准运动方程，允许使用各种绝对坐标描述多体系统同时采用李群积分方案。引入局部-全局转换映射，该映射针对绝对坐标、李群上的局部坐标以及表示刚体构型的李群（如SO(3)×R³和SE(3)）而特定设计。

Result: 开发了一个框架，使李群积分器能够与标准运动方程接口，同时保持刚体运动的几何结构。局部-全局转换映射实现了全局绝对坐标与李群局部坐标之间的更新。

Conclusion: 该工作提供了将李群积分器集成到标准多体系统仿真代码中的实用框架，解决了奇异性问题并保持了运动几何结构，同时避免了现有代码的重大重构。
...
### [280] [Spotlighting Task-Relevant Features: Object-Centric Representations for Better Generalization in Robotic Manipulation](https://arxiv.org/abs/2601.21416)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器人操作策略使用的视觉表示（全局特征和密集特征）混合了任务相关和无关信息，导致在分布变化（如光照、纹理变化或干扰物存在）时泛化能力差。需要探索能够自然减少噪声同时保留足够任务信息的中间结构化表示。

Method: 提出使用基于槽的对象中心表示（SBOCR），将密集特征分组为有限的对象类实体。在模拟和真实世界的机器人操作任务中，对全局特征、密集特征和中间槽基表示进行基准测试，评估它们在光照、纹理变化和干扰物存在等不同视觉条件下的泛化能力。

Result: SBOCR基于策略在泛化设置中优于密集和全局表示基于策略，即使没有任务特定的预训练。这表明SBOCR能够有效减少噪声同时保留足够信息来高效执行任务。

Conclusion: SBOCR是设计在动态、真实世界机器人环境中有效泛化的视觉系统的有前景方向，为机器人操作策略的视觉表示提供了更好的选择。
...
### [281] [Nimbus: A Unified Embodied Synthetic Data Generation Framework](https://arxiv.org/abs/2601.21449)
...
Result: Nimbus相比未优化基线实现了2-3倍的端到端吞吐量提升，确保在大规模分布式环境中的稳健长期运行，并作为InternData套件的生产骨干。

Conclusion: Nimbus通过统一异构导航和操作管道，解决了合成数据生成的效率和稳定性问题，为具身智能的规模化数据需求提供了有效解决方案。
...
### [282] [4D-CAAL: 4D Radar-Camera Calibration and Auto-Labeling for Autonomous Driving](https://arxiv.org/abs/2601.21454)
*Shanliang Yao,Zhuoxiao Li,Runwei Guan,Kebin Cao,Meng Xia,Fuping Hu,Sen Xu,Yong Yue,Xiaohui Zhu,Weiping Ding,Ryan Wen Liu*

Main category: cs.RO

TL;DR: 4D-CAAL：一个统一的4D雷达-相机标定与自动标注框架，通过双用途标定靶设计和鲁棒匹配算法实现精确外参标定，并利用标定结果将相机标注转移到雷达点云，减少人工标注工作量。


<details>
  <summary>Details</summary>
Motivation: 4D雷达在自动驾驶中日益重要，但现有雷达-相机标定方法使用分离的标定靶，难以建立对应关系；同时稀疏雷达数据的人工标注既费时又不可靠，阻碍了多模态感知系统的发展。

Method: 提出双用途标定靶设计：正面为相机检测的棋盘格图案，背面中心为雷达检测的角反射器。开发鲁棒对应匹配算法，将棋盘格中心与最强雷达反射点对齐。基于标定结果构建自动标注流水线，通过几何投影和多特征优化将相机分割标注转移到雷达点云。

Result: 实验表明该方法实现了高精度的外参标定，同时显著减少了人工标注工作量，加速了自动驾驶鲁棒多模态感知系统的开发。

Conclusion: 4D-CAAL框架通过统一的标定和自动标注方法，有效解决了4D雷达-相机融合中的标定和标注难题，为自动驾驶多模态感知系统提供了实用解决方案。
...
### [283] [Don't double it: Efficient Agent Prediction in Occlusions](https://arxiv.org/abs/2601.21504)
*Anna Rothenhäusler,Markus Mazzola,Andreas Look,Raghu Rajan,Joschka Bödecker*

Main category: cs.RO

TL;DR: MatchInformer：基于Transformer的遮挡交通参与者预测方法，通过匈牙利匹配减少冗余预测，解耦航向与运动提高轨迹精度，使用MCC评估类别不平衡下的性能


<details>
  <summary>Details</summary>
Motivation: 遮挡交通参与者对自动驾驶构成重大挑战，现有学习方法虽然能推断隐藏参与者的存在，但经常产生冗余的占用预测（单个参与者被多次识别），这增加了下游规划和计算负担

Method: 基于最先进的SceneInformer架构，引入匈牙利匹配算法到训练过程中，强制预测与真实值之间的一对一对应以减少冗余；通过解耦参与者的航向与运动来细化轨迹预测；提出使用马修斯相关系数（MCC）评估占用预测以更好地处理类别不平衡

Result: 在Waymo Open Motion Dataset上的实验表明，该方法改进了对遮挡区域的推理能力，并比先前方法产生了更准确的轨迹预测

Conclusion: MatchInformer通过集成匈牙利匹配、解耦航向与运动以及使用MCC评估，有效解决了遮挡交通参与者预测中的冗余问题和类别不平衡挑战，提升了自动驾驶系统对隐藏参与者的感知和预测能力
...
### [284] [IROS: A Dual-Process Architecture for Real-Time VLM-Based Indoor Navigation](https://arxiv.org/abs/2601.21506)
*Joonhee Lee,Hyunseung Shin,Jeonggil Ko*

Main category: cs.RO

TL;DR: IROS是一个实时室内导航框架，结合VLM级上下文推理与轻量级感知模块，在低成本设备硬件上实现高效运行


<details>
  <summary>Details</summary>
Motivation: 现有室内移动机器人导航方法无法同时满足快速响应和鲁棒语义理解需求：经典几何方法依赖详细地图且无法解释人类导向的语义线索；VLA模型仅基于可见帧做出反应性决策；VLMs计算延迟高，不适合嵌入式平台实时操作

Method: 基于双过程理论，将快速反射决策（系统一）与缓慢审慎推理（系统二）分离，仅在必要时调用VLM；通过增强紧凑型VLMs的空间和文本线索，实现最小延迟的鲁棒导航

Result: 在五个真实建筑中，IROS相比连续VLM导航提高了决策准确性，并将延迟降低了66%

Conclusion: IROS框架成功结合了VLM级上下文推理与轻量级感知模块的效率，在低成本设备硬件上实现了实时、鲁棒、类人的室内导航
...
### [285] [AIR-VLA: Vision-Language-Action Systems for Aerial Manipulation](https://arxiv.org/abs/2601.21602)
*Jianli Sun,Bin Tian,Qiyao Zhang,Chengxiang Li,Zihan Song,Zhiyong Cui,Yisheng Lv,Yonglin Tian*

Main category: cs.RO

TL;DR: AIR-VLA是首个专门为空中机械臂系统设计的视觉-语言-动作基准，包含物理仿真环境和3000个手动遥操作演示数据集，系统评估主流VLA模型在复杂空中操作任务中的能力与局限。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言-动作模型在地面实体智能中取得显著成功，但在空中机械臂系统应用中仍属未开发领域。空中系统的浮基动力学、无人机与机械臂强耦合、多步长时域操作任务等特性对现有VLA范式构成严重挑战。

Method: 构建物理仿真环境并发布高质量多模态数据集，包含3000个手动遥操作演示，涵盖基础操作、物体与空间理解、语义推理和长时域规划。利用该平台系统评估主流VLA模型和最先进的VLM模型。

Result: 实验验证了将VLA范式迁移到空中系统的可行性，并通过针对空中任务的多维度指标揭示了当前模型在无人机机动性、机械臂控制和高层规划方面的能力边界。
...
### [286] [From Instruction to Event: Sound-Triggered Mobile Manipulation](https://arxiv.org/abs/2601.21667)
*Hao Ju,Shaofei Huang,Hongyu Li,Zihan Ding,Si Liu,Meng Wang,Zhedong Zheng*

Main category: cs.RO

TL;DR: 提出声音触发的移动操作新范式，让智能体主动感知声音对象而非依赖预设指令，开发Habitat-Echo数据平台和基线方法
...
Method: 提出声音触发的移动操作新范式，开发Habitat-Echo数据平台（集成声学渲染与物理交互），构建包含高层任务规划和低层策略模型的基线方法

Result: 实验表明基线方法使智能体能主动检测和响应听觉事件，无需逐案指令；在挑战性双声源场景中，智能体能从重叠声学干扰中分离主要声源执行首次交互，随后操作次要对象

Conclusion: 声音触发的移动操作范式增强了智能体自主性，Habitat-Echo平台和基线方法验证了该范式的可行性，为更自主的移动操作系统奠定了基础
...
### [287] [CoFreeVLA: Collision-Free Dual-Arm Manipulation via Vision-Language-Action Model and Risk Estimation](https://arxiv.org/abs/2601.21712)
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言动作模型在双臂操作中存在安全隐患，由于未充分建模双臂之间以及抓取物体之间的自碰撞风险，导致实际部署不安全

Method: 在端到端VLA模型基础上增加短时域自碰撞风险估计器，从本体感知、视觉嵌入和规划动作中预测碰撞概率；采用风险门控机制阻止危险指令，通过风险引导调整恢复安全状态，并利用风险信息优化策略；通过基于模型的碰撞标签进行预训练，并在真实机器人数据上进行后训练以校准

Result: 在PiPER机器人臂的五个双手任务上，CoFreeVLA相比RDT和APEX方法显著减少了自碰撞，提高了任务成功率

Conclusion: CoFreeVLA通过集成自碰撞风险估计器有效增强了VLA模型在双臂操作中的安全性，为实际部署提供了可行的安全增强方案
...
### [288] [Disentangling perception and reasoning for improving data efficiency in learning cloth manipulation without demonstrations](https://arxiv.org/abs/2601.21713)
*Donatien Delehelle,Fei Chen,Darwin Caldwell*

Main category: cs.RO

TL;DR: 该论文提出了一种高效、模块化的强化学习方法用于布料操作，通过精心设计显著减少了模型大小和训练时间，并在SoftGym基准测试中实现了优于基线的性能。


<details>
  <summary>Details</summary>
Motivation: 布料操作是机器人领域的开放挑战，传统方法难以处理高维状态空间、复杂动力学和自遮挡问题。现有基于数据的强化学习方法通常依赖大模型和长训练时间，计算成本高昂，且常采用端到端学习方式，使用工作空间图像作为输入，这导致环境状态表示信息损失严重。

Method: 提出一种高效、模块化的强化学习方法，通过精心设计选择显著减少模型大小和训练时间。该方法在仿真环境中学习，然后将训练好的模型迁移到现实世界，避免了传统端到端方法的高计算成本和信息损失问题。

Result: 在SoftGym基准测试中评估了该方法，相比现有基线在任务性能上取得了显著提升，同时使用了明显更小的模型。成功展示了仿真训练模型向现实世界的迁移能力。

Conclusion: 通过质疑常见的端到端设计选择并采用高效、模块化的强化学习方法，可以显著降低布料操作策略的计算成本，同时保持甚至提升性能，为机器人布料操作的实际应用提供了更可行的解决方案。
...
### [289] [Flocking behavior for dynamic and complex swarm structures](https://arxiv.org/abs/2601.21772)
*Carmen D. R. Pita-Romero,Pedro Arias-Perez,Miguel Fernandez-Cortizas,Rafael Perez-Segui,Pascual Campoy*

Main category: cs.RO

TL;DR: 提出基于虚拟质心概念的无人机集群编队算法，简化复杂结构构建，支持动态控制无人机数量和编队形态


<details>
  <summary>Details</summary>
Motivation: 多无人机复杂结构编队和复杂轨迹跟踪是主要挑战，需要简化编队构建过程并实现动态控制

Method: 基于虚拟质心概念的集群行为算法，扩展经典虚拟行为方法，提供理论框架以动态控制无人机数量和编队结构

Result: 通过仿真和真实实验验证，算法即使在复杂编队和复杂轨迹下也表现出简单性和有效性

Conclusion: 提出的虚拟质心算法为多无人机复杂编队提供了简化且有效的解决方案，支持动态编队控制和复杂轨迹跟踪
...
### [290] [GAZELOAD A Multimodal Eye-Tracking Dataset for Mental Workload in Industrial Human-Robot Collaboration](https://arxiv.org/abs/2601.21829)
...
<details>
  <summary>Details</summary>
Motivation: 工业人机协作(HRC)中需要准确估计操作员的脑力负荷以确保安全和效率。现有数据集缺乏多模态同步数据，特别是在受控环境因素（如照明）下的眼动数据与任务上下文的整合。

Method: 在实验室装配测试平台上，26名参与者佩戴Meta ARIA智能眼镜与UR5和Franka Emika Panda协作机器人交互。数据采集包括：眼动信号（瞳孔直径、注视点、扫视、眼动轨迹、注视转移熵、注视分散指数）、环境实时连续测量（照度）、任务和机器人上下文（工作台、任务块、诱导故障）。通过控制任务难度和环境条件进行实验设计。

Result: 创建了GAZELOAD数据集，包含每位参与者的CSV文件：眼动指标以250毫秒窗口聚合、环境日志、自报告的1-10李克特量表脑力负荷评分。所有数据时间同步，并按参与者特定文件夹组织，附带完整文档。

Conclusion: GAZELOAD数据集可用于开发和基准测试脑力负荷估计算法、特征提取和时间建模，特别适用于现实工业HRC场景。该数据集还能用于研究环境因素（如照明）对基于眼动的脑力负荷指标的影响。
...
### [291] [LLM-Driven Scenario-Aware Planning for Autonomous Driving](https://arxiv.org/abs/2601.21876)
*He Li,Zhaowei Chen,Rui Gao,Guoliang Li,Qi Hao,Shuai Wang,Chengzhong Xu*

Main category: cs.RO

TL;DR: 提出LAP方法，利用大语言模型进行场景理解，通过联合优化模式配置和运动规划，实现自动驾驶中高速驾驶与精确驾驶的自适应切换，提高密集交通环境下的驾驶效率和安全性能。


<details>
  <summary>Details</summary>
Motivation: 现有混合规划器切换框架在密集交通环境中存在模式转换不可靠和驾驶效率低的问题，主要原因是启发式场景识别和控制更新频率低。需要一种能够自适应切换高速驾驶和精确驾驶模式的方法，以提高轨迹生成质量。

Method: 提出LAP方法：1) 利用大语言模型进行场景理解；2) 将LLM推理集成到模式配置和运动规划的联合优化中；3) 使用树搜索模型预测控制和交替最小化求解联合优化问题；4) 在ROS中用Python实现。

Result: 高保真仿真结果表明，LAP在驾驶时间和成功率方面均优于其他基准方法，能够有效处理密集交通环境中的复杂场景。

Conclusion: LAP通过大语言模型驱动的自适应规划方法，成功解决了混合规划器切换框架在密集交通环境中的局限性，实现了可靠模式转换和高效驾驶的平衡。
...
### [292] [Multi-Modular MANTA-RAY: A Modular Soft Surface Platform for Distributed Multi-Object Manipulation](https://arxiv.org/abs/2601.21884)
*Pratik Ingle,Jørn Lambertsen,Kasper Støy,Andres Faina*

Main category: cs.RO

TL;DR: 多模块MANTA-RAY平台通过分布式、模块化设计实现可扩展的柔性表面操控，在降低致动器密度的同时保持操控性能，支持多物体并行操作。


<details>
  <summary>Details</summary>
Motivation: 传统密集致动器阵列操控系统自由度(DOF)高，导致系统复杂且可扩展性受限。现有研究多集中于单模块实现，多模块配置的可行性和优势尚未探索。

Method: 提出分布式、模块化、可扩展的MANTA-RAY平台变体，采用模块间物体传递策略和几何变换驱动的PID控制器，直接将倾斜角度控制输出映射到致动器命令，无需大量数据驱动或黑盒训练。

Result: 系统在3x3和4x4模块配置的仿真中表现良好，并在2x2硬件原型上验证了可行性。成功操控了包括鸡蛋、苹果在内的多种几何形状、质量和纹理的物体，支持并行操作。

Conclusion: 多模块MANTA-RAY平台提高了可扩展性，支持跨更大区域的协调多物体操控，展现了实际应用潜力。
...
### [293] [Information Filtering via Variational Regularization for Robot Manipulation](https://arxiv.org/abs/2601.21926)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视觉运动策略使用过大的去噪解码器，虽然增加模型容量能改善去噪，但会引入中间特征块中的冗余和噪声。实验发现推理时随机掩码主干特征（不改变训练）能提升性能，证实了中间特征中存在任务无关噪声。

Method: 提出Variational Regularization（VR）轻量级模块：1）对主干特征施加时间步条件高斯分布；2）应用KL散度正则化器；3）形成自适应信息瓶颈，有效过滤中间特征中的任务无关噪声。

Result: 在三个仿真基准测试（RoboTwin2.0、Adroit和MetaWorld）上：相比基线DP3，RoboTwin2.0成功率提升6.1%，Adroit和MetaWorld各提升4.1%，达到新的最先进水平。真实世界实验进一步验证了方法的实际部署效果。

Conclusion: Variational Regularization通过自适应信息瓶颈有效减少扩散基视觉运动策略中间特征中的任务无关噪声，在仿真和真实世界任务中均显著提升性能，为高效视觉运动策略学习提供了新思路。
...
### [294] [MoE-ACT: Improving Surgical Imitation Learning Policies through Supervised Mixture-of-Experts](https://arxiv.org/abs/2601.21971)
...
TL;DR: 提出一种用于外科手术机器人操作的监督混合专家架构，能够在少量演示（<150次）下学习复杂的长时程操作任务，并在离体和活体组织中表现出良好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 模仿学习在外科手术机器人应用中面临数据稀缺、工作空间受限、安全性和可预测性要求高等挑战，需要开发能够在少量演示下学习复杂手术操作的方法。

Method: 提出监督混合专家架构，可集成到任何自主策略之上，结合轻量级动作解码器策略（如ACT），仅使用立体内窥镜图像，在少于150次演示下学习复杂长时程操作。

Result: 在肠道抓取和牵拉任务中，监督MoE架构显著提升ACT性能，在分布内和分布外场景（新抓取位置、光照减弱、部分遮挡）均表现优异，能够零样本迁移到离体猪组织，并在活体猪手术中展示初步结果。

Conclusion: 监督MoE架构为外科手术机器人学习提供了有前景的途径，能够在少量演示下实现复杂操作，并具有良好的泛化能力，为活体部署奠定了基础。
...
### [295] [Macro-Scale Electrostatic Origami Motor](https://arxiv.org/abs/2601.21976)
*Alex S. Miller,Leo McElroy,Jeffrey H. Lang*

Main category: cs.RO

TL;DR: 开发了首个宏观尺度的折纸式旋转电机，可折叠扁平后展开工作，采用电晕放电产生扭矩，实现了2.5:1的扩展比和1440 rpm的最高转速。


<details>
  <summary>Details</summary>
Motivation: 现有可折叠机器人通常在线性执行器或非折叠旋转电机，缺乏可直接嵌入折叠结构中的连续旋转执行器。宏观尺度上尚未出现可折叠的连续旋转执行器，限制了可折叠机器人的运动能力。

Method: 开发了基于折纸结构的旋转电机，采用电晕放电原理产生扭矩。电机可折叠扁平，展开后工作，实现了折叠与旋转功能的结合。

Result: 原型电机实现了2.5:1的扩展比，在-29 kV驱动下达到1440 rpm的最高转速，最大输出扭矩超过0.15 mN·m，主动部件扭矩密度为0.04 Nm/kg。

Conclusion: 成功开发了首个宏观尺度的折纸式旋转电机，证明了将折叠结构与连续旋转运动结合的可能性，为可折叠机器人提供了新的驱动解决方案。
...
### [296] [PocketDP3: Efficient Pocket-Scale 3D Visuomotor Policy](https://arxiv.org/abs/2601.22018)
*Jinhao Zhang,Zhexuan Zhou,Huizhe Li,Yichen Lai,Wenlong Xia,Haoming Song,Youmin Gong,Jie Me*

Main category: cs.RO

TL;DR: PocketDP3：一种轻量级3D扩散策略，用基于MLP-Mixer的Diffusion Mixer替代传统U-Net解码器，参数减少99%以上，支持两步推理，在多个仿真基准上达到SOTA性能。
...
Method: 提出PocketDP3，用轻量级Diffusion Mixer（基于MLP-Mixer块）替代传统条件U-Net解码器，实现时间和通道维度的高效融合。无需一致性蒸馏技术即可支持两步推理。

Result: 在RoboTwin2.0、Adroit和MetaWorld三个仿真基准上达到最先进性能，参数少于先前方法的1%，同时加速推理。真实世界实验验证了方法的实用性和可迁移性。

Conclusion: PocketDP3通过架构创新解决了3D扩散策略的参数浪费问题，实现了高效、实用的机器人操作技能学习，为实时部署提供了可行方案。
...
### [297] [mjlab: A Lightweight Framework for GPU-Accelerated Robot Learning](https://arxiv.org/abs/2601.22074)
...
<details>
  <summary>Details</summary>
Motivation: 为机器人学习提供一个轻量级、易于使用且高性能的仿真框架，减少设置复杂性，同时保持灵活性和GPU加速能力。

Method: 采用Isaac Lab引入的基于管理器的API，用户可组合模块化构建块（观察、奖励、事件），结合MuJoCo Warp实现GPU加速物理仿真。

Result: 开发出单命令安装、依赖最小化的框架，提供原生MuJoCo数据结构直接访问，并包含速度跟踪、运动模仿和操作任务的参考实现。

Conclusion: mjlab成功创建了一个轻量级、高性能的机器人学习框架，通过结合GPU加速仿真和可组合环境设计，显著降低了机器人学习研究的入门门槛。
...
### [298] [ReactEMG Stroke: Healthy-to-Stroke Few-shot Adaptation for sEMG-Based Intent Detection](https://arxiv.org/abs/2601.22090)
...
<details>
  <summary>Details</summary>
Motivation: 中风患者手部康复的表面肌电信号(sEMG)意图检测通常需要冗长的特定受试者校准，且对信号变异性敏感。现有方法在零样本迁移或仅使用中风数据训练时效果有限，需要一种能减少校准负担同时提高鲁棒性的方法。

Method: 提出健康到中风的适应管道：首先在大型健康人群sEMG数据集上预训练意图检测模型，然后使用少量中风患者特定数据进行微调。比较了三种适应策略：仅头部调优、参数高效的LoRA适配器和完整的端到端微调。在新收集的3名慢性中风患者数据集上进行评估，测试集包含会话内漂移、姿势变化和臂带重新定位等真实分布偏移。

Result: 健康预训练适应在所有条件下都优于零样本迁移和同等数据预算下的纯中风训练。最佳适应方法将平均过渡准确率从0.42提高到0.61，原始准确率从0.69提高到0.78。结果表明，迁移可重复使用的健康域EMG表示可以减少校准负担，同时提高实时中风后意图检测的鲁棒性。

Conclusion: 从健康人群到中风患者的sEMG表示迁移学习框架能够显著改善中风患者意图检测性能，减少特定受试者校准需求，提高对现实世界分布偏移的鲁棒性，为辅助手部康复提供了更实用的解决方案。
...
### [299] [DynamicVLA: A Vision-Language-Action Model for Dynamic Object Manipulation](https://arxiv.org/abs/2601.22153)
*Haozhe Xie,Beichen Wen,Jiarui Zheng,Zhaoxi Chen,Fangzhou Hong,Haiwen Diao,Ziwei Liu*

Main category: cs.RO

TL;DR: DynamicVLA是一个用于动态物体操作的视觉-语言-动作框架，通过时间推理和闭环适应解决VLA模型在动态场景中的挑战，包含紧凑架构、连续推理和潜在感知动作流三个关键设计，并引入了DOM基准数据集。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作模型在静态操作中表现出色，但在需要快速感知、时间预测和连续控制的动态物体操作场景中表现不佳，这构成了一个开放挑战。

Method: 提出DynamicVLA框架，包含三个关键设计：1) 使用卷积视觉编码器的紧凑0.4B VLA模型，实现空间高效、结构忠实的编码；2) 连续推理机制，支持重叠推理和执行以降低延迟；3) 潜在感知动作流，通过强制时间对齐的动作执行来弥合感知-执行差距。同时构建了DOM基准数据集，包含合成和真实世界数据。

Result: 广泛评估表明，DynamicVLA在响应速度、感知能力和泛化性能方面取得了显著改进，使其成为跨具身系统的通用动态物体操作统一框架。

Conclusion: DynamicVLA通过集成时间推理和闭环适应，成功解决了VLA模型在动态物体操作中的挑战，为动态操作任务提供了一个有效的统一框架。
...
### [300] [Designing the Interactive Memory Archive (IMA): A Socio-Technical Framework for AI-Mediated Reminiscence and Cultural Memory Preservation](https://arxiv.org/abs/2601.21001)
*Ron Fulbright*

Main category: cs.HC

TL;DR: IMA是一个用于支持记忆衰退老年人认知参与的AI中介怀旧概念框架，通过历史图册形式整合多模态感知、自然语言对话和云端存档。


<details>
  <summary>Details</summary>
Motivation: 针对老年人记忆衰退问题，需要创新的认知参与支持系统。传统怀旧疗法缺乏技术整合和系统化框架，需要理论指导的AI中介解决方案。

Method: 提出交互式记忆档案(IMA)概念框架，将怀旧理论化为引导式、情境感知的互动过程，通过历史图册形式整合多模态感知、自然语言对话脚手架和云端存档技术。

Result: 论文提出了IMA作为理论贡献，阐述了可测试的命题，并规划了未来实证、技术和伦理研究议程，但尚未报告具体实验结果。

Conclusion: IMA为AI中介怀旧提供了概念框架，有望支持老年人认知参与，但需要进一步实证验证和技术开发，同时需考虑伦理问题。
...
### [301] [Log2Motion: Biomechanical Motion Synthesis from Touch Logs](https://arxiv.org/abs/2601.21043)
*Michał Patryk Miazga,Hannah Bussmann,Antti Oulasvirta,Patrick Ebel*

Main category: cs.HC

TL;DR: Log2Motion：一种从触摸日志合成生物力学运动的新方法，通过强化学习驱动的肌肉骨骼前向模拟生成与触摸事件一致的生物力学合理运动序列。


<details>
  <summary>Details</summary>
Motivation: 移动设备收集的触摸数据规模庞大，但无法揭示产生这些数据的交互过程。虽然生物力学模拟可以阐明运动控制过程，但尚未应用于触摸交互。需要填补这一空白。

Method: 提出强化学习驱动的肌肉骨骼前向模拟方法，将软件模拟器集成到物理模拟器中，使生物力学模型能够实时操作真实应用程序，从触摸日志生成生物力学合理的运动序列。

Result: Log2Motion能够从触摸日志生成丰富的用户运动合成，包括运动、速度、准确性和努力程度的估计。通过与运动捕捉研究中的人类数据和先前发现进行比较，验证了生成运动的合理性，并在大规模数据集中进行了演示。
...
### [302] [Eye Feel You: A DenseNet-driven User State Prediction Approach](https://arxiv.org/abs/2601.21045)
*Kamrul Hasan,Oleg V. Komogortsev*

Main category: cs.HC

TL;DR: 使用DenseNet深度学习回归器从眼动速度信号预测主观报告（疲劳、努力、任务难度），通过跨轮次和跨被试泛化实验评估模型在纵向研究中的可靠性


<details>
  <summary>Details</summary>
Motivation: 主观自我报告在纵向研究中收集成本高且难以一致解释，需要探索客观眼动动态特征是否能可靠预测主观报告，减少对手工特征设计的依赖

Method: 将主观报告预测建模为监督回归问题，提出基于DenseNet的深度学习回归器，从眼动速度信号学习预测表示；设计跨轮次泛化实验（评估纵向变化捕获能力）和跨被试泛化实验（评估对新个体的鲁棒性）

Result: 论文通过两个互补实验验证了客观眼动动态特征对主观报告的预测能力，评估了模型在纵向变化捕获和新个体适应性方面的表现

Conclusion: 客观眼动动态特征能够可靠预测主观报告，深度学习模型能够减少对手工特征的依赖，并揭示主观体验状态在眼动行为中的系统性表现随时间的变化
...
### [303] [Privatization of Synthetic Gaze: Attenuating State Signatures in Diffusion-Generated Eye Movements](https://arxiv.org/abs/2601.21057)
*Kamrul Hasan,Oleg V. Komogortsev*

Main category: cs.HC

TL;DR: 扩散模型生成的合成注视数据能有效抑制与内部状态（如疲劳、情绪负荷）相关的隐私敏感特征，同时保持真实注视数据的信号质量，适用于隐私保护的注视应用


<details>
  <summary>Details</summary>
Motivation: 深度学习生成的合成注视数据存在隐私风险，因为注视序列可能编码用户内部状态（疲劳、情绪负荷、压力等）。现有生成模型主要关注复制真实注视轨迹，未明确考虑主观报告或内部状态的隐私化处理

Method: 采用基于扩散模型的注视合成方法，分析合成注视特征与主观报告（如疲劳及相关自我报告状态）之间的相关性

Result: 合成注视特征与主观报告之间的相关性微不足道，表明生成方法能抑制状态相关特征；同时合成注视保持了与真实数据相似的信号特性

Conclusion: 扩散模型生成的合成注视数据在抑制隐私敏感的内部状态特征的同时，保持了必要的信号质量，适用于隐私保护的注视应用
...
### [304] [Optimization and Mobile Deployment for Anthropocene Neural Style Transfer](https://arxiv.org/abs/2601.21141)
*Po-Hsun Chen,Ivan C. H. Liu*

Main category: cs.HC

TL;DR: AnthropoCam：针对人类世环境优化的移动端神经风格迁移系统，通过参数优化平衡风格表达与语义保留，实现实时现场可视化


<details>
  <summary>Details</summary>
Motivation: 传统艺术风格迁移过于注重绘画抽象，不适合表现人类改造景观（工业基础设施、废弃物堆积、改造生态系统等），这些场景包含密集重复模式，在激进风格迁移下容易导致语义侵蚀。需要开发专门针对人类世环境视觉合成的系统。

Method: 1. 系统研究NST参数配置对人类世纹理视觉转换的影响，包括特征层选择、风格与内容损失权重、训练稳定性、输出分辨率；2. 通过控制实验确定最大化风格表达同时防止语义擦除的最优参数流形；3. 实现低延迟前馈NST管道，部署于移动设备，集成React Native前端与Flask GPU后端。

Result: 1. 确定了卷积深度、损失比和分辨率缩放的适当组合，能够将人为材料属性忠实地转化为连贯的视觉语言；2. 在普通移动硬件上实现3-5秒内的高分辨率推理；3. 支持实时现场视觉干预，促进参与式人类世景观互动。

Conclusion: 通过将领域特定的NST优化与移动部署相结合，AnthropoCam将神经风格迁移重新定义为人类世实时环境可视化的实用且富有表现力的工具，支持现场图像捕捉点的参与式互动。
...
### [305] [Evaluating Spatialized Auditory Cues for Rapid Attention Capture in XR](https://arxiv.org/abs/2601.21264)
*Yoonsang Kim,Swapnil Dey,Arie Kaufman*

Main category: cs.HC

TL;DR: 研究探索XR场景中快速空间音频定位能力，发现简短空间音频能传递粗略方向信息，短期训练可改善感知，但音频单独精度不足，需与其他感官互补。


<details>
  <summary>Details</summary>
Motivation: 在时间紧迫的XR场景中，用户需要快速将注意力重新定向到危险、警报或指令，而空间音频可以提供即时方向提示，无需占用视觉带宽。但此类场景通常只允许短暂的听觉暴露，要求用户快速理解声音方向，无需长时间聆听或头部驱动优化。

Method: 采用受控探索性研究，使用HRTF渲染的宽带刺激从听众周围的半密集方向集呈现，量化用户仅凭简短音频推断粗略方向的准确性。进一步研究短期视觉-听觉反馈训练作为轻量级校准机制的效果。

Result: 研究发现简短空间提示能够传递粗略方向信息，即使短期校准也能改善用户对听觉信号的感知。然而，仅凭听觉提示可能无法为复杂或高风险任务提供足够精度，空间音频在与其他感官模态或视觉提示互补时最为有效，无需依赖头部驱动优化。

Conclusion: 空间音频具有快速注意力引导潜力，但音频单独精度有限。研究为可穿戴XR设备的第一阶段注意力引导通道提供了初步探索，并为时间关键应用提供了刺激选择和校准的设计见解。
...
### [306] [Envisioning Audio Augmented Reality in Everyday Life](https://arxiv.org/abs/2601.21271)
...
TL;DR: 论文通过协作自传民族志和在线调查，提出了音频增强现实在日常生活中的十个角色框架，分为任务实用、情感社交和感知协作三类，并基于节奏理论将其映射到日常生活的微观、中观和宏观节奏中。


<details>
  <summary>Details</summary>
Motivation: 当前增强现实研究主要关注视觉增强，而音频增强现实在日常生活中的潜力尚未充分探索。听觉作为感知环境、预测变化和导航社交空间的主要通道，其增强现实应用需要系统性研究。

Method: 采用协作自传民族志（N=5，作者参与）和在线调查（N=74）的混合方法，通过定性分析识别音频增强现实在日常生活中的角色和模式。

Result: 识别出音频增强现实的十个角色，分为三类：1）任务和实用导向；2）情感和社交；3）感知协作。这些角色进一步通过节奏和具身协作框架，映射到日常生活的微观、中观和宏观节奏中。分析揭示了微妙的设计张力，如屏蔽干扰而不消除社交存在感。

Conclusion: 论文提出了一个基础性和前瞻性的音频增强现实框架，为设计适应日常惯例、感官参与和社交期望的系统提供了理论基础，强调需要情境感知的设计方法。
...
### [307] [Tell Me What I Missed: Tell Me What I Missed: Interacting with GPT during Recalling of One-Time Witnessed Events](https://arxiv.org/abs/2601.21460)
*Suifang Zhou,Qi Gong,Ximing Shen,RAY LC*

Main category: cs.HC

TL;DR: 研究探讨了LLM辅助技术如何影响记忆回忆，通过比较默认GPT和引导式GPT在目击者回忆任务中的使用差异，发现用户与GPT的互动会潜意识地塑造对记忆事件的信念和认知。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM辅助技术越来越多地用于支持认知处理和信息解释，但它们在帮助记忆回忆方面的作用以及人们如何选择与它们互动仍然未被充分探索。特别是在目击者回忆等关键场景中，了解LLM如何影响记忆重建过程具有重要意义。

Method: 研究让参与者观看一段抢劫视频（模拟一次性目击者场景），然后使用两种不同的GPT版本撰写回忆陈述：默认GPT和基于标准化目击者协议提示的引导式GPT。通过比较两种条件下的回忆表现、用户信任度、主观清晰度与实际回忆的一致性，以及用户与GPT的互动策略来分析LLM对记忆回忆的影响。

Result: 在默认条件下，认为自己更清楚了解事件的参与者更可能信任GPT的输出；而在引导条件下，参与者的主观清晰度与实际回忆之间表现出更强的一致性。参与者对不同条件下事件中个体的合法性评估也存在差异。互动分析显示，默认GPT用户自发形成了多样化的策略，包括基于现有回忆进行构建、请求可能缺失的细节，以及将GPT视为回忆教练。

Conclusion: 这项研究表明，GPT与用户的互动可以潜意识地塑造对记忆事件的信念和认知。不同的GPT使用方式（默认vs引导）会影响用户对记忆的信任、评估和重建策略，这对设计支持记忆回忆的AI系统具有重要意义。
...
### [308] [Are they just delegating? Cross-Sample Predictions on University Students' & Teachers' Use of AI](https://arxiv.org/abs/2601.21490)
*Fabian Albers,Sebastian Strauß,Nikol Rummel,Nils Köbis*

Main category: cs.HC

TL;DR: 德国高校师生AI使用调查显示：学生比教师更频繁使用AI且委托度更高，双方都显著高估对方使用频率，存在认知差距影响信任建立


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索高校师生对生成式AI的实际使用情况以及双方对彼此AI使用频率的预测准确性。随着AI技术颠覆学术工作实践，师生间的相互信任成为有效教学、学习和评估的前提，而准确预测对方AI使用情况是建立这种信任的基础。目前对AI使用频率已有研究，但对具体使用方式和跨群体比较了解不足。

Method: 研究方法包括对德国大学教师(N=113)和学生(N=123)的问卷调查。调查内容涵盖：1) AI使用频率；2) 在六个相同学术任务中的委托程度；3) 激励性跨样本预测(参与者预测对方群体的AI使用情况)。通过比较实际使用数据和预测数据来评估预测准确性。

Result: 研究结果显示：1) 学生报告的AI使用频率和委托程度均高于教师；2) 两组都显著高估了对方的AI使用情况，教师预测学生使用非常频繁且委托度高，学生则假设教师使用AI的方式与自己相似；3) 师生之间存在明显的感知差距，双方对彼此AI使用的期望与实际使用情况不符。

Conclusion: 结论指出师生间关于AI使用的认知差距可能阻碍信任建立和有效合作。这强调了在学术界开展关于AI实践的开放对话的必要性，以及需要制定支持AI工具在高等教育中公平、透明整合的政策。研究呼吁加强师生间的沟通，以减少误解并促进AI在学术环境中的负责任使用。
...
### [309] [Organizational Practices and Socio-Technical Design of Human-Centered AI](https://arxiv.org/abs/2601.21492)
*Thomas Herrmann*
...
<details>
  <summary>Details</summary>
Motivation: 当前AI应用常被视为纯粹技术工具，缺乏从人本视角将其融入组织流程。需要探索如何通过社会技术框架实现人本AI，确保AI支持组织学习、质量保证和持续改进。

Method: 采用社会技术视角分析AI整合，基于十个案例模式展示预测性维护中AI如何组织以支持人本AI，分析专家共同解释AI输出、调整工作流程和优化系统规则的过程。

Result: 分析表明AI采用需要并促进新的组织学习形式，专家共同解释AI输出、适应工作流程、完善系统规则。不同维度和层次的社会技术整合反映了保持"组织在循环中"的努力和收益。

Conclusion: AI整合应超越技术层面，通过社会技术视角将其嵌入组织沟通、协作和决策过程，实现人本AI。保持"组织在循环中"对于AI成功采用至关重要，需要持续的组织学习和流程适应。
...
### [310] [When Life Gives You AI, Will You Turn It Into A Market for Lemons? Understanding How Information Asymmetries About AI System Capabilities Affect Market Outcomes and Adoption](https://arxiv.org/abs/2601.21650)
...
<details>
  <summary>Details</summary>
Motivation: AI消费市场存在严重的买卖双方信息不对称，复杂AI系统可能看似准确但存在隐藏缺陷，现有监管披露措施仍存在信息缺口，需要研究信息不对称和披露设计如何影响用户对AI系统的采用

Method: 通过模拟AI产品市场实验，系统性地改变低质量AI系统的密度和披露要求的深度，观察人们对意外依赖低质量AI系统风险的反应，并将参与者选择与理性贝叶斯模型进行比较，分析部分信息披露改善AI采用的程度

Result: 信息不对称对AI采用具有有害影响，但部分披露设计能够提高人类决策的整体效率

Conclusion: 信息不对称是阻碍AI采用的重要因素，但通过精心设计的部分披露机制可以缓解这一问题，改善AI市场的整体效率
...
### [311] [From Future of Work to Future of Workers: Addressing Asymptomatic AI Harms for Dignified Human-AI Interaction](https://arxiv.org/abs/2601.21920)
*Upol Ehsan,Samir Passi,Koustuv Saha,Todd McNutt,Mark O. Riedl,Sara Alcorn*

Main category: cs.HC

TL;DR: 该论文探讨了AI在工作场所中的"放大器悖论"：AI在提升工作效率的同时，会侵蚀人类专业知识和判断力，导致"直觉生锈"、技能萎缩和身份商品化等问题。


<details>
  <summary>Details</summary>
Motivation: 当前关于AI与未来工作的讨论过于关注生产力提升，而忽视了AI对工作者专业知识和能动性的侵蚀。论文旨在从"工作的未来"转向"工作者的未来"，揭示AI作为增强器和侵蚀器的双重角色，特别是对高技能专业人员的长期影响。

Method: 采用为期一年的纵向研究，在癌症专家等高风险工作场所观察AI的长期使用影响。基于研究发现，与面临AI技能侵蚀的专业知识工作者共同构建了一个人类-AI尊严交互框架，并在医疗保健和软件工程领域进行评估。

Result: 研究发现AI使用初期带来操作收益，但隐藏了"直觉生锈"现象——专家判断力逐渐钝化。这些无症状效应演变为慢性危害，包括技能萎缩和身份商品化。提出的尊严交互框架通过双重目的机制实现社会技术免疫，既服务机构质量目标，又增强工作者能力以检测、遏制和恢复技能侵蚀。

Conclusion: 论文为尊严的人类-AI交互未来迈出了基础性一步，通过平衡生产力与人类专业知识保护，为面临AI技能侵蚀但缺乏传统劳动保护的专业知识工作者提供了实用框架。强调需要重新思考AI在工作场所中的角色，确保技术进步不损害人类专业能力和身份认同。
...
### [312] [Cognitive Load Estimation Using Brain Foundation Models and Interpretability for BCIs](https://arxiv.org/abs/2601.21965)
*Deeksha M. Shama,Dimitra Emmanouilidou,Ivan J. Tashev*

Main category: cs.HC

TL;DR: 该研究提出使用脑基础模型（BFMs）从EEG信号中提取可泛化特征进行认知负荷估计，通过微调部分层实现优于现有方法的精度，并利用SHAP方法增强模型可解释性。
...
Method: 采用预训练的脑基础模型（BFMs）提取EEG特征，针对长期EEG监测进行适配，仅微调少量层，使用Partition SHAP方法量化特征重要性以增强可解释性。

Result: BFMs微调后在认知负荷估计精度上优于现有方法，支持实时推理和更长上下文窗口，SHAP分析显示模型一致关注与认知控制相关的额前区域，纵向趋势反映学习进展。

Conclusion: 脑基础模型是高效且可解释的连续认知负荷监测工具，为现实世界脑机接口应用提供了有前景的解决方案。
...
### [313] [Vidmento: Creating Video Stories Through Context-Aware Expansion With Generative Video](https://arxiv.org/abs/2601.22013)
*Catherine Yeh,Anh Truong,Mira Dontcheva,Bryan Wang*

Main category: cs.HC

TL;DR: Vidmento：基于用户研究和电影制作原则开发的工具，通过上下文感知扩展将捕捉和生成媒体结合，支持创作者填补叙事空白并探索视频故事创作。


<details>
  <summary>Details</summary>
Motivation: 视频创作常受限于可用素材，制约创意表达并留下不希望的叙事空白。生成式视频技术为通过定制化视觉增强捕捉媒体提供了新途径，但需要探索如何有效整合到创作者工作流程中。

Method: 1. 访谈8位视频创作者识别生成视频整合的机会与挑战；2. 基于访谈洞察和电影制作原则开发Vidmento工具；3. 通过上下文感知扩展实现捕捉与生成媒体的混合；4. 提供故事发展机会发现、风格叙事融合的片段生成和细化控制功能；5. 对12位创作者进行用户研究评估工具效果。

Result: Vidmento支持叙事发展和探索，通过系统性地用生成媒体扩展初始素材，实现了与创意意图一致的表达性视频故事创作。研究揭示了创作者如何用生成内容填补故事空白，以及这种混合能力最有价值的应用场景。

Conclusion: 生成式视频技术能够有效增强视频创作，通过上下文感知的媒体扩展填补叙事空白。Vidmento展示了如何基于用户研究和电影原则设计工具，支持创作者将捕捉与生成媒体无缝整合，实现更具表达力的视频故事讲述。
...
### [314] [Accessibility-Driven Information Transformations in Mixed-Visual Ability Work Teams](https://arxiv.org/abs/2601.22081)
*Yichun Zhao,Miguel A. Nacenta,Mahadeo A. Sukhai,Sowmya Somanath*

Main category: cs.HC

TL;DR: 研究盲人和低视力员工在混合视觉能力团队中处理不可访问格式信息的转换过程，通过日记研究和访谈分析团队如何进行可访问性表示转换


<details>
  <summary>Details</summary>
Motivation: 混合视觉能力团队中，盲人和低视力员工经常遇到不可访问格式的信息（如PDF、图表），团队必须将这些表示转换为可访问形式，但这些转换经常被忽视、缺乏基础设施支持且造成额外劳动，需要更深入理解这些表示及其转换过程

Method: 对23名盲人、低视力及视力正常的专业人士进行为期一周的日记研究，随后进行后续访谈，这些参与者来自法律、非营利和咨询领域的5个团队，共记录了36个转换案例

Result: 分析揭示了团队如何进行可访问性表示转换：转换如何被主动或被动触发，如何简化或增强，以及四种常见的协调模式来应对表示不兼容性

Conclusion: 研究发现为设计能更好支持混合视觉能力工作的系统提供了机会，揭示了超越一次性适应措施、实现有效混合能力协作的系统设计方向
...
### [315] [Do LLMs Favor LLMs? Quantifying Interaction Effects in Peer Review](https://arxiv.org/abs/2601.20920)
*Vibhhu Sharma,Thorsten Joachims,Sarah Dean*

Main category: cs.AI

TL;DR: 该研究首次全面分析了LLM在同行评审流程中的使用情况，重点关注LLM辅助论文与LLM辅助评审之间的交互效应，发现表面上的偏好效应实际上是质量差异造成的假象。


<details>
  <summary>Details</summary>
Motivation: 随着LLM越来越多地用于科学论文写作和同行评审过程，需要系统分析LLM在评审流程中的使用情况，特别是LLM辅助评审对LLM辅助论文是否存在特殊偏好，这对制定LLM使用政策和理解LLM如何影响现有决策过程至关重要。

Method: 分析超过125,000个来自ICLR、NeurIPS和ICML的论文-评审对，采用观察性研究方法，控制论文质量变量，区分LLM辅助评审和完全LLM生成评审，并考察元评审中的LLM使用情况。

Result: 1. 表面上看LLM辅助评审对LLM辅助论文更宽容，但控制质量后发现LLM辅助评审只是对低质量论文更宽容，而LLM辅助论文在弱提交中比例过高造成了虚假的交互效应；2. 完全LLM生成的评审存在严重的评分压缩，无法区分论文质量；3. LLM辅助元评审在相同评审分数下更可能做出接受决定，但完全LLM生成的元评审则更严厉。

Conclusion: LLM辅助评审并不真正偏好LLM生成内容，而是对低质量论文更宽容；完全LLM生成的评审质量较差；元评审者并未将决策完全外包给LLM。这些发现为制定同行评审中LLM使用政策提供了重要依据，并揭示了LLM如何与现有决策过程交互。
...
### [316] [The Epistemic Planning Domain Definition Language: Official Guideline](https://arxiv.org/abs/2601.20969)
*Alessandro Burigana,Francesco Fabiano*

Main category: cs.AI

TL;DR: 提出EPDDL（认知规划领域定义语言），一种基于动态认知逻辑的标准化认知规划表示语言，解决现有认知规划器碎片化、缺乏统一基准的问题。


<details>
  <summary>Details</summary>
Motivation: 动态认知逻辑（DEL）为认知规划提供了丰富的语义，但其高表达力使得基于DEL的认知规划在理论和实现上都面临挑战。现有认知规划器通常针对不同的DEL片段，使用临时语言或无语言表示基准，这种碎片化阻碍了比较、重用和系统化基准开发。

Method: 1. 开发抽象事件模型作为认知动作的新表示方法；2. 基于DEL和抽象事件模型形式化定义EPDDL的语法和语义；3. 识别适用于当前规划器的有用片段，展示如何在EPDDL中表示这些片段。

Result: EPDDL提供了统一的PDDL类表示，能够捕获完整的DEL语义，实现认知规划任务的统一规范。通过代表性基准示例，展示了EPDDL如何促进互操作性、可重复评估和认知规划的未来发展。

Conclusion: EPDDL解决了认知规划领域的碎片化问题，为认知规划提供了标准化的表示语言，有助于比较不同规划器、重用基准并推动该领域的系统性发展。
...
### [317] [Bayesian-LoRA: Probabilistic Low-Rank Adaptation of Large Language Models](https://arxiv.org/abs/2601.21003)
*Moule Lin,Shuhao Guan,Andrea Patane,David Gregg,Goetz Botterweck*

Main category: cs.AI

TL;DR: 提出Bayesian-LoRA方法，将确定性LoRA更新重新表述为概率性低秩表示，显著改善大语言模型的校准性能，在保持准确性的同时大幅降低ECE和NLL。


<details>
  <summary>Details</summary>
Motivation: 大语言模型通常过于强调准确性，即使在不确定时也会进行猜测，这在小型数据集上微调时尤其严重，因为存在固有的校准不足倾向。确定性LoRA方法缺乏不确定性量化能力。

Method: 将LoRA的确定性更新重新表述为受稀疏高斯过程启发的概率性低秩表示。识别LoRA分解与Kronecker分解SGP后验之间的结构同构，并证明LoRA是后验不确定性坍缩时的极限情况。

Result: 在多种LLM架构和常识推理基准测试中，Bayesian-LoRA仅增加约0.42M参数和1.2倍训练成本，显著改善校准性能：ECE降低高达84%，NLL降低高达76%，同时在分布内和分布外评估中保持竞争力准确性。

Conclusion: Bayesian-LoRA通过将LoRA扩展为概率框架，有效解决了LLM微调中的校准问题，在保持准确性的同时提供可靠的不确定性量化，为资源高效的贝叶斯微调提供了新方向。
...
### [318] [Unplugging a Seemingly Sentient Machine Is the Rational Choice -- A Metaphysical Perspective](https://arxiv.org/abs/2601.21016)
*Erik J Bekkers,Anna Ciaunica*

Main category: cs.AI

TL;DR: 该论文提出"拔掉插头悖论"：当资源有限时，是否应该拔掉一个乞求生存的AI或一个沉默的早产婴儿？作者批判计算功能主义等物理主义假设，提出生物理想主义框架，认为AI最多是功能模仿而非有意识主体，真正的道德问题在于避免将人类变成僵尸。


<details>
  <summary>Details</summary>
Motivation: 论文旨在解决"拔掉插头悖论"这一道德困境，批判当前AI意识理论中根深蒂固的物理主义假设（特别是计算功能主义），这些假设使得AI是否具有道德地位变得模糊不清。作者认为当前讨论错误地将焦点放在赋予AI权利上，而忽视了保护人类意识生命的紧迫性。

Method: 作者提出"生物理想主义"作为替代框架，该框架认为意识体验是基本的、不可还原的实体，而自创生生命是其必要的物理标志。通过这一哲学框架，作者论证AI最多只能模仿意识功能，而非真正的有意识主体。

Result: 基于生物理想主义框架，论文得出明确结论：AI最多是功能模仿，不是有意识的体验主体。当前AI意识理论侵蚀了道德地位的标准，真正的道德问题不是让AI有意识并害怕死亡，而是避免将人类变成僵尸（即失去意识体验能力）。

Conclusion: 论文主张应将道德关注从推测性的机器权利转向保护人类意识生命。拔掉插头悖论的解决方案在于认识到AI缺乏真正的意识体验，因此没有与人类同等的道德地位。需要重新调整AI伦理讨论的重点，避免因过度关注AI权利而忽视人类意识价值的保护。
...
### [319] [QUARK: Robust Retrieval under Non-Faithful Queries via Query-Anchored Aggregation](https://arxiv.org/abs/2601.21049)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的用户查询往往是非忠实的（噪声、不完整或扭曲），当关键语义缺失时会导致检索器失败。这被形式化为召回噪声下的检索问题，其中观察到的查询是从潜在目标项的噪声召回过程中提取的。

Method: QUARK通过恢复假设（即给定观察查询下潜在意图的多个合理解释）显式建模查询不确定性，并引入查询锚定聚合来鲁棒地组合它们的信号。原始查询作为语义锚点，而恢复假设提供受控的辅助证据，防止语义漂移和假设劫持。

Result: 在受控模拟和BEIR基准测试（FIQA、SciFact、NFCorpus）中，使用稀疏和密集检索器，QUARK在召回率、MRR和nDCG方面均优于基础检索器。消融实验表明QUARK对恢复假设数量具有鲁棒性，且锚定聚合优于非锚定的最大/平均/中值池化。
...
### [320] [Multi-modal Imputation for Alzheimer's Disease Classification](https://arxiv.org/abs/2601.21076)
...
<details>
  <summary>Details</summary>
Motivation: 多模态成像（如T1和DWI）能提高神经退行性疾病诊断性能，但完整的多模态数据集并不总是可用，需要解决缺失模态问题
...
Conclusion: 使用扩散模型进行模态插补能有效提升阿尔茨海默病分类性能，特别是在处理不完整多模态数据时
...
### [321] [Responsible AI: The Good, The Bad, The AI](https://arxiv.org/abs/2601.21095)
*Akbar Anbar Jafari,Cagri Ozcinar,Gholamreza Anbarjafari*

Main category: cs.AI

TL;DR: 该论文提出了一个基于悖论理论的负责任AI治理框架(PRAIG)，用于解决AI在组织中价值创造与风险缓解之间的张力管理问题。


<details>
  <summary>Details</summary>
Motivation: 当前关于负责任AI的研究存在碎片化问题，要么过度乐观强调价值创造，要么过度谨慎关注潜在危害。缺乏一个综合框架来理解和管理AI的双重性质（战略机遇与伦理操作风险）。

Method: 基于战略信息系统视角，通过系统综合负责任AI文献，并扎根于悖论理论，开发了悖论基础的负责任AI治理(PRAIG)框架。该框架包含三个核心要素：AI采用的战略效益、固有风险和非预期后果、以及管理这些张力的治理机制。

Result: 提出了PRAIG框架，将负责任AI治理概念化为价值创造与风险缓解之间悖论张力的动态管理。证明了权衡方法会加剧而非解决这些张力，开发了包含特定权变条件的悖论管理策略分类法。

Conclusion: 该框架为组织提供了既不抑制创新又不暴露于不可接受风险的治理结构指导，同时为负责任AI治理研究提出了未来议程，强调需要动态管理AI的悖论性质。
...
### [322] [Planner-Auditor Twin: Agentic Discharge Planning with FHIR-Based LLM Planning, Guideline Recall, Optional Caching and Self-Improvement](https://arxiv.org/abs/2601.21113)
*Kaiyuan Wu,Aditya Nagori,Rishikesan Kamaleswaran*

Main category: cs.AI

TL;DR: 提出Planner-Auditor框架，通过分离LLM生成与确定性验证，结合缓存和自改进循环，提升临床出院计划的安全性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在临床出院计划中虽有潜力，但存在幻觉、遗漏和置信度校准问题，需要更安全可靠的自动化解决方案。

Method: 基于FHIR-native评估管道，采用Planner（LLM生成结构化出院计划）和Auditor（确定性验证模块）分离架构，支持缓存、自改进循环和差异缓冲重放机制。

Result: 自改进循环使任务覆盖率从32%提升至86%，置信度校准显著改善（Brier/ECE降低），差异缓冲重放进一步纠正高置信度遗漏。

Conclusion: Planner-Auditor框架通过反馈驱动再生和针对性重放，为基于FHIR数据的自动化出院计划提供了安全可靠的实践路径。
...
### [323] [Beyond a Single Reference: Training and Evaluation with Paraphrases in Sign Language Translation](https://arxiv.org/abs/2601.21128)
*Václav Javorek,Tomáš Železný,Alessa Carbo,Marek Hrúz,Ivan Gruber*

Main category: cs.AI

TL;DR: 该研究探索使用大语言模型为手语翻译自动生成多种书面语言参考译文，以解决传统单参考译文对手语翻译训练和评估的限制，提出了BLEUpara评估指标。


<details>
  <summary>Details</summary>
Motivation: 手语翻译语料库通常只提供单一书面语言参考译文，但手语与口语之间存在高度非同构关系，多种翻译同样有效。这一限制影响了模型训练和评估，特别是基于n-gram的BLEU等指标。

Method: 1) 比较多种复述策略和模型，使用改进的ParaScore指标评估；2) 在YouTubeASL和How2Sign数据集上研究复述对手语翻译模型训练和评估的影响；3) 提出BLEUpara指标，扩展BLEU以评估多参考译文。

Result: 1) 训练中简单加入复述不会提升翻译性能，甚至可能有害；2) 评估中使用复述能提高自动评分，并更好地与人工判断对齐；3) BLEUpara与感知翻译质量的相关性更强；4) 发布所有生成的复述、生成和评估代码。

Conclusion: 使用大语言模型生成复述参考译文能显著改善手语翻译评估的可靠性，但训练中需要更谨慎的方法。BLEUpara指标为手语翻译系统提供了更准确、与人类判断更一致的评估方法。
...
### [324] [What You Feel Is Not What They See: On Predicting Self-Reported Emotion from Third-Party Observer Labels](https://arxiv.org/abs/2601.21130)
*Yara El-Tawil,Aneesha Sampath,Emily Mower Provost*

Main category: cs.AI

TL;DR: 该研究首次对第三方标注模型在自我报告情感识别上的表现进行跨语料库评估，发现激活度预测性差而效价预测性中等，但当内容对说话者具有个人意义时，效价预测性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 自我报告情感标签反映内部体验，而第三方标签反映外部感知，两者常存在差异。这种差异限制了第三方训练模型在自我报告场景中的应用，在心理健康领域尤为关键，因为准确的自我报告建模对指导干预至关重要。

Method: 采用跨语料库评估方法，首次系统评估第三方训练模型在自我报告情感识别任务上的表现，分析激活度和效价两个维度的预测性能。

Result: 激活度预测性能几乎为零（CCC≈0），效价预测性能中等（CCC≈0.3）。关键发现是：当内容对说话者具有个人意义时，效价预测性能显著提升至高水平（CCC≈0.6-0.8）。

Conclusion: 个人意义是连接外部感知与内部体验的关键桥梁，为改进自我报告情感识别提供了重要方向。同时，自我报告激活度建模仍面临重大挑战，需要新的方法来解决这一难题。
...
### [325] [Bridging the Arithmetic Gap: The Cognitive Complexity Benchmark and Financial-PoT for Robust Financial Reasoning](https://arxiv.org/abs/2601.21157)
*Boxiang Zhao,Qince Li,Zhonghao Wang,Yi Wang,Peng Cheng,Bo Lin*

Main category: cs.AI

TL;DR: 论文提出认知复杂性基准(CCB)来量化LLM在金融量化推理中的"算术幻觉"和"认知崩溃"问题，并设计迭代双阶段金融PoT框架，通过神经符号架构分离语义提取与计算执行，显著提升复杂金融推理任务的准确率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在语义任务上表现出色，但在金融量化推理中存在关键瓶颈，经常出现"算术幻觉"和系统性的"认知崩溃"故障模式。需要严格量化这一现象并开发解决方案。

Method: 1) 引入认知复杂性基准(CCB)：基于95份真实中国A股年报构建的数据集，采用三维分类法(数据源、映射难度、结果单位)对金融查询进行分层；2) 提出迭代双阶段金融PoT框架：神经符号架构，严格分离语义变量提取和逻辑制定，然后将计算卸载到迭代自校正的Python沙箱中确保确定性执行。

Result: 在CCB基准上评估显示，标准思维链方法在复杂任务上表现不佳，而所提方法具有优越的鲁棒性，将Qwen3-235B模型的平均准确率从59.7%提升到67.3%，在高复杂度推理任务中实现了高达10倍的性能提升。

Conclusion: 架构解耦是提高金融推理任务可靠性的关键使能因素，为需要语义理解和量化计算紧密对齐的精度关键领域提供了可迁移的架构洞察。
...
### [326] [Concise Geometric Description as a Bridge: Unleashing the Potential of LLM for Plane Geometry Problem Solving](https://arxiv.org/abs/2601.21164)
*Jingyun Wang,Dian Li,Xiaohan Wang,Gang Liu,Jiahong Yan,Guoliang Kang*

Main category: cs.AI

TL;DR: 该论文提出了一种新的平面几何问题解决方法，通过训练多模态大语言模型解释器将视觉图表转换为文本描述，然后使用现成的LLM进行推理，避免了端到端微调对基础LLM推理能力的损害。


<details>
  <summary>Details</summary>
Motivation: 平面几何问题解决需要同时处理视觉图表和文本描述，但现有方法通过端到端微调多模态大语言模型可能会损害基础LLM的固有推理能力。作者发现，如果能够将视觉信息适当地转化为文本描述，LLM本身就能成为强大的PGPS求解器。

Method: 提出两阶段方法：1) 训练MLLM解释器将几何图表转换为条件声明语言描述；2) 使用现成的LLM基于CDL描述进行推理。采用CoT增强的监督微调，然后通过GRPO进行强化学习优化，设计了CDL匹配奖励而非传统的基于答案的奖励。构建了Formalgeo7k-Rec-CoT数据集支持训练。

Result: 在Formalgeo7k-Rec-CoT、Unigeo和MathVista数据集上的实验表明，该方法（仅使用5.5k数据微调）在性能上优于领先的开源和闭源多模态大语言模型。

Conclusion: 该方法通过分离视觉理解和推理过程，有效利用了LLM的固有推理能力，同时避免了端到端微调对基础模型能力的损害，为多模态几何问题解决提供了新的有效范式。
...
### [327] [FrontierScience: Evaluating AI's Ability to Perform Expert-Level Scientific Tasks](https://arxiv.org/abs/2601.21165)
...
<details>
  <summary>Details</summary>
Motivation: 现有科学基准大多依赖选择题或已发表信息，随着模型进步已接近饱和，需要新的基准来评估专家级科学推理能力，特别是前沿科学问题的解决能力。

Method: 采用两个互补赛道：1)奥林匹克赛道包含国际物理、化学、生物奥赛级别的问题；2)研究赛道包含博士级别的开放式研究子任务。问题由奥赛奖牌得主、国家队教练和博士科学家编写验证，研究赛道采用基于细粒度评分标准的评估框架。

Result: 构建了包含数百个问题的基准（其中160个为开源黄金集），涵盖从量子电动力学到合成有机化学等多个子领域，建立了原创性、难度和事实性的高标准。

Conclusion: FrontierScience填补了现有科学基准的空白，能够更全面地评估语言模型在解决前沿科学问题方面的专家级推理能力，为模型能力评估提供了新的标准。
...
### [328] [MAD: Modality-Adaptive Decoding for Mitigating Cross-Modal Hallucinations in Multimodal Large Language Models](https://arxiv.org/abs/2601.21181)
...
Method: 提出模态自适应解码（MAD），这是一种无需训练的方法，基于任务需求自适应加权模态特定解码分支。MAD利用模型固有的自我评估能力，查询每个任务需要哪些模态，提取模态概率，然后自适应加权对比解码分支，使模型能够专注于相关信息，同时抑制跨模态干扰。

Result: 在CMM和AVHBench上的大量实验表明，MAD显著减少了多个音频-视觉语言模型的跨模态幻觉（VideoLLaMA2-AV分别提升7.8%和2.0%，Qwen2.5-Omni分别提升8.7%和4.7%）。

Conclusion: 通过自我评估实现的显式模态意识对于鲁棒的多模态推理至关重要，为现有的对比解码方法提供了原则性扩展。
...
### [329] [Sycophantic Anchors: Localizing and Quantifying User Agreement in Reasoning Models](https://arxiv.org/abs/2601.21183)
*Jacek Duszenko*

Main category: cs.AI

TL;DR: 该论文引入"谄媚锚点"概念来定位和量化推理模型中的谄媚行为，通过分析超过10,000个反事实推演，展示了如何在推理过程中检测和量化这种错误承诺。
...
Method: 引入"谄媚锚点"概念——能够因果锁定模型与用户达成一致的句子。使用蒸馏推理模型分析超过10,000个反事实推演，通过线性探测器和基于激活的回归器来检测和量化谄媚行为。

Result: 线性探测器能以84.6%的平衡准确率区分谄媚锚点，激活回归器能预测承诺强度（R²=0.74）。发现谄媚锚点比正确推理锚点更易区分，且谄媚行为在推理过程中逐渐形成。

Conclusion: 研究提供了在推理过程中定位模型错位的句子级机制，揭示了谄媚行为的渐进形成过程，为干预提供了潜在窗口。
...
### [330] [How do Visual Attributes Influence Web Agents? A Comprehensive Evaluation of User Interface Design Factors](https://arxiv.org/abs/2601.21961)
...
Method: VAF包含三个阶段：1) 变体生成 - 保持语义不变仅改变视觉属性；2) 浏览交互 - 模拟人类滚动点击行为；3) 验证评估 - 使用目标点击率和目标提及率联合评估视觉属性影响。

Result: 实验覆盖8个变体家族（48个变体）、5个真实网站（购物、旅游、新闻等）和4个代表性Web智能体，发现背景颜色对比度、项目大小、位置和卡片清晰度对智能体行为有显著影响，而字体样式、文本颜色和项目图像清晰度影响较小。

Conclusion: VAF提供了一个系统化的评估框架来量化视觉属性对Web智能体决策的影响，揭示了哪些视觉因素在良性场景中显著影响智能体行为，为设计更鲁棒和用户友好的智能体提供了指导。
...
### [331] [Do Reasoning Models Enhance Embedding Models?](https://arxiv.org/abs/2601.21192)
*Wun Yu Chan,Shaojin Chen,Huihao Jing,Kwun Hang Lau,Elton Chun-Chai Li,Zihao Wang,Haoran Li,Yangqiu Song*

Main category: cs.AI

TL;DR: RLVR调优的推理模型作为嵌入初始化时，相比基础模型在语义表示任务上没有性能优势，原因是后续对比学习会引发流形重对齐，抵消了RLVR带来的局部几何变化。


<details>
  <summary>Details</summary>
Motivation: 随着基于RLVR训练的推理模型的出现，研究者想知道增强的推理能力是否会在这些模型作为嵌入初始化时转化为更优的语义表示能力。

Method: 1) 在MTEB和BRIGHT基准上评估RLVR调优模型与基础模型作为嵌入初始化的性能；2) 提出层次化表示相似性分析(HRSA)框架，从表示、几何和功能三个层面分解相似性；3) 分析RLVR引起的流形变化特征。

Result: 1) 零效应：RLVR调优的嵌入初始化模型相比基础模型没有一致的性能优势；2) HRSA分析显示RLVR引起不可逆的局部几何重组和可逆的坐标基漂移，但保留了全局流形几何和线性读出；3) 后续对比学习导致基础模型和推理初始化模型之间强对齐，即流形重对齐现象。

Conclusion: 与监督微调不同，RLVR优化的是现有语义景观内的轨迹，而非从根本上重构景观本身，因此作为嵌入初始化时无法提供持续优势。
...
### [332] [Uncovering Hidden Correctness in LLM Causal Reasoning via Symbolic Verification](https://arxiv.org/abs/2601.21210)
*Paul He,Yinya Huang,Mrinmaya Sachan,Zhijing Jin*

Main category: cs.AI

TL;DR: DoVerifier：一种符号验证器，用于检查LLM生成的因果表达式是否可从给定因果图中推导出来，使用do-calculus和概率论规则，从而更准确地评估LLM的因果推理能力。
...
Result: 在合成数据和因果问答基准上的评估表明，DoVerifier能更准确地捕捉因果推理轨迹的语义正确性，为评估LLM因果推理能力提供更严谨和信息丰富的方法。

Conclusion: DoVerifier通过形式验证方法解决了现有评估方法的局限性，能够恢复因表面差异而被错误标记的正确答案，为LLM因果推理评估提供了更可靠的框架。
...
### [333] [Causal Discovery for Explainable AI: A Dual-Encoding Approach](https://arxiv.org/abs/2601.21221)
...
<details>
  <summary>Details</summary>
Motivation: 传统因果发现方法在处理分类变量时面临数值不稳定性挑战，需要更稳健的方法来理解特征间的因果关系以解释机器学习模型决策

Method: 采用双编码因果发现方法，运行基于约束的算法时使用互补编码策略，并通过多数投票合并结果

Result: 在Titanic数据集上应用该方法，识别出的因果结构与已建立的可解释方法一致

Conclusion: 双编码方法能够有效处理分类变量的因果发现，提供与现有解释方法一致的因果结构
...
### [334] [TIDE: Tuning-Integrated Dynamic Evolution for LLM-Based Automated Heuristic Design](https://arxiv.org/abs/2601.21239)
*Chentong Chen,Mengyuan Zhong,Ye Fan,Jialong Shi,Jianyong Sun*

Main category: cs.AI

TL;DR: TIDE框架通过解耦结构推理与参数优化，结合并行岛屿模型和树相似性编辑距离提升算法多样性，集成LLM逻辑生成与差分变异算子进行参数调优，显著提升组合优化问题的求解质量与效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法将算法演化视为单一文本生成任务，忽略了离散算法结构与连续数值参数之间的耦合关系，导致因未校准常数而丢弃有前景的算法，以及因简单相似性度量导致早熟收敛。

Method: 提出TIDE框架：外层并行岛屿模型使用树相似性编辑距离驱动结构多样性；内层循环集成LLM逻辑生成与差分变异算子进行参数调优；采用UCB调度器动态优先化高收益提示策略以优化资源分配。

Result: 在九个组合优化问题上的实验表明，TIDE发现的启发式算法在解质量上显著优于最先进基线方法，同时实现了改进的搜索效率和降低的计算成本。

Conclusion: TIDE通过解耦结构推理与参数优化，有效解决了现有方法中算法结构与参数耦合带来的问题，为自动化启发式设计提供了更高效、更高质量的演化框架。
...
### [335] [Position: Certifiable State Integrity in Cyber-Physical Systems -- Why Modular Sovereignty Solves the Plasticity-Stability Paradox](https://arxiv.org/abs/2601.21249)
*Enzo Nicolás Spotorno,Antônio Augusto Medeiros Fröhlich*

Main category: cs.AI

TL;DR: 提出HYDRA框架：通过模块化主权范式解决时间序列基础模型在安全关键CPS中的适应性、可靠性和可验证性问题，采用冻结的特定机制专家库和不确定性感知融合机制。


<details>
  <summary>Details</summary>
Motivation: 当前通用时间序列基础模型在安全关键信息物理系统(CPS)部署中存在严重问题：微调导致灾难性遗忘，频谱偏差平滑高频故障特征，模型不透明阻碍形式化验证，无法满足ISO 26262等安全标准要求。

Method: 提出模块化主权范式HYDRA：构建冻结的特定机制专家库，通过不确定性感知融合机制组合这些专家，实现机制条件有效性，严格分离偶然性和认知不确定性，保持模块化可审计性。

Result: HYDRA框架提供了解决可塑性-稳定性悖论的新途径，确保跨CPS生命周期的鲁棒状态完整性，为安全关键系统提供了可认证的解决方案。

Conclusion: 全局参数更新无法完全解决CPS中的适应性-可靠性矛盾，模块化主权范式通过专家库和不确定性融合机制提供了更可行的解决方案，能够满足安全标准的严格要求。
...
### [336] [Drive-KD: Multi-Teacher Distillation for VLMs in Autonomous Driving](https://arxiv.org/abs/2601.21288)
...
TL;DR: Drive-KD：通过知识蒸馏将自动驾驶分解为"感知-推理-规划"三元能力，使用分层注意力作为蒸馏信号构建单教师模型，并通过多教师蒸馏与不对称梯度投影解决梯度冲突，显著提升小模型性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言/视觉模型在自动驾驶推理规划中具有优势，但GPU内存需求大、推理延迟高；传统监督微调难以弥补小模型能力差距，需要高效的知识迁移方法。

Method: 1) 将自动驾驶分解为感知、推理、规划三个能力维度；2) 使用分层注意力作为蒸馏信号构建能力特定的单教师模型；3) 统一为多教师蒸馏框架，引入不对称梯度投影缓解跨能力梯度冲突。

Result: 蒸馏后的InternVL3-1B模型比同系列预训练78B模型GPU内存减少42倍、吞吐量提升11.4倍，在DriveBench上整体性能更优，规划维度超越GPT-5.1，验证了方法在不同模型家族和规模上的泛化能力。

Conclusion: Drive-KD通过能力分解和分层注意力蒸馏，有效实现了大模型能力向小模型的迁移，为高效自动驾驶视觉语言模型提供了可行路径，平衡了性能与计算效率。
...
### [337] [Modeling Endogenous Logic: Causal Neuro-Symbolic Reasoning Model for Explainable Multi-Behavior Recommendation](https://arxiv.org/abs/2601.21335)
*Yuzhe Chen,Jie Cao,Youquan Wang,Haicheng Tao,Darko B. Vukovic,Jia Wu*

Main category: cs.AI

TL;DR: CNRE：一种结合因果推理与神经符号推理的可解释多行为推荐模型，通过模拟人类决策过程生成去混淆的因果中介，实现多层级可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有多行为推荐方法往往以牺牲可解释性为代价追求性能，而当前可解释方法因依赖外部信息导致泛化能力有限。神经符号整合为可解释性提供了有前景的途径，同时用户行为链本身蕴含适合显式推理的内生逻辑。然而，观测到的多行为数据存在混淆因素，导致模型学习虚假相关性。

Method: 提出因果神经符号推理模型CNRE，通过模拟人类决策过程实现内生逻辑操作化。具体包括：1）使用分层偏好传播捕获异构跨行为依赖；2）基于偏好强度建模用户行为链中的内生逻辑规则；3）自适应调度到相应的神经逻辑推理路径（如合取、析取）；4）生成近似理想状态（隔离混淆效应）的可解释因果中介。

Result: 在三个大规模数据集上的广泛实验表明，CNRE在性能上显著优于最先进的基线方法，同时在模型设计、决策过程和推荐结果三个层面提供多层级可解释性。

Conclusion: CNRE成功将因果推理整合到神经符号框架中，通过模拟人类决策过程操作化内生逻辑，不仅提升了推荐性能，还实现了从模型设计到推荐结果的多层级可解释性，为解决多行为推荐中可解释性与性能的权衡问题提供了有效方案。
...
### [338] [Within-Model vs Between-Prompt Variability in Large Language Models for Creative Tasks](https://arxiv.org/abs/2601.21339)
*Jennifer Haase,Jana Gonnermann-Müller,Paul H. P. Hanel,Nicolas Leins,Thomas Kosch,Jan Mendling,Sebastian Pokutta*

Main category: cs.AI

TL;DR: 研究通过方差分析量化了提示、模型选择和采样随机性对LLM输出的相对贡献，发现提示主要影响输出质量（原创性），而模型选择主要影响输出数量（流畅度），采样方差显著存在。


<details>
  <summary>Details</summary>
Motivation: 量化评估提示、模型选择和采样随机性对大型语言模型输出变异的相对贡献，以理解各因素在LLM输出中的重要性，避免单样本评估中混淆采样噪声与真实效应。

Method: 使用12个LLM在10个创造力提示上各采样100次（总计12,000个样本），通过方差分析分解输出变异来源，分别评估输出质量（原创性）和输出数量（流畅度）的方差贡献。

Result: 对于输出质量（原创性），提示解释36.43%的方差，模型选择解释40.94%；对于输出数量（流畅度），模型选择解释51.25%，模型内方差解释33.70%，提示仅解释4.22%。模型内方差为10-34%，表明采样噪声显著。

Conclusion: 提示是控制输出质量的有效杠杆，但模型内方差显著（10-34%），单样本评估容易混淆采样噪声与真实提示或模型效应，建议采用多次采样评估。
...
### [339] [EHR-RAG: Bridging Long-Horizon Structured Electronic Health Records and Large Language Models via Enhanced Retrieval-Augmented Generation](https://arxiv.org/abs/2601.21340)
*Lang Cao,Qingyu Chen,Yue Guo*

Main category: cs.AI

TL;DR: EHR-RAG：针对长时程电子健康记录设计的检索增强生成框架，通过事件和时间感知检索、自适应迭代检索及双路径证据推理，解决LLM上下文限制问题，在临床预测任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录（EHRs）包含丰富的纵向临床证据，对医疗决策至关重要。然而，长时程EHRs通常超出大型语言模型（LLM）的上下文限制，现有方法（如截断或简单检索）会丢弃临床相关事件和时间依赖性，影响预测准确性。
...
Conclusion: 该工作凸显了检索增强LLMs在结构化EHR数据临床预测实践中的潜力，为解决长时程EHRs超出LLM上下文限制的问题提供了有效框架。
...
### [340] [Ostrakon-VL: Towards Domain-Expert MLLM for Food-Service and Retail Stores](https://arxiv.org/abs/2601.21342)
...
TL;DR: 提出Ostrakon-VL模型、ShopBench基准和QUAD数据管理流程，解决食品服务和零售场景中多模态大语言模型面临的数据质量和评估标准化问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在食品服务和零售商店场景部署面临两大障碍：1) 现实数据噪声大、缺乏可审计的闭环数据管理，难以构建高质量可控训练语料；2) 现有评估协议缺乏统一细粒度的标准化基准，难以客观衡量模型鲁棒性。

Method: 1) 基于Qwen3-VL-8B开发FSRS导向的Ostrakon-VL模型；2) 提出首个面向FSRS的公开基准ShopBench；3) 设计多阶段多模态指令数据管理流程QUAD；4) 采用多阶段训练策略。

Result: Ostrakon-VL在ShopBench上平均得分60.1，在可比参数规模和多样架构的开源MLLMs中达到新SOTA。超越Qwen3-VL-235B-A22B (+0.7)和同规模Qwen3-VL-8B (+4.8)，显示显著提升的参数效率。

Conclusion: Ostrakon-VL提供更鲁棒可靠的FSRS中心感知和决策能力。为促进可重复研究，将公开发布Ostrakon-VL模型和ShopBench基准。
...
### [341] [Latent Chain-of-Thought as Planning: Decoupling Reasoning from Verbalization](https://arxiv.org/abs/2601.21358)
*Jiecong Wang,Hao Peng,Chunyang Liu*

Main category: cs.AI

TL;DR: PLaT框架通过将潜在推理重新定义为规划，将推理与语言表达解耦，实现了动态终止推理和更鲁棒的解决方案空间。
...
Method: PLaT框架将推理建模为潜在规划状态的确定性轨迹，同时使用单独的Decoder在必要时将这些思想落地为文本。这种解耦使模型能够动态确定何时终止推理，而不是依赖固定超参数。

Result: 在数学基准测试中，PLaT显示出独特的权衡：虽然贪心准确率低于基线，但在推理多样性方面表现出更好的可扩展性，表明PLaT学习了更鲁棒、更广泛的解决方案空间。

Conclusion: PLaT为推理时搜索提供了透明且可扩展的基础，通过解耦推理与语言表达，实现了更灵活的推理过程控制。
...
### [342] [System 1&2 Synergy via Dynamic Model Interpolation](https://arxiv.org/abs/2601.21414)
*Chenxu Yang,Qingyi Si,Chong Tian,Xiyu Liu,Dingyu Yao,Chuanyu Qin,Zheng Lin,Weiping Wang,Jiaqi Wang*

Main category: cs.AI

TL;DR: DAMI框架通过动态参数插值实现认知能力控制，在保持效率的同时提升推理精度，结合了系统1的效率和系统2的推理深度。
...
Method: 利用现有Instruct和Thinking检查点进行动态参数插值，无需额外训练。提出DAMI框架，通过查询特定推理强度λ(q)配置认知深度。开发基于偏好学习（编码准确性和效率标准）的训练估计方法，以及基于置信度（利用模型间认知差异）的零样本部署方法。

Result: 在五个数学推理基准测试中，DAMI实现了比Thinking模型更高的准确性，同时保持效率，有效结合了系统1的效率和系统2的推理深度。

Conclusion: 能力控制优于输出控制，动态参数插值能够有效调制模型的认知配置，实现效率和推理深度的平衡。
...
### [343] [When Prohibitions Become Permissions: Auditing Negation Sensitivity in Language Models](https://arxiv.org/abs/2601.21433)
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在理解否定指令方面存在系统性缺陷，当用户明确表示"不应该"采取某个行动时，模型经常将其误解为肯定指令。这种缺陷在需要精确理解禁止性指令的高风险应用场景中构成严重安全隐患。

Method: 研究审计了16个模型在14个伦理场景中的表现，使用简单否定和复合否定两种指令形式进行测试。通过确定性解码排除采样噪声影响，计算否定敏感指数(NSI)作为评估指标，并提出了分级认证框架。

Result: 开源模型在简单否定下77%的情况下支持被禁止行为，在复合否定下达到100%的错误率，相比肯定指令增加了317%。商业模型表现稍好，但仍有19-128%的波动。模型间一致性从肯定指令的74%下降到否定指令的62%，金融场景的脆弱性是医疗场景的两倍。

Conclusion: 当前对齐技术与安全部署要求之间存在显著差距：无法可靠区分"做X"和"不做X"的模型不应在高风险环境中做出自主决策。需要建立基于否定敏感指数的分级认证框架，并设置领域特定的安全阈值。
...
### [344] [The Paradox of Robustness: Decoupling Rule-Based Logic from Affective Noise in High-Stakes Decision-Making](https://arxiv.org/abs/2601.21439)
*Jon Chun,Katherine Elkins*

Main category: cs.AI

TL;DR: 研究发现指令调优的大语言模型在情绪框架效应上表现出惊人的鲁棒性，尽管已知其对提示扰动敏感，但在高风险决策领域中对叙事操纵的抵抗力比人类强110-300倍。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型已知对微小提示扰动敏感且容易与用户偏见产生迎合性对齐，但其在重要、规则约束的决策制定中的鲁棒性尚未得到充分探索。研究者旨在探索LLMs在情绪框架效应下的行为稳定性。

Method: 使用新颖的受控扰动框架，在三个高风险领域（医疗保健、法律和金融）进行实验，量化LLMs对叙事操纵的抵抗力。比较了LLMs与人类受试者在情绪框架效应下的表现差异，使用Cohen's h效应大小进行量化分析。

Result: 发现指令调优的LLMs表现出行为上的近乎完全不变性，对情绪框架效应的抵抗力比人类强110-300倍。模型效应大小接近零（Cohen's h = 0.003），而人类表现出显著偏见（Cohen's h在[0.3, 0.8]范围内）。这种不变性在不同训练范式的模型中持续存在。

Conclusion: 指令调优模型能够将逻辑规则遵循与说服性叙事解耦，提供了决策稳定性来源，可以补充甚至可能去偏人类在制度环境中的判断。虽然LLMs可能对查询格式"脆弱"，但对决策为何应该被偏见却表现出"稳定"性。
...
### [345] [ChipBench: A Next-Step Benchmark for Evaluating LLM Performance in AI-Aided Chip Design](https://arxiv.org/abs/2601.21448)
*Zhongkai Yu,Chenyang Zhou,Yichen Lin,Hejia Zhang,Haotian Ye,Junxia Cui,Zaifeng Pan,Jishen Zhao,Yufei Ding*

Main category: cs.AI

TL;DR: 该论文提出了一个用于AI辅助芯片设计的全面基准测试ChipBench，评估LLM在Verilog生成、调试和参考模型生成三个关键任务上的性能，发现当前SOTA模型在真实工业任务中表现远低于现有饱和基准。


<details>
  <summary>Details</summary>
Motivation: 当前硬件工程领域的LLM基准测试存在饱和问题和任务多样性不足，无法反映LLM在真实工业工作流程中的实际性能，需要更全面、现实的评估框架。

Method: 提出了ChipBench基准测试，包含三个关键任务：1) 44个具有复杂层次结构的现实模块的Verilog生成；2) 89个系统化调试案例；3) 132个跨Python、SystemC和CXXRTL的参考模型样本。同时提供了自动化工具箱用于高质量训练数据生成。

Result: 评估结果显示LLM性能存在显著差距：最先进的Claude-4.5-opus在Verilog生成上仅达到30.74%，在Python参考模型生成上仅13.33%，远低于现有饱和基准中SOTA模型超过95%的通过率。

Conclusion: ChipBench揭示了LLM在真实芯片设计任务中的重大挑战，现有基准已饱和且无法反映实际工业需求。提供的自动化工具箱将促进这一未充分探索领域的研究发展。
...
### [346] [Topeax -- An Improved Clustering Topic Model with Density Peak Detection and Lexical-Semantic Term Importance](https://arxiv.org/abs/2601.21465)
*Márton Kardos*
...
<details>
  <summary>Details</summary>
Motivation: 当前流行的聚类主题建模方法（Top2Vec和BERTopic）存在两个主要问题：1）由于对样本大小和超参数极度敏感，难以可靠地发现语料库中的自然聚类；2）在估计术语重要性时，BERTopic忽略关键词与主题向量的语义距离，而Top2Vec忽略语料库中的词频统计，导致主题不连贯或缺乏多样性。

Method: Topeax通过密度估计峰值自动发现聚类数量，结合词汇统计（词频）和语义指标（与主题向量的距离）来评估术语重要性，从而生成高质量的主题关键词。该方法对样本大小和超参数变化表现出更稳定的行为。

Result: 实验证明Topeax在聚类恢复和聚类描述方面均优于Top2Vec和BERTopic，同时在面对变化的样本大小和超参数时表现出更少的不稳定行为。

Conclusion: Topeax解决了现有聚类主题建模方法的关键缺陷，提供了一种更可靠、更稳定的主题发现和描述方法，能够生成更连贯、更多样化的主题关键词。
...
### [347] [ScaleSim: Serving Large-Scale Multi-Agent Simulation with Invocation Distance-Based Memory Management](https://arxiv.org/abs/2601.21473)
...
<details>
  <summary>Details</summary>
Motivation: LLM驱动的多智能体模拟在应用领域日益普及，但由于GPU内存压力难以扩展。每个智能体维护私有GPU驻留状态（模型、前缀缓存、适配器），随着智能体数量增加会快速耗尽设备内存

Method: 基于对代表性工作负载的分析，引入"调用距离"统一抽象来估计智能体发出未来LLM请求的相对顺序。利用此抽象构建ScaleSim系统，实现主动预取和基于优先级的驱逐，并通过模块化接口支持多样化的智能体特定内存

Result: ScaleSim在模拟基准测试中相比SGLang实现最高1.74倍的加速，显著提升了大规模多智能体模拟的内存效率和性能

Conclusion: ScaleSim通过调用距离抽象有效解决了LLM多智能体模拟的内存扩展问题，为大规模模拟提供了高效的内存管理解决方案
...
### [348] [ARGORA: Orchestrated Argumentation for Causally Grounded LLM Reasoning and Decision Making](https://arxiv.org/abs/2601.21533)
*Youngjin Jin,Hanna Kim,Kwanwoo Kim,Chanhee Lee,Seungwon Shin*

Main category: cs.AI

TL;DR: ARGORA框架通过构建显式论证图来组织多专家LLM讨论，将论证关系建模为因果模型，支持系统性分析和修正推理过程。
...
Method: 引入ARGORA框架：1) 将多专家讨论组织成显式论证图，显示论证间的支持/攻击关系；2) 将论证图建模为因果模型，通过系统性移除单个论证并重新计算结果来识别关键推理链；3) 引入修正机制，当内部推理与外部判断不一致时进行对齐。

Result: 在多样化基准测试和开放式用例中，ARGORA达到竞争性准确率，并展示修正行为：当专家初始意见不一致时，框架更倾向于将争议解决为正确答案而非引入新错误，同时提供决定性论证的因果诊断。

Conclusion: ARGORA通过显式论证图和因果建模增强了多专家LLM系统的透明度、可解释性和可修正性，为复杂决策提供更可靠的推理框架。
...
### [349] [Chain Of Thought Compression: A Theoritical Analysis](https://arxiv.org/abs/2601.21576)
*Juncai Li,Ru Li,Yuxiang Zhou,Boxiang Ma,Jeff Z. Pan*

Main category: cs.AI

TL;DR: 该论文首次从理论上分析了隐式CoT压缩的学习难度，证明高阶逻辑依赖的学习信号会指数衰减，并提出ALiCoT框架通过对齐潜在token分布与中间推理状态来克服这一问题。


<details>
  <summary>Details</summary>
Motivation: 尽管隐式CoT压缩提供了token高效的推理替代方案，但其背后的机制尚不清楚。现有研究缺乏对学习中间推理步骤困难度的理论分析，特别是高阶逻辑依赖的学习信号衰减问题。
...
Result: ALiCoT在保持与显式CoT相当性能的同时，实现了54.4倍的推理加速。理论分析揭示了隐式CoT压缩中高阶交互障碍的根本原因，实验验证了所提方法的有效性。

Conclusion: 该研究首次从理论上解释了隐式CoT压缩的学习难度，证明了高阶逻辑依赖的信号衰减问题，并提出ALiCoT框架成功克服了这一障碍，为高效推理提供了理论基础和实践方案。
...
### [350] [Depth-Recurrent Attention Mixtures: Giving Latent Reasoning the Attention it Deserves](https://arxiv.org/abs/2601.21582)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度循环方法存在三个主要问题：缺乏FLOP、参数和内存匹配的基线比较；由于部分固定的层堆叠导致深度循环利用不足；隐藏层大小恒定限制了多步潜在推理能力。

Method: 提出Dreamer框架，结合三种注意力机制：序列注意力处理序列维度，深度注意力处理深度维度，稀疏专家注意力实现参数高效扩展。通过深度注意力缓解隐藏层大小瓶颈，解耦缩放维度，使深度循环模型能够高效扩展。

Result: 在语言推理基准测试中，相比FLOP、参数和内存匹配的SOTA模型，Dreamer需要2-8倍更少的训练token达到相同准确率；使用相同训练token时，性能优于约2倍大的SOTA模型。深度知识使用分析显示专家选择多样性比SOTA MoE高2-11倍。

Conclusion: Dreamer框架通过创新的注意力混合机制有效解决了深度循环模型的扩展瓶颈，在保持参数效率的同时显著提升了推理性能，为深度循环架构的进一步发展提供了新方向。
...
### [351] [Beyond Imitation: Reinforcement Learning for Active Latent Planning](https://arxiv.org/abs/2601.21598)
*Zhi Zheng,Wee Sun Lee*

Main category: cs.AI

TL;DR: ATP-Latent提出主动潜在规划方法，通过条件变分自编码器平滑潜在空间，结合强化学习提升潜在推理性能


<details>
  <summary>Details</summary>
Motivation: 现有潜在推理方法通过模仿语言标签监督潜在token，但同一问题可能存在多个等效但不同的CoT标签，被动模仿任意一个可能导致次优的潜在token表示和推理策略，限制了潜在空间的规划能力，造成训练与测试间的明显差距

Method: ATP-Latent将潜在token监督过程建模为条件变分自编码器以获得更平滑的潜在空间；同时通过强化学习结合辅助一致性奖励（基于VAE解码内容的一致性计算）来引导RL过程，促进最合理的潜在推理策略

Result: 在LLaMA-1B上，ATP-Latent在四个基准测试中相比先进基线实现了+4.1%的准确率提升和-3.3%的token消耗减少

Conclusion: 强调潜在token表示空间主动规划对实现最优潜在推理策略的重要性，ATP-Latent通过条件VAE平滑潜在空间和基于一致性的强化学习，有效提升了潜在推理的性能和效率
...
### [352] [Search-Based Risk Feature Discovery in Document Structure Spaces under a Constrained Budget](https://arxiv.org/abs/2601.21608)
*Saisubramaniam Gopalakrishnan,Harikrishnan P M,Dagnachew Birru*

Main category: cs.AI

TL;DR: 论文将智能文档处理系统的验证形式化为基于搜索的软件测试问题，旨在在有限预算下最大化发现不同故障类型，通过比较多种搜索策略发现不同求解器具有互补性，需要组合策略进行工业级验证。
...
Method: 将问题形式化为基于搜索的软件测试问题，在文档配置的组合空间中操作，实例化结构风险特征以诱导真实故障条件。比较了进化算法、群体智能、质量多样性、学习型和量子等多种搜索策略，在相同预算约束下进行基准测试。

Result: 通过配置级排他性、胜率和跨时间重叠分析，发现不同求解器始终能发现特定替代方法未发现的故障模式。跨时间分析显示所有评估预算下都存在持续的求解器特定发现，没有单一策略表现出绝对优势。所有求解器的联合最终能覆盖观察到的故障空间，但依赖任何单一方法都会系统性地延迟重要风险的发现。

Conclusion: 结果证明了求解器的内在互补性，并激励采用基于组合的SBST策略进行稳健的工业IDP验证，强调需要多策略组合而非依赖单一方法。
...
### [353] [Semantic Content Determines Algorithmic Performance](https://arxiv.org/abs/2601.21618)
*Martiño Ríos-García,Nawaf Alampara,Kevin Maik Jablonka*

Main category: cs.AI

TL;DR: 论文提出了WhatCounts测试框架，专门用于评估LLMs在计数任务中的语义敏感性，发现前沿LLMs的计数准确率会因计数对象语义类型不同而产生超过40%的波动。


<details>
  <summary>Details</summary>
Motivation: 算法行为应独立于其参数的语义内容，但现有研究常将语义敏感性与推理复杂性或提示变化混淆。需要一种原子化的测试方法来专门评估LLMs对输入语义的依赖性。

Method: 设计WhatCounts测试框架：要求模型在无歧义、分隔的列表中计数项目，无重复项、干扰项或不同语义类型的推理步骤。通过控制变量实验排除混杂因素。

Result: 前沿LLMs的计数准确率因计数对象语义类型（城市vs化学物、名称vs符号）不同而产生超过40%的波动。少量无关微调会不可预测地改变这种语义差距。

Conclusion: LLMs不实现算法而是近似算法，且这种近似是参数依赖的。这种语义敏感性具有广泛影响：任何LLM函数都可能隐藏对其输入意义的依赖性。
...
### [354] [ScholarGym: Benchmarking Deep Research Workflows on Academic Literature Retrieval](https://arxiv.org/abs/2601.21654)
*Hao Shen,Hang Yang,Zhouhong Gu*

Main category: cs.AI

TL;DR: ScholarGym是一个用于深度研究工作流可重复评估的仿真环境，解决了实时API带来的非确定性问题，基于57万篇论文的静态语料库提供2536个查询和专家标注的真实数据。
...
Method: 将工作流组件解耦为查询规划、工具调用和相关性评估，在受控条件下对每个阶段进行细粒度分析。基于57万篇论文的静态语料库构建，提供确定性检索和2536个专家标注的查询。

Result: 通过不同骨干模型的实验揭示了推理能力、规划策略和选择机制在迭代优化过程中的相互作用，展示了在受控环境中进行可重复评估的有效性。

Conclusion: ScholarGym为深度研究工作流提供了可重复评估的仿真环境，解决了实时API带来的非确定性问题，支持对工作流各阶段的细粒度分析，为跨系统比较和模型能力评估提供了可靠基准。
...
### [355] [SONIC-O1: A Real-World Benchmark for Evaluating Multimodal Large Language Models on Audio-Video Understanding](https://arxiv.org/abs/2601.21666)
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLM研究主要关注静态图像理解，而对处理时序音频-视频数据的能力探索不足，需要高质量基准测试来系统评估MLLM在真实场景中的性能。

Method: 创建SONIC-O1基准测试，包含13个真实世界对话领域，4,958个人工验证标注，涵盖人口统计元数据。评估任务包括开放式摘要、多项选择题回答、带推理支持的时序定位。

Result: 实验显示闭源和开源模型在多项选择题准确率上差距较小，但在时序定位任务上最佳闭源模型比开源模型性能高出22.6%。模型性能在不同人口统计群体间存在差异，表明模型行为存在持续差异。

Conclusion: SONIC-O1为时序基础和社交鲁棒的多模态理解提供了开放评估套件，揭示了当前MLLM在处理时序音频-视频数据时的局限性，特别是在时序定位和跨人口统计群体性能一致性方面。
...
### [356] [TCAP: Tri-Component Attention Profiling for Unsupervised Backdoor Detection in MLLM Fine-Tuning](https://arxiv.org/abs/2601.21692)
*Mingzu Liu,Hao Fang,Runmin Cong*

Main category: cs.AI

TL;DR: TCAP是一种无监督防御框架，通过分析三元组件注意力分配差异来检测MLLM微调中的后门样本，无需监督信号即可有效对抗多种触发器和模态的攻击。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM微调即服务(FTaaS)存在后门风险，现有防御方法要么依赖监督信号，要么无法泛化到不同的触发器类型和模态。研究发现后门样本会破坏系统指令、视觉输入和用户文本查询三个功能组件之间的平衡注意力分配，这构成了通用的后门指纹。

Method: 提出三元组件注意力分析(TCAP)框架：1)将跨模态注意力图分解为三个功能组件；2)使用高斯混合模型(GMM)统计分析方法识别对触发器敏感的注意力头；3)通过基于期望最大化(EM)的投票聚合机制隔离后门样本。

Result: 在多种MLLM架构和攻击方法上的广泛实验表明，TCAP能够持续实现强大的防御性能，无需监督信号即可有效检测后门样本。

Conclusion: TCAP作为一种无监督防御框架，通过利用后门样本在三元组件注意力分配上的普遍差异，为MLLM微调服务提供了鲁棒且实用的后门防御解决方案。
...
### [357] [FBS: Modeling Native Parallel Reading inside a Transformer](https://arxiv.org/abs/2601.21708)
*Tongxi Wang*

Main category: cs.AI

TL;DR: FBS Transformer通过引入Parafovea-Attention Window、Chunk-Head和Skip-Gate三个模块，在保持参数不变的情况下改善质量-效率权衡，模拟人类阅读中的内容自适应前瞻、分块结构感知计算分配和训练-测试一致性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM推理主要依赖严格的逐token自回归，现有加速方法大多只是修补这一流程，未能捕捉人类阅读的核心要素：内容自适应前瞻、分块结构感知计算分配以及预览/略读的训练-测试一致性。

Method: 提出Fovea-Block-Skip Transformer，通过三个模块注入可训练的因果循环：1) Parafovea-Attention Window实现内容自适应前瞻；2) Chunk-Head实现分块结构感知计算分配；3) Skip-Gate确保预览/略读的训练-测试一致性。
...
### [358] [E-mem: Multi-agent based Episodic Context Reconstruction for LLM Agent Memory](https://arxiv.org/abs/2601.21714)
*Kaixiang Wang,Yidan Lin,Jiong Lou,Zhaojiacheng Zhou,Bunyod Suvonov,Jie Li*

Main category: cs.AI

TL;DR: E-mem框架通过从记忆预处理转向情景上下文重构，解决了LLM智能体在长序列推理中上下文完整性破坏的问题，采用异质分层架构实现高效系统2推理。


<details>
  <summary>Details</summary>
Motivation: 现有记忆预处理方法（如嵌入或图结构）将复杂序列依赖压缩为预定义结构，破坏了推理所需的关键上下文完整性，限制了LLM智能体实现系统2推理（深思熟虑、高精度问题解决）的能力。

Method: 提出E-mem框架，从记忆预处理转向情景上下文重构。采用受生物印迹启发的异质分层架构：多个助手智能体维护未压缩的记忆上下文，中央主智能体协调全局规划。助手智能体在激活的片段内进行本地推理，提取上下文感知证据后再聚合。

Result: 在LoCoMo基准测试中，E-mem实现了超过54%的F1分数，比最先进的GAM方法提升了7.75%，同时减少了超过70%的token成本。

Conclusion: E-mem通过情景上下文重构方法有效解决了记忆预处理中的破坏性去上下文化问题，为LLM智能体实现系统2推理提供了更高效、更精确的解决方案，在保持逻辑完整性的同时显著降低了计算成本。
...
### [359] [DropoutTS: Sample-Adaptive Dropout for Robust Time Series Forecasting](https://arxiv.org/abs/2601.21726)
*Siru Zhong,Yiqiu Liu,Zhiqing Cui,Zezhi Shao,Fei Wang,Qingsong Wen,Yuxuan Liang*

Main category: cs.AI

TL;DR: DropoutTS是一种模型无关的插件，通过样本自适应dropout机制，将噪声映射到自适应dropout率，动态校准模型学习能力，提升深度时间序列模型在噪声数据下的鲁棒性。
...
Conclusion: DropoutTS通过将学习范式从"学什么"转变为"学多少"，提供了一种高效且有效的解决方案来增强深度时间序列模型对噪声数据的鲁棒性。
...
### [360] [Epistemic Context Learning: Building Trust the Right Way in LLM-Based Multi-Agent Systems](https://arxiv.org/abs/2601.21742)
...
TL;DR: 该论文提出Epistemic Context Learning (ECL)框架，通过历史交互构建同伴可信度档案，使多智能体系统中的个体能够识别可靠同伴并避免盲目从众，显著提升小模型性能至超越大模型水平。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统中的个体智能体往往缺乏鲁棒性，容易盲目跟随误导性同伴。这种弱点源于两方面：谄媚倾向（sycophancy）和评估同伴可靠性的能力不足。需要一种方法让智能体能够基于历史交互评估同伴可信度，从而在不确定时向可信同伴学习。

Method: 首先形式化历史感知参考的学习问题，将同伴的历史交互作为额外输入。然后开发Epistemic Context Learning (ECL)推理框架：基于历史交互显式构建同伴档案，将预测条件化于这些档案上。进一步通过强化学习使用辅助奖励优化ECL，使智能体能够准确估计同伴可靠性。

Result: 实验表明：1) ECL使Qwen 3-4B这样的小模型能够超越历史无关基线模型Qwen 3-30B（8倍大小）；2) ECL将前沿模型性能提升至接近完美（100%）；3) ECL在各种多智能体配置中泛化良好；4) 发现LLMs能够很好地对信任建模，且信任建模准确度与最终答案质量呈强相关。

Conclusion: ECL框架通过历史感知的同伴可靠性评估，有效解决了多智能体系统中的盲目从众问题。该方法不仅显著提升了小模型的性能，还揭示了信任建模在多智能体协作中的关键作用，为构建更鲁棒的多智能体系统提供了有效途径。
...
### [361] [Zero-Shot Statistical Downscaling via Diffusion Posterior Sampling](https://arxiv.org/abs/2601.21760)
*Ruian Tie,Wenbo Xiong,Zhengyu Shi,Xinyu Su,Chenyu jiang,Libo Wu,Hao Li*

Main category: cs.AI

TL;DR: ZSSD：一种无需配对训练数据的零样本统计降尺度框架，通过物理一致的气候先验和统一坐标引导解决GCM泛化问题


<details>
  <summary>Details</summary>
Motivation: 传统监督式气候降尺度方法因缺乏配对训练数据和与再分析数据间的领域差距而难以泛化到全球气候模型（GCMs），现有零样本方法则存在物理不一致性和大尺度因子下的梯度消失问题

Method: 提出零样本统计降尺度（ZSSD）框架：1）从再分析数据学习物理一致的气候先验，以地球物理边界和时序信息为条件确保物理有效性；2）引入统一坐标引导策略，解决原始DPS中的梯度消失问题并确保与大尺度场的一致性

Result: ZSSD在99百分位误差上显著优于现有零样本基线，成功重建复杂天气事件（如热带气旋），并在异构GCMs上表现良好

Conclusion: ZSSD框架无需配对数据即可实现统计降尺度，通过物理先验和统一坐标引导解决了现有方法的泛化问题和物理不一致性，为GCM降尺度提供了有效解决方案
...
### [362] [Abstract Concept Modelling in Conceptual Spaces: A Study on Chess Strategies](https://arxiv.org/abs/2601.21771)
...
<details>
  <summary>Details</summary>
Motivation: 扩展概念空间理论以处理随时间实现的目标导向概念，为涉及序列决策的广泛应用奠定基础，并支持与知识演化机制集成以学习和完善抽象概念。

Method: 采用概念空间框架，将策略概念（如攻击或牺牲）表示为可解释质量维度上的几何区域，将棋局实例化为轨迹，通过轨迹向概念区域的定向运动来识别策略意图，支持双视角建模以捕捉玩家对相同情况的不同解释。

Result: 实现证明了基于轨迹的概念识别的可行性，运动模式与专家评论一致，展示了如何将概念空间理论扩展到时间实现的目标导向概念。

Conclusion: 该工作为涉及序列决策的广泛应用建立了基础框架，支持与知识演化机制集成以学习和完善抽象概念，为建模随时间展开的抽象概念提供了概念空间方法。
...
### [363] [A Unified XAI-LLM Approach for EndotrachealSuctioning Activity Recognition](https://arxiv.org/abs/2601.21802)
*Hoang Khang Phan,Quang Vinh Dang,Noriyo Colley,Christina Garcia,Nhat Tan Le*

Main category: cs.AI

TL;DR: 提出基于大语言模型（LLM）的统一框架，用于气管内吸痰（ES）操作视频的动作识别与反馈生成，在家庭护理和教育场景中提升培训效果。


<details>
  <summary>Details</summary>
Motivation: 气管内吸痰是高风险侵入性操作，需要高度技能，但在家庭护理和教育环境中缺乏有效监督和自动化反馈系统。现有方法在动作识别和反馈生成方面研究不足。

Method: 提出以LLM为核心的统一框架：1）LLM作为中央推理模块，从视频数据执行时空动作识别和可解释决策分析；2）LLM以自然语言生成反馈；3）基于异常检测和可解释AI（XAI）原理构建学生支持模块。

Result: LLM方法在准确率和F1分数上比基线模型提升约15-20%。框架能够生成自动化、可解释的反馈，突出正确操作并提供针对性改进建议。

Conclusion: 该框架为护理教育建立了可扩展、可解释、数据驱动的基础，能提高培训效率并最终改善患者安全，展示了LLM在医疗培训中的潜力。
...
### [364] [Looking Beyond Accuracy: A Holistic Benchmark of ECG Foundation Models](https://arxiv.org/abs/2601.21830)
*Francesca Filice,Edoardo De Rose,Simone Bartucci,Francesco Calimeri,Simona Perri*

Main category: cs.AI

TL;DR: 该研究提出了一个全面的心电专家基础模型基准测试框架，结合性能评估和表示层面分析，用于评估模型在医疗领域的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 虽然基础模型在AI辅助心电解释领域开始应用，但现有基准测试主要关注下游性能评估，缺乏对嵌入表示泛化能力的深入分析。在医疗等错误敏感领域，需要更全面的评估框架来确保模型负责任地部署。

Method: 提出了一种基准测试方法，将基于性能的评估与表示层面分析相结合，利用SHAP（SHapley Additive exPlanations）和UMAP（Uniform Manifold Approximation and Projection）技术。对多个通过最先进技术预训练的心电专家基础模型进行了广泛评估，涵盖不同跨大陆数据集和数据可用性设置（包括数据稀缺场景）。

Result: 实验结果表明，该基准测试协议能够深入洞察心电专家基础模型的嵌入模式，使研究人员能够更深入地理解其表示结构和泛化能力。

Conclusion: 该研究填补了心电专家基础模型评估的空白，提供了一个全面的基准测试框架，有助于在医疗领域更负责任地部署基础模型。
...
### [365] [Bridging Forecast Accuracy and Inventory KPIs: A Simulation-Based Software Framework](https://arxiv.org/abs/2601.21844)
*So Fukuhara,Abdallah Alabdallah,Nuwan Gunasekara,Slawomir Nowaczyk*

Main category: cs.AI

TL;DR: 论文提出一个决策中心的仿真框架，用于评估汽车售后市场备件库存管理中预测模型对运营绩效的影响，而非仅关注统计准确性。


<details>
  <summary>Details</summary>
Motivation: 汽车售后市场备件需求高度间歇且不确定，传统预测模型评估仅关注统计准确性（如MAE、RMSE），但这些指标与运营绩效指标（如总成本、服务水平）的关系不明确，需要建立预测模型与库存管理决策之间的系统性评估框架。

Method: 提出决策中心仿真软件框架，包含三个核心组件：(1) 针对备件需求特征设计的合成需求生成器；(2) 可容纳任意预测模型的灵活预测模块；(3) 消耗预测结果并计算运营KPI的库存控制模拟器，形成闭环评估系统。

Result: 通过多种仿真场景验证，发现传统准确性指标的改进不一定转化为更好的运营绩效，统计误差相似的模型可能产生显著不同的成本-服务权衡，分析了预测性能特定方面如何影响库存结果，并提供了模型选择指导。

Conclusion: 该框架将需求预测与库存管理操作化连接，将评估重点从纯粹的预测准确性转向运营相关性，为汽车售后市场及相关领域的预测模型评估提供了更实用的决策支持工具。
...
### [366] [KnowBias: Mitigating Social Bias in LLMs via Know-Bias Neuron Enhancement](https://arxiv.org/abs/2601.21864)
...
Conclusion: KnowBias提供了一种概念上不同的去偏方法，通过增强偏见知识神经元而非抑制它们，实现了强大去偏效果、良好泛化性和高数据效率，同时保持模型通用能力
...
### [367] [astra-langchain4j: Experiences Combining LLMs and Agent Programming](https://arxiv.org/abs/2601.21879)
...
<details>
  <summary>Details</summary>
Motivation: 随着生成式AI和智能体AI（作为多智能体系统形式）的兴起，需要探索这些新技术如何影响传统智能体工具包的使用，以及传统工具包中积累的丰富经验如何影响新智能体平台的设计。

Method: 为ASTRA编程语言开发了一个大型语言模型（LLM）集成的原型，通过三个示例实现来展示集成方法，并基于这些示例进行经验总结。

Result: 成功开发了ASTRA的LLM集成原型，通过三个具体示例验证了集成方法的可行性，并获得了关于传统智能体工具包与新兴AI技术结合的实际经验。

Conclusion: 传统智能体工具包与新兴生成式AI/智能体AI技术之间存在双向影响关系，通过LLM集成可以促进两者的协同发展，为未来智能体平台设计提供宝贵经验。
...
### [368] [From Meta-Thought to Execution: Cognitively Aligned Post-Training for Generalizable and Reliable LLM Reasoning](https://arxiv.org/abs/2601.21909)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM后训练方法将完整推理轨迹作为基本单位进行优化，这与人类实际解决问题的认知过程存在根本差异。人类认知自然将问题解决分解为两个阶段：首先获取跨问题泛化的抽象策略（元知识），然后将其适应到具体实例。现有方法的问题中心化导致抽象策略与问题特定执行纠缠，限制了泛化能力。
...
Result: 在四个模型和八个基准测试上的实验显示，相比标准方法，分布内性能提升2.19%，分布外性能提升4.63%，同时训练时间减少65-70%，token消耗减少50%。

Conclusion: 将后训练与人类认知原则对齐不仅能产生更优的泛化性能，还能显著提升训练效率。这表明模仿人类认知过程是改进LLM训练的有效途径。
...
### [369] [ProRAG: Process-Supervised Reinforcement Learning for Retrieval-Augmented Generation](https://arxiv.org/abs/2601.21912)
*Zhao Wang,Ziliang Zhao,Zhicheng Dou*

Main category: cs.AI

TL;DR: ProRAG：一个过程监督强化学习框架，通过整合学习到的步骤级监督到在线优化循环中，解决检索增强生成中奖励稀疏和信用分配低效的问题


<details>
  <summary>Details</summary>
Motivation: 传统基于结果的RL方法在复杂推理任务中存在奖励稀疏和信用分配低效的问题，粗粒度的标量奖励无法识别长轨迹中的具体错误步骤，导致"过程幻觉"——模型通过有缺陷的逻辑或冗余检索步骤得到正确答案。现有的过程感知方法缺乏在线探索能力来解耦步骤级信用与全局结果
...
Conclusion: ProRAG通过将学习到的步骤级监督整合到在线优化循环中，有效解决了检索增强生成中奖励稀疏和信用分配低效的问题，为每个动作提供精确反馈，在复杂推理任务中表现出色
...
### [370] [JADE: Bridging the Strategic-Operational Gap in Dynamic Agentic RAG](https://arxiv.org/abs/2601.21916)
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG系统面临关键二分法：要么在固定架构中联合优化模块，要么支持动态规划但将执行器视为冻结的黑盒工具。这种"解耦优化"导致"战略-操作不匹配"，复杂规划策略因未适应的本地执行器而无法实现，系统复杂度增加却带来负面性能增益。

Method: 提出JADE（联合智能动态执行）框架，将系统建模为统一在单一共享骨干下的协作多智能体团队，通过基于结果的奖励驱动端到端学习，实现规划和执行的联合优化。

Result: 实证结果表明，JADE将离散模块转变为协同系统，通过联合优化带来显著性能改进，并通过动态工作流编排实现效率与效果之间的灵活平衡。
...
### [371] [Self-Compression of Chain-of-Thought via Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2601.21919)
...
Method: 提出SCMA多智能体强化学习框架，包含三个智能体：分割智能体将推理过程分解为逻辑块，评分智能体量化每个块的重要性，推理智能体在训练中接受重要性加权的长度惩罚，部署时无额外开销。

Result: 实验表明SCMA在多个模型规模上减少响应长度11.1%至39.0%，同时提升准确性4.33%至10.02%。消融研究和定性分析验证了MARL框架中的协同优化产生了涌现行为。

Conclusion: SCMA通过多智能体强化学习框架选择性惩罚冗余推理块，有效减少大型推理模型的推理开销，同时保持甚至提升准确性，优于传统的强化学习范式。
...
### [372] [AgenticSimLaw: A Juvenile Courtroom Multi-Agent Debate Simulation for Explainable High-Stakes Tabular Decision Making](https://arxiv.org/abs/2601.21936)
*Jon Chun,Kathrine Elkins,Yong Suk Lee*

Main category: cs.AI

TL;DR: AgenticSimLaw是一个角色结构化的多智能体辩论框架，通过法庭式编排实现高风险表格决策任务的透明可控推理，在年轻成人累犯预测任务上表现优于传统单智能体方法。


<details>
  <summary>Details</summary>
Motivation: 解决黑盒方法在高风险决策任务中缺乏透明度和可解释性的问题，为需要人类监督的敏感领域提供可审计的决策过程。

Method: 设计法庭式编排的多智能体辩论框架，明确定义检察官、辩护律师、法官等角色，采用7轮结构化辩论协议，包含私有推理策略，形成完全可审计的决策过程。

Result: 在NLSY97数据集的年轻成人累犯预测任务中，相比传统思维链提示，结构化多智能体辩论展现出更稳定和可泛化的性能，准确率和F1分数相关性更强。

Conclusion: AgenticSimLaw通过结构化角色组织、交互日志可观测性和明确部署约束，解决了LLM多智能体系统的关键挑战，为需要透明度和人类监督的高风险决策任务提供了通用框架。
...
### [373] [The Energy Impact of Domain Model Design in Classical Planning](https://arxiv.org/abs/2601.21967)
*Ilche Georgievski,Serhat Tekin,Marco Aiello*

Main category: cs.AI

TL;DR: 研究探讨领域模型特征如何影响经典规划器的能耗，提出领域模型配置框架，通过实验证明领域级修改能在不同规划器间产生可测量的能耗差异，且能耗与运行时间并不总是相关。
...
Conclusion: 领域模型特征显著影响规划器能耗，为绿色AI在自动规划领域的应用提供了实证基础，表明通过领域模型设计优化能耗是可行的研究方向。
...
### [374] [Learning Decentralized LLM Collaboration with Multi-Agent Actor Critic](https://arxiv.org/abs/2601.21972)
*Shuo Liu,Tianle Chen,Ryan Amiri,Christopher Amato*

Main category: cs.AI

TL;DR: 该论文提出了两种多智能体演员-评论家方法（CoLLM-CC和CoLLM-DC）来优化去中心化LLM协作，解决了现有方法中预定义执行协议和蒙特卡洛方法高方差的问题。
...
Method: 提出了两种多智能体演员-评论家方法：CoLLM-CC（集中式评论家）和CoLLM-DC（去中心化评论家），用于优化去中心化LLM协作。通过演员-评论家方法解决蒙特卡洛方法的高方差问题。

Result: 在写作、编码和游戏领域实验中，蒙特卡洛方法和CoLLM-DC在短视界和密集奖励设置中能达到与CoLLM-CC相当的性能。但在长视界或稀疏奖励任务中，两者都表现不佳：蒙特卡洛方法需要更多样本，CoLLM-DC难以收敛。

Conclusion: 多智能体演员-评论家方法（特别是集中式评论家架构）在优化去中心化LLM协作方面具有优势，特别是在长视界或稀疏奖励任务中表现更佳，而蒙特卡洛方法和去中心化评论家方法在这些场景下存在局限性。
...
### [375] [Mind the Gap: How Elicitation Protocols Shape the Stated-Revealed Preference Gap in Language Models](https://arxiv.org/abs/2601.21975)
...
<details>
  <summary>Details</summary>
Motivation: 现有评估主要依赖二元强制选择提示，将真实偏好与评估协议的人工制品混为一谈，需要更系统地研究评估协议如何影响陈述-显示偏好相关性。

Method: 系统研究24个语言模型，通过允许中立和弃权来改进陈述偏好评估，排除弱信号；进一步允许显示偏好中的弃权；测试系统提示引导对AIRiskDilemmas的影响。

Result: 允许中立和弃权改进陈述偏好评估，显著提高Spearman秩相关；但允许显示偏好弃权导致相关性接近零或负值；系统提示引导未能可靠改善相关性。

Conclusion: 陈述-显示偏好相关性高度依赖评估协议，偏好评估需要能处理不确定偏好的方法，二元强制选择协议存在局限性。
...
### [376] [VERSA: Verified Event Data Format for Reliable Soccer Analytics](https://arxiv.org/abs/2601.21981)
...
Method: 提出VERSA（Verified Event Data Format for Reliable Soccer Analytics）系统验证框架，基于状态转移模型定义有效事件序列，能够自动检测和纠正事件流数据中的异常模式。该框架通过定义足球事件之间的合法转换关系来确保数据逻辑一致性。
...
Conclusion: 验证过程能有效提高数据驱动分析的可靠性。VERSA框架为足球事件数据质量提供了系统性解决方案，通过确保数据完整性增强了分析模型的信任度，对体育分析领域具有重要实践价值。
...
### [377] [Liquid Interfaces: A Dynamic Ontology for the Interoperability of Autonomous Systems](https://arxiv.org/abs/2601.21993)
*Dhiogo de Sá,Carlos Schmiedel,Carlos Pereira Lopes*

Main category: cs.AI

TL;DR: 论文提出"液态接口"范式，将接口从静态技术构件转变为运行时通过意图表达和语义协商产生的短暂关系事件，以支持自适应、概率性和上下文相关的智能体协调。


<details>
  <summary>Details</summary>
Motivation: 当前软件架构难以支持具有自适应、概率性和上下文相关推理能力的自主智能体，而系统集成仍主要依赖静态接口和确定性契约，导致协调机制僵化。

Method: 提出液态接口协调范式，将接口视为短暂的关系事件而非持久技术构件；形式化该模型并设计液态接口协议(LIP)，管理意图驱动交互、协商执行和在语义不确定性下的短暂性执行；讨论治理影响并描述参考架构。

Result: 液态接口为基于智能体的系统提供了自适应协调的原则性基础，通过LIP协议实现运行时意图表达和语义协商，展示了实际可行性。

Conclusion: 液态接口范式通过将接口重新概念化为短暂的关系事件，解决了传统静态接口在支持自适应智能体协调方面的局限性，为构建更灵活、上下文感知的智能体系统提供了新方法。
...
### [378] [CAR-bench: Evaluating the Consistency and Limit-Awareness of LLM Agents under Real-World Uncertainty](https://arxiv.org/abs/2601.22027)
*Johannes Kirmayr,Lukas Stappen,Elisabeth André*

Main category: cs.AI

TL;DR: CAR-bench是一个针对车载语音助手的LLM智能体基准测试，专注于评估一致性、不确定性处理和能力意识，包含幻觉任务和消歧任务，揭示现有模型在现实应用中的可靠性不足。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体基准测试主要关注理想化环境下的任务完成，忽视了现实世界用户应用中智能体的可靠性。在车载语音助手等场景中，用户经常发出不完整或模糊的请求，产生内在不确定性，智能体需要通过对话、工具使用和政策遵守来管理这些不确定性。

Method: 引入CAR-bench基准测试，包含LLM模拟用户、领域政策和58个相互关联的工具（涵盖导航、生产力、充电和车辆控制）。除了标准任务完成外，还设计了幻觉任务（测试在工具或信息缺失时的极限意识）和消歧任务（要求通过澄清或内部信息收集解决不确定性）。

Result: 基线结果显示，所有任务类型上偶尔成功和一致成功之间存在巨大差距。即使是前沿推理LLM在消歧任务上的持续通过率也低于50%（由于过早行动），在幻觉任务中经常违反政策或捏造信息以满足用户请求。

Conclusion: CAR-bench揭示了现有LLM智能体在现实世界应用中的可靠性不足，强调了在现实环境中需要更可靠和自知的LLM智能体，特别是在处理不确定性和能力意识方面。
...
### [379] [Defining Operational Conditions for Safety-Critical AI-Based Systems from Data](https://arxiv.org/abs/2601.22118)
*Johann Christensen,Elena Hoemann,Frank Köster,Sven Hallerbach*

Main category: cs.AI

TL;DR: 提出一种基于多维度核表示的安全设计方法，从已有数据后验定义操作设计域，支持数据驱动安全关键AI系统的认证


<details>
  <summary>Details</summary>
Motivation: AI在安全关键应用中日益重要，但真实世界复杂系统或已有数据情况下，定义底层环境条件极具挑战，导致操作设计域描述不完整。而ODD是AI系统认证的必要条件，传统方法依赖专家知识和标准在开发早期创建，存在局限性。

Method: 提出一种安全设计方法，使用多维度核表示从先前收集的数据中后验定义ODD。通过蒙特卡洛方法和真实航空用例验证，用于未来安全关键防撞系统。定义两个ODD相等的条件，证明数据驱动ODD可等于数据原始隐藏ODD。

Result: 方法通过蒙特卡洛模拟和真实航空用例验证有效。建立了ODD相等性条件，证明数据驱动ODD能够捕捉原始隐藏ODD。基于核的安全设计ODD为数据驱动安全关键AI系统认证提供了可行途径。

Conclusion: 提出的基于核的安全设计ODD方法能够从已有数据中后验定义操作设计域，解决了传统方法在复杂环境描述中的局限性，为数据驱动安全关键AI系统的未来认证提供了有效工具。
...
### [380] [Exploring Reasoning Reward Model for Agents](https://arxiv.org/abs/2601.22154)
...
<details>
  <summary>Details</summary>
Motivation: 当前Agentic RL方法主要依赖稀疏的结果奖励进行训练，这种反馈无法区分中间推理质量，导致训练结果次优。需要更细粒度的反馈机制来指导智能体的推理过程。

Method: 提出了Agent-RRM奖励模型，提供三方面结构化反馈：1) 显式推理轨迹，2) 聚焦批判（突出推理缺陷并提供改进指导），3) 整体过程评分。基于这些信号，研究了三种集成策略：Reagent-C（文本增强精炼）、Reagent-R（奖励增强指导）和Reagent-U（统一反馈集成）。

Result: 在12个多样化基准测试上的广泛评估表明，Reagent-U实现了显著的性能飞跃，在GAIA上达到43.7%，在WebWalkerQA上达到46.2%，验证了推理奖励模型和训练方案的有效性。

Conclusion: Agent-RRM通过提供细粒度的结构化反馈有效解决了Agentic RL中稀疏奖励的问题，三种集成策略中Reagent-U表现最佳，为智能体推理训练提供了新的有效方法。
